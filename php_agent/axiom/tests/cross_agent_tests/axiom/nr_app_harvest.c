...
uint64_t nr_app_harvest_calculate_threshold(uint64_t target, uint64_t seen) {
  uint64_t threshold;

  if ((0 == seen) || (0 == target) || (seen < target)) {
    return 0;
  }

  /* The spec provides the following expression for the exponential
   * back-off strategy:
   *
  ...
  threshold = pow((double)target, (double)target / (double)seen)
...
bool nr_app_harvest_private_should_sample(nr_app_harvest_t* ah,
  ...
  if (now >= ah->next_harvest) {
    ah->threshold = nr_app_harvest_calculate_threshold(
        ah->target_transactions_per_cycle, ah->transactions_seen);
  ...
  if (ah->transactions_sampled < ah->target_transactions_per_cycle) {
     ...
     * sample a transaction. The spec provides the following exponential
     * back-off strategy:
     *
     *   sampled = rand(decided_count) <
     *               (target ** (target / sampled_true_count) - target ** 0.5)
