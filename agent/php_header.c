...
char *
nr_php_get_request_header (const char *name TSRMLS_DC)
{
  /*
...
int
nr_php_has_request_header (const char *name TSRMLS_DC)
{
...
static void
nr_php_get_response_header_search (void *data, void *arg TSRMLS_DC)
{
...
static char *
nr_php_get_response_header (const char *name TSRMLS_DC)
{
  ...
  zend_llist_apply_with_argument (nr_php_response_headers (TSRMLS_C), nr_php_get_response_header_search, (void *)&search TSRMLS_CC);

  return search.value;
}

int
nr_php_has_response_content_length (TSRMLS_D)
{
...
int
nr_php_get_response_content_length (TSRMLS_D)
{
  int content_length;
  char *str = nr_php_get_response_header ("content-length:" TSRMLS_CC);
...
static int
has_prefix (const char *s, size_t slen, const char *prefix, size_t prefix_len)
{
  return (slen >= prefix_len) && (0 == nr_strnicmp (s, prefix, prefix_len));
}

char *
nr_php_get_response_content_type (TSRMLS_D)
{
   ...
   * SG(sapi_headers).mimetype, see PHP-579.
   *
   ...
   * charset=UTF-8'
   */
  for (hdr = (sapi_header_struct *)zend_llist_get_first_ex (headers, &pos);
       hdr;
...
static nr_status_t
nr_php_add_response_header (const char *name, const char *value TSRMLS_DC)
{
  int zend_rv;
...
void
nr_php_header_output_handler (char *output NRUNUSED,
                              ...
                              int mode TSRMLS_DC)
{
  ...
  if (0 == nr_php_output_is_start (mode)) {
    return;
  }

  if (SG (headers_sent)) {
    nrl_verbosedebug (NRL_TXN, "CAT: unable to add cross process response header: headers already sent");
    return;
  }

  if (0 == nr_php_output_is_end (mode)) {
    nrl_verbosedebug (NRL_TXN, "CAT: adding cross process response header before buffer's end");
  }
  ...
  if (0 == response) {
    nrl_verbosedebug (NRL_TXN, "CAT: unable to create cross process response header");
  } else {
    nr_status_t rv = nr_php_add_response_header (X_NEWRELIC_APP_DATA, response TSRMLS_CC);
 ...
 * Situations where the struct has been contracted are highly unlikely. See
 * PHP-1057.
 */
static int
nr_php_sapi_headers_pointer_is_plausible (sapi_headers_struct *sapi_headers TSRMLS_DC)
{
...
int
nr_php_header_handler (sapi_header_struct *sapi_header, sapi_header_op_enum op, sapi_headers_struct *sapi_headers TSRMLS_DC)
{
  /*
   * Capture a pointer to SG(sapi_headers) to prevent segfaults accessing response
   * headers or status code via the SAPI globals on OpenSUSE. OpenSUSE PHP SAPI
   * globals have a different memory layout. See PHP-581.
   */
  ...
  if (NR_PHP_PROCESS_GLOBALS (orig_header_handler)) {
    return NR_PHP_PROCESS_GLOBALS (orig_header_handler) (sapi_header, op, sapi_headers TSRMLS_CC);
  }
...
zend_llist *
nr_php_response_headers (TSRMLS_D)
{
...
void
nr_php_capture_sapi_headers (TSRMLS_D)
{
  sapi_header_line ctr = {
 ...
 *
 * We do not use REPLACE for all PHPs because it is riskier than simply deleting
...
int
nr_php_http_response_code (TSRMLS_D)
{
