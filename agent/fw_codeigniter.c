 ...
 */
static void
nr_codeigniter_name_the_wt (zend_function *func,
                            const zend_function *caller NRUNUSED TSRMLS_DC)
{
  zend_op_array *op_array = nr_php_current_op_array (TSRMLS_C);

  if ((NR_FW_CODEIGNITER != NRPRG (current_framework) ||
      (NULL == func) || (NULL == func->common.scope))) {
    return;
  }
  ...
  if (OP_ARRAY_IS_FILE (op_array, "CodeIgniter.php")) {
    ...
    action = nr_formatf ("%.*s/%.*s",
      (int) nr_php_class_entry_name_length (ce),
      nr_php_class_entry_name (ce),
      (int) nr_php_function_name_length (func),
      nr_php_function_name (func));

    nr_txn_set_path ("CodeIgniter", NRPRG (txn), action, NR_PATH_TYPE_ACTION, NR_NOT_OK_TO_OVERWRITE);
 ...
 */
void
nr_codeigniter_enable (TSRMLS_D)
{
  nr_php_add_call_user_func_array_pre_callback (nr_codeigniter_name_the_wt TSRMLS_CC);
}
