 ...
 * https://newrelic.atlassian.net/wiki/display/eng/Transaction+trace+linking+phase+1
 */

/*
 * Message queue variants of the above names, per
 ...
 * easier: although the spec specifies exact casing, we should generally prefer
 * to match in a case insensitive manner wherever possible to avoid interop
 * concerns if an agent implements the case slightly incorrectly (as has
 * happened depressingly often in the traditional CAT world).
 */
 ...
 *
 * Example : nr_header_extract_encoded_value ("Data", "XXXX Data: a1b2c3 XXXX") => "a1b2c3"
 */
extern char *nr_header_extract_encoded_value (const char *header_name, const char *string);
 ...
 *
 * Example :  nr_header_format_name_value ("alpha", "beta", 1) => "alpha: beta\r\n"
 */
extern char *nr_header_format_name_value (const char *name, const char *value, int include_return_newline);
 ...
 *           failure.
 *
 * Example : nr_header_parse_content_type ("text/html; charset=\"utf-8\"") => "text/html"
 */
