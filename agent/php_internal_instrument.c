...
static void nr_php_instrument_delegate(nrinternalfn_t* wraprec,
  ...
  if ((0 == wraprec) || (0 == wraprec->oldhandler)
    ...
    if (FAILURE
      ...
      if (FAILURE
          == zend_parse_parameters_ex(
                 ...
                 "|s!s!s!bl", &host_and_port, &host_len, &user, &user_len,
                 &passwd, &passwd_len, &ignore2, &ignore1)) {
        nr_wrapper->oldhandler(INTERNAL_FUNCTION_PARAM_PASSTHRU);
        return;
      }
    }
...
NR_INNER_WRAPPER(mysqli_construct) {
  ...
  if (FAILURE
      == zend_parse_parameters_ex(ZEND_PARSE_PARAMS_QUIET,
                                  ZEND_NUM_ARGS() TSRMLS_CC, "|ssssls", &host,
                                  &host_len, &username, &username_len,
                                  &password, &password_len, &database,
                                  &database_len, &port, &socket, &socket_len)) {
    nr_wrapper->oldhandler(INTERNAL_FUNCTION_PARAM_PASSTHRU);
    return;
  }
...
NR_INNER_WRAPPER(mysqli_stmt_construct) {
  ...
  if (sqlstr) {
    nr_php_prepared_statement_save(NR_PHP_INTERNAL_FN_THIS, "mysqli", sqlstr,
                                   sqlstrlen TSRMLS_CC);
  }
...
NR_INNER_WRAPPER(redis_connect) {
  ...
  if (SUCCESS
      == zend_parse_parameters_ex(
             ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS() TSRMLS_CC, "s|lzzz",
             &host, &host_len, &port, &ignore1, &ignore2, &ignore3)) {
...
NR_INNER_WRAPPER(redis_select) {
  ...
  if (SUCCESS
    ...
    instance = nr_php_redis_retrieve_datastore_instance(
        NR_PHP_INTERNAL_FN_THIS TSRMLS_CC);
    nr_datastore_instance_set_database_name(instance, database);
    nr_free(database);
  }
...
NR_INNER_WRAPPER(pdo_exec) {
  ...
  nr_php_pdo_end_segment_sql(segment, sqlstr, sqlstrlen,
