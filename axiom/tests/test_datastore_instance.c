...
test_getters (void)
{
  ...
  tlib_pass_if_null ("null port_path_or_id if instance is null", port_path_or_id);
  tlib_pass_if_null ("null database_name if instance is null", database_name);

  instance = nr_datastore_instance_create ("bluestar", "1234", "lemon_poppyseed");
  ...
  tlib_pass_if_str_equal ("port_path_or_id in matches port_path_or_id out", "1234", port_path_or_id);
  tlib_pass_if_str_equal ("database_name in matches database_name out", "lemon_poppyseed", database_name);

  nr_datastore_instance_set_host (instance, "localhost");
  host = nr_datastore_instance_get_host (instance);
  system_host = nr_system_get_hostname ();
  tlib_pass_if_str_equal ("localhost appropriately transformed", system_host, host);
...
test_setters (void)
{
  ...
  tlib_pass_if_str_equal ("null host results in unknown", "unknown", instance->host);
  tlib_pass_if_str_equal ("null port_path_or_id results in unknown", "unknown", instance->port_path_or_id);
  tlib_pass_if_str_equal ("null database_name results in unknown", "unknown", instance->database_name);
  ...
  tlib_pass_if_str_equal ("empty host results in unknown", "unknown", instance->host);
  tlib_pass_if_str_equal ("empty port_path_or_id results in unknown", "unknown", instance->port_path_or_id);
  tlib_pass_if_str_equal ("empty database_name results in unknown", "unknown", instance->database_name);
  ...
  tlib_pass_if_str_equal ("port_path_or_id in matches host out", "4321", instance->port_path_or_id);
  tlib_pass_if_str_equal ("database_name in matches host out", "chocolate", instance->database_name);
