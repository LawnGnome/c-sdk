...
func NewClient(cfg *ClientConfig) (Client, error) {
	...
	// TODO(msl): Make these timeouts configurable.
	dialer := &net.Dialer{
...
func (c *clientImpl) perform(url string, data []byte) ([]byte, error) {
	...
	default:
		...
		return nil, fmt.Errorf("unexpected collector HTTP status code: %d", resp.StatusCode)
	}
...
func (c *clientImpl) Execute(cmd Cmd) ([]byte, error) {
	...
	if nil != err {
		return nil, fmt.Errorf("unable to create json payload for '%s': %s", cmd.Name, err)
	}
