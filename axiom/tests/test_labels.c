...
static void
labels_cross_agent_testcase_fn (
  const char *testname, 
  const char *input, 
  ...
  int line)
{
  ...
  test_pass_if_true (testname, 0 == nr_strcmp (actual_json, expected_json),
    "actual_json=%s expected_json=%s",
    NRSAFESTR (actual_json), NRSAFESTR (expected_json));
...
#define labels_testcase(...) \
  labels_testcase_fn (__VA_ARGS__, __FILE__, __LINE__)
...
static void
test_nr_labels_parse_and_format_cross_agent (void)
{
  char *json = 0;
  nrobj_t *array = 0;
  nrotype_t otype;
...
#define LABELS_CLEAN_TESTS_FILE CROSS_AGENT_TESTS_DIR "/labels.json"
  ...
  if (array && (NR_OBJECT_ARRAY == nro_type (array))) {
    for (i = 1; i <= nro_getsize (array); i++) {
      ...
      tlib_pass_if_true ("tests valid", 0 != labelString, "labelString=%p", labelString);
