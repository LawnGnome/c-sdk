...
struct _nr_slowsql_t {
  ...
  uint32_t   sql_id;            /* Hash of obfuscated and normalized sql used for aggregation */
  ...
  char      *params_json;       /* A JSON hash containing a backtrace and possibly an explain plan */
...
uint32_t
nr_slowsql_id (const nr_slowsql_t *slow)
{
  if (slow) {
    return slow->sql_id;
  }
...
int
nr_slowsql_count (const nr_slowsql_t *slow)
{
  if (slow) {
    return slow->count;
  }
...
nrtime_t
nr_slowsql_min (const nr_slowsql_t *slow)
{
  if (slow) {
...
nrtime_t
nr_slowsql_max (const nr_slowsql_t *slow)
{
  if (slow) {
...
nrtime_t
nr_slowsql_total (const nr_slowsql_t *slow)
{
  if (slow) {
    return slow->total;
  }
...
const char *
nr_slowsql_metric (const nr_slowsql_t *slow)
{
  if (slow) {
...
const char *
nr_slowsql_query (const nr_slowsql_t *slow)
{
  if (slow) {
    return slow->sql;
  }
...
const char *
nr_slowsql_params (const nr_slowsql_t *slow)
{
  if (slow) {
...
static nr_slowsql_t *
nr_slowsql_dup (const nr_slowsql_t *orig)
{
...
static void
nr_slowsql_destroy (nr_slowsql_t **slowsql_ptr)
{
...
void
nr_slowsqls_destroy (nr_slowsqls_t **slowsqls_ptr)
{
  int i;
...
nr_slowsqls_t *
nr_slowsqls_create (int max_slowsqls)
{
  nr_slowsqls_t *slowsqls;
  ...
  slowsqls->slowsqls = (nr_slowsql_t **)nr_zalloc (max_slowsqls * sizeof (nr_slowsql_t *));

  return slowsqls;
}

int
nr_slowsqls_saved (const nr_slowsqls_t *slowsqls)
{
  if (slowsqls) {
...
const nr_slowsql_t *
nr_slowsqls_at (const nr_slowsqls_t *slowsqls, int i)
{
  if (NULL == slowsqls) {
    return NULL;
  }
...
static void
nr_slowsqls_add_internal (nr_slowsqls_t *slowsqls, const nr_slowsql_t *slow)
{
  int i;
...
static char *
nr_slowsqls_create_params_json (const nr_slowsqls_params_t *params)
{
  char *json;
  ...
  if (params->instance) {
    if (params->instance_reporting_enabled) {
      ...
      nro_set_hash_string (obj, "port_path_or_id", params->instance->port_path_or_id);      
    }
    if (params->database_name_reporting_enabled) {
      nro_set_hash_string (obj, "database_name", params->instance->database_name);
    }
  }
...
void
nr_slowsqls_add (nr_slowsqls_t *slowsqls, const nr_slowsqls_params_t *params)
{
  nr_slowsql_t slow;
