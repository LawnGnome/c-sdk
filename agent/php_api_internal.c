...
PHP_FUNCTION (newrelic_get_request_metadata)
{
  ...
  if (NRPRG (txn) && NRTXN (special_flags.debug_cat)) {
    nrl_verbosedebug (NRL_CAT,
      ...
      X_NEWRELIC_ID,          NRP_CAT (id),
      X_NEWRELIC_TRANSACTION, NRP_CAT (transaction));
  }
...
PHP_FUNCTION (newrelic_get_metric_table)
{
  ...
  if (FAILURE == zend_parse_parameters (ZEND_NUM_ARGS () TSRMLS_CC, "b", &scoped)) {
    goto end;
  }
...
PHP_FUNCTION (newrelic_get_slowsqls)
{
  ...
  for (i = 0; i < count; i++) {
    ...
    nr_php_add_assoc_string (ss_zv, "metric", nr_remove_const (nr_slowsql_metric (slowsql)));
    nr_php_add_assoc_string (ss_zv, "query", nr_remove_const (nr_slowsql_query (slowsql)));
    nr_php_add_assoc_string (ss_zv, "params", nr_remove_const (nr_slowsql_params (slowsql)));
