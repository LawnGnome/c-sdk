...
/*
 * Set the WT name from the action
 */
...
NR_PHP_WRAPPER (nr_yii_runWithParams_wrapper)
{
  ...
  if (!nr_php_is_zval_valid_object (classz)) {
    nrl_warning (NRL_FRAMEWORK, "getController does not return an object (%d)", Z_TYPE_P (classz));
  } else {
    ...
    if (!nr_php_is_zval_valid_string (idz)) {
      nrl_warning (NRL_FRAMEWORK, "getId does not return a string (%d)", Z_TYPE_P (idz));
    } else {
      id_name = Z_STRVAL_P (idz);
      id_name_length = Z_STRLEN_P (idz);

      if (class_name_length + id_name_length > 256) {
        nrl_warning (NRL_FRAMEWORK, "Yii class and id names are too long (> %d); Yii naming not used", 256);
      } else {
        ...
        nr_txn_set_path ("Yii", NRPRG (txn), buf, NR_PATH_TYPE_ACTION, NR_NOT_OK_TO_OVERWRITE);
      }
    }
  }
 ...
 */
void
nr_yii_enable (TSRMLS_D)
{
  nr_php_wrap_user_function (NR_PSTR (      "CAction::runWithParams"), nr_yii_runWithParams_wrapper TSRMLS_CC);
  nr_php_wrap_user_function (NR_PSTR ("CInlineAction::runWithParams"), nr_yii_runWithParams_wrapper TSRMLS_CC);
}
