...
nr_status_t
nr_txn_freeze_name_update_apdex (nrtxn_t *txn)
{
  return ((const mock_txn *)txn)->freeze_name_return;
}

const char *
nr_txn_get_cat_trip_id (const nrtxn_t *txn)
{
  return txn->cat.trip_id ? txn->cat.trip_id : txn->guid;
}

char *
nr_txn_get_path_hash (nrtxn_t *txn NRUNUSED)
{
  return nr_strdup ("12345678");
}

nrtime_t
nr_txn_queue_time (const nrtxn_t *txn)
{
  return ((const mock_txn *)txn)->fake_queue_time;
}

int
nr_txn_is_account_trusted (const nrtxn_t *txn, int account_id NRUNUSED)
{
  return ((const mock_txn *)txn)->fake_trusted;
}

nrtime_t
nr_txn_unfinished_duration (const nrtxn_t *txn)
{
  return ((const mock_txn *)txn)->unfinished_duration;
}

#define test_metric_created(...) test_header_test_metric_created_fn (__VA_ARGS__, __FILE__, __LINE__)

static void
test_header_test_metric_created_fn (const char *testname, nrmtable_t *metrics, nrtime_t val, uint32_t flags,
                        const char *name, const char *file, int line)
{
  ...
  if (m) {
    ...
    test_pass_if_true (testname, nrm_total (m) == val,
      "name=%s nrm_total (m)=" NR_TIME_FMT " val=" NR_TIME_FMT, name, nrm_total (m), val);
    test_pass_if_true (testname, nrm_exclusive (m) == val,
      "name=%s nrm_exclusive (m)=" NR_TIME_FMT " val=" NR_TIME_FMT, name, nrm_exclusive (m), val);
    test_pass_if_true (testname, nrm_min (m) == val,
      "name=%s nrm_min (m)=" NR_TIME_FMT " val=" NR_TIME_FMT, name, nrm_min (m), val);
    test_pass_if_true (testname, nrm_max (m) == val,
      "name=%s nrm_max (m)=" NR_TIME_FMT " val=" NR_TIME_FMT, name, nrm_max (m), val);
    test_pass_if_true (testname, nrm_sumsquares (m) == (val * val),
      "name=%s nrm_sumsquares (m)=" NR_TIME_FMT " val=" NR_TIME_FMT, name, nrm_sumsquares (m), val);
  }
}

#define test_metrics_empty(T, M) test_metrics_empty_fn ((T), (M),__FILE__, __LINE__)

static void
test_metrics_empty_fn (const char *testname, const nrmtable_t *table, const char *file, int line)
{
...
test_encode_decode (void)
{
  ...
  nro_set_hash_string (app_connect_reply, "encoding_key", "d67afc830dab717fd163bfcb0b8b88423e9a1a3b");
...
test_validate_decoded_id (void)
{
  ...
  rv = nr_header_validate_decoded_id (txn, "0x3039#6789"); /* 0x3039 is 12345 in hex */
...
#define failed_inbound_response_testcase(...) failed_inbound_response_testcase_fn (__VA_ARGS__,__FILE__,__LINE__)

static void
failed_inbound_response_testcase_fn (const char *testname, const nrtxn_t *txn,
                                     const char *response, const char *file, int line)
{
  test_pass_if_true (testname, 0 != txn, "txn=%p", txn);
  test_pass_if_true (testname, 0 == response, "response=%p", response);

  if (txn) {
    test_pass_if_true (testname, NR_STATUS_CROSS_PROCESS_START == txn->status.cross_process,
      "txn->status.cross_process=%d", (int) txn->status.cross_process);
    test_pass_if_true (testname, 0 == txn->status.has_inbound_record_tt,
      "txn->status.has_inbound_record_tt=%d", txn->status.has_inbound_record_tt);
...
static void
test_inbound_response_internal (void)
{
  char *guid;
  nrobj_t *app_connect_reply;
  char *response;
  ...
  app_connect_reply = nro_create_from_json ("{\"cross_process_id\":\"1#1\",\"encoding_key\":\"d67afc830dab717fd163bfcb0b8b88423e9a1a3b\",\"trusted_account_ids\":[12345]}");
  txn->app_connect_reply = app_connect_reply;
   ...
   */
  response = nr_header_inbound_response_internal (txn, -1);
  tlib_pass_if_int_equal ("no decoded_x_newrelic_transaction", txn->status.cross_process, NR_STATUS_CROSS_PROCESS_RESPONSE_CREATED);
  tlib_pass_if_str_equal ("no decoded_x_newrelic_transaction", response, "[\"1#1\",\"txnname\",1.00000,123.00000,-1,\"FEDCBA9876543210\",false]");
  tlib_pass_if_int_equal ("no decoded_x_newrelic_transaction", txn->status.has_inbound_record_tt, 0);
  test_obj_as_json ("no decoded_x_newrelic_transaction", txn->intrinsics, "{\"client_cross_process_id\":\"12345#6789\"}");
  test_metric_created ("no decoded_x_newrelic_transaction", txn->unscoped_metrics, 123000000, 0, "ClientApplication/12345#6789/all");
...
test_inbound_response (void)
{
  ...
  tlib_pass_if_true ("success", 0 != decoded_response, "decoded_response=%p", decoded_response);
...
#define failed_outbound_response_testcase(...) failed_outbound_response_testcase_fn (__VA_ARGS__,__FILE__,__LINE__)

static void
failed_outbound_response_testcase_fn (const char *testname, const nrtxn_t *txn, const char *id,
                                      const char *txnname, const char *guid, const char *file, int line)
{
  test_pass_if_true (testname, 0 != txn, "txn=%p", txn);
  ...
  if (txn) {
    test_pass_if_true (testname, 0 == txn->status.has_outbound_record_tt,
      "txn->status.has_outbound_record_tt=%d", txn->status.has_outbound_record_tt);
  }
}

static void
test_outbound_response_decoded (void)
{
  mock_txn txnv;
  nrtxn_t *txn = &txnv.txn;
  char *id = 0;
   ...
   * Test : Bad Parameters
   */
  nr_header_outbound_response_decoded (0, "[\"12345#6789\", \"txnname\", 1.1, 2.2, -1]", &id, &txnname, &guid);
  ...
  nr_header_outbound_response_decoded (txn, "[\"12345#6789\", \"txnname\"]", &id, &txnname, &guid);
  failed_outbound_response_testcase ("only two elements", txn, id, txnname, guid);

  nr_header_outbound_response_decoded (txn, "[\"12345#6789\", 123, 1.1, 2.2, -1]", &id, &txnname, &guid);
  failed_outbound_response_testcase ("bad txnname", txn, id, txnname, guid);

  nr_header_outbound_response_decoded (txn, "[123, \"txnname\", 1.1, 2.2, -1]", &id, &txnname, &guid);
  failed_outbound_response_testcase ("bad id", txn, id, txnname, guid);

  nr_header_outbound_response_decoded (txn, "[\"99999#6789\", 123, 1.1, 2.2, -1]", &id, &txnname, &guid);
  failed_outbound_response_testcase ("id not in trusted list", txn, id, txnname, guid);

  txn->options.cross_process_enabled = 0;
  nr_header_outbound_response_decoded (txn, "[\"12345#6789\", \"txnname\", 1.1, 2.2, -1]", &id, &txnname, &guid);
  failed_outbound_response_testcase ("cross process disabled", txn, id, txnname, guid);
  txn->options.cross_process_enabled = 1;

  nr_header_outbound_response_decoded (txn, "[\"12345#6789\", \"txnname\", 1.1, 2.2, -1, 123, false]", &id, &txnname, &guid);
  failed_outbound_response_testcase ("bad guid", txn, id, txnname, guid);

  nr_header_outbound_response_decoded (txn, "[\"12345#6789\", \"txnname\", 1.1, 2.2, -1, \"0123456789ABCDEF\", 123]", &id, &txnname, &guid);
  ...
  tlib_pass_if_int_equal ("5 element success", txn->status.has_outbound_record_tt, 0);
  nr_free (id);
  nr_free (txnname);
   ...
   */
  nr_header_outbound_response_decoded (txn, "[\"12345#6789\", \"txnname\", 1.1, 2.2, -1, \"0123456789ABCDEF\", false]", &id, &txnname, &guid);
  ...
  tlib_pass_if_int_equal ("7 element success", txn->status.has_outbound_record_tt, 0);
  nr_free (id);
  nr_free (txnname);
  nr_free (guid);
   ...
   */
  txn->status.has_outbound_record_tt = 0;
  nr_header_outbound_response_decoded (txn, "[\"12345#6789\", \"txnname\", 1.1, 2.2, -1, \"0123456789ABCDEF\", true]", &id, &txnname, &guid);
  ...
  tlib_pass_if_int_equal ("true record_tt", txn->status.has_outbound_record_tt, 1);
  nr_free (id);
  nr_free (txnname);
  nr_free (guid);
   ...
   */
  txn->status.has_outbound_record_tt = 0;
  nr_header_outbound_response_decoded (txn, "[\"12345#6789\", \"txnname\", 1.1, 2.2, -1, \"0123456789ABCDEF\", true]", 0, 0, 0);
  ...
  tlib_pass_if_int_equal ("only record_tt", txn->status.has_outbound_record_tt, 1);

  /*
   * Test : Success with 8 elements (to allow future extension)
   */
  txn->status.has_outbound_record_tt = 0;
  nr_header_outbound_response_decoded (txn, "[\"12345#6789\", \"txnname\", 1.1, 2.2, -1, \"0123456789ABCDEF\", true, \"FUTURISTIC\"]", &id, &txnname, &guid);
  ...
  tlib_pass_if_int_equal ("future support", txn->status.has_outbound_record_tt, 1);
  nr_free (id);
  nr_free (txnname);
  nr_free (guid);

  txn->status.has_outbound_record_tt = 0;
  nr_header_outbound_response_decoded (txn, "[\"12345#6789\",\"Controller/admin/blogs/index\",0.0,0.0,-1,\"1a7b1067d671f6b3\"]", &id, &txnname, &guid);
...
test_outbound_response (void)
{
  ...
  const char *five_element_response = "PxQGU1VXDRAGU1lbFR0XRBBJWF0DCwZAHEIJTAkUFAAdVxVBHFBu";
  /* Encoded: [\"12345#6789\", \"txnname\", 1.1, 2.2, -1, \"0123456789ABCDEF\", true]*/
  const char *seven_element_response = "PxQGU1VXDRAGU1lbFR0XRBBJWF0DCwZAHEIJTAkUFAAdVxVBHFAfQkYGBlNVVw0FB1xYI3VycyMiExoTFhQWB20=";
  ...
  tlib_pass_if_int_equal ("null response", txn->status.has_outbound_record_tt, 0);
  ...
  tlib_pass_if_int_equal ("no app_connect_reply", txn->status.has_outbound_record_tt, 0);
  txn->app_connect_reply = app_connect_reply;
  ...
  tlib_pass_if_int_equal ("empty response", txn->status.has_outbound_record_tt, 0);
  ...
  tlib_pass_if_int_equal ("junk response", txn->status.has_outbound_record_tt, 0);
   ...
   */
  nr_header_outbound_response (txn, seven_element_response, &id, &txnname, &guid);
  tlib_pass_if_str_equal ("5 element success", "12345#6789", id);
  tlib_pass_if_str_equal ("5 element success", "txnname", txnname);
  tlib_pass_if_str_equal ("5 element success", "0123456789ABCDEF", guid);
  tlib_pass_if_int_equal ("5 element success", txn->status.has_outbound_record_tt, 1);
  nr_free (id);
  nr_free (txnname);
  nr_free (guid);
...
test_outbound_request (void)
{
   ...
   * Test : Bad Parameters
   */
  nr_header_outbound_request (0, &x_newrelic_id, &x_newrelic_transaction, &x_newrelic_synthetics);
  ...
  txn->options.cross_process_enabled = 0;
  nr_header_outbound_request (txn, &x_newrelic_id, &x_newrelic_transaction, &x_newrelic_synthetics);
  ...
  tlib_pass_if_str_equal ("success", "PxQHUFRQDAYGU1lbdnN0IiF3FB8EBw8RVU4aUgkKBwYGUw5ZCCBxISBzcUNKQQkBA1BUVAAJFTs=", x_newrelic_transaction);
  tlib_pass_if_null ("success", x_newrelic_synthetics);
  tlib_fail_if_int_equal ("txn type", 0, NR_TXN_TYPE_CAT_OUTBOUND & txn->type);

  decoded_x_newrelic_id = nr_deobfuscate (x_newrelic_id, "d67afc830dab717fd163bfcb0b8b88423e9a1a3b", 0);
  decoded_x_newrelic_transaction = nr_deobfuscate (x_newrelic_transaction, "d67afc830dab717fd163bfcb0b8b88423e9a1a3b", 0);

  tlib_pass_if_str_equal ("success", "12345#6789", decoded_x_newrelic_id);
  tlib_pass_if_str_equal ("success", "[\"0123456789ABCDEF\",false,\"0123456789ABCDEF\",\"12345678\"]", decoded_x_newrelic_transaction);
  ...
  tlib_pass_if_str_equal ("synthetics", "PxQHUFRQDAYGU1lbdnN0IiF3FB8EBw8RVU4aUgkKBwYGUw5ZCCBxISBzcUNKQQkBA1BUVAAJFTs=", x_newrelic_transaction);
  tlib_pass_if_str_equal ("synthetics", "PwcbUFZTFBFRRk1AVRMbRAcTaw==", x_newrelic_synthetics);

  decoded_x_newrelic_id = nr_deobfuscate (x_newrelic_id, "d67afc830dab717fd163bfcb0b8b88423e9a1a3b", 0);
  decoded_x_newrelic_transaction = nr_deobfuscate (x_newrelic_transaction, "d67afc830dab717fd163bfcb0b8b88423e9a1a3b", 0);
  decoded_x_newrelic_synthetics = nr_deobfuscate (x_newrelic_synthetics, "d67afc830dab717fd163bfcb0b8b88423e9a1a3b", 0);

  tlib_pass_if_str_equal ("synthetics", "12345#6789", decoded_x_newrelic_id);
  tlib_pass_if_str_equal ("synthetics", "[\"0123456789ABCDEF\",false,\"0123456789ABCDEF\",\"12345678\"]", decoded_x_newrelic_transaction);
  tlib_pass_if_str_equal ("synthetics", "[1,100,\"a\",\"b\",\"c\"]", decoded_x_newrelic_synthetics);
  ...
  tlib_pass_if_str_equal ("synthetics", "VQQEVVNADgQIXQ==", x_newrelic_id);
  tlib_pass_if_str_equal ("synthetics", "PxQHUFRQDAYGU1lbdnN0IiF3FB8EBw8RVU4aUgkKBwYGUw5ZCCBxISBzcUNKQQkBA1BUVAAJFTs=", x_newrelic_transaction);
  tlib_pass_if_null ("synthetics", x_newrelic_synthetics);
...
test_lifecycle (void)
{
  ...
  shared_app_connect_reply = nro_create_from_json ("{\"cross_process_id\":\"12345#6789\",\"encoding_key\":\"d67afc830dab717fd163bfcb0b8b88423e9a1a3b\",\"trusted_account_ids\":[12345]}");
   ...
   */
  nr_header_outbound_request (client_txn, &x_newrelic_id, &x_newrelic_transaction, &x_newrelic_synthetics);
  ...
  test_obj_as_json ("full lifecycle", external_txn->intrinsics,
    "{\"referring_transaction_guid\":\"CLIENT_GUID\",\"client_cross_process_id\":\"12345#6789\"}");
  tlib_pass_if_str_equal ("full lifecycle", external_id, "12345#6789");
  tlib_pass_if_str_equal ("full lifecycle", external_txnname, "EXTERNAL_TXNNAME");
...
#define extract_testcase(...) extract_testcase_fn (__VA_ARGS__,__FILE__,__LINE__)

static void
extract_testcase_fn (const char *testname, const char *header_name,
                     ...
                     const char *file, int line)
{
  ...
  test_pass_if_true (testname, 0 == nr_strcmp (expected, actual),
    "input=%s expected=%s actual=%s header_name=%s", NRSAFESTR (input), NRSAFESTR (expected),
    NRSAFESTR (actual), NRSAFESTR (header_name));
  nr_free (actual);
}

static void
test_extract_encoded_value (void)
{
...
static void
test_validate_encoded_string (void)
{
  nr_status_t rv;
...
test_bad_content_type (void)
{
  ...
  tlib_pass_if_null ("all whitespace content-type", nr_header_parse_content_type ("     "));
  tlib_pass_if_null ("missing media-type", nr_header_parse_content_type ("Content-Type: ; foo=bar"));
  tlib_pass_if_null ("missing media-type type", nr_header_parse_content_type ("Content-Type: /html"));
  tlib_pass_if_null ("missing media subtype", nr_header_parse_content_type ("Content-Type: text;"));
  tlib_pass_if_null ("empty media subtype", nr_header_parse_content_type ("Content-Type: text/"));
  tlib_pass_if_null ("invalid media subtype", nr_header_parse_content_type ("Content-Type: audio/mp3[]"));
  tlib_pass_if_null ("leading colon", nr_header_parse_content_type (":text/html"));
  tlib_pass_if_null ("double colon", nr_header_parse_content_type ("Content-Type::text/html"));
}

static void
test_extract_content_type (void)
{
  char *mimetype;
  ...
  mimetype = nr_header_parse_content_type ("Content-Type: \t  text/html");
  tlib_pass_if_str_equal ("content-type leading whitespace", mimetype, "text/html");
  nr_free (mimetype);

  mimetype = nr_header_parse_content_type ("Content-Type:text/html \t ");
  tlib_pass_if_str_equal ("content-type trailing whitespace", mimetype, "text/html");
  nr_free (mimetype);

  mimetype = nr_header_parse_content_type ("Content-Type: text/html; charset=\"utf-8\"");
  tlib_pass_if_str_equal ("content-type with charset", mimetype, "text/html");
  nr_free (mimetype);

  mimetype = nr_header_parse_content_type ("Content-Type:text/html;charset=\"utf-8\";foo=bar;");
  tlib_pass_if_str_equal ("content-type with multiple parameters", mimetype, "text/html");
  nr_free (mimetype);
  ...
  tlib_pass_if_str_equal ("content-type with no name and with charset", mimetype, "text/html");
  nr_free (mimetype);

  mimetype = nr_header_parse_content_type ("text/html");
  tlib_pass_if_str_equal ("content-type with only media-type", mimetype, "text/html");
  nr_free (mimetype);

  mimetype = nr_header_parse_content_type ("     text/html");
  tlib_pass_if_str_equal ("content-type with leading whitespace and only media-type", mimetype, "text/html");
  nr_free (mimetype);
...
test_set_cat_txn (void)
{
  ...
  encoded_transaction_v1 = nr_obfuscate (trusted_transaction_v1, ENCODING_KEY, 0);
  encoded_transaction_v2 = nr_obfuscate (trusted_transaction_v2, ENCODING_KEY, 0);

  txn->app_connect_reply = nro_create_from_json ("{\"trusted_account_ids\":[1,3],\"encoding_key\":\"" ENCODING_KEY "\"}");
   ...
   * Test : Bad parameters.
   */
  tlib_pass_if_status_failure ("NULL txn",        nr_header_set_cat_txn (NULL, encoded_id, encoded_transaction_v1));
  tlib_pass_if_status_failure ("NULL id header",  nr_header_set_cat_txn (txn,  NULL,       encoded_transaction_v1));

  /*
   * Test : Invalid headers.
   */
  tlib_pass_if_status_failure ("invalid id header",  nr_header_set_cat_txn (txn, trusted_id, encoded_transaction_v1));
  tlib_pass_if_int_equal ("invalid id header", 0, txn->type);

  tlib_pass_if_status_failure ("invalid txn header", nr_header_set_cat_txn (txn, encoded_id, trusted_transaction_v1));
  tlib_pass_if_str_equal ("invalid txn header", trusted_id, txn->cat.client_cross_process_id);
  nr_free (txn->cat.client_cross_process_id);
   ...
   */
  txnv.fake_trusted = 0;
  tlib_pass_if_status_failure ("untrusted", nr_header_set_cat_txn (txn, encoded_id, encoded_transaction_v1));
   ...
   * Test : No txn header
   */
  tlib_pass_if_status_failure ("Only X-NewRelic-ID present", nr_header_set_cat_txn (txn, encoded_id, NULL));
  tlib_pass_if_str_equal ("Only X-NewRelic-ID present", trusted_id, txn->cat.client_cross_process_id);
  nr_free (txn->cat.client_cross_process_id);
  ...
  tlib_pass_if_status_success ("CATv1", nr_header_set_cat_txn (txn, encoded_id, encoded_transaction_v1));
  tlib_pass_if_str_equal ("CATv1 type", trusted_id, txn->cat.client_cross_process_id);
  tlib_pass_if_true ("CATv1 type", NR_TXN_TYPE_CAT_INBOUND & txn->type, "txn->type=%d", txn->type);
  tlib_pass_if_str_equal ("CATv1 guid", "guid", txn->cat.inbound_guid);
  tlib_pass_if_int_equal ("CATv1 record_tt", 0, txn->status.has_inbound_record_tt);
   ...
   */
  txn->status.has_inbound_record_tt = 1;
  tlib_pass_if_status_success ("CATv2", nr_header_set_cat_txn (txn, encoded_id, encoded_transaction_v2));
  tlib_pass_if_str_equal ("CATv2 type", trusted_id, txn->cat.client_cross_process_id);
  tlib_pass_if_true ("CATv2 type", NR_TXN_TYPE_CAT_INBOUND & txn->type, "txn->type=%d", txn->type);
  tlib_pass_if_str_equal ("CATv2 guid", "guid", txn->cat.inbound_guid);
  tlib_pass_if_int_equal ("CATv2 record_tt", 0, txn->status.has_inbound_record_tt);
  tlib_pass_if_str_equal ("CATv2 referring_path_hash", "01234567", txn->cat.referring_path_hash);
...
test_set_synthetics_txn (void)
{
   ...
   * Test : Bad parameters.
   */
  tlib_pass_if_status_failure ("NULL txn", nr_header_set_synthetics_txn (NULL, encoded));

  tlib_pass_if_status_failure ("NULL header", nr_header_set_synthetics_txn (txn, NULL));
  ...
  tlib_pass_if_status_failure ("synthetics txn", nr_header_set_synthetics_txn (txn, encoded));
  nr_synthetics_destroy (&txn->synthetics);

  /*
   * Test : Invalid header.
   */
  tlib_pass_if_status_failure ("invalid header", nr_header_set_synthetics_txn (txn, "foo"));
   ...
   */
  txnv.fake_trusted = 0;
  tlib_pass_if_status_failure ("untrusted", nr_header_set_synthetics_txn (txn, encoded));
   ...
   */
  txnv.fake_trusted = 1;
  tlib_pass_if_status_success ("valid", nr_header_set_synthetics_txn (txn, encoded));
  tlib_pass_if_not_null ("valid", txn->synthetics);
  tlib_pass_if_true ("valid", NR_TXN_TYPE_SYNTHETICS & txn->type, "txn->type=%d", txn->type);
  nr_synthetics_destroy (&txn->synthetics);
...
static void
test_account_id_from_cross_process_id (void)
{
  /*
   * Test : Bad parameters.
   */
  tlib_pass_if_int64_t_equal ("NULL string", -1, nr_header_account_id_from_cross_process_id (NULL));
  tlib_pass_if_int64_t_equal ("empty string", -1, nr_header_account_id_from_cross_process_id (""));
  tlib_pass_if_int64_t_equal ("malformed string", -1, nr_header_account_id_from_cross_process_id ("foo"));
  tlib_pass_if_int64_t_equal ("malformed string", -1, nr_header_account_id_from_cross_process_id ("foo#bar"));
  tlib_pass_if_int64_t_equal ("missing hash", -1, nr_header_account_id_from_cross_process_id ("01234"));
  tlib_pass_if_int64_t_equal ("overflow", -1, nr_header_account_id_from_cross_process_id ("3000000000#1"));

  /*
   * Test : Good parameters.
   */
  tlib_pass_if_int64_t_equal ("normal operation", 10, nr_header_account_id_from_cross_process_id ("10#10"));
}
