...
test_save_datastore_instance (TSRMLS_D)
{
  ...
  assert_datastore_instance_equals ("null conn and null host_and_port", expected_default,
  ...
  assert_datastore_instance_equals ("null conn and empty host_and_port", expected_default,
    nr_hashmap_get (NRPRG (datastore_connections), key, nr_strlen (key)));
  ...
  assert_datastore_instance_equals ("null host_and_port", expected_default,
    nr_hashmap_get (NRPRG (datastore_connections), key, nr_strlen (key)));

  nr_hashmap_delete (NRPRG (datastore_connections), key, nr_strlen (key));
  nr_php_mysql_save_datastore_instance (conn, "" TSRMLS_CC);
  assert_datastore_instance_equals ("empty host_and_port", expected_default,
    nr_hashmap_get (NRPRG (datastore_connections), key, nr_strlen (key)));
  ...
  assert_datastore_instance_equals ("same conn won't save new instance", expected_default,
    nr_hashmap_get (NRPRG (datastore_connections), key, nr_strlen (key)));
...
test_retrieve_datastore_instance (TSRMLS_D)
{
  ...
  tlib_pass_if_null ("an unknown non-null connection should not update the global",
   ...
   */
  assert_datastore_instance_equals ("unknown null connection saves a default instance", expected,
...
static void
test_host_and_port_path_or_id (const char *host_and_port,
  const char *expected_host, const char *expected_port_path_or_id)
{
  char *host = NULL;
  char *port_path_or_id = NULL;

  nr_php_mysql_get_host_and_port_path_or_id (host_and_port, &host, &port_path_or_id);
  tlib_pass_if_str_equal ("correct host", expected_host, host);
  tlib_pass_if_str_equal ("correct port_path_or_id", expected_port_path_or_id, port_path_or_id);
...
test_get_host_and_port_path_or_id ()
{
  ...
  test_host_and_port_path_or_id ("localhost:", "localhost", DEFAULT_SOCKET);
  test_host_and_port_path_or_id (":", "localhost", DEFAULT_SOCKET);
  test_host_and_port_path_or_id ("localhost:/path/to/socket", "localhost", "/path/to/socket");
  test_host_and_port_path_or_id (":/path/to/socket", "localhost", "/path/to/socket");

  /*
   * Test: Non-localhost
   */
  test_host_and_port_path_or_id ("blue:star", "blue", "star");
  test_host_and_port_path_or_id ("blue:/path/to/socket", "blue", "/path/to/socket");
...
static void
test_instance (const char *message, const char *host_and_port,
...
test_create_datastore_instance ()
{
   ...
   * Test: Bad input
   */
  test_instance ("null", NULL, &((nr_datastore_instance_t) {
    .database_name   = DEFAULT_DATABASE_NAME,
    .host            = system_host_name,
    .port_path_or_id = DEFAULT_SOCKET,
  }));

  test_instance ("empty", "", &((nr_datastore_instance_t) {
    .database_name   = DEFAULT_DATABASE_NAME,
    .host            = system_host_name,
    .port_path_or_id = DEFAULT_SOCKET,
  }));
   ...
   * Test: localhost
   */
  test_instance ("localhost", "localhost", &((nr_datastore_instance_t) {
    .database_name   = DEFAULT_DATABASE_NAME,
    .host            = system_host_name,
    .port_path_or_id = DEFAULT_SOCKET,
  }));
   ...
   * Test: Non-localhost
   */
  test_instance ("non-localhost", "blue:1234", &((nr_datastore_instance_t) {
    .database_name   = DEFAULT_DATABASE_NAME,
    .host            = "blue",
