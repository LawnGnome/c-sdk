...
int
nr_php_output_has_content (int flags)
{
...
void
nr_php_output_install_handler (const char *name,
                               php_output_handler_func_t handler TSRMLS_DC)
{
  if (0 == name) {
    return;
  }
...
#if ZEND_MODULE_API_NO >= ZEND_5_4_X_API_NO
  {
    ...
    php_output_start_internal (name, name_len, handler, chunk_size, flags TSRMLS_CC);
  }
#else /* PHP < 5.4 */
  /* Everything else before it */
  {
    ...
    php_ob_set_internal_handler (handler, buffer_size, name_duplicate, erase TSRMLS_CC);
  }
#endif
}

int
nr_php_output_is_end (int flags)
{
  return flags & PHP_OUTPUT_HANDLER_END;
}

int
nr_php_output_is_start (int flags)
{
