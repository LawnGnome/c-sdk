 ...
 */
int
nr_attribute_destination_modifier_match (
...
uint32_t
nr_attribute_destination_modifier_apply (
  const nr_attribute_destination_modifier_t *modifier,
  const char *key,
  uint32_t key_hash,
...
void
nr_attribute_destination_modifier_destroy (nr_attribute_destination_modifier_t **entry_ptr)
{
...
nr_attribute_config_create (void)
{
...
nr_attribute_destination_modifier_create (
  const char            *match,
  ...
  uint32_t               exclude_destinations)
{
  ...
  new_entry = (nr_attribute_destination_modifier_t *)nr_zalloc (sizeof (nr_attribute_destination_modifier_t));
 ...
 */
static int
nr_attribute_destination_modifier_compare (
  ...
  uint32_t               exclude_destinations)
{
  ...
  new_entry = nr_attribute_destination_modifier_create (match, include_destinations, exclude_destinations);
  if (0 == new_entry) {
    return;
  }
...
nr_attribute_destination_modifier_copy (const nr_attribute_destination_modifier_t *entry)
{
  nr_attribute_destination_modifier_t *new_entry;

  new_entry = (nr_attribute_destination_modifier_t *)nr_zalloc (sizeof (*new_entry));
...
nr_attribute_config_t *
nr_attribute_config_copy (const nr_attribute_config_t *config)
{
  ...
  for (entry = config->modifier_list; entry; entry = entry->next) {
    nr_attribute_destination_modifier_t *new_entry = nr_attribute_destination_modifier_copy (entry);
  ...
  uint32_t destinations)
{
  ...
  for (modifier = config->modifier_list; modifier; modifier = modifier->next) {
    destinations = nr_attribute_destination_modifier_apply (modifier, key, key_hash, destinations);
  }
...
void
nr_attribute_config_destroy (nr_attribute_config_t **config_ptr)
{
  nr_attribute_destination_modifier_t *modifier;
  nr_attribute_config_t *config;
...
nr_attributes_t *
nr_attributes_create (const nr_attribute_config_t *config)
{
...
void
nr_attribute_destroy (nr_attribute_t **attribute_ptr)
{
...
static void
nr_attribute_list_destroy (nr_attribute_t *attribute)
{
  while (attribute) {
...
void
nr_attributes_destroy (nr_attributes_t **attributes_ptr)
{
  nr_attributes_t *attributes;
...
void
nr_attributes_remove_duplicate (nr_attributes_t *ats, const char *key, uint32_t key_hash, int is_user)
{
  nr_attribute_t *attribute;
...
static void
nr_attributes_log_destination_change (const char *key, uint32_t default_destinations, uint32_t final_destinations)
{
  nrl_verbosedebug (NRL_TXN, "attribute '%.128s' destinations modified by configuration: "
...
static int
nr_attributes_is_valid_value (const nrobj_t *value)
{
  double dbl;
  ...
  const nrobj_t   *value)
{
  ...
  if (nr_strlen (key) > NR_ATTRIBUTE_KEY_LENGTH_LIMIT) {
    if (is_user) {
      nrl_warning (NRL_TXN, "potential attribute discarded: key '%.128s' exceeds size limit %d",
       ...
       */
      nrl_debug (NRL_TXN, "potential attribute discarded: key '%.128s' exceeds size limit %d",
        key, NR_ATTRIBUTE_KEY_LENGTH_LIMIT);
    }
    return NR_FAILURE;
  }
  ...
  final_destinations = nr_attribute_config_apply (ats->config, key, key_hash, default_destinations);

  if (0 == final_destinations) {
    /* There is no purpose in saving attributes which will not be used. */
    nrl_verbosedebug (NRL_TXN, "attribute '%.128s' disabled by configuration", key);
    return NR_FAILURE;
  }

  if (final_destinations != default_destinations) {
    nr_attributes_log_destination_change (key, default_destinations, final_destinations);
  }
  ...
  if (is_user && (NR_ATTRIBUTE_USER_LIMIT == ats->num_user_attributes)) {
    ...
    nrl_warning (NRL_TXN, "attribute '%.128s' discarded: user limit of %d reached.",
      key, NR_ATTRIBUTE_USER_LIMIT);
    return NR_FAILURE;
  }
  ...
  const nrobj_t   *value)
{
  ...
  if (NR_OBJECT_STRING != nro_type (value)) {
    return nr_attributes_add_internal (ats, default_destinations, is_user, key, value);
  }
  ...
  rv = nr_attributes_add_internal (ats, default_destinations, is_user, key, bounded_value);
  nro_delete (bounded_value);
  return rv;
}

nr_status_t
nr_attributes_user_add (nr_attributes_t *ats, uint32_t default_destinations, const char *key, const nrobj_t *value)
{
  return nr_attributes_add (ats, default_destinations, 1, key, value);
}

nr_status_t
nr_attributes_user_add_string (nr_attributes_t *ats, uint32_t default_destinations, const char *key, const char *value)
{
  nr_status_t rv;
...
nr_status_t
nr_attributes_user_add_long (nr_attributes_t *ats, uint32_t default_destinations, const char *key, int64_t lng)
{
  nr_status_t rv;
...
nr_status_t
nr_attributes_agent_add_long (nr_attributes_t *ats, uint32_t default_destinations, const char *key, int64_t lng)
{
  nr_status_t rv;
...
nr_status_t
nr_attributes_agent_add_string (nr_attributes_t *ats, uint32_t default_destinations, const char *key, const char *str)
{
  nr_status_t rv;
...
static nrobj_t *
nr_attributes_to_obj_internal (const nr_attribute_t *attribute_list, uint32_t destination)
{
  nrobj_t *obj;
...
nrobj_t *
nr_attributes_user_to_obj (const nr_attributes_t *attributes, uint32_t destination)
{
  if (0 == attributes) {
    return 0;
  }
  return nr_attributes_to_obj_internal (attributes->user_attribute_list, destination);
}

nrobj_t *
nr_attributes_agent_to_obj (const nr_attributes_t *attributes, uint32_t destination)
{
  if (0 == attributes) {
    return 0;
  }
  return nr_attributes_to_obj_internal (attributes->agent_attribute_list, destination);
}

static char *
nr_attribute_debug_json (const nr_attribute_t *attribute)
{
  nrobj_t *dests;
  nrobj_t *obj;
  char *json;
 ...
 */
char *
nr_attributes_debug_json (const nr_attributes_t *attributes)
{
  nrobj_t *obj;
  nrobj_t *agent;
  nrobj_t *user;
  ...
  for (attribute = attributes->user_attribute_list; attribute; attribute = attribute->next) {
  ...
  for (attribute = attributes->agent_attribute_list; attribute; attribute = attribute->next) {
    json = nr_attribute_debug_json (attribute);
