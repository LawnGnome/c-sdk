           ...
           const char *agent_specific_info)
{
  ...
  snprintf (process_information, sizeof (process_information), " pid=%d ppid=%d uid=%d euid=%d gid=%d egid=%d",
    nr_getpid (),
    nr_getppid (),
    nr_getuid (),
    nr_geteuid (),
...

  {
    ...
    if (sys) {
      snprintf (osinfo, sizeof (osinfo), " os='%s' rel='%s' mach='%s' ver='%s' node='%s'",
