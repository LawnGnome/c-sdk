...
static char *
nr_slim_path_from_route (zval *route TSRMLS_DC)
{
  zval *name = NULL;
  zval *pattern = NULL;
  ...
  if (pattern) {
    ...
    nrl_verbosedebug (NRL_FRAMEWORK, "Slim: unable to read route pattern property");
  }
...
NR_PHP_WRAPPER (nr_slim2_route_dispatch)
{
  ...
  if (txn_name && retval_ptr && nr_php_is_zval_true (*retval_ptr)) {
    nr_txn_set_path ("Slim", NRPRG (txn), txn_name, NR_PATH_TYPE_ACTION, NR_OK_TO_OVERWRITE);
  }
 ...
 * to a route.
 */
NR_PHP_WRAPPER (nr_slim3_route_run)
{
  zval *this_var = NULL;
  char *txn_name = NULL;
  ...
  if (txn_name) {
    nr_txn_set_path ("Slim", NRPRG (txn), txn_name, NR_PATH_TYPE_ACTION, NR_NOT_OK_TO_OVERWRITE);
    nr_free (txn_name);
  }
...
void
nr_slim_enable (TSRMLS_D)
{
  NR_UNUSED_TSRMLS;

  nr_php_wrap_user_function (NR_PSTR ("Slim\\Route::dispatch"), nr_slim2_route_dispatch TSRMLS_CC);
  nr_php_wrap_user_function (NR_PSTR ("Slim\\Route::run"), nr_slim3_route_run TSRMLS_CC);
}
