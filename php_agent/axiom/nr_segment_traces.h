 ...
 *
 * The userdata is structured to accomodate that: the topmost struct holds data
...
typedef struct {
  ...
  nr_set_t* sample; /* The set of segments that should be added to the list of
                       spans */
  nr_vector_t* current_path; /* The path of ancestor segments that were added to
                                the list of spans; used to determine state in
                                the post traversal callback */
  nr_vector_t*
      current_span_path; /* The path of ancestor span events. This is a list of
                            span events corresponding to the segments in
                            current_path; used for parenting of span events */
 ...
 *           collector.  If a segment is a member of metadata->trace_set, it is
 *           added to the transaction trace JSON. If metadata->trace_set is
 *           NULL, all segments are added.
 *
