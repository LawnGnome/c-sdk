 ...
 * ======================
 *
 * Fundamentally, our Predis instrumentation relies on looking for matched
 ...
 * writeRequest() becomes writeCommand(), and on Predis 0.7 -- which is the
 * oldest version we support -- the interfaces are generally called IFoo
 ...
 * parallel. Until version 7.0 of the PHP agent, we supported these only to the
 * extent of reporting each pipeline as its own operation, but we were unable to
 * break out the individual commands. From version 7.0 onwards, we use the
 * agent's limited async support to correctly break out each command.
 *
 ...
 * the appropriate default behaviour from the datastore instance functions, but
...
char *
nr_predis_get_operation_name_from_object (zval *command_obj TSRMLS_DC)
{
  char *lower = NULL;
  zval *op;

  if (!nr_predis_is_command (command_obj TSRMLS_CC)) {
    nrl_verbosedebug (NRL_INSTRUMENT, "Predis command does not seem to be a command.");
    return NULL;
  }
...
nr_predis_create_datastore_instance_from_fields (zval *scheme,
                                                 zval *host,
                                                 zval *port,
                                                 zval *path,
                                                 zval *database)
...
nr_datastore_instance_t *
nr_predis_create_datastore_instance_from_array (zval *params)
{
...
nr_datastore_instance_t *
nr_predis_create_datastore_instance_from_parameters_object (zval *params
                                                            TSRMLS_DC)
{
...
nr_datastore_instance_t *
nr_predis_create_datastore_instance_from_string (zval *params TSRMLS_DC)
{
  zval *database = NULL;
  zval *host = NULL;
  zval *path = NULL;
  ...
  if (!nr_php_is_zval_valid_array (parts)) {
    nrl_verbosedebug (NRL_INSTRUMENT,
                      "%s: parse_url failed on string %s",
  ...
  if (nr_php_is_zval_valid_string (query)) {
    zval *query_parts = nr_php_parse_str (Z_STRVAL_P (query),
                                          Z_STRLEN_P (query) TSRMLS_CC);

    if (nr_php_is_zval_valid_array (query_parts)) {
      zval *database_zv = nr_php_zend_hash_find (Z_ARRVAL_P (query_parts), "database");
  ...
  instance = nr_predis_create_datastore_instance_from_fields (scheme, host,
                                                              port, path,
                                                              database);
...
nr_datastore_instance_t *
nr_predis_create_datastore_instance_from_connection_params (zval *params
                                                            TSRMLS_DC)
{
  /*
  ...
  if (nr_php_is_zval_valid_string (params)) {
    ...
    return nr_predis_create_datastore_instance_from_parameters_object (params
                                                                       TSRMLS_CC);
     ...
     * at the connection parameters. (Also, why the hell would your connection
     * parameter callable have side effects?)
     */
    retval = nr_php_call_callable (params);

    /*
     * Since we should have something this function can chew on, we'll just
     * recursively call back into it.
     */
    instance = nr_predis_create_datastore_instance_from_connection_params (retval
                                                                           TSRMLS_CC);
    ...
    /*
     * Log something, since this means the input is just straight up invalid.
     *
     * Technically, boolean values (which are exempted above) are also invalid,
     * but Laravel is buggy and provides a boolean when instantiating the
   ...
   * we'll let the defaults handle everything, because that's effectively what
   * Predis is going to do anyway.
   */
  return nr_predis_create_datastore_instance_from_fields (NULL, NULL, NULL,
                                                          NULL, NULL);
}

nr_datastore_instance_t *
nr_predis_save_datastore_instance (const zval *conn,
                                   zval *params TSRMLS_DC)
{
  ...
  instance = nr_predis_create_datastore_instance_from_connection_params (params
                                                                         TSRMLS_CC);
...
nr_datastore_instance_t *
nr_predis_retrieve_datastore_instance (const zval *conn TSRMLS_DC)
{
  char *key;
  nr_datastore_instance_t *instance;
...
zval *
nr_predis_get_parameter (zval *params, const char *name TSRMLS_DC)
{
  zval *name_zv;
  zval *retval;
 ...
 */
static inline int
nr_predis_is_object_one_of (const zval *obj, const char *classes[],
...
int
nr_predis_is_aggregate_connection (const zval *obj TSRMLS_DC)
{
  return nr_predis_is_object_one_of (obj, ((const char *[2]) {
...
int
nr_predis_is_command (const zval *obj TSRMLS_DC)
{
  return nr_predis_is_object_one_of (obj, ((const char *[2]) {
...
int
nr_predis_is_connection (const zval *obj TSRMLS_DC)
{
  return nr_predis_is_object_one_of (obj, ((const char *[2]) {
...
int
nr_predis_is_node_connection (const zval *obj TSRMLS_DC)
{
  return nr_predis_is_object_one_of (obj, ((const char *[2]) {
...
int
nr_predis_is_parameters (const zval *obj TSRMLS_DC)
{
  return nr_predis_is_object_one_of (obj, ((const char *[2]) {
...
NR_PHP_WRAPPER (nr_predis_aggregateconnection_getConnection)
{
  ...
  if (NULL == retval_ptr) {
    nrl_verbosedebug (NRL_INSTRUMENT,
                      "%s: error retrieving return value pointer",
                      __func__);
    ...
    /*
     * Lazily add the datastore instance metadata if it hasn't already happened.
     */
...
NR_PHP_WRAPPER (nr_predis_client_construct)
{
  ...
  if (nr_predis_is_aggregate_connection (conn TSRMLS_CC)) {
     ...
     * were given to instrument the connection and create the datastore instance
     * metadata right here.
     */
...
nr_predis_enable (TSRMLS_D)
{
   ...
   */
  nr_php_wrap_user_function (NR_PSTR ("Predis\\Client::__construct"), nr_predis_client_construct TSRMLS_CC);

  /*
   * Instrument the pipeline classes that are bundled with Predis so that we
   * correctly set up async contexts.
   */
  nr_php_wrap_user_function (NR_PSTR ("Predis\\Pipeline\\Pipeline::executePipeline"), nr_predis_pipeline_executePipeline TSRMLS_CC);
  nr_php_wrap_user_function (NR_PSTR ("Predis\\Pipeline\\Atomic::executePipeline"), nr_predis_pipeline_executePipeline TSRMLS_CC);
  nr_php_wrap_user_function (NR_PSTR ("Predis\\Pipeline\\ConnectionErrorProof::executePipeline"), nr_predis_pipeline_executePipeline TSRMLS_CC);
  nr_php_wrap_user_function (NR_PSTR ("Predis\\Pipeline\\FireAndForget::executePipeline"), nr_predis_pipeline_executePipeline TSRMLS_CC);

  /*
   * Instrument Webdis connections, since they don't use the same
   * writeRequest()/readResponse() pair as the other connection types.
   */
  nr_php_wrap_user_function (NR_PSTR ("Predis\\Connection\\WebdisConnection::executeCommand"), nr_predis_webdisconnection_executeCommand TSRMLS_CC);
}
