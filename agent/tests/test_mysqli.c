...
test_save_datastore_instance (TSRMLS_D)
{
  ...
  assert_datastore_instance_equals ("null conn and null host", expected_default,
  ...
  assert_datastore_instance_equals ("null conn and empty host", expected_default,
    nr_hashmap_get (NRPRG (datastore_connections), key, nr_strlen (key)));
  ...
  assert_datastore_instance_equals ("null host saves default instance", expected_default,
    nr_hashmap_get (NRPRG (datastore_connections), key, nr_strlen (key)));

  nr_php_mysqli_save_datastore_instance (conn, "blue", 0, NULL, NULL TSRMLS_CC);
  assert_datastore_instance_equals ("same conn saves new instance", expected,
    nr_hashmap_get (NRPRG (datastore_connections), key, nr_strlen (key)));

  nr_hashmap_delete (NRPRG (datastore_connections), key, nr_strlen (key));
  nr_php_mysqli_save_datastore_instance (conn, "blue", 0, NULL, NULL TSRMLS_CC);
  assert_datastore_instance_equals ("new conn saves new instance", expected,
    nr_hashmap_get (NRPRG (datastore_connections), key, nr_strlen (key)));
...
test_retrieve_datastore_instance (TSRMLS_D)
{
  ...
  assert_datastore_instance_equals ("connection info is found", expected,
    nr_php_mysqli_retrieve_datastore_instance (NULL TSRMLS_CC));
...
static void
test_remove_datastore_instance (TSRMLS_D)
{
  zval *conn;
  char *key = NULL;
  ...
  tlib_pass_if_int_equal ("removing unknown non-null connection has no effect", 0,
    nr_php_datastore_has_conn (key TSRMLS_CC));
...
static void
test_host_and_port_path_or_id_early_return (void) {
   ...
   */
  nr_php_mysqli_get_host_and_port_path_or_id ("", 0, "", &host, &port_path_or_id);
...
static void
test_host_and_port_path_or_id (const char *message, const char *host_param,
  zend_long port, const char *socket, const char *expected_host,
  const char *expected_port_path_or_id)
{
  char *host = NULL;
  char *port_path_or_id = NULL;

  nr_php_mysqli_get_host_and_port_path_or_id (host_param, port, socket,
...
test_get_host_and_port_path_or_id ()
{
  ...
  test_host_and_port_path_or_id ("empty host", "", 0, NULL, "localhost", DEFAULT_SOCKET);
  test_host_and_port_path_or_id ("null host", NULL, 0, NULL, "localhost", DEFAULT_SOCKET);

  /*
   * Test: Localhost
   */
  test_host_and_port_path_or_id ("localhost", "localhost", 0, NULL, "localhost", DEFAULT_SOCKET);
  test_host_and_port_path_or_id ("localhost ignores port", "localhost", 1234, NULL, "localhost", DEFAULT_SOCKET);
  test_host_and_port_path_or_id ("localhost ignores port with empty socket", "localhost", 1234, "", "localhost", DEFAULT_SOCKET);
  test_host_and_port_path_or_id ("localhost custom socket", "localhost", 0, "/path/to/socket", "localhost", "/path/to/socket");
  test_host_and_port_path_or_id ("localhost custom socket ignores port", "localhost", 4321, "/path/to/socket","localhost", "/path/to/socket");

  /*
   * Test: Looks like localhost, but isn't
   */
  test_host_and_port_path_or_id ("colons not meaningful", "localhost:", 0, NULL, "localhost:", DEFAULT_PORT);
  test_host_and_port_path_or_id ("colons not meaningful", "localhost:/path/to/socket", 0, NULL, "localhost:/path/to/socket", DEFAULT_PORT);
  test_host_and_port_path_or_id ("colons not meaningful", ":/path/to/socket", 0, NULL, ":/path/to/socket", DEFAULT_PORT);

  /*
   * Test: Non-localhost
   */
  test_host_and_port_path_or_id ("non-localhost socket ignored", "blue", 0, "/path/to/socket", "blue", DEFAULT_PORT);
  test_host_and_port_path_or_id ("non-localhost socket ignored", "blue", 42, "", "blue", "42");
  test_host_and_port_path_or_id ("weird host and port", "12", 41, NULL, "12", "41");
}

static void
test_instance (const char *message, const char *host_param, zend_long port,
  const char *socket, const char *database, const nr_datastore_instance_t *expected)
{
  nr_datastore_instance_t *actual;

  actual = nr_php_mysqli_create_datastore_instance (host_param, port, socket, database);
...
test_create_datastore_instance ()
{
   ...
   * Test: Bad input
   */
  test_instance ("null", NULL, 0, NULL, NULL, &((nr_datastore_instance_t) {
    .database_name   = DEFAULT_DATABASE_NAME,
    .host            = system_host_name,
    .port_path_or_id = DEFAULT_SOCKET,
  }));

  test_instance ("empty", "", 0, "", "", &((nr_datastore_instance_t) {
    .database_name   = DEFAULT_DATABASE_NAME,
    .host            = system_host_name,
    .port_path_or_id = DEFAULT_SOCKET,
  }));
   ...
   * Test: localhost
   */
  test_instance ("localhost without db", "localhost", 0, NULL, NULL, &((nr_datastore_instance_t) {
    .database_name   = DEFAULT_DATABASE_NAME,
    .host            = system_host_name,
    .port_path_or_id = DEFAULT_SOCKET,
  }));

  test_instance ("localhost with db", "localhost", 0, NULL, "lemon_poppyseed", &((nr_datastore_instance_t) {
   ...
   * Test: Non-localhost
   */
  test_instance ("non-localhost without db", "blue", 1234, NULL, NULL, &((nr_datastore_instance_t) {
    .database_name   = DEFAULT_DATABASE_NAME,
    .host            = "blue",
    .port_path_or_id = "1234",
  }));

  test_instance ("non-localhost with db", "blue", 1234, NULL, "lemon_poppyseed", &((nr_datastore_instance_t) {
    .database_name   = "lemon_poppyseed",
    .host            = "blue",
    .port_path_or_id = "1234",
  }));
