...
#ifdef PHP7
static int
nr_php_zend_hash_ptr_apply_wrapper (zval *value, int num_args, va_list args,
...
#else
static int
nr_php_zend_hash_ptr_apply_wrapper (void *value TSRMLS_DC,
...
void
nr_php_zend_hash_ptr_apply (HashTable *ht,
                            nr_php_ptr_apply_t apply_func,
                            void *arg TSRMLS_DC)
{
  zend_hash_apply_with_arguments (ht TSRMLS_CC,
                                  (apply_func_args_t) nr_php_zend_hash_ptr_apply_wrapper,
                                  2, apply_func, arg);
}

#ifdef PHP7
static int
nr_php_zend_hash_zval_apply_wrapper (zval *value, int num_args, va_list args,
                                     zend_hash_key *hash_key)
{
...
#else
static int
nr_php_zend_hash_zval_apply_wrapper (zval **value TSRMLS_DC,
                                     int num_args, va_list args,
                                     zend_hash_key *hash_key)
{
  nr_php_zval_apply_t apply_func;
...
void
nr_php_zend_hash_zval_apply (HashTable *ht,
                             nr_php_zval_apply_t apply_func,
                             void *arg TSRMLS_DC)
{
  zend_hash_apply_with_arguments (ht TSRMLS_CC,
                                  (apply_func_args_t) nr_php_zend_hash_zval_apply_wrapper,
                                  2, apply_func, arg);
}

int
nr_php_zend_hash_del (HashTable *ht, const char *key)
{
  if ((NULL == ht) || (NULL == key)) {
    return 0;
  }
...
int
nr_php_zend_hash_exists (const HashTable *ht, const char *key)
{
  /*
...
#ifdef PHP7
zval *
nr_php_zend_hash_find (const HashTable *ht, const char *key)
{
...
void *
nr_php_zend_hash_find_ptr (const HashTable *ht, const char *key)
{
  if ((NULL == ht) || (NULL == key) || ('\0' == key[0])) {
    return NULL;
  }
...
zval *
nr_php_zend_hash_index_find (const HashTable *ht, ulong index)
{
  if (NULL == ht) {
    return NULL;
  }
