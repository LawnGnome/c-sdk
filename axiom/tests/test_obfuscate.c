...
test_main (void *p NRUNUSED)
{
  ...
  tlib_pass_if_true ("obfuscate success", 0 == nr_strcmp (rv, "NikyPBs8OisiJg=="), "rv=%s", NRSAFESTR (rv));
  rp = nr_deobfuscate (rv, "BLAHHHH", 0);
  tlib_pass_if_true ("deobfuscate success", 0 == nr_strcmp (rp, "testString"), "rp=%s", NRSAFESTR (rp));
  nr_free (rv);
  nr_free (rp);
  ...
  tlib_pass_if_true ("obfuscate success", 0 == nr_strcmp (rv, "NikyNh81MCUvJQ=="), "rv=%s", NRSAFESTR (rv));
