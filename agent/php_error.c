...
void
nr_php_exception_filters_init (zend_llist *chain)
{
  if (chain) {
...
void
nr_php_exception_filters_destroy (zend_llist *chain)
{
  if (chain) {
...
static int
nr_php_exception_filters_compare (void *a, void *b)
{
...
nr_status_t
nr_php_exception_filters_add (zend_llist *chain, nr_php_exception_filter_fn fn)
{
...
nr_status_t
nr_php_exception_filters_remove (zend_llist *chain, nr_php_exception_filter_fn fn)
{
  if (chain) {
...
static nr_php_exception_action_t
nr_php_exception_filters_apply (zend_llist *chain, zval *exception TSRMLS_DC)
{
...
nr_php_ignore_exceptions_ini_filter (zval *exception TSRMLS_DC)
{
  nrobj_t *names;
  ...
  for (int i = 1, n = nro_getsize (names); i <= n; i++) {
    ...
    if (name && nr_php_class_entry_instanceof_class (Z_OBJCE_P (exception), name TSRMLS_CC)) {
...
PHP_FUNCTION (newrelic_exception_handler)
{
  ...
  if ((FAILURE == zend_parse_parameters_ex (ZEND_PARSE_PARAMS_QUIET,
      ...
      (NULL == exception)) {
    /*
     * There isn't much useful that we can do here. Let's log this weirdness
     * and generate an error to get the hell out.
     */
    ...
    /*
     * zend_error won't return for an E_ERROR, but just in case, let's salt the
     * earth and return.
     */
    return;
  }
  ...
  /*
   * Rightio. Let's use this exception to generate an error. The error
   * priority is set to NR_PHP_ERROR_PRIORITY_UNCAUGHT_EXCEPTION to override
   * anything else, _including_ API noticed errors (in case the user uses
   * newrelic_notice_error as their error handler with prioritize_api_errors
   * enabled).
   */
  nr_php_error_record_exception (NRPRG (txn), exception,
    NR_PHP_ERROR_PRIORITY_UNCAUGHT_EXCEPTION, "Uncaught exception ",
...
int
nr_php_error_get_priority (int type)
{
  switch (type) {
    case E_PARSE:
      return 50;
...
void
nr_php_error_install_exception_handler (TSRMLS_D)
{
  ...
  if (has_user_exception_handler) {
     ...
     */
#ifdef PHP7
    zend_stack_push (&EG (user_exception_handlers),
  ...
  /*
   * Actually allocate and set the user_exception_handler zval. PHP itself will
   * destroy this at the end of the request.
   */
#ifdef PHP7
 ...
 */
static zval *
nr_php_error_exception_stack_trace (zval *exception TSRMLS_DC)
{
  zval *trace;
 ...
 */
static char *
nr_php_error_exception_file (zval *exception TSRMLS_DC)
{
 ...
 */
static long
nr_php_error_exception_line (zval *exception TSRMLS_DC)
{
  long line = 0;
 ...
 */
static char *
nr_php_error_exception_message (zval *exception TSRMLS_DC)
{
  /*
...
static const char *
get_error_type_string (int type)
{
  switch (type) {
    case E_ERROR:
      return "E_ERROR";
...
static int
nr_php_should_record_error (int type, const char *format TSRMLS_DC)
{
  int errprio;
  ...
  /*
   * Note: The sense of this check is reversed compared to the error_reporting setting.
   */
...
void
nr_php_error_cb (int type, const char *error_filename, uint error_lineno, const char *format, va_list args)
{
  TSRMLS_FETCH ();
  ...
  if (nr_php_should_record_error (type, format TSRMLS_CC)) {
    ...
    nr_txn_record_error (NRPRG (txn), nr_php_error_get_priority (type), msg, errclass, stack_json);
  ...
  if (0 != NR_PHP_PROCESS_GLOBALS (orig_error_cb)) {
    NR_PHP_PROCESS_GLOBALS (orig_error_cb) (type, error_filename, error_lineno, format, args);
  }
}

nr_status_t
nr_php_error_record_exception (nrtxn_t *txn, zval *exception, int priority,
  const char *prefix, zend_llist *filters TSRMLS_DC)
{
  zend_class_entry *ce;
  ...
  if ((NULL == txn) || (0 == nr_php_error_zval_is_exception (exception TSRMLS_CC))) {
    return NR_FAILURE;
  }
  ...
  if (file && line) {
    if (message) {
      error_message = nr_formatf ("%s'%s' with message '%s' in %s:%ld", prefix, klass, message, file, line);
    } else {
      error_message = nr_formatf ("%s'%s' in %s:%ld", prefix, klass, file, line);
    }
  } else if (message) {
    error_message = nr_formatf ("%s'%s' with message '%s'", prefix, klass, message);
  } else {
...
int
nr_php_error_zval_is_exception (zval *zv TSRMLS_DC)
{
#ifdef PHP7
