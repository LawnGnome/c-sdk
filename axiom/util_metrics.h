 ...
 *
 * Unfortunately, there is no spec and little consistency between agents
 * in the maximum number of metrics.
 *
 * TODO(willhf) This number should be determined using the maximum collector
 * message payload size.  (This is not an immediate big problem:  If the
 * metric command payload is too large for the collector, all metrics will
 * be saved for the next harvest cycle, but this will only happen a certain
 * number of times before all metrics will be discarded).
 */
 ...
 *           been reached.
 */
extern void nrm_add_ex       (nrmtable_t *table, const char *name, nrtime_t duration, nrtime_t exclusive);
extern void nrm_force_add_ex (nrmtable_t *table, const char *name, nrtime_t duration, nrtime_t exclusive);
extern void nrm_add          (nrmtable_t *table, const char *name, nrtime_t duration);
extern void nrm_force_add    (nrmtable_t *table, const char *name, nrtime_t duration);

extern void nrm_add_apdex       (nrmtable_t *table, const char *name,
                                 nrtime_t satisfying, nrtime_t tolerating, nrtime_t failing, nrtime_t apdex);
extern void nrm_force_add_apdex (nrmtable_t *table, const char *name,
                                 nrtime_t satisfying, nrtime_t tolerating, nrtime_t failing, nrtime_t apdex);
 ...
 */
extern void nrm_add_internal (int force, nrmtable_t *table, const char *name,
                              nrtime_t count, nrtime_t total, nrtime_t exclusive, nrtime_t min,
                              nrtime_t max, nrtime_t sum_of_squares);
extern void nrm_add_apdex_internal (int force, nrmtable_t *table, const char *name,
                                    nrtime_t satisfying, nrtime_t tolerating,
                                    nrtime_t failing, nrtime_t min_apdex, nrtime_t max_apdex);
 ...
 *           not found.
 */
extern void nrm_duplicate_metric (nrmtable_t *table, const char *current_name, const char *new_name);
