...
nruserfn_t *
nr_php_wrap_user_function (const char *name, size_t namelen,
  ...
  if (wraprec && callback) {
    if ((NULL != wraprec->special_instrumentation) &&
      ...
      nrl_verbosedebug (NRL_INSTRUMENT,
                        ...
                        "it is already set",
                        __func__,
                        NRSAFELEN (namelen), NRBLANKSTR (name));
    } else {
...
nruserfn_t *
nr_php_wrap_user_function_extra (const char *name, size_t namelen,
                                 nrspecialfn_t callback,
                                 const char *extra TSRMLS_DC)
{
  nruserfn_t *wraprec = nr_php_wrap_user_function (name, namelen, callback TSRMLS_CC);
  wraprec->extra = extra;
...
nruserfn_t *
nr_php_wrap_callable (zend_function *callable, nrspecialfn_t callback TSRMLS_DC)
{
...
zval *
nr_php_arg_get (ssize_t index, NR_EXECUTE_PROTO TSRMLS_DC)
{
  zval *arg;
...
#else
  arg = nr_php_get_user_func_arg ((zend_uint) index, NR_EXECUTE_ORIG_ARGS TSRMLS_CC);
...
void
nr_php_arg_release (zval **ppzv)
{
  release_zval (ppzv);
}

zval *
nr_php_scope_get (NR_EXECUTE_PROTO TSRMLS_DC)
{
  zval *this_ptr;
...
void
nr_php_scope_release (zval **ppzv)
{
  release_zval (ppzv);
}

zval **
nr_php_get_return_value_ptr (TSRMLS_D)
{
#ifdef PHP7
