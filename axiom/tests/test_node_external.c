...
static nr_node_external_params_t mock_external_params_url(nrtime_t duration,
                                                          char* url) {
  nr_node_external_params_t params = {
      .start = {.stamp = 1, .when = 1 * NR_TIME_DIVISOR},
      .stop = {.stamp = 2, .when = 1 * NR_TIME_DIVISOR + duration},
      .url = url,
      .urllen = nr_strlen(url),
  };

  return params;
}

static nr_node_external_params_t mock_external_params(nrtime_t duration) {
  return mock_external_params_url(duration, "newrelic.com");
}
...
static void test_node_rollup(void) {
  nrtxn_t txnv;
  nrtxn_t* txn = &txnv;
  nrtxnnode_t rollup;
  nrtxntime_t stop;
  ...
  tlib_pass_if_true("no start", (NR_FAILURE == rv) && (0 == rollup.count),
                    "rv=%d rollup.count=%d", (int)rv, rollup.count);

  rv = nr_txn_node_rollup(txn, &start, 0, "myname");
  tlib_pass_if_true("no stop", (NR_FAILURE == rv) && (0 == rollup.count),
                    "rv=%d rollup.count=%d", (int)rv, rollup.count);
  ...
  tlib_pass_if_true("no last added", (NR_FAILURE == rv) && (0 == rollup.count),
                    "rv=%d rollup.count=%d", (int)rv, rollup.count);
  txn->last_added = &rollup;
  ...
  rv = nr_txn_node_rollup(txn, &start, &stop, "myname");
  tlib_pass_if_true("rollup has kids",
                    (NR_FAILURE == rv) && (0 == rollup.count),
                    "rv=%d rollup.count=%d", (int)rv, rollup.count);
  rollup.start_time.stamp = 7;
  ...
  tlib_pass_if_true("node rollup count", 1 == rollup.count, "rollup.count=%d",
                    rollup.count);
  ...
  tlib_pass_if_true("node rollup start time", 100 == rollup.start_time.when,
                    "rollup.start_time.when=" NR_TIME_FMT,
                    rollup.start_time.when);
  tlib_pass_if_true("node rollup stop time", 500 == rollup.stop_time.when,
                    "rollup.stop_time.when=" NR_TIME_FMT,
                    rollup.stop_time.when);
...
static void test_bad_parameters(void) {
  ...
  nr_node_external_params_t params = mock_external_params(duration);
  nrtxntime_t start = params.start;
  nrtxntime_t stop = params.stop;

  nr_txn_end_node_external(NULL, NULL);
  nr_txn_end_node_external(txn, NULL);
  nr_txn_end_node_external(NULL, &params);

  params.start = stop;
  params.stop = start;
  nr_txn_end_node_external(txn, &params);
  test_txn_untouched("stop time is before start time", txn);
  params.start = start;
  params.stop = stop;
...
static void test_zero_urllen(void) {
...
static void test_domain_parsing_fails(void) {
  ...
  nrtime_t duration = 4 * NR_TIME_DIVISOR;
  nr_node_external_params_t params
      = mock_external_params_url(duration, "@@@@@");
 ...
 */
static void test_junk_url(void) {
  const char* tname = "junk url";
  nrtxn_t* txn = new_txn(0);
  nrtime_t duration = 4 * NR_TIME_DIVISOR;
  nr_node_external_params_t params
      = mock_external_params_url(duration, "this is a junk url");

  nr_txn_end_node_external(txn, &params);
  tlib_pass_if_time_equal(tname, txn->root_kids_duration, duration);
  test_one_node_populated(tname, txn, "External/this is a junk url/all",
                          duration, "{\"uri\":\"this is a junk url\"}");
  test_metric_table_size(tname, txn->scoped_metrics, 1);
  test_metric_table_size(tname, txn->unscoped_metrics, 1);
  test_metric_created(tname, txn->unscoped_metrics, MET_FORCED, duration,
                      "External/all");
  test_metric_created(tname, txn->scoped_metrics, 0, duration,
                      "External/this is a junk url/all");
  nr_txn_destroy(&txn);
}
...
static void test_url_saving_strips_parameters(void) {
  ...
  nrtxn_t* txn = new_txn(0);
  nrtime_t duration = 4 * NR_TIME_DIVISOR;
  nr_node_external_params_t params = mock_external_params_url(
      duration, "http://newrelic.com?secret=shhhhhh");
...
static void test_node_rollup_prevention(void) {
  /*
  ...
  params.start.stamp = 3;
  params.stop.stamp = 4;
  params.start.when = 3 * NR_TIME_DIVISOR;
  params.stop.when = params.start.when + duration;
  params.do_rollup = true;
  nr_txn_end_node_external(txn, &params);
  tlib_pass_if_true("node rollup prevents new node", 1 == txn->nodes_used,
                    "txn->nodes_used=%d", txn->nodes_used);
  tlib_pass_if_true("node rollup increases count", 1 == txn->nodes[0].count,
                    "txn->nodes[0].count=%d", txn->nodes[0].count);
  test_metric_table_size(tname, txn->scoped_metrics, 1);
  test_metric_table_size(tname, txn->unscoped_metrics, 1);
  nr_txn_destroy(&txn);
}
...
static void test_only_external_id(void) {
  /* No cross process metrics here: Both external_id and external_txnname are
   * needed. */
  ...
  params.encoded_response_header = "{\"id\":\"12345#6789\"}";
...
static void test_only_external_txnname(void) {
  ...
  params.encoded_response_header = "{\"txnname\":\"my_txn\"}";
...
static void test_external_id_and_txnname(void) {
  ...
  params.encoded_response_header
  ...
  test_metric_created(tname, txn->unscoped_metrics, 0, duration,
                      "External/newrelic.com/all");
  test_metric_created(tname, txn->unscoped_metrics, 0, duration,
                      "ExternalApp/newrelic.com/12345#6789/all");
  test_metric_created(tname, txn->scoped_metrics, 0, duration,
                      "ExternalTransaction/newrelic.com/12345#6789/my_txn");
  nr_txn_destroy(&txn);
}

static void test_external_id_txnname_and_guid(void) {
  ...
  params.encoded_response_header
      = "{\"id\":\"12345#6789\",\"txnname\":\"my_txn\",\"guid\":"
        "\"0123456789ABCDEF\"}";

  nr_txn_end_node_external(txn, &params);
  tlib_pass_if_time_equal(tname, txn->root_kids_duration, duration);
  test_one_node_populated(
      tname, txn, "ExternalTransaction/newrelic.com/12345#6789/my_txn",
      duration,
...
static void test_async_external_call(void) {
