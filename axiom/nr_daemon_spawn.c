...
void
nr_argv_append (nr_argv_t *argv, const char *flag_or_value)
{
...
void
nr_argv_append_flag (nr_argv_t *argv, const char *flag, const char *fmt, ...)
{
  int rv;
  va_list ap;
  char *tmp = NULL;
...
void
nr_argv_destroy (nr_argv_t *argv)
{
  size_t i;
...
nr_argv_t *
nr_daemon_args_to_argv (const char *name, const nr_daemon_args_t *args)
{
  nr_argv_t *argv = NULL;
  ...
  if (args) {
    ...
    if (args->app_timeout && ('\0' != args->app_timeout[0])) {
      nr_argv_append_flag (argv, "--define", "app_timeout=%s", args->app_timeout);
    }

    /* utilization */
    nr_argv_append_flag (argv, "--define", "utilization.detect_aws=%s",     args->utilization.aws     ? "true" : "false");
    nr_argv_append_flag (argv, "--define", "utilization.detect_azure=%s",   args->utilization.azure   ? "true" : "false");
    nr_argv_append_flag (argv, "--define", "utilization.detect_gcp=%s",     args->utilization.gcp     ? "true" : "false");
    nr_argv_append_flag (argv, "--define", "utilization.detect_pcf=%s",     args->utilization.pcf     ? "true" : "false");
    nr_argv_append_flag (argv, "--define", "utilization.detect_docker=%s",  args->utilization.docker  ? "true" : "false");
...
pid_t
nr_spawn_daemon (const char *path, const nr_daemon_args_t *args)
{
  pid_t dpid;
  int ret;
  int logfd = -1;
  ...
  if (-1 == nr_access (path, X_OK)) {
    nrl_warning (NRL_INIT, "couldn't find daemon=" NRP_FMT " (%.16s)", NRP_FILENAME (path), nr_errno (errno));
    return -1;
  }
  ...
  for (i = 0; i < argv->count; i++) {
    nrl_verbosedebug (NRL_INIT, "exec[%zu]=" NRP_FMT, i, NRP_PROCARG (argv->data[i]));
  }
  ...
  if (0 != ret) {
    nrl_warning (NRL_INIT, "failed to spawn daemon: (%.16s) - please start it manually", nr_errno (errno));
  }
  _exit (0);
}
