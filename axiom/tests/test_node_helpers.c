...
void test_metric_table_size_fn(const char* testname,
...
void test_txn_untouched_fn(const char* testname,
                           const nrtxn_t* txn,
                           const char* file,
                           int line) {
...
void test_node_helper_metric_created_fn(const char* testname,
                                        nrmtable_t* metrics,
                                        uint32_t flags,
                                        nrtime_t duration,
                                        const char* name,
...
void test_one_node_populated_fn(const char* testname,
                                const nrtxn_t* txn,
  ...
  test_pass_if_true(testname, 1 == txn->nodes_used, "txn->nodes_used=%d",
                    txn->nodes_used);
  ...
  test_pass_if_true(testname, n->stop_time.when > n->start_time.when,
                    "n->stop_time.when=" NR_TIME_FMT
                    "n->start_time.when=" NR_TIME_FMT,
                    n->stop_time.when, n->start_time.when);
  test_pass_if_true(testname, n->stop_time.stamp > n->start_time.stamp,
                    "n->stop_time.stamp=%d n->start_time.stamp=%d",
                    n->stop_time.stamp, n->start_time.stamp);

  test_pass_if_true(testname,
                    duration == (n->stop_time.when - n->start_time.when),
                    "duration= " NR_TIME_FMT "n->stop_time.when " NR_TIME_FMT
                    "n->start_time.when " NR_TIME_FMT,
                    duration, n->stop_time.when, n->start_time.when);
  ...
  test_pass_if_true(testname, 0 == nr_strcmp(expected_name, name),
                    "expected_name=%s name=%s", NRSAFESTR(expected_name),
                    NRSAFESTR(name));

  if (NULL == expected_data_hash) {
    test_pass_if_true(testname, NULL == n->data_hash, "data_hash=%p",
                      n->data_hash);
  } else {
    char* data_hash = nro_to_json(n->data_hash);

    test_pass_if_true(testname, 0 == nr_strcmp(data_hash, expected_data_hash),
                      "data_hash=%s expected=%s", NRSAFESTR(data_hash),
...
nrtxn_t* new_txn(int background) {
  nrapp_t app;
  nrtxn_t* txn;
