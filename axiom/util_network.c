...
static nr_status_t
nr_wait_fd (int fd, int events, nrtime_t deadline)
{
  struct pollfd pfd;
  int timeout_msec;
...
nr_status_t
nr_write_full (int fd, const void *buf, size_t len, nrtime_t deadline)
{
  const char *p;
  ssize_t remaining;
  int err;
...
nr_status_t
nr_write_message (int fd, const void *buf, size_t len, nrtime_t deadline)
{
  nrbuf_t *header;
  nr_status_t st;
  ...
  /*
   * TODO(msl): It would be better to use writev(2) here to prevent short
   * reads in the daemon, which are costly and latency inducing at high RPM.
   */

  st = nr_write_full (fd, nr_buffer_cptr (header), nr_buffer_len (header), deadline);
...
static nrbuf_t *
nrn_read_internal (int fd, int nbytes, nrtime_t deadline, char *tmp)
{
  int      remaining;
...
nrbuf_t *
nrn_read (int fd, int nbytes, nrtime_t deadline)
{
  char *tmp;
  nrbuf_t *reply;
...
void
nr_protocol_write_preamble (nrbuf_t *buf, uint32_t datalen)
{
...
nr_status_t
nr_protocol_parse_preamble (nrbuf_t *buf, uint32_t *return_length)
{
  uint32_t length = 0;
  uint32_t format = 0;
  ...
  if (NR_PREAMBLE_FORMAT != format) {
    nrl_error (NRL_NETWORK, "parse preamble failure: invalid format: %u", format);
    return NR_FAILURE;
  }

  if (length > NR_PROTOCOL_CMDLEN_MAX_BYTES) {
    nrl_error (NRL_NETWORK, "parse preamble failure: invalid length: %d", length);
    return NR_FAILURE;
  }
...
nrbuf_t *
nr_network_receive (int fd, nrtime_t deadline)
{
  nrbuf_t *preamble;
  nrbuf_t *msg;
  nr_status_t st;
  ...
  if (NULL == preamble) {
    nrl_error (NRL_NETWORK, "failed to read reply preamble: fd=%d errno="
      NRP_FMT_UQ, fd, NRP_STRERROR (nr_errno (errno)));
    return NULL;
  }
  ...
  if (NULL == msg) {
    nrl_error (NRL_NETWORK, "failed to read reply msg: len=%d fd=%d errno="
      NRP_FMT_UQ, len, fd, NRP_STRERROR (nr_errno (errno)));
    return NULL;
  }
  return msg;
}
...
nr_status_t
nr_network_set_non_blocking (int fd)
{
  int flags;
