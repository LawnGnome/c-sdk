...
nr_php_pdo_execute_query (zval *stmt, zval *parameters TSRMLS_DC)
{
  ...
  if (parameters) {
    ...
    if (NULL == result) {
      nrl_verbosedebug (NRL_SQL, "%s: error calling PDOStatement::execute with parameters", __func__);
      return NR_FAILURE;
    }
  } else {
    result = nr_php_call (stmt, "execute");
    if (NULL == result) {
      nrl_verbosedebug (NRL_SQL, "%s: error calling PDOStatement::execute", __func__);
      return NR_FAILURE;
    }
  }
  ...
  if (!nr_php_is_zval_true (result)) {
    ...
    if (nr_php_is_zval_valid_array (error_info)) {
      ...
      if (nr_php_is_zval_valid_string (message)) {
        nrl_verbosedebug (NRL_SQL,
                          "%s: PDOStatement::execute failed with error %.*s",
                          __func__,
                          (int) Z_STRLEN_P (message),
                          Z_STRVAL_P (message));
     ...
     */
    nrl_verbosedebug (NRL_SQL,
                      "%s: PDOStatement::execute failed, and no error information is available",
                      __func__);
    nr_php_zval_free (&error_info);
    goto end;
  }
...
nr_php_pdo_prepare_query (zval *dbh, const char *query TSRMLS_DC)
{
  ...
  if (!nr_php_object_instanceof_class (stmt, "PDOStatement" TSRMLS_CC)) {
    nrl_verbosedebug (NRL_SQL, "%s: prepare did not return a PDOStatement", __func__);
...
nr_php_pdo_rebind_parameters (zval *source, zval *destination TSRMLS_DC)
{
  ...
  if ((NULL != pdo_stmt) && (NULL != pdo_stmt->bound_params)) {
    nr_php_zend_hash_ptr_apply (pdo_stmt->bound_params,
...
int
nr_php_pdo_rebind_apply_parameter (struct pdo_bound_param_data *param,
                                   zval *stmt,
                                   zend_hash_key *hash_key TSRMLS_DC)
{
  ...
  if (nr_php_zend_hash_key_is_string (hash_key)) {
    /*
     * String keys require no munging, and can just be reused.
     */
    nr_php_zval_str_len (key,
                         nr_php_zend_hash_key_string_value (hash_key),
...
pdo_dbh_t *
nr_php_pdo_get_database_object_from_object (zval *obj TSRMLS_DC)
{
  if (NULL == obj) {
    return NULL;
  }
...
const char *
nr_php_pdo_get_driver_internal (pdo_dbh_t *dbh)
{
  if (NULL == dbh) {
    return NULL;
  }
...
nr_datastore_t
nr_php_pdo_get_datastore_for_driver (const char *driver_name)
{
  size_t i;
...
nr_datastore_t
nr_php_pdo_get_datastore_internal (pdo_dbh_t *dbh)
{
...
char *
nr_php_pdo_datastore_make_key (pdo_dbh_t *dbh)
{
...
static const size_t num_instance_handlers = sizeof (instance_handlers) /
                                            sizeof (instance_handlers[0]);
...
nr_php_pdo_get_datastore_instance (zval *obj TSRMLS_DC)
{
   ...
   * that.
   *
   * TODO(aharvey): unify with datastore instance API when it's split out.
   */
  ...
  /*
   * OK, it's not in the cache. Let's rotate the board!
   */
...
{
  nr_datastore_t datastore;
  pdo_dbh_t *dbh = nr_php_pdo_get_database_object_from_object (stmt_obj TSRMLS_CC);
  nr_datastore_instance_t *instance;
  nr_explain_plan_t *plan = NULL;

  if (try_explain) {
    plan = nr_php_explain_pdo_statement (txn, stmt_obj, parameters,
                                         start, stop TSRMLS_CC);
  }

  datastore = nr_php_pdo_get_datastore_internal (dbh);
  instance = nr_php_pdo_get_datastore_instance (stmt_obj TSRMLS_CC);
  nr_php_txn_end_node_sql (txn, start, stop, sqlstr, sqlstrlen, plan,
                           datastore, instance TSRMLS_CC);

  nr_explain_plan_destroy (&plan);
}

static zval *
nr_php_pdo_options_get (zval *dbh TSRMLS_DC)
{
...
nr_php_pdo_duplicate (zval *dbh TSRMLS_DC)
{
  ...
  dsn_len = asprintf (&dsn, "%s:%.*s", driver,
                      NRSAFELEN (pdo_dbh->data_source_len),
                      pdo_dbh->data_source);
  ...
  /*
   * OK, let's create the object and construct it.
   */
...
nr_php_pdo_options_save (zval *dbh, zval *options TSRMLS_DC)
{
   ...
   */
  if (NULL == NRPRG (pdo_link_options)) {
    NRPRG (pdo_link_options) = nr_hashmap_create ((nr_hashmap_dtor_func_t) free_options);
  }
 ...
 * Fortunately, its signature doesn't change between PHP 5 and 7, with the
 * exception of the way zend_ulong itself changed.
 */
extern int __attribute__((weak))
php_pdo_parse_data_source (const char *data_source, zend_ulong data_source_len,
                              ...
                              size_t nparams)
{
  if (!php_pdo_parse_data_source) {
    nrl_verbosedebug (NRL_SQL,
                      "%s: php_pdo_parse_data_source() unavailable",
                      __func__);
    return NR_FAILURE;
  }
  ...
  if (nparams > INT_MAX) {
    nrl_verbosedebug (NRL_SQL,
                      "%s: invalid number of parameters provided: %zu",
                      __func__, nparams);
    return NR_FAILURE;
  }

  php_pdo_parse_data_source (data_source, (zend_ulong) data_source_len,
                             parsed, (int) nparams);
  return NR_SUCCESS;
}
