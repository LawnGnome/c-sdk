...
type Error struct {
	...
	// limit to the number of errors is reached.  Higher numbers have
...
// Pop removes the lowest priority element.
...
func (h *ErrorHeap) AddError(priority int, dataNeedsCopy []byte) {
	if len(*h) == cap(*h) {
		// If the priority is the same, errors are saved on a first
