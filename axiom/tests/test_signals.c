...
static void
nr_test_signals_do_kill (nr_signaller_t signaller, int signaller_arg)
{
  if (0 != signaller) {
 ...
 * may call malloc, which is verboten in signal handlers. The thread
 * sanitizer discovers this, and will complain about it.
 */

static void
sigsegv_signal_tracer (int sig)
{
...
static void
sigfpe_real_signal_tracer (int sig)
{
  nr_signal_tracer_common (sig);
...
static void
send_signal (int signal_to_send)
{
  kill (nr_getpid (), signal_to_send);  /* assumes synchronous send and receive to self */
}

#if defined(TEST_DODGY_SIGNALS) /* { */
static int
nr_test_signals_zero (void)
{
  return 0;
}

static void
do_integer_zero_divide (int arg NRUNUSED)
{
  int x = 0;
...
static void
do_integer_corner_divide (int arg NRUNUSED)
{
  int x = 0;

  (void) x;
  nrl_send_log_message (NRL_ALWAYS, "before integer INT_MIN / -1 divide x=%d", x);
  x = INT_MIN / (nr_test_signals_zero () - 1);  /* eg, INT_MIN / -1 , which isn't representable */
  nrl_send_log_message (NRL_ALWAYS, "after  integer INT_MIN / -1 divide x=%d", x);
}
#endif /* } */

/*
 * Execute backtrace and related just for the side effect
 * of loading the DSO that does that,
 * so that we don't call malloc from a signal handler.
 */
static void
test_signals_prime_backtrace (void)
{
#ifdef HAVE_BACKTRACE
  void *array[100];
  size_t size;
