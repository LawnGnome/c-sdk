...
static int
nr_kohana_is_external_request (zval *request TSRMLS_DC)
{
  int is_external = 0;
...
NR_PHP_WRAPPER (nr_kohana_name_the_wt)
{
  ...
  if (!nr_php_is_zval_valid_object (this_var)) {
    nrl_verbosedebug (NRL_FRAMEWORK, "Kohana: improper this");
    goto leave;
  }

  if (nr_kohana_is_external_request (this_var TSRMLS_CC)) {
    nrl_verbosedebug (NRL_FRAMEWORK, "Kohana: request is external, no name this time");
    goto leave;
  }
  ...
  if ((0 == nr_php_object_has_method (this_var, "route"      TSRMLS_CC)) ||
    ...
    nrl_verbosedebug (NRL_FRAMEWORK, "Kohana: object does not quack like a Kohana_Request");
    goto leave;
  }
  ...
  if (nr_php_is_zval_valid_object (route)) {
    ...
    nr_txn_set_path ("Kohana", NRPRG (txn), name, NR_PATH_TYPE_ACTION, NR_OK_TO_OVERWRITE);
  }
 ...
 */
void
nr_kohana_enable (TSRMLS_D)
{
   /* We set the path to 'unknown' to prevent having to name routing errors. */
  nr_txn_set_path ("Kohana", NRPRG (txn), "unknown", NR_PATH_TYPE_ACTION, NR_NOT_OK_TO_OVERWRITE);  
  nr_php_wrap_user_function (NR_PSTR ("Kohana_Request::execute"), nr_kohana_name_the_wt TSRMLS_CC);
}
