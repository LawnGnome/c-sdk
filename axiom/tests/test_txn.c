...
static void test_freeze_name_update_apdex(void) {
   ...
   */
  {
    ...
    /*             Test Name                         path
     * is_apdex_f   apdex_t   tt_threshold */
...
static void test_apdex_metrics_fn(const char* txn_name,
  ...
  if (has_error) {
    ...
    txn->error = nr_error_create(priority, "my/msg", "my/class",
...
static void test_create_apdex_metrics(void) {
  ...
  /*                                                        has error
   *                                                        *  duration
   * Test : Apdex value is properly calculated.             *  *   apdex
   * Satisfying
   *                                                        *  *   *
   * *  Tolerating txn_name                              *  *   *  Specific
   * Metric Name       *  *  Failing    */
...
static void test_end(void) {
  ...
  nr_error_destroy(&txn->error);
  nr_txn_end(txn);
  test_end_testcase("no error", txn, 1 /* apdex */, 0 /* error*/,
                    1 /* queue */);
  nr_txn_destroy(&txn);
...
static void test_txn_accept_distributed_trace_payload_metrics(void) {
  ...
  test_txn_metric_created("background no exclusive", txn.unscoped_metrics,
                          ...
                          999, 998001);
  test_txn_metric_created("background", txn.unscoped_metrics, MET_FORCED,
                          "DurationByCaller/App/9123/51424/Unknown/all", 1,
                          999, 999, 999, 999, 998001);
  test_txn_metric_created("background", txn.unscoped_metrics, MET_FORCED,
                          "DurationByCaller/App/9123/51424/Unknown/allOther",
                          1, 999, 999, 999, 999, 998001);
  nrm_table_destroy(&txn.unscoped_metrics);
  nr_distributed_trace_destroy(&txn.distributed_trace);
  ...
  test_txn_metric_created(
      ...
      "ErrorsByCaller/App/9123/51424/Other/all", 1, 0, 0, 0, 0, 0);
  test_txn_metric_created(
      "background error with dt", txn.unscoped_metrics, MET_FORCED,
