...
test_create_destroy (void)
{
  ...
  tlib_pass_if_true ("pool destroy zeros pointer", 0 == empty, "empty=%p", empty);
}
...
test_nr_string_get (void)
{
  ...
  string = nr_string_get (pool, 1);
  tlib_pass_if_true ("success", 0 == nr_strcmp ("alpha", string), "string=%s", NRSAFESTR (string));
...
test_find_add_bad_params (void)
{
  ...
  tlib_pass_if_true ("add with hash length null params", 0 == idx, "idx=%d", idx);
  idx = nr_string_add_with_hash_length (pool, 0, 123, 5);
  tlib_pass_if_true ("add with hash length null string", 0 == idx, "idx=%d", idx);
  ...
  tlib_pass_if_true ("add with hash length negative length", 0 == idx, "idx=%d", idx);
  ...
  tlib_pass_if_true ("find with hash length null params", 0 == idx, "idx=%d", idx);
  ...
  tlib_pass_if_true ("find with hash length empty pool", 0 == idx, "idx=%d", idx);
  idx = nr_string_find_with_hash_length (pool, "alpha", 123, -1);
  tlib_pass_if_true ("find with hash length negative length", 0 == idx, "idx=%d", idx);
...
test_find_add (void)
{
  ...
  idx = nr_string_add_with_hash_length (pool, "Alpha", 123, 5); /* Different bytes */
  tlib_pass_if_true ("add", 2 == idx, "idx=%d", idx);
  idx = nr_string_add_with_hash_length (pool, "alpha", 234, 5); /* Different hash */
  tlib_pass_if_true ("add", 3 == idx, "idx=%d", idx);
  idx = nr_string_add_with_hash_length (pool, "alpha", 123, 4); /* Different length */
...
const char *example_strings[] = {
  "UDS", "only", "1", "1-65534,", "1023", "<code>newreliccfg</code>", "If",
  "In", "Please", "Sets", "TCP", "This", "UNIX", "a", "absolute", "agent",
  "also", "an", "and", "are", "as", "be", "by", "can", "case", "communicating",
  "communications", "configured", "daemon", "domain", "endpoint", "fact",
  "file", "first", "for", "form,", "forms", "if", "in", "is", "mechanism",
  "name", "no", "non-standard", "not", "note", "number", "of", "operating",
  "path", "paths", "please", "port", "ports", "preferred", "provide", "range",
  "relative", "remember", "require", "restriction", "run", "second", "sets",
  "setting", "socket", "specified", "specify", "standard", "startup", "string",
  "super-user", "system", "that", "the", "then", "this", "through", "to",
  "two", "use", "used", "uses", "using", "valid,", "variable", "where",
  "will", "with", "you", 0
};

const char *underscore_example_strings[] = {
  "_UDS", "_only", "_1", "_1-65534,", "_1023", "_<code>newreliccfg</code>",
  "_If", "_In", "_Please", "_Sets", "_TCP", "_This", "_UNIX", "_a",
  "_absolute", "_agent", "_also", "_an", "_and", "_are", "_as", "_be",
  "_by", "_can", "_case", "_communicating", "_communications", "_configured",
  "_daemon", "_domain", "_endpoint", "_fact", "_file", "_first", "_for",
  "_form,", "_forms", "_if", "_in", "_is", "_mechanism", "_name", "_no",
  "_non-standard", "_not", "_note", "_number", "_of", "_operating", "_path",
  "_paths", "_please", "_port", "_ports", "_preferred", "_provide", "_range",
  "_relative", "_remember", "_require", "_restriction", "_run", "_second",
  "_sets", "_setting", "_socket", "_specified", "_specify", "_standard",
  "_startup", "_string", "_super-user", "_system", "_that", "_the", "_then",
  "_this", "_through", "_to", "_two", "_use", "_used", "_uses", "_using",
  "_valid,", "_variable", "_where", "_will", "_with", "_you", 0
};
...
test_add_find (void)
{
  ...
  for (i = 0; example_strings[i]; i++) {
    ...
    tlib_pass_if_true ("add string", (1 + i) == rv_length, "i=%d rv_length=%d", i, rv_length);
  }

  for (i = 0; example_strings[i]; i++) {
    rv_length = nr_string_find (in, example_strings[i]);
    tlib_pass_if_true ("find string", (1 + i) == rv_length, "i=%d rv_length=%d", i, rv_length);
  }

  for (i = 0; underscore_example_strings[i]; i++) {
    rv_length = nr_string_find (in, underscore_example_strings[i]);
    tlib_pass_if_true ("find absent string", 0 == rv_length, "i=%d rv_length=%d", i, rv_length);
  }
...
test_pool_to_json (void)
{
  ...
  tlib_pass_if_true ("empty pool", 0 == nr_strcmp ("[]", json), "json=%s", NRSAFESTR (json));
  nr_free (json);
  ...
  tlib_pass_if_true ("normal pool", 0 == nr_strcmp ("[\"alpha\",\"beta\",\"gamma\"]", json),
...
static void
test_apply_callback (const char *str, int len, apply_state_t *state)
{
...
static void
test_apply_callback_never (const char *str, int len, void *data NRUNUSED)
{
