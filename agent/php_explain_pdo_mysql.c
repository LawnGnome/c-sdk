...
/*
 * At least some of the below functions will probably end up being generic once
 * other database extensions are supported, but let's jump off that bridge when
 * we come to it.
 */
 ...
 */
static int
add_column_to_explain_plan (zval *value NRUNUSED,
 ...
 */
static int
add_row_to_explain_plan (zval *row,
                         nr_explain_plan_t *plan,
 ...
 */
static int
add_value_to_explain_plan (zval *value, nrobj_t *row,
                           zend_hash_key *hash_key NRUNUSED TSRMLS_DC);
 ...
 */
static nr_explain_plan_t *
fetch_explain_plan_from_stmt (zval *stmt TSRMLS_DC);
 ...
 */
static zval *
issue_explain_query (zval *dbh, zval *original_stmt, zval *parameters TSRMLS_DC);
 ...
 */
static enum pdo_error_mode
set_pdo_silent (zval *dbh TSRMLS_DC);

/* }}} */

nr_explain_plan_t *
nr_php_explain_pdo_mysql_statement (zval *stmt, zval *parameters TSRMLS_DC)
{
  zval *dbh = NULL;
  zval *dup = NULL;
                            ...
                            zend_hash_key *hash_key TSRMLS_DC)
{
  ...
  if (nr_php_zend_hash_key_is_numeric (hash_key)) {
    nrl_verbosedebug (NRL_SQL, "%s: Unexpected non-string column name", __func__);
...
issue_explain_query (zval *dbh, zval *original_stmt, zval *parameters TSRMLS_DC)
{
   ...
   */
  explain_query = (char *) nr_zalloc (pdo_stmt->query_stringlen + sizeof ("EXPLAIN "));
  ...
  if (NR_FAILURE == nr_php_pdo_execute_query (explain_stmt, parameters TSRMLS_CC)) {
    nr_php_zval_free (&explain_stmt);
    goto end;
  }
