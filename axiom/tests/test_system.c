...
static void
test_system_get_hostname (void)
{
  char *hostname;

  hostname = nr_system_get_hostname ();
  tlib_pass_if_true ("hostname not null", 0 != hostname, "hostname=%p", hostname);
  nr_free (hostname);
}

static void
test_get_system (void)
{
  ...
  /* TODO(msl): Use NR_SYSTEM_* instead. */
  ...
  tlib_pass_if_true ("sys->sysname", (0 == nr_strcmp (sys->sysname, "SunOS")) ||
...
# error Unsupported OS: please add the expected uname to this file.
  ...
  tlib_pass_if_true ("sys value not null", 0 != sys->nodename, "sys->nodename=%p", sys->nodename);
  tlib_pass_if_true ("sys value not null", 0 != sys->release, "sys->release=%p", sys->release);
  tlib_pass_if_true ("sys value not null", 0 != sys->version, "sys->version=%p", sys->version);
  tlib_pass_if_true ("sys value not null", 0 != sys->machine, "sys->machine=%p", sys->machine);
...
static void
test_system_destroy_bad_params (void)
{
  nr_system_t *sys;
