 ...
 */
static int
fuzz_time_match (uint64_t expected, uint64_t actual)
{
  return (expected == actual) || (expected - 1ULL == actual) || (expected + 1ULL == actual);
}
...
test_parse_unix_time (void)
{
   ...
   */
  t1 = nr_parse_unix_time ("1368811467146000");
  tlib_pass_if_true ("parse unix time", 1368811467146000ULL == t1, "t1=" NR_TIME_FMT, t1);
  ...
  tlib_pass_if_true ("parse unix time", fuzz_time_match (1368811467146000ULL, t1), "t1=" NR_TIME_FMT, t1);
  t1 = nr_parse_unix_time ("1368811467.146");
  tlib_pass_if_true ("parse unix time", fuzz_time_match (1368811467146000ULL, t1), "t1=" NR_TIME_FMT, t1);
  t1 = nr_parse_unix_time ("1368811467");
  tlib_pass_if_true ("parse unix time", fuzz_time_match (1368811467000000ULL, t1), "t1=" NR_TIME_FMT, t1);
}
