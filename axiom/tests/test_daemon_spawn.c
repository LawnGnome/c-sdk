...
static void
pass_if_argv_has_flag_f (const char *msg, const nr_argv_t *argv,
  const char* flag, const char *file, int line)
{
  size_t i;
...
static void
fail_if_argv_has_flag_f (const char *msg, const nr_argv_t *argv,
  const char* flag, const char *file, int line)
{
  size_t i;
...
static void
pass_if_flag_has_value_f (const char *msg, const nr_argv_t *argv,
  const char* flag, const char* value)
{
   ...
   */
  tlib_pass_if_true (msg, argv->count - i > 1, "i=%zu count=%zu", i, argv->count);
...
static char *
nr_argv_get (const nr_argv_t *argv, size_t i)
{
  if (i < argv->count) {
    return argv->data[i];
  }
...
test_argv_resize (void)
{
  ...
  tlib_pass_if_true (__func__, argv.capacity >= 3, "capacity=%zu", argv.capacity);
  tlib_pass_if_int_equal (__func__, 3, argv.count);
...
static void
test_sockfile_is_abs_path (void)
{
  nr_argv_t *argv;
...
static void
test_sockfile_is_rel_path (void)
{
  nr_argv_t *argv;
  nr_daemon_args_t args;
...
test_tls_disabled (void)
{
  ...
  pass_if_argv_has_flag (argv, "--tls=false");
  fail_if_argv_has_flag (argv, "--tls");
  fail_if_argv_has_flag (argv, "--tls=true");
...
static void
test_integration_mode_enabled (void)
{
  nr_argv_t *argv;
  nr_daemon_args_t args;
...
static void
test_integration_mode_disabled (void)
{
  nr_argv_t *argv;
  nr_daemon_args_t args;
...
static void
test_spawn_daemon_bad_input (const char *fake_daemon_path)
{
  pid_t pid;
  nr_daemon_args_t args;
...
static void
test_fork_error (const char *fake_daemon_path)
{
  pid_t pid;
  nr_daemon_args_t args;
  pid_t (*saved_fork_fn) (void) = nr_daemon_fork_hook;
...
static void
test_fork_success (const char *fake_daemon_path)
{
  pid_t pid;
  nr_daemon_args_t args;
  pid_t (*saved_fork_fn) (void) = nr_daemon_fork_hook;
 ...
 */
static int
daemon_exec_wrapper (const char *path, char *const argv[])
{
   ...
   */
  nr_write (nrl_get_log_fd (), NR_PSTR ("info: inherited file descriptors > 2 should be closed\n"));

  return execvp (path, argv);
}

static void
test_spawn_daemon (const char *fake_daemon_path)
{
  pid_t daemon_pid;
  int daemon_exit_status;
  nr_status_t st;
  nr_daemon_args_t args;
  int (*saved_exec_fn) (const char* path, char *const argv[]) = nr_daemon_execvp_hook;
