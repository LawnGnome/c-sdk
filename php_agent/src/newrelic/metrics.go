...
// MetricForce describes the kind of a metric. Metrics can be either
// forced or unforced.
...
func (mt *MetricTable) mergeMetric(nameSlice []byte, nameString, scope string, m *metric) {
...
func (mt *MetricTable) add(nameSlice []byte, nameString, scope string, data metricData, force MetricForce) {
	mt.mergeMetric(nameSlice, nameString, scope, &metric{data: data, forced: force})
}

// AddRaw adds a metric to mt. If mt is full, and the metric is unforced,
// the metric will not be added.
func (mt *MetricTable) AddRaw(nameSlice []byte, nameString, scope string, data [6]float64, force MetricForce) {
	d := metricData{
...
func (mt *MetricTable) AddCount(name, scope string, count float64, force MetricForce) {
	mt.add(nil, name, scope, metricData{countSatisfied: count}, force)
}

// AddValue adds a metric with the given duration to mt. If mt is
// full, and the metric is unforced, the metric will not be added.
func (mt *MetricTable) AddValue(name, scope string, value float64, force MetricForce) {
	data := metricData{
		countSatisfied:  1,
...
func (mt *MetricTable) CollectorJSON(id AgentRunID, now time.Time) ([]byte, error) {
...
// name and scope.
func (mt *MetricTable) CollectorJSONSorted(id AgentRunID, now time.Time) ([]byte, error) {
...
// by the collector.
func (mt *MetricTable) Data(id AgentRunID, harvestStart time.Time) ([]byte, error) {
...
func (mt *MetricTable) Audit(id AgentRunID, harvestStart time.Time) ([]byte, error) {
...
func parseCollectorMetrics(data interface{}, scrub []*regexp.Regexp) collectorMetrics {
