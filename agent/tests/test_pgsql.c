...
test_save_datastore_instance (TSRMLS_D)
{
  ...
  assert_datastore_instance_equals ("null conn and null conn_info", expected_default,
  ...
  assert_datastore_instance_equals ("null conn and empty conn_info", expected_default,
    nr_hashmap_get (NRPRG (datastore_connections), key, nr_strlen (key)));
  ...
  assert_datastore_instance_equals ("null conn_info", expected_default,
    nr_hashmap_get (NRPRG (datastore_connections), key, nr_strlen (key)));

  nr_hashmap_delete (NRPRG (datastore_connections), key, nr_strlen (key));
  nr_php_pgsql_save_datastore_instance (conn, "" TSRMLS_CC);
  assert_datastore_instance_equals ("empty conn_info", expected_default,
    nr_hashmap_get (NRPRG (datastore_connections), key, nr_strlen (key)));
   ...
   */
  nr_php_pgsql_save_datastore_instance (conn, "host=spock port=2345 dbname=kirk" TSRMLS_CC);
...
static void
test_instance (const char *message, const char *conn_info,
...
test_create_datastore_instance ()
{
   ...
   * Test: localhost
   */
  test_instance ("localhost port", "host=localhost", &((nr_datastore_instance_t) {
    .database_name   = DEFAULT_DATABASE_NAME,
    .host            = system_host_name,
    .port_path_or_id = DEFAULT_PORT,
  }));

  test_instance ("localhost socket", "host=/tmp", &((nr_datastore_instance_t) {
    .database_name   = DEFAULT_DATABASE_NAME,
    .host            = system_host_name,
    .port_path_or_id = DEFAULT_SOCKET,
  }));
   ...
   * Test: Non-localhost
   */
  test_instance ("non-localhost", "host=spock port=2345 dbname=kirk", &((nr_datastore_instance_t) {
    .database_name   = "kirk",
    .host            = "spock",
    .port_path_or_id = "2345",
  }));
