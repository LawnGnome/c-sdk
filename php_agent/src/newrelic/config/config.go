...
func lexInitial(d *Decoder, v reflect.Value) (next stateFn, err error) {
	...
	for {
		...
		default:
			return nil, fmt.Errorf("config: syntax error, expected keyword or comment, got %q", ch)
		}
	}
}
...
func lexKeyword(d *Decoder, v reflect.Value) (next stateFn, err error) {
	...
	for {
		...
		case ch == '=':
			...
			return lexValue, nil
		default:
			return nil, fmt.Errorf("invalid character %q following keyword: %s", ch, d.token.String())
		}
	}
}
...
func lexDelimiter(d *Decoder, v reflect.Value) (next stateFn, err error) {
	...
	for {
		...
		if err != nil {
			if err == io.EOF {
				return nil, fmt.Errorf("expected delimiter after keyword %q", d.keyword)
			}
			return nil, err
		}
		...
		return nil, fmt.Errorf("expected delimiter after keyword '%s', got %q", d.keyword, ch)
	}
}
...
// consumed.
func lexSingleQuoteString(d *Decoder, v reflect.Value) (next stateFn, err error) {
	value, err := d.r.ReadBytes('\'')
	if err != nil {
		if err != io.EOF {
			return nil, err
		}
		return nil, fmt.Errorf("unexpected EOF: %q is missing a closing quote", d.keyword)
	}
...
// When this function is called, the opening quote has already been
// consumed.
func lexDoubleQuoteString(d *Decoder, v reflect.Value) (next stateFn, err error) {
	value, err := d.r.ReadBytes('"')
	if err != nil {
		if err != io.EOF {
			return nil, err
...
func (d *Decoder) processKeyword(v reflect.Value) error {
	...
	// couldn't find a matching field, give the caller a chance to
	// process the keyword.
	//if d.Unmatched != nil {
	//	return d.Unmatched(d.keyword, d.token.Bytes())
	//}
...
// String always returns an empty string. It's implemented to satisify the
