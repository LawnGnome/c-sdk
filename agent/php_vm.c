 ...
 */
static void
nr_php_set_opcode_handler (zend_uchar opcode, user_opcode_handler_t handler)
{
  ...
  if (SUCCESS != zend_set_user_opcode_handler (opcode, handler)) {
     ...
     */
    nrl_info (NRL_AGENT,
              "%s: error setting handler for opcode %u",
              __func__, (unsigned int) opcode);
  }
}
 ...
 *
 * This function is written unusually for our code base: it uses the EXPECTED
 * and UNEXPECTED macros heavily.
 *
 * The reason here is performance: without wanting to re-establish the worst
 * excesses of our old nr_php_execute_*() family of functions, this is on a hot
 * path in the PHP 7 agent (every function call -- internal or user function
 * alike -- goes through here), and we want to minimise our overhead. Where
 * there's a normal case, branch prediction macros are used to (hopefully)
 * speed up execution by reducing the likelihood of a branch misprediction
 * stall.
 *
 * The logic here is fundamentally fairly simple: If the cufa_callback global
 * is set, then we're instrumenting call_user_func_array() and should invoke it
 * (provided we can pull the fields we need out of execute_data). After that,
 * regardless of whether cufa_callback was set, we should invoke any previous
 * opcode handler for the same opcode, otherwise return
 * ZEND_USER_OPCODE_DISPATCH to the Zend Engine, which signals that the Zend
 * Engine should execute the opline normally.
 */
static int
nr_php_handle_cufa_fcall (zend_execute_data *execute_data)
{
   ...
   * change coming in PHP 7.2, but it only optimises array_slice() calls, which
  ...
  if (ZEND_SEND_ARRAY == prev_opline->opcode) {
    if (UNEXPECTED ((NULL == execute_data->call) ||
      ...
      nrl_verbosedebug (NRL_AGENT,
                        "%s: cannot get function from call",
                        __func__);
      return ZEND_USER_OPCODE_DISPATCH;
    }

    nr_php_call_user_func_array_handler (cufa_callback,
                                         execute_data->call->func,
                                         execute_data);
  }
...
void
nr_php_set_opcode_handlers (void)
{
  nr_php_set_opcode_handler (ZEND_DO_FCALL, nr_php_handle_cufa_fcall);
}

void
nr_php_remove_opcode_handlers (void)
{
  size_t i;
