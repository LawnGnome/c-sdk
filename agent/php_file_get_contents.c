...
static int
nr_php_file_get_contents_header_search (zval *element, char **header_ptr,
                                        zend_hash_key *key NRUNUSED TSRMLS_DC)
{
  ...
  if ((0 == nr_php_is_zval_valid_string (element)) ||
      ...
      (0 != nr_strnicmp (Z_STRVAL_P (element), NR_PSTR (X_NEWRELIC_APP_DATA)))) {
    return ZEND_HASH_APPLY_KEEP;
  }
...
char *
nr_php_file_get_contents_response_header (TSRMLS_D)
{
  zval *header_zv;
  char *header = NULL;
  ...
  nr_php_zend_hash_zval_apply (Z_ARRVAL_P (header_zv),
...
static void
nr_php_file_get_contents_add_headers_internal (zval *context, zval *context_options, char *headers TSRMLS_DC)
{
  ...
  http_context_options = nr_php_zend_hash_find (Z_ARRVAL_P (context_options),
                                                "http");
  if (NULL == http_context_options) {
    ...
    retval = nr_php_call (NULL, "stream_context_set_option", context,
                          http_string_zval, header_string_zval,
                          buf_string_zval);
  ...
  http_header = nr_php_zend_hash_find (Z_ARRVAL_P (http_context_options),
                                       "header");
  if (NULL == http_header) {
  ...
  /* There is a non-empty header string which must be preserved. */
  {
     ...
     * changed.
     */
    all_headers = nr_formatf ("%s%.*s", headers,
      NRSAFELEN (Z_STRLEN_P (http_header)),
...
static char *
nr_php_file_get_contents_create_outbound_headers (nrtxn_t *txn)
{
  char *headers = NULL;
  ...
  if (txn && txn->special_flags.debug_cat) {
    nrl_verbosedebug (NRL_CAT,
      "CAT: outbound request: transport='file_get_contents' %s=" NRP_FMT " %s=" NRP_FMT,
...
void
nr_php_file_get_contents_add_headers (zval *context TSRMLS_DC)
{
  char *headers = 0;
  ...
  nr_php_file_get_contents_add_headers_internal (context, context_options, headers TSRMLS_CC);
...
static int
nr_php_file_get_contents_remove_outbound_headers_iterator (zval *element,
                                                           void *extra NRUNUSED,
                                                           zend_hash_key *key NRUNUSED TSRMLS_DC)
{
  NR_UNUSED_TSRMLS;
  ...
  if (nr_strncaseidx (Z_STRVAL_P (element), X_NEWRELIC_ID, Z_STRLEN_P (element)) >= 0) {
    return ZEND_HASH_APPLY_REMOVE;
  } else if (nr_strncaseidx (Z_STRVAL_P (element), X_NEWRELIC_SYNTHETICS, Z_STRLEN_P (element)) >= 0) {
    return ZEND_HASH_APPLY_REMOVE;
  } else {
    return ZEND_HASH_APPLY_KEEP;
  }
...
static void
nr_php_file_get_contents_remove_headers_internal (zval *context_options TSRMLS_DC)
{
  zval *http_context_options = 0;
  zval *http_header = 0;
  ...
  if (IS_ARRAY == Z_TYPE_P (http_header)) {
    nr_php_zend_hash_zval_apply (Z_ARRVAL_P (http_header),
                                 (nr_php_zval_apply_t) nr_php_file_get_contents_remove_outbound_headers_iterator,
                                 NULL TSRMLS_CC);
    return;
  }
   ...
   *
   * For Example:
   *  "X-NewRelic-ID: VQQEVVNADgQIXQ==\r\nX-NewRelic-Transaction: PxQHUFRQDAYGU1lbdnN0IiF3FB8EBw8RVT8=\r\nAlpha: Beta\r\n"
   * Or:
   *  "X-NewRelic-ID: VQQEVVNADgQIXQ==\r\nX-NewRelic-Transaction: PxQHUFRQDAYGU1lbdnN0IiF3FB8EBw8RVT8=\r\nX-NewRelic-Synthetics: PwcbUFZTFBFRRk1AVRMbRAcTaw==\r\nAlpha: Beta\r\n"
   *
   * See:
   ...
   */
  {
    ...
    if (0 == nr_strncaseidx (dup, X_NEWRELIC_ID, nr_strlen (X_NEWRELIC_ID))) {
      ...
      for (i = 0; dup[i]; i++) {
        if ('\n' == dup[i]) {
          newlines_seen += 1;
          if (((2 == newlines_seen) && 0 != nr_strncaseidx (dup + i + 1, X_NEWRELIC_SYNTHETICS, nr_strlen (X_NEWRELIC_SYNTHETICS))) ||
              (3 == newlines_seen)) {
            nr_php_add_assoc_string (http_context_options, "header", dup + i + 1);
            break;
          }
        }
      }
...
void
nr_php_file_get_contents_remove_headers (zval *context TSRMLS_DC)
{
  zval *context_options = 0;
...
nr_status_t
nr_php_file_get_contents_recurse_with_context (
  zval *return_value,
  zval *file_zval,
...
PHP_FUNCTION (newrelic_add_headers_to_context)
{
  ...
  rv = zend_parse_parameters_ex (ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS () TSRMLS_CC, "z", &context);
  if (SUCCESS != rv) {
    return;
  }
...
/* Test scaffolding */
#ifdef TAGS
void zif_newrelic_remove_headers_from_context (void);  /* ctags landing pad only */
