...
static const char *
nr_magento_name_to_string (const zval *zv)
{
...
void
nr_magento_name_transaction (zval *action_obj TSRMLS_DC)
{
  zval *request = NULL;
  zval *module = NULL;
  ...
  nrl_verbosedebug (NRL_FRAMEWORK, "Magento: module=%s controller=%s action=%s",
    NRSAFESTR (modulename), NRSAFESTR (controllername), NRSAFESTR (actionname));
   ...
   *
   */
  nr_txn_set_path ("Magento", NRPRG (txn), txn_name, NR_PATH_TYPE_ACTION, NR_OK_TO_OVERWRITE);
