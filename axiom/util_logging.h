...
static inline int
nrl_should_print (nrloglev_t level, uint32_t subsystem)
{
  if (NRL_ALWAYS == level) {
    return 1;
  }
 ...
 * We need to mediate strings so that we aren't susceptible to logfile injection attacks.
 *
 ...
 * and then clamp the printed length of the string to be no more than the given value.
 * Use the NRP_FMT macro with ANSI string concatentation to build up format specifiers.
 *
 ...
 * allocation time.
 *
 * Please keep the format strings on the same line as the call to nrl_foo.
...
#define NRP_CONNECT(N)     256, NRSAFESTR (N)  /* echoing connect messages with text from the collector */
...
#define NRP_PHP(N)         100, NRBLANKSTR (N) /* names derived from PHP engine, such as functions, namespaces, modules, etc */
#define NRP_ARGUMENTS(N)   100, NRBLANKSTR (N) /* arguments given to PHP functions */
#define NRP_ARGSTR(N)       80,           (N)  /* arguments given to PHP functions, copied into local buffer */
#define NRP_FILENAME(N)    250, NRSAFESTR (N)  /* source code file names (these tend to be long in modern frameworks) */
#define NRP_PROCARG(N)     300, NRSAFESTR (N)  /* arguments given to process invocation */
#define NRP_CONFIG(N)      256, NRSAFESTR (N)  /* internally created agent configuration string */
#define NRP_BUFFER(N)    ((int) sizeof (N)), (N)  /* local char[] array; NRSAFESTR doesn't work on such beasts */
#define NRP_ERROR(N)       256, NRSAFESTR (N)  /* error type and info from PHP exception frame */
#define NRP_COMMAND(N)      16,           (N)  /* internal command name; doesn't need to be safed */
#define NRP_STRERROR(N)     64,           (N)  /* string from strerror, which will be non-null */

/*
 * Use one of these format directive strings to build up format directives
 * to print the result of the above macros.
 *
 * NRP_FMT quotes the string using '' in order to delimit the scope of the string.
 * NRP_FMT_UQ doesn't quote the string.  Use this sparingly, as for example when printing banner information.
 */
#define NRP_FMT    "'%.*s'"
#define NRP_FMT_UQ "%.*s"
 ...
 *           DSO is loaded
 */
extern nr_status_t nrl_send_log_message (nrloglev_t level, const char *fmt, ...) NRPRINTFMT(2);
 ...
 *           verbose,rpm=debug,daemon=verbosedebug - sets all subsystems to
 *              the verbose level then selectively sets the RPM subsystem to
 *              the debug level and the daemon subsystem to verbosedebug.
 *
...
/*
 * Purpose : Write a log message, but using a va_list rather than variable number
 *           of arguments.
 */
extern void nrl_vlog (nrloglev_t level, uint32_t subsystem, const char *fmt, va_list ap);
