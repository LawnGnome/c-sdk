...
static int
nr_php_curl_do_cross_process (TSRMLS_D)
{
...
static void
nr_php_curl_save_response_header_from_zval (const zval *zstr TSRMLS_DC)
{
  char *str;
  char *hdr;

  if ((0 == zstr) ||
      (0 == nr_php_is_zval_non_empty_string (zstr))) {
    return;
  }
...
PHP_FUNCTION (newrelic_curl_header_callback)
{
  ...
  rv = zend_parse_parameters_ex (ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS () TSRMLS_CC, "zz",
                                 &curl_resource, &header_data);
...
static void
nr_php_curl_set_default_response_header_callback (zval *curlres TSRMLS_DC)
{
  zval *callback_name;
  zval *retval = 0;
  zval *curlopt;
...
static void
nr_php_curl_set_default_request_headers (zval *curlres TSRMLS_DC)
{
  zval *arr;
  zval *retval = 0;
  zval *curlopt;
...
void
nr_php_curl_init (zval *curlres TSRMLS_DC)
{
  if (0 == nr_php_curl_do_cross_process (TSRMLS_C)) {
    return;
  }
...
static int
nr_php_curl_copy_outbound_headers_iterator (zval *element, zval *dest,
                                            ...
                                            TSRMLS_DC)
{
  ...
  if (nr_php_is_zval_valid_string (element) &&
      ((nr_strncaseidx (Z_STRVAL_P (element), X_NEWRELIC_ID, Z_STRLEN_P (element)) >= 0) ||
      (nr_strncaseidx (Z_STRVAL_P (element), X_NEWRELIC_TRANSACTION, Z_STRLEN_P (element)) >= 0) ||
      (nr_strncaseidx (Z_STRVAL_P (element), X_NEWRELIC_SYNTHETICS, Z_STRLEN_P (element)) >= 0))) {
    return ZEND_HASH_APPLY_KEEP;
  }

  /*
   * Otherwise, we copy the header into the destination array, being careful to
   * increment the refcount on the element to avoid double frees.
   */
#ifdef PHP7
...
static void
nr_php_curl_setopt_curlopt_httpheader (zval *curlres, zval *curlopt,
                                       zval *curlval TSRMLS_DC)
{
  ...
  nr_php_zend_hash_zval_apply (Z_ARRVAL_P (curlval),
  ...
  if (x_newrelic_id && x_newrelic_transaction) {
    ...
    char *h2 = nr_header_format_name_value (X_NEWRELIC_TRANSACTION, x_newrelic_transaction, 0);
  ...
  if (x_newrelic_synthetics) {
    char *h3 = nr_header_format_name_value (X_NEWRELIC_SYNTHETICS, x_newrelic_synthetics, 0);
...
static void
nr_php_curl_setopt_curlopt_writeheader (zval *curlval TSRMLS_DC)
{
  if ((0 == curlval) ||
      (IS_RESOURCE != Z_TYPE_P (curlval))) {
    return;
  }
  ...
  nrm_force_add (NRPRG (txn) ? NRPRG (txn)->unscoped_metrics : 0,
    "Supportability/Unsupported/curl_setopt/CURLOPT_WRITEHEADER", 0);
}

static void
nr_php_curl_setopt_curlopt_headerfunction (zval *curlval TSRMLS_DC)
{
  const char *our_callback = NR_CURL_RESPONSE_HEADER_CALLBACK_NAME;
  nr_string_len_t our_callback_len = sizeof (NR_CURL_RESPONSE_HEADER_CALLBACK_NAME) - 1;
  ...
  if (nr_php_is_zval_valid_object (curlval)) {
     ...
     */
    nrm_force_add (NRPRG (txn) ? NRPRG (txn)->unscoped_metrics : 0,
      "Supportability/Unsupported/curl_setopt/CURLOPT_HEADERFUNCTION/closure", 0);
    return;
  }
  ...
  if ((our_callback_len == Z_STRLEN_P (curlval)) &&
      (0 == nr_strncmp (our_callback, Z_STRVAL_P (curlval), our_callback_len))) {
    /*
...
void
nr_php_curl_setopt_pre (zval *curlres, zval *curlopt, zval *curlval TSRMLS_DC)
{
  if (0 == nr_php_curl_do_cross_process (TSRMLS_C)) {
    return;
  }
  ...
  if (nr_php_is_zval_named_constant (curlopt, "CURLOPT_HEADERFUNCTION" TSRMLS_CC)) {
...
void
nr_php_curl_setopt_post (zval *curlres, zval *curlopt, zval *curlval TSRMLS_DC)
{
  if (0 == nr_php_curl_do_cross_process (TSRMLS_C)) {
    return;
  }
...
static int
nr_php_curl_setopt_array_apply (zval *value,
...
void
nr_php_curl_setopt_array (zval *curlres, zval *options,
                          nr_php_curl_setopt_func_t func TSRMLS_DC)
{
  ...
  nr_php_zend_hash_zval_apply (Z_ARRVAL_P (options),
                               (nr_php_zval_apply_t) nr_php_curl_setopt_array_apply,
                               (void *) &app TSRMLS_CC);
}

char *
nr_php_curl_get_url (zval *curlres TSRMLS_DC)
{
  zval *retval = 0;
  char *url = 0;
  ...
  curlinfo_effective_url = nr_php_get_constant ("CURLINFO_EFFECTIVE_URL" TSRMLS_CC);
 ...
 */
int
nr_php_curl_should_instrument_proto (const char *url)
{
