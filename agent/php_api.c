...
void
nr_php_api_error (const char *format, ...)
{
...
void
nr_php_api_add_supportability_metric (const char *name TSRMLS_DC)
{
  char buf[512];
...
PHP_FUNCTION (newrelic_notice_error)
{
  ...
  if (NR_SUCCESS != nr_txn_record_error_worthy (NRPRG (txn), priority)) {
    nrl_debug (NRL_API, "newrelic_notice_error: a higher severity error has already been noticed");
    RETURN_FALSE;
  }
  ...
  switch (ZEND_NUM_ARGS ()) {
    case 1:
       ...
       * the string.
       */
      if (SUCCESS == zend_parse_parameters_ex (ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS () TSRMLS_CC, "o", &exc)) {
        break;
      }

      if (FAILURE == zend_parse_parameters_ex (ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS () TSRMLS_CC, "s", &errormsgstr, &errormsglen)) {
        nrl_debug (NRL_API, "newrelic_notice_error: invalid single argument: expected string");
        RETURN_NULL ();
      }
      break;
    ...
    case 2:
       ...
       * completely unused.
       */
      if (FAILURE == zend_parse_parameters_ex (ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS () TSRMLS_CC, "zo!", &ignore, &exc)) {
        nrl_debug (NRL_API, "newrelic_notice_error: invalid two arguments: expected exception as second argument");
        RETURN_NULL ();
      }
      break;

    case 5:
      if (FAILURE == zend_parse_parameters_ex (ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS () TSRMLS_CC, "lsslz!",
          &ignore1, &errormsgstr, &errormsglen, &ignore2, &ignore3, &ignore4, &ignore5)) {
  ...
  if (exc) {
    if (NR_SUCCESS == nr_php_error_record_exception (
...
PHP_FUNCTION (newrelic_end_transaction)
{
  ...
  if (1 == ZEND_NUM_ARGS ()) {
    if (SUCCESS == zend_parse_parameters (ZEND_NUM_ARGS () TSRMLS_CC, "b", &ignorebool)) {
      ignore = (long)ignorebool;
    } else if (FAILURE == zend_parse_parameters (ZEND_NUM_ARGS () TSRMLS_CC, "l", &ignore)) {
      RETURN_FALSE;
    }
  }
...
PHP_FUNCTION (newrelic_start_transaction)
{
  ...
  if (1 == ZEND_NUM_ARGS ()) {
    if (SUCCESS == zend_parse_parameters (ZEND_NUM_ARGS () TSRMLS_CC, "s", &appstr, &applen)) {
    ...
    if (SUCCESS == zend_parse_parameters (ZEND_NUM_ARGS () TSRMLS_CC, "ss", &appstr, &applen, &licstr, &liclen)) {
      appnames = (char *)nr_alloca (applen + 1);
      nr_strxcpy (appnames, appstr, applen);
...
PHP_FUNCTION (newrelic_background_job)
{
  ...
  if (ZEND_NUM_ARGS () >= 1) {
    if (FAILURE == zend_parse_parameters (ZEND_NUM_ARGS () TSRMLS_CC, "b", &backgroundbool)) {
      if (FAILURE == zend_parse_parameters (ZEND_NUM_ARGS () TSRMLS_CC, "l", &background)) {
        background = 1;
      } else {
  ...
  if (background) {
    nr_txn_set_as_background_job (NRPRG (txn), "newrelic_background_job API call");
  } else {
    nr_txn_set_as_web_transaction (NRPRG (txn), "newrelic_background_job API call");
  }
}

static void
nr_php_api_capture_params_internal (const char *function_name, INTERNAL_FUNCTION_PARAMETERS)
{
  zend_long enable = 0;
  ...
  if (ZEND_NUM_ARGS () >= 1) {
    if (FAILURE == zend_parse_parameters (ZEND_NUM_ARGS () TSRMLS_CC, "b", &enablebool)) {
      if (FAILURE == zend_parse_parameters (ZEND_NUM_ARGS () TSRMLS_CC, "l", &enable)) {
        enable = 1;
      } else {
  ...
  nrl_debug (NRL_API, "capture params enabled='%.10s'", NRPRG (deprecated_capture_request_parameters) ? "true" : "false");
}
...
{
  nr_php_api_capture_params_internal ("enable_params", INTERNAL_FUNCTION_PARAM_PASSTHRU);
}
...
{
  nr_php_api_capture_params_internal ("capture_params", INTERNAL_FUNCTION_PARAM_PASSTHRU);
}
...
PHP_FUNCTION (newrelic_custom_metric)
{
  ...
  if (FAILURE == zend_parse_parameters (ZEND_NUM_ARGS () TSRMLS_CC, "sd", &metricstr, &metriclen, &value_ms)) {
    RETURN_FALSE;
  }
...
#define NR_PHP_API_INVALID_ATTRIBUTE_FMT "%s: expects parameter to be scalar, %s given"

static nrobj_t *
nr_php_api_zval_to_attribute_obj (const zval *z TSRMLS_DC)
{
  char *str;
  nrobj_t *obj;
  ...
  switch (Z_TYPE_P (z)) {
    ...
    case IS_STRING:
      if (!nr_php_is_zval_valid_string (z)) {
        nr_php_api_error (NR_PHP_API_INVALID_ATTRIBUTE_FMT, get_active_function_name (TSRMLS_C), "invalid string");
        return NULL;
      }
    ...
    case IS_ARRAY:
      nr_php_api_error (NR_PHP_API_INVALID_ATTRIBUTE_FMT, get_active_function_name (TSRMLS_C), "array");
      return NULL;

    case IS_OBJECT:
      nr_php_api_error (NR_PHP_API_INVALID_ATTRIBUTE_FMT, get_active_function_name (TSRMLS_C), "object");
      return NULL;

    case IS_RESOURCE:
      nr_php_api_error (NR_PHP_API_INVALID_ATTRIBUTE_FMT, get_active_function_name (TSRMLS_C), "resource");
      return NULL;

    case IS_CONSTANT:
      nr_php_api_error (NR_PHP_API_INVALID_ATTRIBUTE_FMT, get_active_function_name (TSRMLS_C), "constant");
      return NULL;

    default:
      nr_php_api_error (NR_PHP_API_INVALID_ATTRIBUTE_FMT, get_active_function_name (TSRMLS_C), "unknown");
      return NULL;
  }
}
...
PHP_FUNCTION (newrelic_add_custom_parameter)
{
  ...
  if (FAILURE == zend_parse_parameters (ZEND_NUM_ARGS () TSRMLS_CC, "zz", &zzkey, &zzvalue)) {
    RETURN_FALSE;
  }
...
PHP_FUNCTION (newrelic_name_transaction)
{
  ...
  if (1 != ZEND_NUM_ARGS ()) {
    nrl_warning (NRL_API, "newrelic_name_transaction failure: improper number of parameters");
    RETURN_FALSE;
  }

  zend_rv = zend_parse_parameters (ZEND_NUM_ARGS () TSRMLS_CC, "s", &namestr, &namestrlen);
  if ((SUCCESS != zend_rv) ||
      (0 == namestr) ||
      (namestrlen <= 0)) {
    nrl_warning (NRL_API, "newrelic_name_transaction failure: unable to parse string parameter");
    RETURN_FALSE;
  }
  ...
  rv = nr_txn_set_path ("API", NRPRG (txn), s, NR_PATH_TYPE_CUSTOM, NR_OK_TO_OVERWRITE);
  if (NR_SUCCESS != rv) {
    nrl_warning (NRL_API, "newrelic_name_transaction failure: unable to change name to " NRP_FMT, NRP_TXNNAME (s));
  } else {
    nrl_debug (NRL_API, "newrelic_name_transaction: API naming is " NRP_FMT, NRP_TXNNAME (s));
  }
...
PHP_FUNCTION (newrelic_add_custom_tracer)
{
  ...
  if (1 == ZEND_NUM_ARGS ()) {
    if (FAILURE == zend_parse_parameters (ZEND_NUM_ARGS () TSRMLS_CC, "s", &namestr, &namestrlen)) {
      RETURN_FALSE;
    }
  } else {
    RETURN_FALSE;
...
PHP_FUNCTION (newrelic_get_browser_timing_header)
{
   ...
   */
  if (ZEND_NUM_ARGS () >= 1) {
    if (FAILURE == zend_parse_parameters (ZEND_NUM_ARGS () TSRMLS_CC, "b", &tagsbool)) {
      if (FAILURE == zend_parse_parameters (ZEND_NUM_ARGS () TSRMLS_CC, "l", &usetags)) {
        usetags = 1;
      } else {
  ...
  /*
   * This mess is required to silence warnings about PHP's terrible prototypes.
   */
#ifdef PHP7
...
PHP_FUNCTION (newrelic_set_appname)
{
  ...
  if (1 == ZEND_NUM_ARGS ()) {
    ...
    if (SUCCESS == zend_parse_parameters (ZEND_NUM_ARGS () TSRMLS_CC, "ssb", &appstr, &applen, &licstr, &liclen, &xmitbool)) {
      appnames = (char *)nr_alloca (applen + 1);
      nr_strxcpy (appnames, appstr, applen);
      license = (char *)nr_alloca (liclen + 1);
      nr_strxcpy (license, licstr, liclen);
      xmit = (long)xmitbool;
    } else if (SUCCESS == zend_parse_parameters (ZEND_NUM_ARGS () TSRMLS_CC, "ssl", &appstr, &applen, &licstr, &liclen, &xmit)) {
      /* NOTREACHED */
      /* IMPOSSIBLE path through interpreter */
      /*
  ...
  if (NR_SUCCESS != ret) {
    nrl_verbose (NRL_API, "newrelic_set_appname: failed to end current transaction in changing app to " NRP_FMT " [" NRP_FMT "]",
      NRP_APPNAME (appnames),
      NRP_LICNAME (license));
  }

  ret = nr_php_txn_begin (appnames, license TSRMLS_CC);
  if (NR_SUCCESS != ret) {
    nrl_verbose (NRL_API, "newrelic_set_appname: unable to start new transaction with app " NRP_FMT " [" NRP_FMT "]",
      NRP_APPNAME (appnames),
      NRP_LICNAME (license));
    RETURN_FALSE;
  }
  ...
  nrl_debug (NRL_API, "newrelic_set_appname: application changed to " NRP_FMT " [" NRP_FMT "]",
    NRP_APPNAME (appnames),
    NRP_LICNAME (license));
...
static nr_status_t
nr_php_api_add_custom_parameter_string (nrtxn_t *txn, const char *key, const char *val, int val_len)
{
  nr_status_t rv;
  nrobj_t *obj;
...
PHP_FUNCTION (newrelic_set_user_attributes)
{
  ...
  if (FAILURE == zend_parse_parameters (ZEND_NUM_ARGS () TSRMLS_CC, "sss", &userstr, &userlen, &accstr, &acclen, &prodstr, &prodlen)) {
    RETURN_FALSE;
  }

  rv = nr_php_api_add_custom_parameter_string (NRPRG (txn), "user",    userstr, userlen);
  if (NR_FAILURE == rv) {
    RETURN_FALSE;
  }
  rv = nr_php_api_add_custom_parameter_string (NRPRG (txn), "account", accstr,  acclen);
  if (NR_FAILURE == rv) {
    RETURN_FALSE;
  }
  rv = nr_php_api_add_custom_parameter_string (NRPRG (txn), "product", prodstr, prodlen);
  if (NR_FAILURE == rv) {
    RETURN_FALSE;
  }
 ...
 */
static nrobj_t *
nr_php_api_transform_custom_events_attributes (zval *params_zval TSRMLS_DC)
{
  nrobj_t *obj = NULL;
  zval *element = 0;
  ...
  ZEND_HASH_FOREACH_KEY_VAL (Z_ARRVAL_P (params_zval), num_key, string_key, element) {
    nrobj_t *o;
    ...
    if (string_key) {
      ...
      nr_php_api_error ("newrelic_record_custom_event: ignoring non-string array key");
      continue;
    }
...
PHP_FUNCTION (newrelic_record_custom_event)
{
  ...
  if (FAILURE == zend_parse_parameters (ZEND_NUM_ARGS () TSRMLS_CC, "sa", &event_type, &event_type_len, &params_zval)) {
    nrl_warning (NRL_API, "unable to parse parameters to newrelic_record_custom_event. %d parameters received",
      ZEND_NUM_ARGS ());
    return;
  }

  if ((NULL == event_type) || (event_type_len <= 0)) {
    nr_php_api_error ("improper parameter to newrelic_record_custom_event: event_type must be a nonempty string");
    return;
  }

  if (0 == nr_php_is_zval_valid_array (params_zval)) {
    nr_php_api_error ("improper parameter to newrelic_record_custom_event: parameters must be an array");
    return;
  }
