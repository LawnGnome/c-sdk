 ...
 */
nr_status_t
nr_header_validate_encoded_string (const char *encoded_string)
{
  int i;
...
char *
nr_header_encode (const nrtxn_t *txn, const char *string)
{
...
char *
nr_header_decode (const nrtxn_t *txn, const char *encoded_string)
{
  const char *encoding_key;
...
int64_t
nr_header_account_id_from_cross_process_id (const char *cross_process_id)
{
  int64_t account_id;
  char *end = NULL;
...
nr_status_t
nr_header_validate_decoded_id (const nrtxn_t *txn, const char *decoded_id)
{
  int64_t account_id;
  int len;
  ...
  if (nr_txn_is_account_trusted (txn, (int) account_id)) {
    ...
    if (txn->special_flags.debug_cat) {
      nrl_verbosedebug (NRL_CAT, "CAT: account is untrusted: id=%d", (int)account_id);
    }
    return NR_FAILURE;
  }
}
...
char *
nr_header_inbound_response_internal (nrtxn_t *txn, int content_length)
{
  nrtime_t qtime;
  ...
  cross_process_id = nro_get_hash_string (txn->app_connect_reply, "cross_process_id", 0);
  ...
  nro_set_hash_string (txn->intrinsics, "client_cross_process_id", txn->cat.client_cross_process_id);
  snprintf (buf, sizeof (buf), "ClientApplication/%s/all", txn->cat.client_cross_process_id);
  ...
  obj = nro_new_array ();
  nro_set_array_string  (obj, (int)NR_RESPONSE_HDR_FIELD_INDEX_CROSS_PROCESS_ID, cross_process_id);
  nro_set_array_string  (obj, (int)NR_RESPONSE_HDR_FIELD_INDEX_TXN_NAME,         txn->name);
  nro_set_array_double  (obj, (int)NR_RESPONSE_HDR_FIELD_INDEX_QUEUE_TIME,       qtime_seconds);
  nro_set_array_double  (obj, (int)NR_RESPONSE_HDR_FIELD_INDEX_RESPONSE_TIME,    apptime_seconds);
  nro_set_array_int     (obj, (int)NR_RESPONSE_HDR_FIELD_INDEX_CONTENT_LENGTH,   content_length);
  nro_set_array_string  (obj, (int)NR_RESPONSE_HDR_FIELD_INDEX_GUID,             txn->guid);
  nro_set_array_boolean (obj, (int)NR_RESPONSE_HDR_FIELD_INDEX_RECORD_TT,        0); /* record_tt currently always false */
...
char *
nr_header_inbound_synthetics (const nrtxn_t *txn,
...
char *
nr_header_inbound_response (nrtxn_t *txn, int content_length)
{
  char *response;
...
char *
nr_header_outbound_request_synthetics_encoded (const nrtxn_t *txn)
{
  const char *decoded;
...
void
nr_header_outbound_request_decoded (
  nrtxn_t *txn,
  char **decoded_id_ptr,
...
void
nr_header_outbound_request (nrtxn_t *txn,
                            ...
                            char **x_newrelic_synthetics_ptr)
{
   ...
   *
   * Having said that, you can disable this altogether with the
   * newrelic.synthetics.enabled setting.
   */
  if (x_newrelic_synthetics_ptr) {
...
static void
nr_header_outbound_response_object (nrtxn_t *txn, const nrobj_t *response_obj,
                                    ...
                                    char **external_guid_ptr)
{
  ...
  external_id = nro_get_array_string (response_obj, (int)NR_RESPONSE_HDR_FIELD_INDEX_CROSS_PROCESS_ID, 0);
  if (0 == external_id) {
    return;
  }
  ...
  external_txnname = nro_get_array_string (response_obj, (int)NR_RESPONSE_HDR_FIELD_INDEX_TXN_NAME, 0);
  if (0 == external_txnname) {
    return;
  }

  if (response_size >= (int)NR_RESPONSE_HDR_FIELD_INDEX_GUID) {
    external_guid = nro_get_array_string (response_obj, (int)NR_RESPONSE_HDR_FIELD_INDEX_GUID, 0);
    if (0 == external_guid) {
      return;
    }
  }
  ...
  if (response_size >= (int)NR_RESPONSE_HDR_FIELD_INDEX_RECORD_TT) {
    ...
    external_record_tt = nro_get_array_boolean (response_obj, (int)NR_RESPONSE_HDR_FIELD_INDEX_RECORD_TT, &err);
    if (NR_SUCCESS != err) {
      return;
    }
...
void
nr_header_outbound_response_decoded (nrtxn_t *txn, const char *decoded_response,
                                     char **external_id_ptr, char **external_txnname_ptr,
                                     char **external_guid_ptr)
{
  nrobj_t *response_obj = nro_create_from_json (decoded_response);

  nr_header_outbound_response_object (txn, response_obj, external_id_ptr, external_txnname_ptr,
                                      external_guid_ptr);
...
nr_header_outbound_response (nrtxn_t *txn,
                             ...
                             char **external_txnname_ptr, char **external_guid_ptr)
{
...
char *
nr_header_extract_encoded_value (const char *header_name, const char *string)
{
  int name_start;
  int value_start;
  int value_end;
...
char *
nr_header_format_name_value (const char *name, const char *value, int include_return_newline)
{
  int len;
  char *hdr;
 ...
 */
static int
nr_header_is_valid_token_char (char ch)
{
  if (('a' <= ch) && (ch <= 'z')) {
    return 1;
  }
 ...
 */
char *
nr_header_parse_content_type (const char *header)
{
  const char *colon;
...
static int
nr_header_obj_is_null_or_string (const nrobj_t *obj)
{
  return (NULL == obj) ||
    (NR_OBJECT_NONE == nro_type (obj)) ||
    (NR_OBJECT_STRING == nro_type (obj));
}

nr_status_t
nr_header_process_x_newrelic_transaction (nrtxn_t *txn, const nrobj_t *x_newrelic_txn)
{
  ...
  nro_set_hash_string (txn->intrinsics, "referring_transaction_guid", inbound_guid);
...
nr_status_t
nr_header_set_cat_txn (nrtxn_t *txn,
...
nr_status_t
nr_header_set_synthetics_txn (nrtxn_t *txn, const char *header)
{
  int account = 0;
  char *decoded = NULL;
  ...
  if (NULL != txn->synthetics) {
    nrl_verbosedebug (NRL_TXN, "%s: transaction already has synthetics", __func__);
    return NR_FAILURE;
  }
