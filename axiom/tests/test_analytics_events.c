...
static void
add_event_from_json (nr_analytics_events_t *events, const char *event_json, nr_random_t *rrnd)
{
  nr_analytics_event_t *event = nr_analytics_event_create_from_string (event_json);

  nr_analytics_events_add_event (events, event, rrnd);
  nr_analytics_event_destroy (&event);
}

#define test_json_is_valid(...) test_json_is_valid_fn (__VA_ARGS__,__FILE__,__LINE__)

static void
test_json_is_valid_fn (const char *testname, const char *json, const char *file, int line)
{
  nrobj_t *obj;
...
static nr_analytics_event_t *
create_sample_event (void)
{
  ...
  event = nr_analytics_event_create (builtin_fields, agent_attributes, user_attributes);
...
test_event_create (void)
{
  ...
  event = nr_analytics_event_create (builtin_fields, agent_attributes, user_attributes);
  tlib_pass_if_true ("event created", 0 == nr_strcmp ((const char *)event,
    "["
      "{"
        "\"type\":\"Transaction\","
        "\"name\":\"escape\\/me\","
  ...
  tlib_pass_if_true ("empty attributes", 0 == nr_strcmp ((const char *)event,
    "["
      "{},"
      "{},"
      "{}"
  ...
  tlib_pass_if_true ("null attributes", 0 == nr_strcmp ((const char *)event,
    "["
      "{},"
      "{},"
      "{}"
...
static void
test_event_create_bad_params (void)
{
  nr_analytics_event_t *event;
  nrobj_t *builtin_fields = nro_new_hash ();
  nrobj_t *user_attributes = nro_new_hash ();
  ...
  event = nr_analytics_event_create (not_hash, user_attributes, agent_attributes);
...
static void
test_events_add_event_success (void)
{
  ...
  tlib_pass_if_true ("event added", 0 == nr_strcmp (json,
      "["
        "{"
          "\"type\":\"Transaction\","
          "\"name\":\"escape\\/me\","
  ...
  tlib_pass_if_true ("only user params", 0 == nr_strcmp (json,
    "[{},{\"x\":123,\"y\":\"z\"}]"),
    "json=%s", NRSAFESTR (json));
  nr_analytics_event_destroy (&event);
  nr_analytics_events_destroy (&events);
...
static void
test_events_create_bad_param (void)
{
  nr_analytics_events_t *events;
...
static void
test_events_add_event_failure (void)
{
  nr_analytics_events_t *events;
  nr_analytics_event_t *event;
  nr_random_t *rnd = nr_random_create_from_seed (12345);
...
test_max_observed (void)
{
  ...
  tlib_pass_if_int_equal ("max observed", max, nr_analytics_events_number_saved (events));
  tlib_pass_if_int_equal ("max observed", max + 1, nr_analytics_events_number_seen (events));

  for (i = 0; i < max; i++) {
    tlib_pass_if_str_equal ("max observed", event_json, nr_analytics_events_get_event_json (events, 0));
  }
...
static void
test_reservoir_replacement (void)
{
  int i;
  int max = 100;
  nr_analytics_events_t *events = nr_analytics_events_create (max);
  ...
  for (i = 0; i < max; i++) {
    ...
    tlib_pass_if_true ("number saved", i + 1 == saved, "i=%d saved=%d", i, saved);
  }
  ...
  for (i = 0; i < 10 * max; i++) {
    ...
    seen = nr_analytics_events_number_seen (events);
    saved = nr_analytics_events_number_saved (events);
    tlib_pass_if_true ("number seen", max + (2*(i+1)) == seen, "max=%d i=%d seen=%d", max, i, seen);
    tlib_pass_if_true ("number saved", max == saved, "max=%d saved=%d", max, saved);
  }
  ...
  tlib_pass_if_true ("test is valid", max == count1 + count2, "max=%d count1=%d count2=%d", max, count1, count2);
  tlib_pass_if_true ("approx equal counts", count1 > (max / 4), "max=%d count1=%d", max, count1);
  tlib_pass_if_true ("approx equal counts", count2 > (max / 4), "max=%d count2=%d", max, count2);
...
static void
test_events_destroy_bad_params (void)
{
...
static void
test_number_seen_bad_param (void)
{
  tlib_pass_if_int_equal ("null events", nr_analytics_events_number_seen (NULL), 0);
}

static void
test_number_saved_bad_param (void)
{
  tlib_pass_if_int_equal ("null events", nr_analytics_events_number_saved (NULL), 0);
}

static void
test_event_int_long (void)
{
  nr_analytics_events_t *events = nr_analytics_events_create (10);
  const char *json;
  nr_random_t *rnd = nr_random_create_from_seed (12345);

  add_event_from_json (events, "[{\"my_int\":123,\"my_long\":9223372036854775807},{}]", rnd);

  json = nr_analytics_events_get_event_json (events, 0);

  test_json_is_valid ("event added", json);
  tlib_pass_if_str_equal ("event added", json, "[{\"my_int\":123,\"my_long\":9223372036854775807},{}]");
...
static void
test_analytics_events_get_event_json (void)
{
  ...
  tlib_pass_if_null ("negative index", nr_analytics_events_get_event_json (events, -1));
  tlib_pass_if_null ("high index", nr_analytics_events_get_event_json (events, 1));

  tlib_pass_if_str_equal ("success", nr_analytics_events_get_event_json (events, 0), "[{\"a\":1},{\"b\":2}]");
