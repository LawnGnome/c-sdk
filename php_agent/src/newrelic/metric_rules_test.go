...
func (tc *CrossAgentRulesTestcase) run(t *testing.T) {
	// This test relies upon Perl-specific regex syntax (negative lookahead
	// assertions) which are not implemented in Go's regexp package.
	// We believe these types of rules are exceedingly rare in practice, so we're
	// skipping implementation of this exotic syntax for now.
