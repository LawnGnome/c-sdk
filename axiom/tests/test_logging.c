...
/*
 * throw away any strings involving the uid or gid.  This filter is pretty coarse.
 */
const char *cleanup_string = "sed "
  "-e 's/ os=[^]]*//g' "
  "-e 's/id=[0-9]*/id=-1/g' "
...
test_logging (void)
{
  ...
  /*
   * Try sending a log message before anything has been initialized. Should fail.
   */
  rv = nrl_send_log_message (NRL_ALWAYS, "test should fail");
  tlib_pass_if_true ("log write before initialization failed", NR_FAILURE == rv, "rv=%d", (int)rv);

  /*
   * Try to initialize to an impossible to write to file. Should fail.
   */
  rv = nrl_set_log_file ("/should/not/exist");
  tlib_pass_if_true ("initialization to bad path fails", NR_FAILURE == rv, "rv=%d", (int)rv);

  /*
   * Attempts to log should still fail.
   */
  rv = nrl_send_log_message (NRL_ALWAYS, "test should fail");
  tlib_pass_if_true ("log write after failed init fails", NR_FAILURE == rv, "rv=%d", (int)rv);

  /*
   * Open to a valid file should succeed.
   */
  rv = nrl_set_log_file ("./logtest1.tmp");
  tlib_pass_if_true ("log initialization succeeds", NR_SUCCESS == rv, "rv=%d", (int)rv);
   ...
   */
  {
    ...
    const char *long_appname = "01234567890123456789012345678901234567890123456789";
    nrl_error (NRL_TEST, "A short appname " NRP_FMT, NRP_APPNAME (short_appname));
    nrl_error (NRL_TEST, "A 50 char appname truncated " NRP_FMT, NRP_APPNAME (long_appname));  /* 50 chars */
  }
   ...
   */
  rv = nrl_send_log_message (NRL_ALWAYS, "test should fail");
  tlib_pass_if_true ("log write after close failed", NR_FAILURE == rv, "rv=%d", (int)rv);
 ...
 * We will also actually call the macro functions at key points too and
...
static void
test_set_log_level_simple (void)
{
  nr_status_t rv;
  ...
  tlib_pass_if_true ("mask[always] = ALL", NRL_ALL_FLAGS == nrl_level_mask_ptr[NRL_ALWAYS],
    "mask=0x%08x", nrl_level_mask_ptr[NRL_ALWAYS]);
  tlib_pass_if_true ("mask[error] = ALL", NRL_ALL_FLAGS == nrl_level_mask_ptr[NRL_ERROR],
  ...
  tlib_pass_if_true ("mask[verbosedebug] = 0", 0 == nrl_level_mask_ptr[NRL_VERBOSEDEBUG],
...
test_set_log_level_null (void)
{
  ...
  tlib_pass_if_true ("mask[error] = ALL", NRL_ALL_FLAGS == nrl_level_mask_ptr[NRL_ERROR],
    "mask=0x%08x", nrl_level_mask_ptr[NRL_ERROR]);
  tlib_pass_if_true ("mask[warning] = ALL", NRL_ALL_FLAGS == nrl_level_mask_ptr[NRL_WARNING],
    "mask=0x%08x", nrl_level_mask_ptr[NRL_WARNING]);
  tlib_pass_if_true ("mask[info] = ALL", NRL_ALL_FLAGS == nrl_level_mask_ptr[NRL_INFO],
    "mask=0x%08x", nrl_level_mask_ptr[NRL_INFO]);
  tlib_pass_if_true ("mask[verbose] = 0", 0 == nrl_level_mask_ptr[NRL_VERBOSE],
    "mask=0x%08x", nrl_level_mask_ptr[NRL_VERBOSE]);
  tlib_pass_if_true ("mask[debug] = 0", 0 == nrl_level_mask_ptr[NRL_DEBUG],
...
static void
test_set_log_level_warning (void)
{
  nr_status_t rv;
...
static void
test_set_log_level_verbose (void)
{
  nr_status_t rv;
  ...
  tlib_pass_if_true ("mask[info] = ALL", NRL_ALL_FLAGS == nrl_level_mask_ptr[NRL_INFO],
    "mask=0x%08x", nrl_level_mask_ptr[NRL_INFO]);
  tlib_pass_if_true ("mask[verbose] = ALL", NRL_ALL_FLAGS == nrl_level_mask_ptr[NRL_VERBOSE],
    "mask=0x%08x", nrl_level_mask_ptr[NRL_VERBOSE]);
  tlib_pass_if_true ("mask[debug] = 0", 0 == nrl_level_mask_ptr[NRL_DEBUG],
    "mask=0x%08x", nrl_level_mask_ptr[NRL_DEBUG]);
  tlib_pass_if_true ("mask[verbosedebug] = 0", 0 == nrl_level_mask_ptr[NRL_VERBOSEDEBUG],
...
test_set_log_level_debug (void)
{
  ...
  tlib_pass_if_true ("mask[verbose] = ALL", NRL_ALL_FLAGS == nrl_level_mask_ptr[NRL_VERBOSE],
    "mask=0x%08x", nrl_level_mask_ptr[NRL_VERBOSE]);
  tlib_pass_if_true ("mask[debug] = ALL", NRL_ALL_FLAGS == nrl_level_mask_ptr[NRL_DEBUG],
    "mask=0x%08x", nrl_level_mask_ptr[NRL_DEBUG]);
  tlib_pass_if_true ("mask[verbosedebug] = 0", 0 == nrl_level_mask_ptr[NRL_VERBOSEDEBUG],
    "mask=0x%08x", nrl_level_mask_ptr[NRL_VERBOSEDEBUG]);
...
static void
test_set_log_level_verbosedebug (void)
{
  nr_status_t rv;

  rv = nrl_set_log_level ("verbosedebug");
  tlib_pass_if_true ("set level (verbosedebug)", NR_SUCCESS == rv, "rv=%d", (int)rv);
  tlib_pass_if_true ("mask[always] = ALL", NRL_ALL_FLAGS == nrl_level_mask_ptr[NRL_ALWAYS],
    "mask=0x%08x", nrl_level_mask_ptr[NRL_ALWAYS]);
  tlib_pass_if_true ("mask[error] = ALL", NRL_ALL_FLAGS == nrl_level_mask_ptr[NRL_ERROR],
    "mask=0x%08x", nrl_level_mask_ptr[NRL_ERROR]);
  ...
  tlib_pass_if_true ("mask[debug] = ALL", NRL_ALL_FLAGS == nrl_level_mask_ptr[NRL_DEBUG],
    "mask=0x%08x", nrl_level_mask_ptr[NRL_DEBUG]);
  tlib_pass_if_true ("mask[verbosedebug] = ALL", NRL_ALL_FLAGS == nrl_level_mask_ptr[NRL_VERBOSEDEBUG],
    "mask=0x%08x", nrl_level_mask_ptr[NRL_VERBOSEDEBUG]);
...
static void
test_set_log_level_overall (void)
{
  nr_status_t rv;
  ...
  tlib_pass_if_true ("set level (warning,autorum=verbose,framework=verbosedebug)", NR_SUCCESS == rv, "rv=%d", (int)rv);
  tlib_pass_if_true ("mask[always] = ALL", NRL_ALL_FLAGS == nrl_level_mask_ptr[NRL_ALWAYS],
    "mask=0x%08x", nrl_level_mask_ptr[NRL_ALWAYS]);
  tlib_pass_if_true ("mask[error] = ALL", NRL_ALL_FLAGS == nrl_level_mask_ptr[NRL_ERROR],
    "mask=0x%08x", nrl_level_mask_ptr[NRL_ERROR]);
  tlib_pass_if_true ("mask[warning] = ALL", NRL_ALL_FLAGS == nrl_level_mask_ptr[NRL_WARNING],
    "mask=0x%08x", nrl_level_mask_ptr[NRL_WARNING]);
  tlib_pass_if_true ("mask[info] = ALL", (NRL_AUTORUM | NRL_FRAMEWORK) == nrl_level_mask_ptr[NRL_INFO],
    "mask=0x%08x", nrl_level_mask_ptr[NRL_INFO]);
  tlib_pass_if_true ("mask[verbose] = 0", (NRL_AUTORUM | NRL_FRAMEWORK) == nrl_level_mask_ptr[NRL_VERBOSE],
    "mask=0x%08x", nrl_level_mask_ptr[NRL_VERBOSE]);
  tlib_pass_if_true ("mask[debug] = 0", NRL_FRAMEWORK == nrl_level_mask_ptr[NRL_DEBUG],
    "mask=0x%08x", nrl_level_mask_ptr[NRL_DEBUG]);
  tlib_pass_if_true ("mask[verbosedebug] = 0", NRL_FRAMEWORK == nrl_level_mask_ptr[NRL_VERBOSEDEBUG],
    "mask=0x%08x", nrl_level_mask_ptr[NRL_VERBOSEDEBUG]);
  ...
  nrl_verbosedebug (NRL_FRAMEWORK, "NRL_VERBOSEDEBUG(FRAMEWORK) should be present (11)");
}

static void
test_set_log_level_invalid_subsystem (void)
{
  nr_status_t rv;
  ...
  tlib_pass_if_true ("set level (verbosedebug,bogus=debug)", NR_FAILURE == rv, "rv=%d", (int)rv);
  tlib_pass_if_true ("mask[always] = ALL", NRL_ALL_FLAGS == nrl_level_mask_ptr[NRL_ALWAYS],
    "mask=0x%08x", nrl_level_mask_ptr[NRL_ALWAYS]);
  tlib_pass_if_true ("mask[error] = ALL", NRL_ALL_FLAGS == nrl_level_mask_ptr[NRL_ERROR],
    "mask=0x%08x", nrl_level_mask_ptr[NRL_ERROR]);
...
static void
test_set_log_level_3_subsystems (void)
{
  nr_status_t rv;
  ...
  tlib_pass_if_true ("set level (metrics=info,listener=verbose,daemon=verbosedebug)", NR_SUCCESS == rv, "rv=%d", (int)rv);
  tlib_pass_if_true ("mask[always] = ALL", NRL_ALL_FLAGS == nrl_level_mask_ptr[NRL_ALWAYS],
    "mask=0x%08x", nrl_level_mask_ptr[NRL_ALWAYS]);
  tlib_pass_if_true ("mask[error] = METRICS|LISTENER|DAEMON", (NRL_METRICS | NRL_LISTENER | NRL_DAEMON) == nrl_level_mask_ptr[NRL_ERROR],
    "mask=0x%08x", nrl_level_mask_ptr[NRL_ERROR]);
  tlib_pass_if_true ("mask[warning] = METRICS|LISTENER|DAEMON", (NRL_METRICS | NRL_LISTENER | NRL_DAEMON) == nrl_level_mask_ptr[NRL_WARNING],
    "mask=0x%08x", nrl_level_mask_ptr[NRL_WARNING]);
  tlib_pass_if_true ("mask[info] = METRICS|LISTENER|DAEMON", (NRL_METRICS | NRL_LISTENER | NRL_DAEMON) == nrl_level_mask_ptr[NRL_INFO],
    "mask=0x%08x", nrl_level_mask_ptr[NRL_INFO]);
  tlib_pass_if_true ("mask[verbose] = LISTENER|DAEMON", (NRL_LISTENER | NRL_DAEMON) == nrl_level_mask_ptr[NRL_VERBOSE],
    "mask=0x%08x", nrl_level_mask_ptr[NRL_VERBOSE]);
  tlib_pass_if_true ("mask[debug] = DAEMON", NRL_DAEMON == nrl_level_mask_ptr[NRL_DEBUG],
    "mask=0x%08x", nrl_level_mask_ptr[NRL_DEBUG]);
  tlib_pass_if_true ("mask[verbosedebug] = DAEMON", NRL_DAEMON == nrl_level_mask_ptr[NRL_VERBOSEDEBUG],
    "mask=0x%08x", nrl_level_mask_ptr[NRL_VERBOSEDEBUG]);
  ...
  nrl_verbosedebug (NRL_TEST, "NRL_VERBOSEDEBUG(TEST) should not be present (13)");
  nrl_verbosedebug (NRL_LISTENER, "NRL_VERBOSEDEBUG(LISTENER) should not be present (13)");
  nrl_verbosedebug (NRL_METRICS, "NRL_VERBOSEDEBUG(METRICS) should not be present (13)");
  nrl_verbosedebug (NRL_DAEMON, "NRL_VERBOSEDEBUG(DAEMON) should be present (13)");
}
...
static void
test_vlog_helper (nrloglev_t level, uint32_t subsystem, const char *fmt, ...)
{
  va_list ap;
...
test_vlog (void)
{
  ...
  tlib_pass_if_not_diff ("vlogtest.tmp", REFERENCE_DIR "/test_vlog.cmp", cleanup_string, 0, 0);
}

static void
test_format_timestamp (const char *msg, time_t utc_time, const char *expected_timestamp)
{
  char buf[64];
  struct timeval tv;
...
test_timezones (void)
{
   ...
   * the local timezone. It would be nice to be able to use timegm(3)
   * instead, but it's not available on Solaris. Alternatively, we
   * could implement our own equivalent (nr_timegm?).
   */
  ...
  test_format_timestamp ("integral timezone with negative offset during daylight savings",
  ...
  test_format_timestamp ("half hour timezone with negative offset during daylight savings",
  ...
  test_format_timestamp ("integral timezone with a positive offset during daylight savings",
  ...
  test_format_timestamp ("quarter hour timezone with a positive offset during daylight savings",
    jul1_midnight_utc, "2015-07-01 12:45:00.000 +1245");

  setenv ("TZ", "Pacific/Honolulu", 1 /* overwrite */);
  tzset ();
  test_format_timestamp ("timezone without daylight savings",
    jan1_midnight_utc, "2014-12-31 14:00:00.000 -1000");
  test_format_timestamp ("timezone without daylight savings",
    jul1_midnight_utc, "2015-06-30 14:00:00.000 -1000");
...
/*
 * TODO: This test has not been reworked to run in parallel.
 */
...
#if HAVE_BACKTRACE
  tlib_pass_if_not_diff ("logtest1.tmp", REFERENCE_DIR "/test_logging_1.cmp", cleanup_string, 0, 0);
#else
  tlib_pass_if_not_diff ("logtest1.tmp", REFERENCE_DIR "/test_logging_1_no_backtrace.cmp", cleanup_string, 0, 0);
#endif

  tlib_pass_if_not_diff ("logtest2.tmp", REFERENCE_DIR "/test_logging_2.cmp", cleanup_string, 0, 0);
