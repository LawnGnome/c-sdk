...
int
nr_reply_get_int (const nrobj_t *reply, const char *name, int dflt)
{
  const nrobj_t *rp;
  int ret;
  nr_status_t err;
...
int
nr_reply_get_bool (const nrobj_t *reply, const char *name, int dflt)
{
  const nrobj_t *rp;
  int ret;
  nr_status_t err;
  ...
  if (('1' == c) || ('y' == c) || ('Y' == c) || ('t' == c) || ('T' == c) || (0 == nr_stricmp (str, "on"))) {
    return 1;
  }

  if (('0' == c) || ('n' == c) || ('N' == c) || ('f' == c) || ('F' == c) || (0 == nr_stricmp (str, "off"))) {
    return 0;
  }
...
double
nr_reply_get_double (const nrobj_t *reply, const char *name, double dflt)
{
  const nrobj_t *rp;
  nr_status_t err;
  double ret;
