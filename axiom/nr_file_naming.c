...
static void
nr_file_namer_destroy_internal (nr_file_naming_t *namer)
{
...
void
nr_file_namer_destroy (nr_file_naming_t **namer_ptr)
{
...
static char *
nr_file_namer_match_one (const nr_file_naming_t *namer, const char *filename)
{
  ...
  if (NULL == s) {
    nrl_error (NRL_AGENT, "unexpected NULL substring for filename="
      NRP_FMT " pattern=" NRP_FMT,
      NRP_FILENAME (filename),
      NRP_PHP (namer->user_pattern));
  }
...
char *
nr_file_namer_match (const nr_file_naming_t *namer, const char *filename)
{
...
static nr_file_naming_t *
nr_file_namer_create (const char *user_pattern)
{
  int len;
  ...
  new_namer->regex = nr_regex_create (regex_pattern,
    nr_file_naming_regex_options, 1);

  nr_free (regex_pattern);

  if (NULL == new_namer->regex) {
    nrl_error (NRL_INSTRUMENT,
      "invalid regular expression pattern used in the value of transaction file namer" NRP_FMT,
...
nr_file_naming_t *
nr_file_namer_append (nr_file_naming_t *curr_head, const char *user_pattern)
{
