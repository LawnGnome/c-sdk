/*
 * This file contains functions dealing with RPM command replies.
...
/*
 * Support functions for extracting real values out of an RPM reply. These do
 * error checking and return sensible values in the face of an error.
 */

/*
 * Purpose : Extract an int, int64_t, boolean or double, with defaults, from
 *           the reply returned by a call to RPM.
 *
 ...
 *              is not present in the RPM reply.
 *
...
extern double nr_reply_get_double (const nrobj_t *reply, const char *name, double dflt);
