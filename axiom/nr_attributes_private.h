/*
 * This file contains private attribute functions and data structures.
 */
...
typedef struct _nr_attribute_destination_modifier_t {
  int        has_wildcard_suffix;    /* Whether or 'match' is exact or a prefix. */
  ...
  uint32_t   include_destinations;   /* Bit set of destinations to be added by this modifier. */
  uint32_t   exclude_destinations;   /* Bit set of destinations to be dropped by this modifier. */
  struct _nr_attribute_destination_modifier_t *next; /* Next linked list entry */
} nr_attribute_destination_modifier_t;

struct _nr_attribute_config_t {
  uint32_t disabled_destinations;    /* Destinations that no attributes should go to. */
  /*
...
typedef struct _nr_attribute_t {
  ...
  uint32_t key_hash;
  nrobj_t *value;
  uint32_t destinations;        /* Set of destinations after config has been applied. */
...
struct _nr_attributes_t {
  ...
  struct _nr_attribute_t         *agent_attribute_list; /* Unordered linked list of agent attributes. */
  struct _nr_attribute_t         *user_attribute_list;  /* Unordered linked list of user attributes. */
};
...
extern void nr_attribute_destination_modifier_destroy (nr_attribute_destination_modifier_t **entry_ptr);
extern nr_attribute_destination_modifier_t *nr_attribute_destination_modifier_create (
  const char            *match,
  uint32_t               include_destinations,
  uint32_t               exclude_destinations);
extern nr_attribute_config_t *nr_attribute_config_copy (const nr_attribute_config_t *config);
extern uint32_t nr_attribute_config_apply (
  const nr_attribute_config_t *config,
  const char *key,
  uint32_t key_hash,
  uint32_t destinations);
extern void nr_attribute_destroy (nr_attribute_t **attribute_ptr);
extern void nr_attributes_remove_duplicate (nr_attributes_t *ats, const char *key, uint32_t key_hash, int is_user);
