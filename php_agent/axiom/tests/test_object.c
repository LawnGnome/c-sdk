...
nrotype_t otypes[] = {
    ...
    NR_OBJECT_LONG,    NR_OBJECT_DOUBLE, NR_OBJECT_JSTRING, NR_OBJECT_STRING,
    NR_OBJECT_HASH,    NR_OBJECT_ARRAY,
};
...
static void test_find_array_int(void) {
  ...
  array_position = nro_find_array_int(ob, 123);
  tlib_pass_if_true("success", 3 == array_position, "array_position=%d",
                    array_position);
 ...
 * but the json encoder should not loop when converting corner cases and bogus
 * UTF8.
 */
...
Object Dump (4):\n\
   ...
   * all it tests is wrap around of 2's complement arithmetic in a given sized
   * integer. It has little value, and it sets off gcc warning messages.
   */
...
Object Dump (9):\n\
  ...
  tlib_pass_if_true("get hash long fails", (-1LL == lv) && (NR_FAILURE == err),
                    "lv=%d err=%d", (int)lv, (int)err);
...
Object Dump (10):\n\
  ...
  tlib_pass_if_true("get array int fails", (-1LL == lv) && (NR_FAILURE == err),
                    "lv=%d err=%d", (int)lv, (int)err);
  ...
  tlib_pass_if_true("populated array to json",
                    (0 != js)
                        && (0
                            == nr_strcmp(js,
                                         "\
[true,123,456,789,101112,131415.16170,\"abc\",[1,2,3],{\"subhash-long\":1,\"subhash-int\":2},[4,5]]")),
                    "js=%s", js);
