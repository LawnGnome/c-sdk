...
static void
test_regex_option_mapping_f (int pcre_option, int nr_option,
                             const char *pcre_name, const char *nr_name)
{
  char *message = NULL;
  ...
  pcre_fullinfo (regex->code, regex->extra, PCRE_INFO_OPTIONS,
                 (void *) &options);

  tlib_pass_if_true (message, options & pcre_option, "options=%lu expected=%d", options, pcre_option);
...
#define test_regex_option_mapping(pcre_option, nr_option) \
  test_regex_option_mapping_f ((pcre_option), (nr_option), #pcre_option, #nr_option)

static void
test_regex_create (void)
{
...
test_regex_match (void)
{
   ...
   * Test : Bad parameters.
   */
  tlib_pass_if_status_failure ("NULL regex", nr_regex_match (NULL, NR_PSTR ("foo")));
  tlib_pass_if_status_failure ("NULL string", nr_regex_match (regex, NULL, 0));
  tlib_pass_if_status_failure ("negative length", nr_regex_match (regex, "foo", -1));

  /*
   * Test : Normal operation.
   */
  tlib_pass_if_status_failure ("non matching", nr_regex_match (regex, NR_PSTR ("foo")));
  tlib_pass_if_status_success ("matching", nr_regex_match (regex, NR_PSTR ("123")));

  nr_regex_destroy (&regex);
}

static void
test_regex_match_capture (void)
{
  nr_regex_t *regex = nr_regex_create ("^[0-9]+$", 0, 1);
  ...
  tlib_pass_if_null ("negative length", nr_regex_match_capture (regex, "foo", -1));

  /*
   * Test : No matches.
   */
  tlib_pass_if_null ("non matching", nr_regex_match_capture (regex, NR_PSTR ("foo")));
...
static void
test_regex_substrings_create (void)
{
   ...
   * Test : Bad parameters.
   */
  tlib_pass_if_null ("negative count", nr_regex_substrings_create (-1));

  /*
   * Test : Normal operation.
   */
  ss = nr_regex_substrings_create (0);
  ...
  ss = nr_regex_substrings_create (1);
...
static void
test_regex_substrings_destroy (void)
{
  nr_regex_substrings_t *ss = nr_regex_substrings_create (1);
...
static void
test_regex_substrings_count (void)
{
  nr_regex_substrings_t *ss = nr_regex_substrings_create (1);
...
static void
test_regex_substrings_get (void)
{
...
static void
test_regex_substrings_get_offsets (void)
{
  int offsets[2] = { -1, -1 };
  nr_regex_t *regex = nr_regex_create ("^([a-z]+)-([0-9]+)$", 0, 0);
  nr_regex_substrings_t *ss = nr_regex_match_capture (regex, NR_PSTR ("foo-123"));
   ...
   * Test : Bad parameters.
   */
  tlib_pass_if_status_failure ("NULL ss", nr_regex_substrings_get_offsets (NULL, 0, offsets));
  tlib_pass_if_status_failure ("negative index", nr_regex_substrings_get_offsets (ss, -1, offsets));
  tlib_pass_if_status_failure ("out of bounds index", nr_regex_substrings_get_offsets (ss, 3, offsets));
  tlib_pass_if_status_failure ("NULL offsets", nr_regex_substrings_get_offsets (ss, 0, NULL));
   ...
   * Test : Normal operation.
   */
  tlib_pass_if_status_success ("whole match", nr_regex_substrings_get_offsets (ss, 0, offsets));
  tlib_pass_if_int_equal ("whole match", 0, offsets[0]);
  tlib_pass_if_int_equal ("whole match", 7, offsets[1]);

  tlib_pass_if_status_success ("subpattern match", nr_regex_substrings_get_offsets (ss, 2, offsets));
...
test_regex_quote (void)
{
  ...
  tlib_pass_if_size_t_equal ("NULL string doesn't change quoted_len", 1, quoted_len);
...
static void
test_regex_escaping_f (const char *message, const char *expected, const char *input, size_t len)
{
  nrbuf_t *buf;
...
#define test_regex_escaping(M, EXP, INPUT) test_regex_escaping_f ((M), (EXP), NR_PSTR (INPUT))

static void
test_regex_add_quoted_to_buffer (void)
{
  nrbuf_t *buf;
