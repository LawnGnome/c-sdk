 ...
 * rather than monkey around with (re)defining STDC_VERSION, just define plausible replacements.
 */
...
#define nro_test(T, O, C) \
 ...
 \
    ...
    tlib_pass_if_true (T, 0 == strcmp_result, "strcmp_result=%d dump=%s correct=%s", strcmp_result, dump, C); \
    nr_free (dump); \
  }
...
#define INT_TEST(S, G, C) \
  { \
    ...
    tlib_pass_if_true ("int get fails OK", (-1 == gotten_int) && \
      (NR_FAILURE == err), "gotten_int=%d err=%d", gotten_int, (int)err); \
    gotten_int = nro_get_int (intob, &err); \
    tlib_pass_if_true ("int get success", ((G) == C gotten_int) && \
      (NR_SUCCESS == err), "gotten_int=%d err=%d", gotten_int, (int)err); \
    nro_delete (intob); \
  }
...
#define LONG_TEST(S, G, C) \
  { \
    ...
    tlib_pass_if_true ("long get fails OK", (-1L == gotten_long) && \
      (NR_FAILURE == err), "gotten_long=" NR_INT64_FMT " err=%d", gotten_long, (int)err); \
    gotten_long = nro_get_long (longob, &err); \
    tlib_pass_if_true ("long set correctly", ((G) == C gotten_long) && \
      (NR_SUCCESS == err), "gotten_long=" NR_INT64_FMT " err=%d", gotten_long, (int)err); \
    nro_delete (longob); \
  }
...
#define DOUBLE_TEST(S, G, C) \
  { \
    ...
    tlib_pass_if_true ("double get fails OK", (gotten_double == -1.0) && \
      (NR_FAILURE == err), "gotten_double=%lf err=%d", gotten_double, (int)err); \
    gotten_double = nro_get_double (doubleob, &err); \
    tlib_pass_if_true ("double set correctly", ((G) == C gotten_double) && \
      (NR_SUCCESS == err), "gotten_double=%lf err=%d", gotten_double, (int)err); \
    nro_delete (doubleob); \
  }

static void
test_find_array_int (void)
{
  nrobj_t *ob;
  int array_position;

  array_position = nro_find_array_int (0, 123);
  tlib_pass_if_true ("zero input", -1 == array_position, "array_position=%d", array_position);

  ob = nro_new_hash ();
  array_position = nro_find_array_int (ob, 123);
  tlib_pass_if_true ("wrong type", -1 == array_position, "array_position=%d", array_position);
  nro_delete (ob);

  ob = nro_new_array ();

  array_position = nro_find_array_int (ob, 123);
  tlib_pass_if_true ("empty array", -1 == array_position, "array_position=%d", array_position);

  nro_set_array_long (ob, 0, 123);
  array_position = nro_find_array_int (ob, 123);
  tlib_pass_if_true ("long not int", -1 == array_position, "array_position=%d", array_position);

  nro_set_array_int (ob, 0, 456);
  array_position = nro_find_array_int (ob, 123);
  tlib_pass_if_true ("wrong int", -1 == array_position, "array_position=%d", array_position);

  nro_set_array_int (ob, 0, 123);
  array_position = nro_find_array_int (ob, 123);
  tlib_pass_if_true ("success", 3 == array_position, "array_position=%d", array_position);

  nro_delete (ob);
}

static void
test_incomensurate_get (void)
{
  nrobj_t *ob;
  nrobj_t *oi;
  const nrobj_t *oh;
  ...
  tlib_pass_if_true ("get string failure", NULL == sv, "sv=%s", sv ? sv : "<null>" );
...
test_nro_getival (void)
{
  ...
  tlib_pass_if_true ("nro_get_ival int extraction", NR_SUCCESS == err, "err=%d", (int)err);
  tlib_pass_if_true ("nro_get_ival int extraction", 3 == rv, "rv=%d", rv);
  nro_delete (ob);

  ob = nro_new_boolean (1);
  rv = nro_get_ival (ob, &err);
  tlib_pass_if_true ("nro_get_ival bool extraction", NR_SUCCESS == err, "err=%d", (int)err);
  tlib_pass_if_true ("nro_get_ival bool extraction", 1 == rv, "rv=%d", rv);
  nro_delete (ob);

  ob = nro_new_long (1U<<31);
  rv = nro_get_ival (ob, &err);
  tlib_pass_if_true ("nro_get_ival long extraction", NR_SUCCESS == err, "err=%d", (int)err);
  tlib_pass_if_true ("nro_get_ival long extraction", (1U<<31) == (unsigned)rv, "rv=%d", rv);
  nro_delete (ob);

  ob = nro_new_double (2.9);
  tlib_pass_if_true ("nro_get_ival double created", 0 != ob, "ob=%p", ob);
  rv = nro_get_ival (ob, &err);
  tlib_pass_if_true ("nro_get_ival double extraction", NR_SUCCESS == err, "err=%d", (int)err);
  ...
  ob = nro_new_array ();
  rv = nro_get_ival (ob, &err);
  tlib_pass_if_true ("nro_get_ival array extraction", NR_FAILURE == err, "err=%d", (int)err);
...
static nr_status_t
hash_visitor (const char *key, const nrobj_t *val, void *vp)
{
  nr_status_t err;
  int intval;
  ...
  *hash_visitor_visits += 1;
  tlib_pass_if_true ("key matches structure", 0 == nr_strncmp ("key", key, 3), "key=%s", key);
  tlib_pass_if_true ("value in range", (0 <= intval) && (intval < 10), "intval=%d", intval);
  return NR_SUCCESS;
}

static nr_status_t
hash_visitor_fails (const char *key, const nrobj_t *val, void *vp)
{
  int *hash_visitor_visits = (int *)vp;
...
test_nro_iteratehash (void)
{
  ...
  tlib_pass_if_int_equal ("test_nro_iteratehash visit count", 10, hash_visitor_visits);
  ...
  tlib_pass_if_int_equal ("test_nro_iteratehash visit count", 3, hash_visitor_visits);

  /*
   * error conditions
   */
  hash_visitor_visits = 0;
  nro_iteratehash (NULL, hash_visitor, &hash_visitor_visits);  /* null hash table */
  tlib_pass_if_int_equal ("test_nro_iteratehash visit count", 0, hash_visitor_visits);
...
static void
test_nro_hash_corner_cases (void)
{
  nrobj_t *hash;
  nrobj_t *obj;
  const char *key;
  ...
  gotten = nro_get_hash_value (obj, NULL, &err);  /* not a hash table, so an error */
  tlib_pass_if_true ("test_nro_hash_corner_cases", NULL == gotten, "gotten=%p", gotten);
  tlib_pass_if_true ("test_nro_hash_corner_cases", NR_FAILURE == err, "err=%d", (int)err);

  gotten = nro_get_hash_value (hash, "qrs", &err);
  tlib_pass_if_true ("test_nro_hash_corner_cases", 0 != gotten, "gotten=%p", gotten);
  tlib_pass_if_true ("test_nro_hash_corner_cases", NR_SUCCESS == err, "err=%d", (int)err);
  ...
  tlib_pass_if_true ("test_nro_hash_corner_cases null hash", NULL == gotten, "gotten=%p", gotten);
  tlib_pass_if_true ("test_nro_hash_corner_cases null hash", NR_FAILURE == err, "err=%d", (int)err);

  gotten = nro_get_hash_value_by_index (obj, 0, &err, &key);
  tlib_pass_if_true ("test_nro_hash_corner_cases not a hash", NULL == gotten, "gotten=%p", gotten);
  tlib_pass_if_true ("test_nro_hash_corner_cases not a hash", NR_FAILURE == err, "err=%d", (int)err);

  gotten = nro_get_hash_value_by_index (hash, 0, &err, &key);
  tlib_pass_if_true ("test_nro_hash_corner_cases out of bounds", NULL == gotten, "gotten=%p", gotten);
  tlib_pass_if_true ("test_nro_hash_corner_cases out of bounds", NR_FAILURE == err, "err=%d", (int)err);
  ...
  tlib_pass_if_true ("test_nro_hash_corner_cases out of bounds", 0 != gotten, "gotten=%p", gotten);
  tlib_pass_if_true ("test_nro_hash_corner_cases out of bounds", NR_SUCCESS == err, "err=%d", (int)err);

  /* can't add a null to an hash */
  setcode = nro_set_hash (hash, "foo", NULL);
  tlib_pass_if_true ("test_nro_hash_corner_cases", NR_FAILURE == setcode, "setcode=%d", (int)setcode);
...
static void
test_nro_array_corner_cases (void)
{
  nrobj_t *array;
  const nrobj_t *gotten;
  nr_status_t setcode;
  ...
  tlib_pass_if_true ("test_nro_array_corner_cases", NULL == gotten, "gotten=%p", gotten);
  tlib_pass_if_true ("test_nro_array_corner_cases", NR_FAILURE == err, "err=%d", (int)err);

  gotten = nro_get_array_value (array, 1, &err);
  tlib_pass_if_true ("test_nro_array_corner_cases", 0 != gotten, "gotten=%p", gotten);
  tlib_pass_if_true ("test_nro_array_corner_cases", NR_SUCCESS == err, "err=%d", (int)err);
  ...
  tlib_pass_if_true ("test_nro_array_corner_cases", NR_FAILURE == setcode, "setcode=%d", (int)setcode);

  nro_delete (array);
}

/*
 * Return an empirically well crafted (hash) object that contains one level of hash nesting,
 * large doubles, and other perversities.
 * The caller owns what is returned.
 */
static nrobj_t *
construct_hairy_object (void)
{
  ...
  nro_set_hash_double (hash_src, "double_posinf", nr_strtod ("Inf", 0)); /* stock json doesn't support ieee abbrevs */
  nro_set_hash_double (hash_src, "double_neginf", nr_strtod ("-Inf", 0)); /* stock json doesn't support ieee abbrevs */
  nro_set_hash_double (hash_src, "double_nan", nr_strtod ("NaN", 0)); /* stock json doesn't support ieee abbrevs */
 ...
 */
static void
test_nro_hairy_object_json (void)
{
  nrobj_t *hash_src;
  nrobj_t *hash_dst;
  char *str_src;
  ...
  tlib_pass_if_true ("test_nro_hairy_object_json", 0 == nr_strcmp (str_src, str_dst),
    "str_src=%s str_dst=%s", str_src, str_dst);
  ...
  tlib_pass_if_true ("test_nro_hairy_object_json copy/json",
    0 == nr_stricmp (expect_json, str_src), "expect_json=>\n%s\nstr_src=>\n%s", expect_json, str_src);
...
static nrobj_t *
construct_hairy_utf8_object (int N)
{
  int i = 0;
  nrobj_t *hash_src = nro_new_hash ();
...
static void
test_nro_hairy_utf8_object_json (void)
{
  nrobj_t *hash_src;
  nrobj_t *hash_dst;
  char *str_src;
  ...
  str_src = nro_to_json (hash_src);
  str_dst = nro_to_json (hash_dst);
  tlib_pass_if_true ("test_nro_hairy_utf8_object_json", 0 == nr_strcmp (str_src, str_dst),
    "str_src=%s str_dst=%s", str_src, str_dst);
  ...
  for (i = 0; i < N; i++) {
    ...
    lg = nr_strlen (expect_json);
    snprintf (expect_json + lg, sizeof (expect_json) - lg, "%s\"%s\":\"%s\"", sep, buf_key, buf_val);
    sep = ",";
  }
...
/*
 * Test data used to provoke PHP-620.
 * It isn't clear where this bogus data comes,
 * but the json encoder should not loop when converting corner cases and bogus UTF8.
 */
static void
test_nro_hairy_mangled_object_json (void)
{
  nrobj_t *hash_src;
  nrobj_t *hash_dst;
  char *str_src;
  ...
  str_src = nro_to_json (hash_src);
  str_dst = nro_to_json (hash_dst);
  tlib_pass_if_true ("test_nro_hairy_mangled_object_json", 0 == nr_strcmp (str_src, str_dst),
    "str_src=%s str_dst=%s", str_src, str_dst);
...
static void
test_nro_json_corner_cases (void)
{
  nrobj_t *obj;
  nrotype_t t;
  nr_status_t err;
  ...
  t = nro_type (obj);
  tlib_pass_if_true ("test_nro_json_corner_cases", NR_OBJECT_INT == t, "t=%d", (int)t);
  v = nro_get_int (obj, &err);
  tlib_pass_if_true ("test_nro_json_corner_cases", NR_SUCCESS == err, "err=%d", (int)err);
  ...
  t = nro_type (obj);
  tlib_pass_if_true ("test_nro_json_corner_cases", NR_OBJECT_NONE == t, "t=%d", (int)t);
  nro_delete (obj);

  obj = nro_create_from_json ("false");  /* tests space skipping */
  t = nro_type (obj);
  tlib_pass_if_true ("test_nro_json_corner_cases", NR_OBJECT_BOOLEAN == t, "t=%d", (int)t);
  ...
  /* json sucks, part N+1: you can't give floats starting with '.' */

  obj = nro_create_from_json ("1.0e100");  /* in range for a double */
  t = nro_type (obj);
  tlib_pass_if_true ("test_nro_json_corner_cases 1.0e100", NR_OBJECT_DOUBLE == t, "t=%d", (int)t);
  dv = nro_get_double (obj, &err);
  tlib_pass_if_true ("test_nro_json_corner_cases 1.0e100", NR_SUCCESS == err, "err=%d", (int)err);
  tlib_pass_if_true ("test_nro_json_corner_cases 1.0e100", 1.0e100 == dv, "dv=%f", dv);
  nro_delete (obj);

  obj = nro_create_from_json ("7e3");  /* in range for a double */
  t = nro_type (obj);
  tlib_pass_if_true ("test_nro_json_corner_cases 7e3", NR_OBJECT_DOUBLE == t, "t=%d", (int)t);
  dv = nro_get_double (obj, &err);
  tlib_pass_if_true ("test_nro_json_corner_cases 7e3", NR_SUCCESS == err, "err=%d", (int)err);
  if (NR_SUCCESS == err) {
    tlib_pass_if_true ("test_nro_json_corner_cases 7e3", 7000.0 == dv, "dv=%f", dv);
  }
  nro_delete (obj);

  obj = nro_create_from_json ("7e+03");  /* in range for a double */
  t = nro_type (obj);
  tlib_pass_if_true ("test_nro_json_corner_cases 7e+03", NR_OBJECT_DOUBLE == t, "t=%d", (int)t);
  dv = nro_get_double (obj, &err);
  tlib_pass_if_true ("test_nro_json_corner_cases 7e+03", NR_SUCCESS == err, "err=%d", (int)err);
  if (NR_SUCCESS == err) {
    tlib_pass_if_true ("test_nro_json_corner_cases 7e+03", 7000.0 == dv, "dv=%f", dv);
  }
  nro_delete (obj);
   ...
   */
  tlib_pass_if_true ("test_nro_json_corner_cases -1.0e500", NR_OBJECT_LONG == t, "t=%d", (int)t);
  l = nro_get_long (obj, &err);
  tlib_pass_if_true ("test_nro_json_corner_cases -1.0e500", NR_SUCCESS == err, "err=%d", (int)err);
  if (NR_SUCCESS == err) {
    tlib_pass_if_true ("test_nro_json_corner_cases -1.0e500", (int64_t)-1 == l, "l=" NR_INT64_FMT, l);
  }
  nro_delete (obj);

  obj = nro_create_from_json ("1000000000000000000000000");  /* exceeds LLONG_MAX */
  t = nro_type (obj);
  tlib_pass_if_true ("test_nro_json_corner_cases >LLONG_MAX", NR_OBJECT_LONG == t, "t=%d", (int)t);
  l = nro_get_long (obj, &err);
  tlib_pass_if_true ("test_nro_json_corner_cases >LLONG_MAX", NR_SUCCESS == err, "err=%d", (int)err);
  if (NR_SUCCESS == err) {
    tlib_pass_if_true ("test_nro_json_corner_cases >LLONG_MAX", LLONG_MAX == l, "l=" NR_INT64_FMT, l);
  }
  nro_delete (obj);

  obj = nro_create_from_json (" [  ]  ");
  t = nro_type (obj);
  tlib_pass_if_true ("test_nro_json_corner_cases empty array", NR_OBJECT_ARRAY == t, "t=%d", (int)t);
  size = nro_getsize (obj);
  tlib_pass_if_true ("test_nro_json_corner_cases empty array", 0 == size, "size=%d", size);
  nro_delete (obj);

  obj = nro_create_from_json (" [ 1  ]  ");
  t = nro_type (obj);
  tlib_pass_if_true ("test_nro_json_corner_cases array lg=1", NR_OBJECT_ARRAY == t, "t=%d", (int)t);
  size = nro_getsize (obj);
  tlib_pass_if_true ("test_nro_json_corner_cases array lg=1", 1 == size, "size=%d", size);
  nro_delete (obj);

  obj = nro_create_from_json (" [ 1 ,  ]  ");  /* can't use , as a terminator in json */
  tlib_pass_if_true ("test_nro_json_corner_cases array lg=1", 0 == obj, "obj=%p", obj);
  ...
  obj = nro_create_from_json (" {  }  ");
  t = nro_type (obj);
  tlib_pass_if_true ("test_nro_json_corner_cases empty hash", NR_OBJECT_HASH == t, "t=%d", (int)t);
  size = nro_getsize (obj);
  tlib_pass_if_true ("test_nro_json_corner_cases empty hash", 0 == size, "size=%d", size);
  nro_delete (obj);

  obj = nro_create_from_json (" { \"foo\" : 17  }  ");
  t = nro_type (obj);
  tlib_pass_if_true ("test_nro_json_corner_cases hash lg=1", NR_OBJECT_HASH == t, "t=%d", (int)t);
  size = nro_getsize (obj);
  tlib_pass_if_true ("test_nro_json_corner_cases hash lg=1", 1 == size, "size=%d", size);
  nro_delete (obj);

  obj = nro_create_from_json (" { \"foo\" ; 17  }  ");  /* use ; instead of : */
  tlib_pass_if_true ("test_nro_json_corner_cases hash lg=1", 0 == obj, "obj=%p", obj);

  obj = nro_create_from_json (" { 1 : 1  }  ");  /* can't use non-string as key */
  tlib_pass_if_true ("test_nro_json_corner_cases hash lg=1", 0 == obj, "obj=%p", obj);

  obj = nro_create_from_json (" { \"foo\" : 1 ,  }  ");  /* can't use , as a terminator */
  tlib_pass_if_true ("test_nro_json_corner_cases hash lg=1", 0 == obj, "obj=%p", obj);

  obj = nro_create_from_json (" { \"foo\" : AAA }  ");  /* illegal value */
  tlib_pass_if_true ("test_nro_json_corner_cases hash lg=1", 0 == obj, "obj=%p", obj);

  obj = nro_create_from_json (" { \"foo\" : 1 ; \"bar\" : 2  }  ");  /* can't use non , as a separator */
  tlib_pass_if_true ("test_nro_json_corner_cases hash lg=1", 0 == obj, "obj=%p", obj);

  obj = nro_create_from_json (" { \"foo\" : 1 , \"bar\" ; 2  }  ");  /* must use : to separate key from value */
  tlib_pass_if_true ("test_nro_json_corner_cases hash lg=1", 0 == obj, "obj=%p", obj);

  obj = nro_create_from_json (" { \"foo\" : 1 , \"bar\" : AA  }  ");  /* bad second value */
  tlib_pass_if_true ("test_nro_json_corner_cases hash lg=1", 0 == obj, "obj=%p", obj);
}

static void
test_nro_mangled_json (void)
{
  nrobj_t *obj;
...
Object Dump (5):\n\
  ...
  nro_test_new (nro_new_double (6.0), "\
Object Dump (6):\n\
  DOUBLE: 6.000000\n");
...
Object Dump (1):\n\
  ...
  size = nro_getsize (ob);
  tlib_pass_if_true ("nro_getsize fails on boolean", -1 == size, "size=%d", size);

  rv = nro_get_boolean (ob, &err);
  tlib_pass_if_true ("get boolean succeeds", (1 == rv) && (NR_SUCCESS == err), "rv=%d err=%d", rv, (int)err);
  ...
  tlib_pass_if_true ("boolean object assert", ob == tob, "ob=%p tob=%p", ob, tob);
  ...
  rv = nro_get_boolean (ob, &err);
  tlib_pass_if_true ("new boolean true (1)", (1 == rv) && (NR_SUCCESS == err), "rv=%d err=%d", rv, (int)err);
  nro_delete (ob);
  ...
  rv = nro_get_boolean (ob, &err);
  tlib_pass_if_true ("new boolean false", (0 == rv) && (NR_SUCCESS == err), "rv=%d err=%d", rv, (int)err);
  nro_delete (ob);
...
Object Dump (9):\n\
  ...
  tlib_pass_if_true ("get hash boolean succeeds", (1 == rv) && (NR_SUCCESS == err), "rv=%d err=%d", rv, (int)err);
  rv = nro_get_hash_int (hash, "abc", &err);
  tlib_pass_if_true ("get hash int fails", (-1 == rv) && (NR_FAILURE == err), "rv=%d err=%d", rv, (int)err);
...
Object Dump (10):\n\
  ...
  tlib_pass_if_true ("get array boolean succeeds", (1 == rv) && (NR_SUCCESS == err), "rv=%d err=%d", rv, (int)err);
  rv = nro_get_array_boolean (NULL, 1, &err);
  tlib_pass_if_true ("get array boolean fails OK", (-1 == rv) && (NR_FAILURE == err), "rv=%d err=%d", rv, (int)err);
  rv = nro_get_array_int (array, 1, &err);
  tlib_pass_if_true ("get array int fails", (-1 == rv) && (NR_FAILURE == err), "rv=%d err=%d", rv, (int)err);
...
Object Dump (4):\n\
  ...
  rv = nro_get_int (ob, &err);
  tlib_pass_if_true ("get int succeeds", (123 == rv) && (NR_SUCCESS == err), "rv=%d err=%d", rv, (int)err);
  ...
  INT_TEST ((unsigned int)INT_MAX+1, (unsigned int)INT_MAX+1, (unsigned int));
  INT_TEST (INT_MIN, INT_MIN,);
  INT_TEST (UINT_MAX+1, 0,);
  /*
   * The following test is too clever by far;
   * all it tests is wrap around of 2's complement arithmetic in a given sized integer.
   * It has little value, and it sets off gcc warning messages.
   */
  /* INT_TEST (INT_MIN-1, INT_MAX,); */ /* Overflow intended */
...
Object Dump (9):\n\
  ...
  rv = nro_get_hash_int (hash, "abc", &err);
  tlib_pass_if_true ("get hash int succeeds", (123 == rv) && (NR_SUCCESS == err), "rv=%d err=%d", rv, (int)err);
  lv = nro_get_hash_long (hash, "abc", &err);
  tlib_pass_if_true ("get hash long fails", (-1LL == lv) && (NR_FAILURE == err), "lv=%d err=%d", (int)lv, (int)err);
...
Object Dump (10):\n\
  ...
  rv = nro_get_array_int (array, 1, &err);
  tlib_pass_if_true ("get array int succeeds", (123 == rv) && (NR_SUCCESS == err), "rv=%d err=%d", rv, (int)err);
  rv = nro_get_array_int (NULL, 1, &err);
  tlib_pass_if_true ("get array int fails OK", (-1 == rv) && (NR_FAILURE == err), "rv=%d err=%d", rv, (int)err);
  lv = nro_get_array_long (array, 1, &err);
  tlib_pass_if_true ("get array int fails", (-1LL == lv) && (NR_FAILURE == err), "lv=%d err=%d", (int)lv, (int)err);
...
Object Dump (5):\n\
  ...
  lv = nro_get_long (ob, &err);
  tlib_pass_if_true ("get long succeeds", (123 == lv) && (NR_SUCCESS == err), "lv=" NR_INT64_FMT " err=%d", lv, (int)err);

  lv = nro_get_long (ob, 0);
  tlib_pass_if_true ("get long without errp", 123 == lv, "lv=" NR_INT64_FMT "", lv);
   ...
   * It has little value, and it sets off gcc warning messages.
   */
  /* LONG_TEST (LLONG_MIN-1, LLONG_MAX,); */ /* Overflow intended */
...
Object Dump (9):\n\
  ...
  lv = nro_get_hash_long (hash, "abc", &err);
  tlib_pass_if_true ("get hash long succeeds", (123 == lv) && (NR_SUCCESS == err), "lv=" NR_INT64_FMT " err=%d", lv, (int)err);
  rv = nro_get_hash_int (hash, "abc", &err);
  tlib_pass_if_true ("get hash int fails", (-1 == rv) && (NR_FAILURE == err), "rv=%d err=%d", rv, (int)err);
...
Object Dump (10):\n\
  ...
  lv = nro_get_array_long (array, 1, &err);
  tlib_pass_if_true ("get array long succeeds", (123 == lv) && (NR_SUCCESS == err), "lv=" NR_INT64_FMT " err=%d", lv, (int)err);
  lv = nro_get_array_long (NULL, 1, &err);
  tlib_pass_if_true ("get array long fails OK", (-1LL == lv) && (NR_FAILURE == err), "lv=%d err=%d", (int)lv, (int)err);
  rv = nro_get_array_int (array, 1, &err);
  tlib_pass_if_true ("get array int fails", (-1 == rv) && (NR_FAILURE == err), "rv=%d err=%d", rv, (int)err);
...
Object Dump (6):\n\
  ...
  dv = nro_get_double (ob, &err);
  tlib_pass_if_true ("get double succeeds", (123.456 == dv) && (NR_SUCCESS == err), "dv=%lf err=%d", dv, (int)err);
...
#if !(defined(__sun__) || defined(__sun))  /* solaris numerics differ from linux or macosx in 1 ulp. */
...
Object Dump (9):\n\
  ...
  tlib_pass_if_true ("get hash double succeeds", (123.456 == dv) && (NR_SUCCESS == err), "dv=%lf err=%d", dv, (int)err);
  rv = nro_get_hash_int (hash, "abc", &err);
  tlib_pass_if_true ("get hash int fails", (-1 == rv) && (NR_FAILURE == err), "rv=%d err=%d", rv, (int)err);
...
Object Dump (10):\n\
  ...
  tlib_pass_if_true ("get array double succeeds", (123.456 == dv) && (NR_SUCCESS == err), "dv=%lf err=%d", dv, (int)err);
  dv = nro_get_array_double (NULL, 1, &err);
  tlib_pass_if_true ("get array double fails OK", (-1.0 == dv) && (NR_FAILURE == err), "dv=%lf err=%d", dv, (int)err);
  rv = nro_get_array_int (array, 1, &err);
  tlib_pass_if_true ("get array int fails", (-1 == rv) && (NR_FAILURE == err), "rv=%d err=%d", rv, (int)err);
...
Object Dump (7):\n\
  ...
  sv = nro_get_string (ob, &err);
  tlib_pass_if_true ("get string succeeds", (0 == nr_strcmp (sv, "123")) && (NR_SUCCESS == err),
  ...
  tlib_pass_if_true ("new string object dups string", s != sv, "s=%s sv=%s", s ? s : "(NULL)", sv ? sv : "(NULL)");
  nro_delete (ob);

  ob = nro_new_string (0);
  tlib_pass_if_true ("new string NULL succeeds", 0 != ob, "ob=%p", ob);
  sv = nro_get_string (ob, &err);
  tlib_pass_if_true ("get NULL string returns empty string", (NR_SUCCESS == err) && (0 == nr_strcmp (sv, "")), "err=%d sv=%s", (int)err, sv ? sv : "(NULL)");
...
Object Dump (9):\n\
  ...
  tlib_pass_if_true ("get hash string", (NR_SUCCESS == err) && (0 == nr_strcmp (sv, "123")), "err=%d sv=%s", (int)err, sv ? sv : "(NULL)");
  rv = nro_get_hash_int (hash, "abc", &err);
  tlib_pass_if_true ("incorrect get hash int", (-1 == rv) && (NR_FAILURE == err), "rv=%d err=%d", rv, (int)err);
...
Object Dump (10):\n\
  ...
  tlib_pass_if_true ("get array string", (NR_SUCCESS == err) && (0 == nr_strcmp (sv, "123")), "err=%d sv=%s", (int)err, sv ? sv : "(NULL)");
  sv = nro_get_array_string (NULL, 1, &err);
  tlib_pass_if_true ("get array string fails OK", (NULL == sv) && (NR_FAILURE == err), "err=%d sv=%s", (int)err, sv ? sv : "(NULL)");
  rv = nro_get_array_int (array, 1, &err);
  tlib_pass_if_true ("incorrect get array int", (-1 == rv) && (NR_FAILURE == err), "rv=%d err=%d", rv, (int)err);
...
Object Dump (8):\n\
  ...
  size = nro_getsize (ob);
  tlib_pass_if_true ("nro_getsize fails on jstring", -1 == size, "size=%d", size);

  sv = nro_get_string (ob, &err);
  tlib_pass_if_true ("get string fails jstring", 0 == sv, "sv=%p err=%d", sv, (int)err);

  sv = nro_get_jstring (ob, &err);
  tlib_pass_if_true ("get jstring succeeds jstring", 0 == nr_strcmp ("[1,2,3]", sv), "sv=%s err=%d",
    NRSAFESTR (sv), (int)err);

  sv = nro_get_jstring (ob, 0);
  tlib_pass_if_true ("get jstring on jstring without errp", 0 == nr_strcmp (sv, "[1,2,3]"),
    "sv=%s", sv ? sv : "(NULL)");

  tlib_pass_if_true ("new jstring object dups jstring", s != sv, "s=%s sv=%s", s ? s : "(NULL)", sv ? sv : "(NULL)");
  nro_delete (ob);
  ...
  sv = nro_get_jstring (ob, &err);
  tlib_pass_if_true ("get NULL jstring returns empty string", (NR_SUCCESS == err) && (0 == nr_strcmp (sv, "")), "err=%d sv=%s", (int)err, sv ? sv : "(NULL)");
...
Object Dump (8):\n\
  ...
  tlib_pass_if_true ("jstring object assert", ob == tob, "ob=%p tob=%p", ob, tob);
...
Object Dump (9):\n\
  ...
  tlib_pass_if_true ("get hash jstring", (NR_SUCCESS == err) && (0 == nr_strcmp (sv, "123")), "err=%d sv=%s", (int)err, sv ? sv : "(NULL)");
  rv = nro_get_hash_int (hash, "abc", &err);
  tlib_pass_if_true ("incorrect get hash int", (-1 == rv) && (NR_FAILURE == err), "rv=%d err=%d", rv, (int)err);
...
Object Dump (10):\n\
  ...
  tlib_pass_if_true ("get array jstring", (NR_SUCCESS == err) && (0 == nr_strcmp (sv, "123")), "err=%d sv=%s", (int)err, sv ? sv : "(NULL)");
  sv = nro_get_array_jstring (NULL, 1, &err);
  tlib_pass_if_true ("get array jstring fails OK", (NULL == sv) && (NR_FAILURE == err), "err=%d sv=%s", (int)err, sv ? sv : "(NULL)");
  rv = nro_get_array_int (array, 1, &err);
  tlib_pass_if_true ("incorrect get array int", (-1 == rv) && (NR_FAILURE == err), "rv=%d err=%d", rv, (int)err);
...
Object Dump (9):\n\
  ...
  js = nro_to_json (ob);
  tlib_pass_if_true ("new hash to json", (0 != js) && (0 == nr_strcmp (js, "{}")), "js=%s", js);
  nr_free (js);
  ...
  tlib_pass_if_true ("NULL key hash set", NR_FAILURE == setcode, "setcode=%d", (int)setcode);

  /* And same with a NULL hash. */
  setcode = nro_set_hash (0, 0, 0);
  tlib_pass_if_true ("NULL hash set", NR_FAILURE == setcode, "setcode=%d", (int)setcode);

  /* And with an empty string as a key. */
  setcode = nro_set_hash (ob, "", ob);
  tlib_pass_if_true ("empty string key", NR_FAILURE == setcode, "setcode=%d", (int)setcode);

  /* Add one of each data type to the hash. */
  setcode = nro_set_hash_boolean (ob, "boolean", 1);
  tlib_pass_if_true ("hash set", NR_SUCCESS == setcode, "setcode=%d", (int)setcode);
...
Object Dump (9):\n\
  ...
  js = nro_to_json (ob);
  tlib_pass_if_true ("populated hash to json", (0 != js) && (0 == nr_strcmp (js, "\
{\"boolean\":true,\"int\":789,\"long\":101112,\"double0\":\
131415.16170,\"double1\":1.11110,\"string\":\"abc\",\"jstring\":[1,2,3],\"hash\":{\"subhash-long\":1,\
\"subhash-int\":2},\"array\":[4,5]}")), "js=%s", js);
...
Object Dump (9):\n\
   ...
   */
  setcode = nro_set_hash_long (ob, "long", 101112);
  tlib_pass_if_true ("replace existing element", NR_SUCCESS == setcode, "setcode=%d", (int)setcode);
  nro_delete (tob);
...
Object Dump (9):\n\
  ...
  for (i = 0; i < 1024; i++) {
    ...
    snprintf (keyval, sizeof (keyval), "Hash Key value #%04d - slightly longer string this time", i);
    setcode = nro_set_hash_string (ob, keyname, keyval);
    tlib_pass_if_true ("hash capacity testing", NR_SUCCESS == setcode, "setcode=%d", (int)setcode);
  }
...
Object Dump (10):\n\
  ...
  js = nro_to_json (ob);
  tlib_pass_if_true ("new array to json", (0 != js) && (0 == nr_strcmp (js, "[]")), "js=%s", js);
  nr_free (js);
  ...
  tlib_pass_if_true ("out of bounds index", NR_FAILURE == setcode, "setcode=%d", (int)setcode);
  ...
  tlib_pass_if_true ("NULL array", NR_FAILURE == setcode, "setcode=%d", (int)setcode);

  /*
   * Add one of each data type to the array.
   */
  setcode = nro_set_array_boolean (ob, 0, 1);
  tlib_pass_if_true ("array set", NR_SUCCESS == setcode, "setcode=%d", (int)setcode);
...
Object Dump (10):\n\
  ...
  js = nro_to_json (ob);
  tlib_pass_if_true ("populated array to json", (0 != js) && (0 == nr_strcmp (js, "\
[true,123,456,789,101112,131415.16170,\"abc\",[1,2,3],{\"subhash-long\":1,\"subhash-int\":2},[4,5]]")), "js=%s", js);
...
Object Dump (10):\n\
  ...
  tlib_pass_if_true ("set array", NR_SUCCESS == setcode, "setcode=%d", (int)setcode);
   ...
   */
  size = nro_getsize (ob);
  tlib_pass_if_true ("correct size", 10 == size, "size=%d", size);
...
test_null_parameters (void)
{
  ...
  t = nro_type (0);
  tlib_pass_if_true ("null object has invalid type", NR_OBJECT_INVALID == t, "t=%d", t);
  size = nro_getsize (0);
  tlib_pass_if_true ("getsize fails on null object", -1 == size, "size=%d", size);
  ob = nro_copy (0);
  ...
  js = nro_to_json (0);
  tlib_pass_if_true ("json from null object", 0 == nr_strcmp (js, "null"), "js=%s", js);
  nr_free (js);
}

static void
test_nro_set_hash_failure (void)
{
  nr_status_t setcode;
  ...
  tlib_pass_if_true ("test_nro_set_hash_failure null object", NR_FAILURE == setcode, "setcode=%d", (int)setcode);
  ...
  tlib_pass_if_true ("test_nro_set_hash_failure null key", NR_FAILURE == setcode, "setcode=%d", (int)setcode);
  ...
  tlib_pass_if_true ("test_nro_set_hash_failure empty key", NR_FAILURE == setcode, "setcode=%d", (int)setcode);
  ...
  tlib_pass_if_true ("test_nro_set_hash_failure success", NR_SUCCESS == setcode, "setcode=%d", (int)setcode);
...
static void
test_nro_set_array_failure (void)
{
  nr_status_t setcode;
  ...
  tlib_pass_if_true ("test_nro_set_array_failure null object", NR_FAILURE == setcode, "setcode=%d", (int)setcode);
  ...
  tlib_pass_if_true ("test_nro_set_array_failure negative index", NR_FAILURE == setcode, "setcode=%d", (int)setcode);
  ...
  tlib_pass_if_true ("test_nro_set_array_failure high index", NR_FAILURE == setcode, "setcode=%d", (int)setcode);
  ...
  tlib_pass_if_true ("test_nro_set_array_failure not array", NR_FAILURE == setcode, "setcode=%d", (int)setcode);
  ...
  tlib_pass_if_true ("test_nro_set_array_failure success", NR_SUCCESS == setcode, "setcode=%d", (int)setcode);
...
static void
test_create_from_json_unterminated (void)
{
  nrobj_t *obj;
  char *json;
