...
static void
nr_php_add_zend_extension_to_hash (zend_extension *ext, void *plugins TSRMLS_DC)
{
...
static int
nr_php_add_dynamic_module_to_hash (zend_module_entry *ext, nrobj_t *plugins,
...
static void
nr_php_gather_dynamic_modules (nrobj_t *env TSRMLS_DC)
{
  nrobj_t *plugins = nro_new (NR_OBJECT_ARRAY);

  zend_llist_apply_with_argument (&zend_extensions, (llist_apply_with_arg_func_t) nr_php_add_zend_extension_to_hash, plugins TSRMLS_CC);

  nr_php_zend_hash_ptr_apply (&module_registry,
                              (nr_php_ptr_apply_t) nr_php_add_dynamic_module_to_hash,
                              plugins TSRMLS_CC);

  nro_set_hash (env, "Plugin List", plugins); /* see environment_value_set.rb ~ line 21 */

  nro_delete (plugins);
}

static void
call_phpinfo (TSRMLS_D)
{
  int save_sapi_flag = sapi_module.phpinfo_as_text;

  sapi_module.phpinfo_as_text = 1;  /* this is a hack to force the output to the text format */
...
static int
phpinfo_output_handler (void **buf_ptr, php_output_context *ctx)
{
  nrbuf_t *buf;
...
static void
nr_php_gather_php_information (nrobj_t *env TSRMLS_DC)
{
  nrbuf_t *buf = nr_buffer_create (65536, 0);
  php_output_handler *handler;

  handler = php_output_handler_create_internal (NR_PSTR ("New Relic phpinfo"),
                                                phpinfo_output_handler,
                                                4096,
  ...
  nr_parse_rocket_assignment_list (nr_remove_const (nr_buffer_cptr (buf)),
                                   nr_buffer_len (buf),
                                   env);
...
nr_php_gather_php_information (nrobj_t *env TSRMLS_DC)
{
  ...
  if (FAILURE == php_start_ob_buffer (output_handler, chunk_size, erase TSRMLS_CC)) {
    return; /* don't call phpinfo() if we can't buffer because otherwise we're going to dump into the user's page */
  }
  ...
  nr_parse_rocket_assignment_list (
    Z_STRVAL_P (tmp_obj),
    Z_STRLEN_P (tmp_obj),
    env);
...
static void
nr_php_gather_machine_information (nrobj_t *env)
{
  ...
  final[0] = 0;
  snprintf (final, sizeof (final), "%s %s %s %s %s",
    NRBLANKSTR (sys->sysname),
    NRBLANKSTR (sys->nodename),
    NRBLANKSTR (sys->release),
    NRBLANKSTR (sys->version),
    NRBLANKSTR (sys->machine));
  ...
  if ((0 < nr_strlen (dyno_value)) && (0 == nr_access ("/app/.heroku/php", F_OK))) {
...
static void
nr_php_gather_dispatcher_information (nrobj_t *env)
{
  char dstring[512];
  char *p;
...
nrobj_t *
nr_php_get_environment (TSRMLS_D)
{
  nrobj_t *env;
