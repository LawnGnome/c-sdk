...
int
nr_rum_do_autorum (const nrtxn_t *txn)
{
  if (0 == txn) {
    return 0;
  }
...
static char *
nr_rum_obfuscate (const char *input, const char *key)
{
...
/*
 * This array is needed by the axiom/tests. As such it is declared in nr_rum_private.h
 * and defined here.
 */
const char nr_rum_footer_prefix[] = "window.NREUM||(NREUM={});NREUM.info=";

char *
nr_rum_produce_header (nrtxn_t *txn, int tags, int autorum)
{
  char *return_header;
  ...
  header_length = (tags ? (nr_strlen (rum_start_tag) + nr_strlen (rum_end_tag)) : 0) +
                  nr_strlen (loader) + 1;

  return_header = (char *)nr_malloc (header_length);
  return_header[0] = '\0';
  snprintf (return_header, header_length, "%s%s%s",
    tags ? rum_start_tag : "",
    loader,
    tags ? rum_end_tag : "");
...
char *
nr_rum_get_attributes (const nr_attributes_t *attributes)
{
  char *json;
  nrobj_t *user;
  nrobj_t *agent;
  ...
  user = nr_attributes_user_to_obj (attributes, NR_ATTRIBUTE_DESTINATION_BROWSER);
  agent = nr_attributes_agent_to_obj (attributes, NR_ATTRIBUTE_DESTINATION_BROWSER);
...
static char *
nr_rum_get_attributes_obfuscated (const nr_attributes_t *attributes, const char *rum_license)
{
...
char *
nr_rum_produce_footer (nrtxn_t *txn, int tags, int autorum)
{
  char *txn_name;
  nrtime_t queue_time;
  nrtime_t app_time;
  ...
  obfuscated_attributes = nr_rum_get_attributes_obfuscated (txn->attributes, txn->license);

  /*
   * For information on this hash contents see:
   * https://newrelic.atlassian.net/wiki/display/eng/BAM+Agent+Auto-Instrumentation
   */
  hash = nro_new_hash ();
  nro_set_hash_string (hash, "beacon",          nro_get_hash_string (txn->app_connect_reply, "beacon", 0));
  nro_set_hash_string (hash, "licenseKey",      nro_get_hash_string (txn->app_connect_reply, "browser_key", 0));
  nro_set_hash_string (hash, "applicationID",   nro_get_hash_string (txn->app_connect_reply, "application_id", 0));
  ...
  nro_set_hash_string (hash, "errorBeacon",     nro_get_hash_string (txn->app_connect_reply, "error_beacon", 0));
  nro_set_hash_string (hash, "agent",           nro_get_hash_string (txn->app_connect_reply, "js_agent_file", 0));
  /*
   * "sslForHttp" unused as of 2013-11-27.  See PHP-460.
   */
...

  {
    ...
    footer_len =
      (tags ? (sizeof (rum_start_tag) + sizeof (rum_end_tag)) : 0) +
      sizeof (nr_rum_footer_prefix) +
      nr_strlen (hash_json) +
      1;

    footer = (char *)nr_malloc (footer_len);
    footer[0] = '\0';
    snprintf (footer, footer_len, "%s%s%s%s",
      tags ? rum_start_tag : "",
      nr_rum_footer_prefix,
      hash_json,
      tags ? rum_end_tag : "");
...
static const char nr_rum_x_ua_compatible_regex[] = "<\\s*meta[^>]+http-equiv\\s*=\\s*['\"]x-ua-compatible['\"][^>]*>";
  ...
  const char **match_end)
{
  ...
  if (NULL == re) {
    nrl_debug (NRL_AUTORUM, "autorum: unable to compile browser monitoring regex %.100s ",
      regex);
    return;
  }
...
/*
 * The spec governing loader script location is located here:
 * https://newrelic.atlassian.net/wiki/display/eng/BAM+Agent+Auto-Instrumentation
 */
const char *
nr_rum_scan_html_for_head (const char *input, const uint input_len)
{
   ...
   * globals.
   */
  nr_rum_regex_search (nr_rum_x_ua_compatible_regex, nr_rum_regex_options, input, input_len, &x_ua_tag_start, &x_ua_tag_end);
  nr_rum_regex_search (nr_rum_charset_regex, nr_rum_regex_options, input, input_len, &charset_tag_start, &charset_tag_end);
  ...
  nr_rum_regex_search (nr_rum_head_open_regex, nr_rum_regex_options, input, input_len, &match_start, &match_end);
  if (match_end) {
    return match_end;
  }

  nr_rum_regex_search (nr_rum_body_open_regex, nr_rum_regex_options, input, input_len, &match_start, &match_end);
  if (match_start) {
    return match_start;
  }
...
const char *
nr_rum_scan_html_for_foot (const char *input, const uint input_len)
{
  int rv;
...
nr_rum_output_handler_worker (
  ...
  char **handled_output, size_t *handled_output_len,
  ...
  int debug_autorum)
{
  ...
  if (0 == handled_output) {
    if (debug_autorum) {
      nrl_verbosedebug (NRL_AUTORUM, "autorum: exiting due to no handled_output");
    }
    return;
  }
  *handled_output = NULL;
  ...
  if (0 == handled_output_len) {
    if (debug_autorum) {
      nrl_verbosedebug (NRL_AUTORUM, "autorum: exiting due to no handled_output_len");
    }
    return;
  }
  *handled_output_len = 0;
  ...
  if (0 == txn->options.autorum_enabled) {
    if (debug_autorum) {
      nrl_verbosedebug (NRL_AUTORUM, "autorum: exiting due to txn->options.autorum_enabled");
    }
    return;
  }
  ...
  if (0 != txn->status.ignore) {
    if (debug_autorum) {
      nrl_verbosedebug (NRL_AUTORUM, "autorum: exiting due to txn->status.ignore");
    }
    return;
  }
  ...
  if (0 != has_response_content_length) {
    if (debug_autorum) {
      nrl_verbosedebug (NRL_AUTORUM, "autorum: exiting due to Content-Length header");
    }
    return;
  }
  ...
  if (debug_autorum) {
    nrl_verbosedebug (NRL_AUTORUM, "autorum: mimetype=" NRP_FMT, NRP_MIME (mimetype));
  }
   ...
   */
  if (0 == nr_strnicmp (mimetype, autorum_text_html, sizeof (autorum_text_html) - 1)) {
    is_html = 1;
  }

  if (0 == is_html) {
    if (debug_autorum) {
      nrl_verbosedebug (NRL_AUTORUM, "autorum: ignoring non text/html (mimetype=" NRP_FMT ") content", NRP_MIME (mimetype));
    }
    return;
  }
  ...
  if (debug_autorum) {
    nrl_verbosedebug (NRL_AUTORUM, "autorum: done_head=%d done_foot=%d", done_head, done_foot);
  }
  ...
  if (output_len >= 6) {
    ...
    if (0 == done_head) {
      ...
      if (0 != head) {
        ...
        if (debug_autorum) {
          nrl_verbosedebug (NRL_AUTORUM, "autorum: header=" NRP_FMT, NRP_RUM (rum_header ? rum_header : "<NULL>"));
        }
        if (0 != rum_header) {
    ...
    if (((0 != done_head) || (0 != head)) && (0 == done_foot)) {
      ...
      if (0 != tail) {
        if (nrunlikely (tail < head)) {
          if (debug_autorum) {
            nrl_verbose (NRL_AUTORUM, "autorum: malformed HTML - </body> appears before <head>");
          }
          tail = 0;
        }
      }
      ...
      if (0 != tail) {
        ...
        if (debug_autorum) {
          nrl_verbosedebug (NRL_AUTORUM, "autorum: footer=" NRP_FMT, NRP_RUM (rum_footer ? rum_footer : "<NULL>"));
        }
        if (0 != rum_footer) {
    ...
    if (final_len != output_len) {
      ...
      *handled_output_len = final_len;
       ...
       */
      if (debug_autorum) {
        nrl_verbosedebug (NRL_AUTORUM, "autorum: head=%p tail=%p bytes_up_to_head=%d header_len=%d bytes_after_head=%d bytes_up_to_tail=%d footer_len=%d bytes_after_tail=%d",
          head, tail, bytes_up_to_head, header_len, bytes_after_head, bytes_up_to_tail, footer_len, bytes_after_tail);
      }
    ...
    if (debug_autorum) {
      nrl_verbosedebug (NRL_AUTORUM, "autorum: short output_len=%zu from %s", output_len, output);
    }
  }
}
