...
nr_php_pgsql_create_datastore_instance (const char *conn_info)
{
...
void
nr_php_pgsql_save_datastore_instance (const zval *pgsql_conn, const char *conn_info TSRMLS_DC)
{
  nr_datastore_instance_t *instance;
...
nr_datastore_instance_t *
nr_php_pgsql_retrieve_datastore_instance (const zval *pgsql_conn TSRMLS_DC)
{
  nr_datastore_instance_t *instance;
  char *key = NULL;
  ...
  if (NULL == pgsql_conn) {
    ...
    if (NRPRG (pgsql_last_conn)) {
      ...
      nrl_verbosedebug (NRL_INSTRUMENT, "could not find previous pgsql connection");
...
void
nr_php_pgsql_remove_datastore_instance (const zval *pgsql_conn TSRMLS_DC)
{
  char *key = NULL;
