...
static void test_json_worker(void) {
  ...
  tlib_pass_if_true(
      "character GBP json string",
      0 == nr_strcmp("\"Mangled Euro sign \\u00e2\\u0082\\u00ecxxx\"", dest),
      "escaped string is %s", dest);
  tlib_pass_if_true("character GBP json string", count == 41, "count==%d",
                    count);
  nr_free(dest);
   ...
   * surrogate pairs. rrh wasn't able to find an example of a 31-bit encoding
   * character, so just made one up. The behavior of the encoder is pretty much
   * undefined.
   */
  count = test_nr_json_escape(&dest,
                              "31-bit encoding "
