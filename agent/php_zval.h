 ...
 *
 * These functions are generally on the hot path for a lot of our
 * instrumentation code, and hence are inlined here. This may be overkill.
 *
...
static inline int
nr_php_is_zval_valid_bool (const zval *z)
{
  if (0 == z) {
    return 0;
  }
...
static inline int
nr_php_is_zval_valid_resource (const zval *z)
{
 ...
 */
static inline int
nr_php_is_zval_valid_string (const zval *z)
{
 ...
 */
static inline int
nr_php_is_zval_non_empty_string (const zval *z)
{
  if (!nr_php_is_zval_valid_string (z) || (0 == Z_STRVAL_P (z)) || (Z_STRLEN_P (z) <= 0)) {
    return 0;
  }
  return 1;
}
...
static inline int
nr_php_is_zval_valid_object (const zval *z)
{
...
static inline int
nr_php_is_zval_valid_array (const zval *z)
{
...
static inline int
nr_php_is_zval_valid_callable (zval *z TSRMLS_DC)
{
  if (NULL == z) {
    return 0;
  }
...
static inline int
nr_php_is_zval_valid_integer (const zval *z)
{
  if ((0 == z) || (IS_LONG != Z_TYPE_P (z))) {
    return 0;
  }
  return 1;
}
...
static inline int
nr_php_is_zval_valid_double (const zval *z)
{
  if ((0 == z) || (IS_DOUBLE != Z_TYPE_P (z))) {
    return 0;
  }
  return 1;
}
...
static inline int
nr_php_is_zval_valid_scalar (const zval *z)
{
  if (NULL == z) {
    return 0;
  }
 ...
 */
static inline long
nr_php_zval_resource_id (const zval *zv)
{
#ifdef PHP7
...
/*
 * These wrapper functions are to avoid warning messages created by PHP's
 * extremely inconsistent and broken const-correctness: even though there's no
 * code behind the ZVAL_STRINGL() macro that breaks const correctness, the way
 * the pointer is handled results in a "cast discards const qualifier" warning.
 *
 * Our only options here are to manually inline the equivalent of ZVAL_STRINGL()
 * or to quiet the compiler warning. For sanity's sake, we'll do the latter.
 */
...
static inline void
nr_php_zval_str_len (zval *zv, const char *str, nr_string_len_t len)
{
#ifdef PHP7
...
static inline void
nr_php_zval_str (zval *zv, const char *str)
{
#ifdef PHP7
...
static inline void
nr_php_zval_bool (zval *zv, int b)
{
  ZVAL_BOOL (zv, b);
}
 ...
 */
static inline void
nr_php_zval_prepare_out_arg (zval *zv)
{
#ifdef PHP7
 ...
 */
static inline zval *
nr_php_zval_real_value (zval *zv)
{
  if (NULL == zv) {
    return zv;
  }
