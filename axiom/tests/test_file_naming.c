...
static void
file_namer_test_null_match (const char *name, nr_file_naming_t *namers, const char *filename)
{
  char *match;
...
test_file_namer (void)
{
  ...
  file_namer_test_null_match ("Empty name list doesn't match", NULL, "docs/a.php");
  ...
  tlib_pass_if_null ("nr_file_namer_append initializes next to NULL", namers->next);
  tlib_pass_if_str_equal ("user_pattern is set", "foo", namers->user_pattern);

  namers = nr_file_namer_append (namers, NULL);
  tlib_pass_if_ptr_equal ("Null filename fails to add to list", NULL, namers->next);
  ...
  tlib_pass_if_ptr_equal ("Name list NULL and NULL doesn't crash or match", NULL, match);
  ...
  tlib_pass_if_ptr_equal ("Destructor sets the pointer passed to NULL", NULL, namers);
}

static void
regexp_tester (const char *test_name, const char *user_expression,
  const char *match_this, const char *expected)
{
...
static void
test_file_namer_regexes (void)
{
  regexp_tester ("Basic usage", "alpha", "alpha.php", "alpha");
  regexp_tester ("Basic regexes the first", "[a-zA-Z]_?[0-9]{2,3}", "brain21","n21");
  regexp_tester ("Basic regexes the second",
      "[a-zA-Z]+_?[0-9]{2,3}", "ab_3335/test.php", "b_333");
