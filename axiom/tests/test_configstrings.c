...
test_nr_parse_time (void)
{
  ...
  struct {
    ...
    /* TODO: decimal pointed numbers aren't allowed, and returns 0 */
  ...
  for (i = 0; i < num_testcases; i++) {
    ...
    tlib_pass_if_true (testcases[i].testname, t == testcases[i].expect,
      "t=" NR_TIME_FMT " expect=" NR_TIME_FMT " input=%s",
      t, testcases[i].expect, NRSAFESTR (testcases[i].input));
  }
}

static void
test_nr_bool_from_str (void)
{
  int x;
  int i;
  struct {
  ...
  for (i = 0; i < num_testcases; i++) {
    ...
    tlib_pass_if_true (testcases[i].input ? testcases[i].input : "NULL",
      x == testcases[i].expect,
      "x=%d expect=%d input=%s",
      x, testcases[i].expect, NRSAFESTR (testcases[i].input));
  }
}
