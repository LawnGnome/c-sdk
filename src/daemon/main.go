...
func main() {
	...
	if printVersion {
		...
		fmt.Println("(C) Copyright 2009-2016 New Relic Inc. All rights reserved.")
		fmt.Println()
		return
	}
...
func createFlagSet(cfg *Config) *flag.FlagSet {
	...
	// TODO: Perhaps this option should be named 'bind' rather than 'port'
	// This would make it less confusing, but it would also not match the
	// config file name.
...
func createLegacyFlagSet(cfg *Config) *flag.FlagSet {
	...
	useTLS := true // to handle the disable SSL flag (-n)
	...
	legacyFlagSet.BoolVar(&cfg.TLS, "s", cfg.TLS, "")
	legacyFlagSet.BoolVar(&useTLS, "n", false, "")
...
//
// TODO(msl): Refactor to return an error rather than exiting. We want
// to ensure validation errors also go to the log file. Writing errors
// to stderr is only useful if there's a terminal and a person on the
// other end.
...
func spawnWatcher(cfg *Config) (*exec.Cmd, error) {
	...
	// TODO(msl): Should we have a whitelist for env vars?
...
func banner(cfg *Config) string {
	...
	if cfg.TLS {
		fmt.Fprint(buf, " ssl=enabled")
	} else {
		fmt.Fprint(buf, " ssl=disabled")
	}
...
// on very old Xen Linux kernels. See PHP-1103.
...
func (e borkedSyscallError) Error() string {
	...
	return fmt.Sprintf(
		"Your operating system is not supported by the New Relic"+
			" PHP agent. If you have any questions or believe you reached this"+
