...
static void
test_buffer_add (nrbuf_t *buf, int dlen, int est, int ext, int *in)
{
  int l1;
  int l2;
  ...
  tlib_pass_if_true ("buffer add increases len correctly", (l2 - l1) == dlen, "l1=%d l2=%d dlen=%d est=%d ext=%d",
    l1, l2, dlen, est, ext);
}

static void
test_buffer_use (nrbuf_t *buf, int dlen, int est, int ext, int *in)
{
  int length;
  int compare_code;
  int l1;
  ...
  tlib_pass_if_true ("buffer use returns correct num", nout == length, "nout=%d length=%d est=%d ext=%d", nout, length, est, ext);
  compare_code = nr_memcmp (in, out, nout);
  tlib_pass_if_true ("buffer use outputs correct bytes", 0 == compare_code, "compare_code=%d est=%d ext=%d", compare_code, est, ext);
  l2 = nr_buffer_len (buf);
  tlib_pass_if_true ("buffer use decreases len correctly", (l1 - l2) == nout, "l1=%d l2=%d nout=%d est=%d ext=%d",
    l1, l2, nout, est, ext);
  nr_free (out);
}
 ...
 */
static nrbuf_t *
test_buffer_test1 (int est, int ext)
{
  nrbuf_t *buf;
  const char *rp;
  int length;
  ...
  tlib_pass_if_true ("new buffer not null", 0 != buf, "buf=%p est=%d ext=%d", buf, est, ext);

  length = nr_buffer_len (buf);
  tlib_pass_if_true ("new buffer has length zero", 0 == length, "length=%d est=%d ext=%d", length, est, ext);

  rp = (const char *)nr_buffer_cptr (buf);
  tlib_pass_if_true ("new buffer has null nr_buffer_cptr", 0 == rp, "rp=%p est=%d ext=%d", rp, est, ext);

  return buf;
}

static void
test_buffer (int est, int ext)
{
  unsigned int i;
  nrbuf_t *buf;
  const void *rp;
  ...
  test_buffer_use (buf, sizeof (int) * 500, est, ext, in); /* more than is in buffer */
  test_buffer_use (buf, sizeof (int) * 500, est, ext, in); /* buffer is empty */

  /*
   * Test 3: nr_buffer_destroy properly disposes, and all functions handle null buffer input correctly.
   */
  nr_buffer_destroy (&buf);
  tlib_pass_if_true ("nr_buffer_destroy disposes buffer", 0 == buf, "buf=%p est=%d ext=%d",
    buf, est, ext);
  rv_buffer = nr_buffer_use (NULL, NULL, 14);
  tlib_pass_if_true ("null buffer cannot be used", -1 == rv_buffer, "rv_buffer=%d est=%d ext=%d", rv_buffer, est, ext);
  nr_buffer_add (NULL, in, 14); /* Don't blow up! */
  rv_buffer = nr_buffer_len (NULL);
  tlib_pass_if_true ("null buffer length returns error", -1 == rv_buffer, "rv_buffer=%d est=%d ext=%d", rv_buffer, est, ext);
  rp = nr_buffer_cptr (NULL);
  tlib_pass_if_true ("null buffer has null ptr", 0 == rp, "rp=%p est=%d ext=%d", rp, est, ext);
}

static void
test_read_write_bad_params (void)
{
  nr_status_t rv;
...
static void
test_write_uint64_t_as_text (void)
{
  nrbuf_t *buf;
  const char *bufptr;
  ...
  nr_buffer_add (buf, "\0", 1); /* Add Nul terminator */
  bufptr = (const char *)nr_buffer_cptr (buf);
  tlib_pass_if_true ("success", 0 == nr_strcmp ("0", bufptr),
    "bufptr=%s", NRSAFESTR (bufptr));
  nr_buffer_destroy (&buf);
}
