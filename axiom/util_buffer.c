...
int
nr_buffer_len (const nrbuf_t *bufp)
{
  if (0 == bufp) {
    return -1;
  }
...
const void *
nr_buffer_cptr (const nrbuf_t *bufp)
{
  if ((0 == bufp) ||
      (bufp->avail <= 0) ||
      (0 == bufp->buf)) {
...
nrbuf_t *
nr_buffer_create (int estsize, int extsize)
{
  nrbuf_t *bp;
...
void
nr_buffer_add (nrbuf_t *bp, const void *data, int dlen)
{
  int bytes_remaining;
  int bytes_needed;
 ...
 */
static void *
nr_buffer_ensure (nrbuf_t *bp, int reqsize)
{
  if (nrunlikely ((0 == bp) || (reqsize < 0))) {
    return 0;
  } else {
...
void
nr_buffer_add_escape_json (nrbuf_t *bufp, const char *raw_string)
{
  int raw_string_len;
  int escaped_space_needed;
  int escaped_len;
  char *bp;  
...
int
nr_buffer_use (nrbuf_t *bufp, void *destp, int dlen)
{
...
void
nr_buffer_destroy (nrbuf_t **bufp)
{
...
void
nr_buffer_write_uint32_t_le (nrbuf_t *bufp, uint32_t val)
{
...
void
nr_buffer_write_uint64_t_as_text (nrbuf_t *bufp, uint64_t val)
{
  char tmp[256];
  int sl;
...
nr_status_t
nr_buffer_read_uint32_t_le (nrbuf_t *bufp, uint32_t *val)
{
  uint32_t x = 0;
