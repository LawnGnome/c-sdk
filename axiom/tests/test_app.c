...
#define TEST_LABELS_JSON  "{\"Data Center\":\"US-East\",\"Server Color\":\"Beige\"}"
...
static void
test_applist_create_destroy (void)
{
  nrapplist_t *applist;
...
test_find_or_add_app (void)
{
  ...
  for (i = 0; i < NR_APP_LIMIT; i++) {
    ...
    if (0 != app) {
      ...
      tlib_pass_if_str_equal ("new app", NR_EXPECTED_PRINTABLE_LICENSE, app->plicense);
      ...
      tlib_pass_if_str_equal ("new app", info.host_display_name, app->info.host_display_name);
      tlib_pass_if_str_equal ("new app", info.redirect_collector, app->info.redirect_collector);
  ...
  for (i = 0; i < NR_APP_LIMIT; i++) {
    ...
    if (0 != app) {
      ...
      tlib_pass_if_str_equal ("find app", NR_EXPECTED_PRINTABLE_LICENSE, app->plicense);
      tlib_pass_if_int_equal ("find app", (int)NR_APP_UNKNOWN, (int)app->state);
      test_obj_as_json ("find app", app->info.settings, "[\"my_settings\"]");
      test_obj_as_json ("find app", app->info.environment, "[\"my_environment\"]");
      test_obj_as_json ("find app", app->info.labels, TEST_LABELS_JSON);
      tlib_pass_if_str_equal ("find app", info.host_display_name, app->info.host_display_name);
      tlib_pass_if_str_equal ("new app", info.redirect_collector, app->info.redirect_collector);
      nrt_mutex_unlock (&app->app_lock);
    }
  }
...
static void
test_find_or_add_app_high_security_mismatch (void)
{
  nrapp_t *app;
  nr_app_info_t info;
  nrapplist_t *applist = nr_applist_create ();
  ...
  if (app) {
    tlib_pass_if_int_equal ("app has high security off", 0, app->info.high_security);
    nrt_mutex_unlock (&app->app_lock);
  }
  ...
  tlib_pass_if_not_null ("app added", app);
  if (app) {
    tlib_pass_if_int_equal ("app has high security on", 1, app->info.high_security);
    nrt_mutex_unlock (&app->app_lock);
  }
  app = nr_app_find_or_add_app (applist, &info);
  tlib_pass_if_not_null ("app found", app);
...
nr_status_t
nr_cmd_appinfo_tx (int daemon_fd NRUNUSED, nrapp_t *app)
{
...
static void
test_agent_should_do_app_daemon_query (void)
{
  int do_query;
  nrapp_t app;
  time_t now = time (0);
  ...
  app.last_daemon_query = now - (NR_APP_UNKNOWN_QUERY_BACKOFF_LIMIT_SECONDS - 1);
  do_query = nr_agent_should_do_app_daemon_query (&app, now);
...
static void
test_agent_find_or_add_app (void)
{
  test_app_state_t *p = (test_app_state_t *)tlib_getspecific ();
  nrapp_t *app;
  nr_app_info_t info;
  ...
  tlib_pass_if_not_null ("new app", app);
  if (0 != app) {
    tlib_pass_if_int_equal ("new app", info.high_security, app->info.high_security);
  ...
  app = applist->apps[0];
  if (0 != app) {
    tlib_pass_if_int_equal ("find app no appinfo", 1, app->failed_daemon_query_count);
    test_obj_as_json ("settings added from callback", app->info.settings, "[\"my_settings\"]");
  }
  ...
  if (0 != app) {
    ...
    tlib_pass_if_int_equal ("app with appinfo", 0, app->failed_daemon_query_count);
    nrt_mutex_unlock (&app->app_lock);
  }
...
tlib_parallel_info_t parallel_info = {.suggested_nthreads = 4, .state_size = sizeof (test_app_state_t)};
