 ...
 *
 * TODO(aharvey): some day these functions should be unit tested.
 */
#include "php_agent.h"
#include "php_call.h"
 ...
 */
static nr_status_t
nr_php_mysqli_link_real_connect (zval *link,
                                 const nr_mysqli_metadata_link_t *metadata
                                 TSRMLS_DC);
 ...
 */
static nr_status_t
nr_php_mysqli_link_set_option (zval *link, long option,
 ...
 */
static zval *
nr_php_mysqli_query_create (void);
 ...
 */
static zval *
nr_php_mysqli_query_find (nr_php_object_handle_t handle TSRMLS_DC);
 ...
 */
static zval *
nr_php_mysqli_query_find_or_create (nr_php_object_handle_t handle TSRMLS_DC);

zval *
nr_php_mysqli_link_duplicate (zval *orig TSRMLS_DC)
{
  zval *dup = NULL;
  int i;
  ...
  if (NR_FAILURE == nr_mysqli_metadata_get (NRPRG (mysqli_links),
                                            Z_OBJ_HANDLE_P (orig),
                                            &metadata)) {
    return NULL;
  }
  ...
  if (metadata.options) {
    for (i = 1; i <= nro_getsize (metadata.options); i++) {
      ...
      if (NR_FAILURE == nr_php_mysqli_link_set_option (dup, option,
                                                       value TSRMLS_CC)) {
        nr_php_zval_free (&dup);
        return NULL;
      }
...
zval *
nr_php_mysqli_query_get_link (nr_php_object_handle_t handle TSRMLS_DC)
{
  zval *link;
  zval *metadata;
...
char *
nr_php_mysqli_query_get_query (nr_php_object_handle_t handle TSRMLS_DC)
{
  const zval *metadata;
  const zval *query;
...
nr_status_t
nr_php_mysqli_query_rebind (nr_php_object_handle_t handle,
                            zval *dest TSRMLS_DC)
{
  zval *args = NULL;
  zval **bind_args = NULL;
   ...
   */
  retval = nr_php_call_user_func (dest, "bind_param",
                                  num_args + 1, bind_args TSRMLS_CC);
...
nr_status_t
nr_php_mysqli_query_set_bind_params (nr_php_object_handle_t handle,
                                     const char *format, size_t format_len,
                                     size_t args_len, zval **args TSRMLS_DC)
{
  char *format_dup;
  size_t i;
  zval *metadata;
...
nr_status_t
nr_php_mysqli_query_set_link (nr_php_object_handle_t query_handle,
                              zval *link TSRMLS_DC)
{
  zval *metadata;
...
nr_status_t
nr_php_mysqli_query_set_query (nr_php_object_handle_t handle,
                               const char *query, int query_len TSRMLS_DC)
{
  char *query_dup;
  zval *metadata;
...
int
nr_php_mysqli_zval_is_link (const zval *zv TSRMLS_DC)
{
  if (NULL == zv) {
    return 0;
  }
...
int
nr_php_mysqli_zval_is_stmt (const zval *zv TSRMLS_DC)
{
  if (NULL == zv) {
    return 0;
  }
...
nr_php_mysqli_query_find (nr_php_object_handle_t handle TSRMLS_DC)
{
  zval *metadata = (zval *) nr_hashmap_index_get (NRPRG (mysqli_queries),
                                                  (uint64_t) handle);
...
static void
nr_php_mysqli_query_destroy (zval *query)
{
...
nr_php_mysqli_query_find_or_create (nr_php_object_handle_t handle TSRMLS_DC)
{
  ...
  if (NULL == NRPRG (mysqli_queries)) {
    NRPRG (mysqli_queries) = nr_hashmap_create ((nr_hashmap_dtor_func_t) nr_php_mysqli_query_destroy);
  } else {
    /*
...
void
nr_php_mysqli_get_host_and_port_path_or_id (const char *host_param,
  const zend_long port, const char *socket, char **host, char **port_path_or_id)
{
  ...
  if (0 == port) {
    /*
     * See: https://github.com/php/php-src/blob/PHP-5.6/ext/mysqlnd/mysqlnd.c#L978-L979
     */
...
nr_php_mysqli_create_datastore_instance (const char *host, const zend_long port,
  const char *socket, const char *database_name)
{
  nr_datastore_instance_t *instance;
  char *actual_host = NULL;
  char *actual_port_path_or_id = NULL;

  nr_php_mysqli_get_host_and_port_path_or_id (host, port, socket, &actual_host, &actual_port_path_or_id);

  instance = nr_datastore_instance_create (actual_host, actual_port_path_or_id, database_name);
...
void
nr_php_mysqli_save_datastore_instance (const zval *mysqli_obj, const char *host,
  const zend_long port, const char *socket, const char *database_name TSRMLS_DC)
{
  nr_datastore_instance_t *instance;
   ...
   */
  instance = nr_php_mysqli_create_datastore_instance (host, port, socket, database_name);
...
nr_datastore_instance_t *
nr_php_mysqli_retrieve_datastore_instance (const zval *mysqli_obj TSRMLS_DC)
{
  nr_datastore_instance_t *instance;
  char *key = nr_php_datastore_make_key (mysqli_obj, "mysqli");
...
void
nr_php_mysqli_remove_datastore_instance (const zval *mysqli_obj TSRMLS_DC)
{
  char *key = nr_php_datastore_make_key (mysqli_obj, "mysqli");
