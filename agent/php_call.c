...
zval *
nr_php_call_user_func (zval *object_ptr, const char *function_name,
                       zend_uint param_count, zval *params[] TSRMLS_DC)
{
  ...
  if ((0 == function_name) || (0 == *function_name)) {
    return NULL;
  }
  ...
  zend_result = call_user_function_ex (EG (function_table), object_ptr, fname, retval, param_count, param_values, no_separation, symbol_table TSRMLS_CC);
  ...
  fname = nr_php_zval_alloc ();
  nr_php_zval_str (fname, function_name);
  zend_result = call_user_function_ex (EG (function_table), &object_ptr, fname, &retval, param_count, param_ptrs, no_separation, symbol_table TSRMLS_CC);
  nr_php_zval_free (&fname);
...
zval *
nr_php_call_user_func_catch (zval *object_ptr, const char *function_name,
                             ...
                             zval **exception TSRMLS_DC)
{
  ...
  /*
   * Short circuit if exception is NULL (which really shouldn't happen!).
   */
  if (NULL == exception) {
   ...
   * thrown. The key difference is that PHP 5 uses a zval, whereas PHP 7 uses a
   * zend_object.
   */
...
zval *
nr_php_call_callable_zval (zval *callable, zend_uint param_count,
                           zval *params[] TSRMLS_DC)
{
  ...
  if (SUCCESS != zend_fcall_info_init (callable, 0, &fci, &fcc, NULL, NULL
                                       TSRMLS_CC)) {
    nrl_verbosedebug (NRL_AGENT,
                      "%s: error in zend_fcall_info_init: %s",
...
zval *
nr_php_call_fcall_info_zval (zend_fcall_info fci, zend_fcall_info_cache fcc,
                             zend_uint param_count, zval *params[] TSRMLS_DC)
{
#ifdef PHP7
  zend_uint i;
...
void
nr_php_call_user_func_array_handler (nrphpcufafn_t handler,
                                     zend_function *func,
...
zval *
nr_php_call_offsetGet (zval *object_ptr, const char *key TSRMLS_DC)
{
  zval *key_zv;
  zval *retval;
