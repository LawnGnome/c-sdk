...
nr_postgres_default_port ()
{
  ...
  if (nr_strlen (port_path_or_id) <= 0) {
     ...
     * See: https://github.com/postgres/postgres/blob/master/configure.in#L151-L164
     */
...
char *
nr_postgres_default_database_name ()
{
...
nr_postgres_parse_conn_info (const char *conn_info, char **host, char **port_path_or_id, char **database_name)
{
  ...
  if ((NULL != *host) || (NULL != *port_path_or_id) || (NULL != *database_name)) {
    return;
  }
  ...
  while (*cp) {
    ...
    if (*cp != '=') {
      nrl_verbosedebug (NRL_INSTRUMENT, "missing \"=\" after \"%s\" in connection info string\n", pname);
      goto fill_defaults;
    }
    *cp++ = '\0';
    ...
    if (*cp != '\'') {
      ...
      *cp2 = '\0';
      ...
      for (;;) {
        if (*cp == '\0') {
          nrl_verbosedebug (NRL_INSTRUMENT, "unterminated quoted string in connection info string\n");
          goto fill_defaults;
        }
        if (*cp == '\\') {
          cp++;
    ...
    if ((0 == nr_strcmp (pname, "host")) && (NULL == *host)) {
       ...
       * See: https://www.postgresql.org/docs/9.3/static/libpq-connect.html#LIBPQ-CONNECT-HOSTADDR
       */
      *host = pval;
