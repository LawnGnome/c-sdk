...
#define rules_apply_testcase(...) rules_apply_testcase_fn (__VA_ARGS__, __FILE__, __LINE__)

static void
rules_apply_testcase_fn (const char *testname, const nrrules_t *rules, const char *input, const char *expected, const char* file, int line)
{
  ...
  if (0 == nr_strcmp (input, expected)) {
     ...
     */
    test_pass_if_true (testname, NR_RULES_RESULT_UNCHANGED == rv, "rv=%d", (int)rv);
     ...
     */
    test_pass_if_true (testname, NR_RULES_RESULT_CHANGED == rv, "rv=%d", (int)rv);
    test_pass_if_true (testname, 0 == nr_strcmp (output, expected), "output=%s expected=%s",
      NRSAFESTR (output), NRSAFESTR (expected));
  }

  nr_free (output);
}

#define rule_parsing_testcase(N, R, M, P, O, F) rule_parsing_testcase_fn ((N), (R), (M), (P), (O), (F), __FILE__, __LINE__)

static void
rule_parsing_testcase_fn (const char *testname, const nrrules_t *rules, const char *match, const char *replacement, int order, int rflags, const char* file, int line)
{
  test_pass_if_true (testname, rules && rules->rules && (1 == rules->nrules), "rules=%p", rules);

  if (rules && rules->rules && (1 == rules->nrules)) {
      nrrule_t *r = rules->rules;

    test_pass_if_true (testname, 0 == nr_strcmp (match, r->match), "match=%s r->match=%s", match, r->match);
    test_pass_if_true (testname, 0 == nr_strcmp (replacement, r->replacement), "replacement=%s r->replacement=%s", replacement, r->replacement);
    test_pass_if_true (testname, order == r->order, "order=%d r->order=%d", order, r->order);
    test_pass_if_true (testname, rflags == r->rflags, "rflags=%d r->rflags=%d", rflags, r->rflags);
    test_pass_if_true (testname, NULL != r->regex, "r->regex=%p", r->regex);
  }
}

static nrrules_t *
build_rules (const char *str)
{
...
test_rule_parsing (void)
{
   ...
   * Some of these appear to be duplicates. Why are duplicate rules sent by the
   * collector? Creating these tests was an exercise in escape sequences.  One
   * escape '\' is needed to create the string in the test source and a second
   '\' is needed for the JSON parsing.  Hence the \\\\1.
   */
  ur = build_rules ("["
  ...
  rule_parsing_testcase ("default rule", ur, "^(test_match_nothing)$", "\\1", 0, NR_RULE_TERMINATE|NR_RULE_HAS_CAPTURES);
  nr_rules_destroy (&ur);
  ...
  ur = build_rules ("["
    ...
    "\"replace_all\":false, "
    "\"terminate_chain\":true}]");
  rule_parsing_testcase ("default rule", ur, ".*\\.(css|gif|ico|jpe?g|js|png|swf)$", "/*.\\1", 0, NR_RULE_TERMINATE|NR_RULE_HAS_CAPTURES|NR_RULE_HAS_ALTS);
  nr_rules_destroy (&ur);
  ...
  rule_parsing_testcase ("default rule", ur, "^[0-9][0-9a-f_,.-]*$", "*", 1, NR_RULE_EACH_SEGMENT);
  nr_rules_destroy (&ur);
  ...
  ur = build_rules ("["
    ...
    "\"replace_all\":false, "
    "\"terminate_chain\":false}]");
  rule_parsing_testcase ("default rule", ur, "^(.*)/[0-9][0-9a-f_,-]*\\.([0-9a-z][0-9a-z]*)$", "\\1/.*\\2", 2, NR_RULE_HAS_CAPTURES);
  nr_rules_destroy (&ur);
...
static void
test_cross_agent_rule_tests (void)
{
  char *json = 0;
  nrobj_t *array = 0;
  nrotype_t otype;
...
#define RULES_TESTS_FILE CROSS_AGENT_TESTS_DIR "/rules.json"
  ...
  if (array && (NR_OBJECT_ARRAY == nro_type (array))) {
    for (i = 1; i <= nro_getsize (array); i++) {
      ...
      tlib_pass_if_true ("tests valid", 0 != test_cases, "test_cases=%p", test_cases);
      ...
      if (test_cases && (NR_OBJECT_ARRAY == nro_type (test_cases))) {
        for (j = 1; j <= nro_getsize (test_cases); j++) {
          ...
          rules_apply_testcase (testname ? testname : input, rules, input, expected);
        }
      }
...
#define process_rule_testcase(...) process_rule_testcase_fn (__VA_ARGS__,__FILE__,__LINE__)

static void
process_rule_testcase_fn (const char *json, int flags, int order, const char *match,
                          const char *replacement, const char *file, int line)
{
  ...
  if (0 == match) {
     ...
     */
    test_pass_if_true ("no rule added", 0 == rules->nrules, "json=%s nrules=%d", json, rules->nrules);
  } else {
    nrrule_t *rule = &rules->rules[0];

    test_pass_if_true ("number rules increased", 1 == rules->nrules, "json=%s nrules=%d", json, rules->nrules);
    test_pass_if_true ("rule added", 0 != rule, "json=%s rule=%p", json, rule);
    if (rule) {
      test_pass_if_true ("correct flags", flags == rule->rflags,
        "json=%s flags=%d rule->rflags=%d",json, flags, rule->rflags);
      test_pass_if_true ("correct order", order == rule->order,
        "json=%s order=%d rule->order=%d",json, order, rule->order);
      test_pass_if_true ("correct match", 0 == nr_strcmp (match, rule->match),
        "json=%s match=%s rule->match=%s",json, NRSAFESTR (match), NRSAFESTR (rule->match));
      test_pass_if_true ("correct replacement", 0 == nr_strcmp (replacement, rule->replacement),
        "json=%s replacement=%s rule->replacement=%s",json, NRSAFESTR (replacement), NRSAFESTR (rule->replacement));
    }
  }
...
test_process_rule (void)
{
  ...
  process_rule_testcase ("{\"replace_all\":false,"
                          "\"terminate_chain\":true,"
                          "\"eval_order\":0,"
                          "\"replacement\":\"\\1\","
                          "\"each_segment\":false,"
                          "\"ignore\":false}", 0, 0, 0, 0);
  /* Missing replacement */
  process_rule_testcase ("{\"match_expression\":\"^(test_match_nothing)$\","
                          "\"replace_all\":false,"
                          "\"terminate_chain\":true,"
                          "\"eval_order\":0,"
                          "\"each_segment\":false,"
   ...
   */
  /* Basic */
  process_rule_testcase ("{\"match_expression\":\"alpha\","
                          "\"replacement\":\"beta\"}",
  ...
  process_rule_testcase ("{\"match_expression\":\"alpha\","
                          ...
                          0, 55, "alpha", "beta");
  /* Has Alts */
  process_rule_testcase ("{\"match_expression\":\"alpha|gamma\","
                          "\"replacement\":\"beta\"}",
...
static void
test_create_from_obj_bad_params (void)
{
  nrrules_t *rules;
...
test_replace_string (void)
{
  ...
  tlib_pass_if_str_equal("basic", "abcabcabc", dest);  /* TODO: WRONG! */
  ...
  ss = nr_regex_match_capture (regex, subject, slen);
  nr_rule_replace_string ("\\0", dest, sizeof (dest), ss);
  tlib_pass_if_str_equal("0-length subject", "\\0", dest); /* WRONG? */
  nr_regex_substrings_destroy (&ss);
