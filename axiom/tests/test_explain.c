...
test_column (void)
{
  ...
  for (i = 1; i < 10; i++) {
    ...
    tlib_pass_if_int_equal ("column count", i, nr_explain_plan_column_count (plan));
  }
...
test_export (void)
{
  ...
  tlib_pass_if_str_equal ("columns and rows", "[[\"a\",\"b\"],[[42,\"foo\"],[\"bar\",0]]]", json);
...
test_row (void)
{
  ...
  tlib_pass_if_int64_t_equal ("add row", 42, nro_get_array_long (added_row, 1, NULL));
