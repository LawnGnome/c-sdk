 ...
 *
 * See the companion script php_test_tools/get_frameworks/installlaravel.sh
 * for a script that can install various versions of Laravel  each running
 * as a blog, populated with "Lorem ipsum"s.
 */
 ...
 */
static void
nr_laravel_name_transaction (zval *router, zval *request TSRMLS_DC);
 ...
 */
static nr_status_t
nr_laravel_should_assign_generic_path (const nrtxn_t *txn, zval *request TSRMLS_DC);
 ...
 * checked: so long as it provides an offsetGet method, it's good enough for
 * us.
 */
static
PHP_NAMED_FUNCTION (nr_laravel_afterfilter_construct)
{
  zval *app = NULL;
  ...
  if (SUCCESS == zend_parse_parameters_ex (ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS () TSRMLS_CC, "o", &app)) {
    if (nr_php_object_has_method (app, "offsetGet" TSRMLS_CC)) {
      /*
       * It's a valid app object. Set $this->app to contain it.
       */
      zend_update_property (nr_laravel_afterfilter_ce, NR_PHP_INTERNAL_FN_THIS, NR_PSTR ("app"), app TSRMLS_CC);
    } else {
      /*
      ...
      nrl_verbosedebug (NRL_FRAMEWORK,
                        "%s: %.*s object doesn't have an offsetGet() method",
                        __func__,
                        NRSAFELEN (nr_php_class_entry_name_length (Z_OBJCE_P (app))),
 ...
 */
static
PHP_NAMED_FUNCTION (nr_laravel_afterfilter_invoke)
{
  zval *app = NULL;
  zval *router = NULL;
  zval *request = NULL;
  ...
  if (SUCCESS != zend_parse_parameters_ex (ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS () TSRMLS_CC, "oo", &request, &response)) {
    nrl_verbosedebug (NRL_FRAMEWORK, "%s: invalid parameters", __func__);
    return;
  }
   ...
   */
  app = nr_php_get_zval_object_property (NR_PHP_USER_FN_THIS, "app" TSRMLS_CC);
  if (!nr_php_is_zval_valid_object (app)) {
    nrl_verbosedebug (NRL_FRAMEWORK, "%s: app property is not an object", __func__);
    return;
  }
...
static zend_function_entry nr_laravel_afterfilter_functions[] = {
  ZEND_FENTRY (__construct, nr_laravel_afterfilter_construct, nr_laravel_afterfilter_construct_arginfo, ZEND_ACC_PUBLIC|ZEND_ACC_CTOR)
  ZEND_FENTRY (__invoke, nr_laravel_afterfilter_invoke, nr_laravel_afterfilter_invoke_arginfo, ZEND_ACC_PUBLIC)
  PHP_FE_END
};

void
nr_laravel_minit (TSRMLS_D)
{
  zend_class_entry ce;

  INIT_CLASS_ENTRY (ce, "newrelic\\Laravel\\AfterFilter", nr_laravel_afterfilter_functions);
  nr_laravel_afterfilter_ce = zend_register_internal_class (&ce TSRMLS_CC);
  zend_declare_property_null (nr_laravel_afterfilter_ce, NR_PSTR ("app"), ZEND_ACC_PRIVATE TSRMLS_CC);
}

static void
nr_laravel_name_transaction_from_zval (const zval *name TSRMLS_DC)
{
  char *name_terminated = nr_alloca (Z_STRLEN_P (name) + 1);

  nr_strxcpy (name_terminated, Z_STRVAL_P (name), Z_STRLEN_P (name));
  nr_txn_set_path ("Laravel", NRPRG (txn), name_terminated, NR_PATH_TYPE_ACTION, NR_OK_TO_OVERWRITE);
}

static nr_status_t
nr_laravel_name_transaction_from_route_action (const zval *action TSRMLS_DC)
{
  ...
  if (nr_php_is_zval_valid_string (action)) {
    nrl_debug (NRL_FRAMEWORK, "%s: using Route::getAction() for transaction naming", __func__);
    nr_laravel_name_transaction_from_zval (action TSRMLS_CC);

    return NR_SUCCESS;
  } else if (nr_php_is_zval_valid_array (action)) {
    const zval *controller = nr_php_zend_hash_find (Z_ARRVAL_P (action), "controller");

    if (NULL == controller) {
      nrl_verbosedebug (NRL_FRAMEWORK, "%s: no controller element in the action array", __func__);
    } else if (!nr_php_is_zval_valid_string (controller)) {
      nrl_debug (NRL_FRAMEWORK, "%s: controller element in the action array is malformed", __func__);
    } else {
      nrl_debug (NRL_FRAMEWORK, "%s: using Route::getAction() for transaction naming", __func__);
    ...
    nrl_debug (NRL_FRAMEWORK, "%s: unexpected type %d returned from Route::getAction()", __func__, Z_TYPE_P (action));
  }
 ...
 */
static nr_status_t
nr_laravel_name_transaction_from_route_method (zval *route, const char *method
                                               TSRMLS_DC)
{
  if (nr_php_object_has_method (route, method TSRMLS_CC)) {
    ...
    if (nr_php_is_zval_valid_string (route_path_zv)) {
      nrl_debug (NRL_FRAMEWORK, "%s: using Route::%s() for transaction naming", __func__, method);
      nr_laravel_name_transaction_from_zval (route_path_zv TSRMLS_CC);

      nr_php_zval_free (&route_path_zv);
      return NR_SUCCESS;
    }
    nrl_verbosedebug (NRL_FRAMEWORK, "%s: Route::%s() returned an unexpected value/type, skipping. ", __func__, method);
    nr_php_zval_free (&route_path_zv);
  }
...
nr_laravel_name_transaction (zval *router, zval *request TSRMLS_DC)
{
  ...
  if (nr_php_object_has_method (router, "current" TSRMLS_CC)) {
    ...
    nrl_debug (NRL_FRAMEWORK, "%s: router does not provide a current() or getCurrentRoute() method", __func__);
  }
  ...
  if (nr_php_is_zval_valid_object (route)) {
    ...
    if (nr_php_object_has_method (route, "getName" TSRMLS_CC)) {
      ...
      if (nr_php_is_zval_valid_string (route_name_zv)) {
        ...
        nrl_debug (NRL_FRAMEWORK, "%s: using Route::getName() for transaction naming", __func__);
        nr_txn_set_path ("Laravel", NRPRG (txn), route_name, NR_PATH_TYPE_ACTION, NR_OK_TO_OVERWRITE);

        nr_php_zval_free (&route_name_zv);
        nr_free (route_name);
        goto leave;
      }
      nrl_verbosedebug (NRL_FRAMEWORK, "%s: Route::getName() returned an unexpected value/type, skipping. ", __func__);
      nr_php_zval_free (&route_name_zv);
    }
    ...
    if (nr_php_object_has_method (route, "getAction" TSRMLS_CC)) {
      ...
      if (route_action) {
        ...
        named = nr_laravel_name_transaction_from_route_action (route_action TSRMLS_CC);
      ...
      nrl_verbosedebug (NRL_FRAMEWORK, "%s: Route::getAction() returned an unexpected value/type, skipping. ", __func__);
    }

    /*
     * The next route-related option is to grab the route pattern from the
     * confusingly named Route::uri(), which is available on Laravel 4.1 to (at
     * least) 5.4, inclusive.
     */
    if (NR_SUCCESS == nr_laravel_name_transaction_from_route_method (route,
                                                                     "uri" TSRMLS_CC)) {
      goto leave;
    }
    ...
    nrl_verbosedebug (NRL_FRAMEWORK, "%s: Route is an unexpected type: %d", __func__,
        Z_TYPE_P (route));
  } else {
  ...
  /*
   * OK, so we weren't able to get the route. That sucks. The final fallback is
   * to just use "$METHOD/index.php": while we can get the requested URL from
   * the request object, we don't actually want to, as that will likely lead to
   * MGI.
   */  
  if (NR_SUCCESS == nr_laravel_should_assign_generic_path (NRPRG (txn), request TSRMLS_CC)) {         
    zval *method_zv = NULL;

    method_zv = nr_php_call (request, "getMethod");
    if (nr_php_is_zval_valid_string (method_zv)) {
      char *name = nr_formatf ("%.*s/index.php",
        NRSAFELEN (Z_STRLEN_P (method_zv)),
        Z_STRVAL_P (method_zv));

      nrl_debug (NRL_FRAMEWORK, "%s: using Request::getMethod() fallback for transaction naming due to invalid Route object", __func__);
      nr_txn_set_path ("Laravel", NRPRG (txn), name, NR_PATH_TYPE_ACTION, NR_OK_TO_OVERWRITE);
  ...
  /*
   * Well, we're in trouble. Let's just log our failure and hope the customer
   * is OK adding some custom code.
   */
  nrl_info (NRL_FRAMEWORK, "%s: unable to name Laravel transaction based on routing or request information", __func__);
 ...
 */
static char *
nr_laravel_version (zval *app TSRMLS_DC)
{
  char *retval = NULL;
  zval *version = NULL;
  ...
  if (0 == nr_php_is_zval_valid_object (app)) {
    nrl_verbosedebug (NRL_FRAMEWORK, "%s: Application object is invalid", __func__);
    return NULL;
  }

  ce = Z_OBJCE_P (app);
  if (NULL == ce) {
    nrl_verbosedebug (NRL_FRAMEWORK, "%s: Application has NULL class entry", __func__);
    return NULL;
  }

  version = nr_php_get_class_constant (ce, "VERSION");
  if (NULL == version) {
    nrl_verbosedebug (NRL_FRAMEWORK, "%s: Application does not have VERSION", __func__);
    return NULL;
  }
...
NR_PHP_WRAPPER (nr_laravel5_exception_report)
{
   ...
   * undersharing.
   *
   * TODO(aharvey): when we implement an API for ignoring errors, we should add
   * tests to ensure that's respected here.
   */
  if (nr_php_object_has_method (this_var, "shouldReport" TSRMLS_CC)) {
    ...
    if (nr_php_is_zval_true (retval)) {
      ...
      st = nr_php_error_record_exception (NRPRG (txn), exception, priority,
        NULL /* use default prefix */, &NRPRG (exception_filters) TSRMLS_CC);
      ...
      nrl_verbosedebug (NRL_FRAMEWORK,
        "%s: ignoring exception due to shouldReport returning false", __func__);
    }
...
static void
nr_laravel_register_after_filter (zval *app TSRMLS_DC)
{
  zval *filter = NULL;
  zval *retval = NULL;
  zval *router = NULL;
   ...
   * captures the Application object. (Sadly, the Zend Engine doesn't provide
   * a rich enough API to just use a true closure.)
   */
  ...
  if (0 == nr_php_object_has_concrete_method (router, "after" TSRMLS_CC)) {
    nrl_verbosedebug (NRL_FRAMEWORK, "%s: Router does not support filters, transaction naming may be degraded", __func__);
    goto end;
  }
  ...
  if (NULL == retval) {
    nrl_verbosedebug (NRL_FRAMEWORK, "%s: error constructing AfterFilter object", __func__);
    goto end;
  }
  nr_php_zval_free (&retval);
  ...
  if (NULL == retval) {
    nrl_verbosedebug (NRL_FRAMEWORK, "%s: error installing AfterFilter", __func__);
    goto end;
  }
...
NR_PHP_WRAPPER (nr_laravel5_middleware_handle)
{
  ...
  if (wraprec->classname) {
    ...
    nr_txn_set_path ("Laravel", NRPRG (txn), wraprec->funcname, NR_PATH_TYPE_ACTION, NR_OK_TO_OVERWRITE);
  }
...
static void
nr_laravel5_wrap_middleware (zval *app TSRMLS_DC)
{
  zval *kernel = NULL;
  zval *middleware = NULL;
  zval *classname = NULL;

  kernel = nr_php_call_offsetGet (app, "Illuminate\\Contracts\\Http\\Kernel" TSRMLS_CC);
  ...
  if (0 == nr_php_is_zval_valid_array (middleware)) {
    if (0 == middleware) {
      nrl_verbosedebug (NRL_FRAMEWORK, "%s: cannot get HTTP middleware", __func__);
      goto leave;
    }

    if (nr_php_is_zval_valid_object (middleware)) {
      nrl_verbosedebug (NRL_FRAMEWORK, "%s: HTTP middleware is an unexpected object: %*s.", __func__,
        NRSAFELEN (nr_php_class_entry_name_length (Z_OBJCE_P (middleware))),
        nr_php_class_entry_name (Z_OBJCE_P (middleware)));
    } else {
      nrl_verbosedebug (NRL_FRAMEWORK, "%s: HTTP middleware is an unexpected type: %d", __func__,
        Z_TYPE_P (middleware));
    }
  ...
  ZEND_HASH_FOREACH_VAL (Z_ARRVAL_P (middleware), classname) {
    if (nr_php_is_zval_valid_string (classname)) {
      ...
      name = nr_formatf ("%.*s::handle", (int)Z_STRLEN_P (classname), Z_STRVAL_P (classname));
      nr_php_wrap_user_function (name, nr_strlen (name), nr_laravel5_middleware_handle TSRMLS_CC);
      nr_free (name);
    }
 ...
 */
static void
nr_laravel_add_callback_method (
  ...
  nrspecialfn_t callback TSRMLS_DC)
{
  ...
  if (NULL == function) {
    nrl_verbosedebug (NRL_FRAMEWORK,
      "cannot get function entry for %.*s::%.*s",
      NRSAFELEN (class_name_len), class_name,
      NRSAFELEN (method_len), method);
    return;
  }

  char *class_method = nr_formatf ("%.*s::%.*s",
    NRSAFELEN (class_name_len), class_name,
    NRSAFELEN (method_len), method);

  nr_php_wrap_user_function (class_method, nr_strlen (class_method), callback TSRMLS_CC);
...
NR_PHP_WRAPPER (nr_laravel5_application_boot)
{
   ...
   */
  exception_handler = nr_php_call_offsetGet (this_var,
    "Illuminate\\Contracts\\Debug\\ExceptionHandler" TSRMLS_CC);
  if (nr_php_is_zval_valid_object (exception_handler)) {
    nr_laravel_add_callback_method (Z_OBJCE_P (exception_handler),
      NR_PSTR ("render"), nr_laravel5_exception_render TSRMLS_CC);

    nr_laravel_add_callback_method (Z_OBJCE_P (exception_handler),
      NR_PSTR ("report"), nr_laravel5_exception_report TSRMLS_CC);
  } else {
    nrl_verbosedebug (NRL_FRAMEWORK, "%s: cannot get exception handler", __func__);
  }
...
NR_PHP_WRAPPER (nr_laravel_router_method_with_request)
{
  ...
  /*
   * Laravel 5.5 turned prepareResponse into a static method.  So, if we're here,
   * and the current function is a static function, we'll use Laravel's userland
   * `app` function to grab an instance of the main router object instead
   */
...
NR_PHP_WRAPPER (nr_laravel_console_application_dorun)
{
  ...
  if (!nr_php_object_instanceof_class (input,
  ...
  if (nr_php_is_zval_non_empty_string (command)) {
    char *name = nr_formatf ("Artisan/%.*s",
      NRSAFELEN (Z_STRLEN_P (command)), Z_STRVAL_P (command));

    nr_txn_set_path ("Laravel", NRPRG (txn), name,
     ...
     */
    nr_txn_set_path ("Laravel", NRPRG (txn), "Artisan/list",
                     NR_PATH_TYPE_ACTION, NR_OK_TO_OVERWRITE);
  }
...
nr_laravel_should_assign_generic_path (const nrtxn_t *txn, zval *request TSRMLS_DC)
{
   ...
   * we don't know what is going on, bail
   */  
  if (!nr_php_object_has_method (request, "getMethod" TSRMLS_CC)) { 
    nrl_verbosedebug (NRL_FRAMEWORK, "%s: Request object has no getMethod method. Bailing.", __func__);
    return NR_FAILURE;
  }
  ...
  /*
   * If the transaction has a name other than "unknown", but its path_type is less than 
   * the NR_PATH_TYPE_ACTION set in `nr_laravel_enable` (i.e. the name is coming from 
   * outside this library), then we should assign the $METHOD/index.php name.
   *
   * See multiverse/laravel/common/test_cannot_get_route.php
   */  
  ...
  nrl_verbosedebug (NRL_FRAMEWORK, 
    "%s: No condition met, so will not assign generic laravel path. path=%s, path_type=%i",
...
void
nr_laravel_enable (TSRMLS_D)
{
  /*
   * We set the path to 'unknown' to prevent having to name routing errors.
   * This follows what is done in symfony2.
   */
  nr_txn_set_path ("Laravel", NRPRG (txn), "unknown", NR_PATH_TYPE_ACTION, NR_NOT_OK_TO_OVERWRITE);
