...
char* nr_txn_create_distributed_trace_payload(nrtxn_t* txn) {
  ...
  /* per the spec: This identifier [ed: trace id] is constant for the entire
   * trip. Its value is equal to the guid of the first span in the trip.
   * Creating a payload should generate a trace ID if one does not exist.
   *
   * TODO for Span Event MMF.
   *
   * nr_distributed_trace_get_guid is not fully supported yet and set at this
   * point. Revisit the usage of nr_txn_get_guid below, probably
   * nr_distributed_trace_get_guid should be used once span event
   * support is implemented.
   */
  if (!nr_distributed_trace_get_trace_id(txn->distributed_trace)) {
    nr_distributed_trace_set_trace_id(txn->distributed_trace,
                                      nr_txn_get_guid(txn));
  }
...
bool nr_txn_accept_distributed_trace_payload_httpsafe(
  ...
  bool rv = nr_txn_accept_distributed_trace_payload(txn, decoded_payload,
                                                    transport_type);
