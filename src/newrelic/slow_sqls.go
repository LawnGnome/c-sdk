...
// A SlowSQL aggregates information about occurences of an SQL statement
// based on observations reported by an agent. Typically, agents do not
// report all occurences, so the Count, TotalMicros and MinMicros fields
...
type SlowSQL struct {
	...
	TotalMicros uint64 // cummulative duration (usecs)
...
func (slow *SlowSQL) merge(other *SlowSQL) {
	// TODO(msl): panic if slow.ID != other.ID?
