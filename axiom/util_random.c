...
void
nr_random_destroy (nr_random_t **rnd_ptr)
{
  nr_realfree ((void **)rnd_ptr);
}

void
nr_random_seed (nr_random_t *rnd, uint64_t seed)
{
  if (0 == rnd) {
    return;
  }
...
nr_random_t *
nr_random_create_from_seed (uint64_t seed)
{
...
void
nr_random_seed_from_time (nr_random_t *rnd)
{
  nr_random_seed (rnd, nr_get_time ());
}

unsigned long
nr_random_range (nr_random_t *rnd, unsigned long max_exclusive)
{
   ...
   */
  largest_multiple = NR_RANDOM_MAX_EXCLUSIVE_LIMIT - (NR_RANDOM_MAX_EXCLUSIVE_LIMIT % max_exclusive);
...
double
nr_random_real (nr_random_t *rnd)
{
  if (0 == rnd) {
    return 0.0;
  }
