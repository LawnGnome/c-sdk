 ...
 *           don't exist.
 */
static nr_php_extension_t *
nr_php_extension_find (nr_php_extensions_t *extensions, int type,
                       int module_number)
{
  int i;
  ...
  for (i = 0; i < extensions->count; i++) {
    ...
    if ((extension->type == type) && (extension->module_number == module_number)) {
      return extension;
    }
  }
 ...
 */
static nr_php_extension_t *
nr_php_extension_next (nr_php_extensions_t *extensions)
{
  ...
  if (count >= extensions->allocated) {
    ...
    extensions->extensions = (nr_php_extension_t *)nr_realloc (
      extensions->extensions, extensions->allocated * sizeof(nr_php_extension_t));
  }
 ...
 */
static void
nr_php_extension_trace_name (const char *ext_name, const char *suffix,
                             char *buffer, size_t buffer_len)
{
  snprintf (buffer, buffer_len, "%s/%s/%s",
            NR_EXTENSION_PREFIX,
            ext_name ? ext_name : NR_EXTENSION_PREFIX_UNKNOWN,
            suffix);
}
 ...
 */
static void
nr_php_save_extension_trace_node (nrtxn_t *txn, const nrtxntime_t *start,
                                  const nrtxntime_t *end, const char *ext_name,
                                  const char *suffix)
{
  char name[512];
 ...
 */
static int
nr_php_extension_shutdown_wrapper (SHUTDOWN_FUNC_ARGS)
{
  nr_php_extension_t *extension = nr_php_extension_find (NRPRG (extensions), type, module_number);
  ...
  if (NULL != extension) {
    ...
    nrl_warning (NRL_INSTRUMENT,
                 "Extension RSHUTDOWN wrapper called for extension %s with no shutdown function",
  ...
  nrl_warning (NRL_INSTRUMENT, "Extension RSHUTDOWN wrapper called for unknown extension");
  return FAILURE;
}
 ...
 */
static int
nr_php_extension_instrument (zend_module_entry *entry,
                             ...
                             zend_hash_key *key NRUNUSED TSRMLS_DC)
{
  ...
  if (NULL != entry) {
    ...
    if ((NULL != entry->request_shutdown_func) &&
        (NULL == nr_php_extension_find (extensions, entry->type, entry->module_number))) {
 ...
 */
static int
nr_php_extension_uninstrument (zend_module_entry *entry,
                               nr_php_extensions_t *extensions,
                               zend_hash_key *key NRUNUSED TSRMLS_DC)
{
  NR_UNUSED_TSRMLS;
  ...
  if (NULL != entry) {
    ...
    if (nr_php_extension_shutdown_wrapper == entry->request_shutdown_func) {
      nr_php_extension_t *extension = nr_php_extension_find (extensions, entry->type, entry->module_number);
      ...
      if (NULL != extension) {
        ...
        nrl_error (NRL_SHUTDOWN,
                   "Extension %s is instrumented, but the original shutdown function cannot be found",
                   entry->name ? entry->name : "(no name)");
      }
    }
  } else {
    nrl_error (NRL_SHUTDOWN, "Attempted to uninstrument a NULL zend_module_entry");
  }
...
nr_php_extensions_t *
nr_php_extension_instrument_create (void)
{
  nr_php_extensions_t *extensions = (nr_php_extensions_t *)nr_malloc (sizeof (nr_php_extensions_t));

  extensions->count = 0;
  extensions->allocated = NR_EXTENSION_CHUNK_SIZE;
  extensions->extensions = (nr_php_extension_t *)nr_calloc (extensions->allocated, sizeof (nr_php_extension_t));

  return extensions;
}

void
nr_php_extension_instrument_rescan (nr_php_extensions_t *extensions TSRMLS_DC)
{
  if (NULL != extensions) {
...
void
nr_php_extension_instrument_destroy (nr_php_extensions_t **extensions_ptr)
{
  nr_php_extensions_t *extensions;
