...
nr_status_t
nr_php_explain_add_value_to_row (const zval *zv, nrobj_t *row)
{
  if ((NULL == zv) || (NULL == row)) {
    return NR_FAILURE;
  }
  ...
  switch (Z_TYPE_P (zv)) {
    ...
    default:
      nrl_verbosedebug (NRL_SQL, "%s: unknown zval type %d",
                        __func__, Z_TYPE_P (zv));
      nro_set_array_string (row, 0, "Unknown value");
      break;
  }
...
int
nr_php_explain_mysql_query_is_explainable (const char *query, int length)
{
  /*
   * Per http://dev.mysql.com/doc/refman/5.5/en/using-explain.html, only SELECT
   * queries can be EXPLAINed.
   *
   * TODO: This does not handle preceding whitespace and comments.
   */
...
nr_explain_plan_t *
nr_php_explain_pdo_statement (nrtxn_t *txn, zval *stmt, zval *parameters,
                              ...
                              TSRMLS_DC)
{
   ...
   */
  if (0 == nr_strncmp (nr_php_pdo_get_driver (stmt TSRMLS_CC), NR_PSTR ("mysql"))) {
...
int
nr_php_explain_wanted (const nrtxn_t *txn, nrtime_t duration TSRMLS_DC)
{
