...
const nr_txn_attribute_t *X \
  = &(nr_txn_attribute_t){(NAME),(DESTS)}
...
NR_TXN_ATTR (nr_txn_host_display_name,      "host.displayName",              NR_TXN_ATTRIBUTE_TRACE_ERROR_EVENT);
NR_TXN_ATTR (nr_txn_request_accept_header,  "request.headers.accept",        NR_TXN_ATTRIBUTE_TRACE_ERROR_EVENT);
NR_TXN_ATTR (nr_txn_request_content_type,   "request.headers.contentType",   NR_TXN_ATTRIBUTE_TRACE_ERROR_EVENT);
NR_TXN_ATTR (nr_txn_request_content_length, "request.headers.contentLength", NR_TXN_ATTRIBUTE_TRACE_ERROR_EVENT);
NR_TXN_ATTR (nr_txn_request_host,           "request.headers.host",          NR_TXN_ATTRIBUTE_TRACE_ERROR_EVENT);
NR_TXN_ATTR (nr_txn_request_method,         "request.method",                NR_TXN_ATTRIBUTE_TRACE_ERROR_EVENT);
NR_TXN_ATTR (nr_txn_request_user_agent,     "request.headers.User-Agent",    NR_TXN_ATTRIBUTE_TRACE_ERROR);
NR_TXN_ATTR (nr_txn_request_referer,        "request.headers.referer",       NR_ATTRIBUTE_DESTINATION_ERROR);
NR_TXN_ATTR (nr_txn_response_content_type,  "response.headers.contentType",  NR_TXN_ATTRIBUTE_TRACE_ERROR_EVENT);
NR_TXN_ATTR (nr_txn_response_content_length,"response.headers.contentLength",NR_TXN_ATTRIBUTE_TRACE_ERROR_EVENT);
/* This "SERVER_NAME" attribute is PHP specific:  It was a custom parameter before attributes happened. */
NR_TXN_ATTR (nr_txn_server_name,            "SERVER_NAME",                   NR_TXN_ATTRIBUTE_TRACE_ERROR);
NR_TXN_ATTR (nr_txn_response_code,          "httpResponseCode",              NR_TXN_ATTRIBUTE_TRACE_ERROR_EVENT);
NR_TXN_ATTR (nr_txn_error_message,          "errorMessage",                  NR_ATTRIBUTE_DESTINATION_TXN_EVENT);
NR_TXN_ATTR (nr_txn_error_type,             "errorType",                     NR_ATTRIBUTE_DESTINATION_TXN_EVENT);

void
nr_txn_set_string_attribute (nrtxn_t *txn, const nr_txn_attribute_t *attribute, const char *value)
{
  if (NULL == txn) {
    return;
  }
  ...
  nr_attributes_agent_add_string (txn->attributes, attribute->destinations, attribute->name, value);
}

void
nr_txn_set_long_attribute (nrtxn_t *txn, const nr_txn_attribute_t *attribute, long value)
{
  if (NULL == txn) {
    return;
  }
  ...
  nr_attributes_agent_add_long (txn->attributes, attribute->destinations, attribute->name, value);
}

char *
nr_txn_create_guid (nr_random_t *rnd)
{
  int i;
...
bool
nr_txn_cmp_options(nrtxnopt_t* o1, nrtxnopt_t* o2) {
  if (o1 == o2)
    return true;
  if (o1 == NULL || o2 == NULL)
    return false;
...
nrtxn_t *
nr_txn_begin (const nrapp_t *app, const nrtxnopt_t *opts, const nr_attribute_config_t *attribute_config)
{
  nrtxn_t *nt;
  ...
  nt->options.apdex_t = (nrtime_t)(nr_reply_get_double (app->connect_reply, "apdex_t", 0.5) * NR_TIME_DIVISOR_D);

  if (nt->options.tt_is_apdex_f) {
    nt->options.tt_threshold = 4 * nt->options.apdex_t;
  }

  if (0 == nr_reply_get_bool (app->connect_reply, "collect_analytics_events", 1)) {
...
#define NR_TXN_MAX_CUSTOM_EVENTS (10*1000)
  ...
  nr_get_cpu_usage (&nt->user_cpu[NR_CPU_USAGE_START], &nt->sys_cpu[NR_CPU_USAGE_START]);
  ...
  nr_txn_set_string_attribute (nt, nr_txn_host_display_name, app->info.host_display_name);

  return nt;
}

void
nr_txn_node_dispose_fields (nrtxnnode_t *node)
{
 ...
 */
static inline int
pq_data_compare (const nrtxnnode_t *a, const nrtxnnode_t *b)
{
...
int
nr_txn_pq_data_compare_wrapper (const nrtxnnode_t *a, const nrtxnnode_t *b)
{
  return pq_data_compare (a, b);
}

nrtxnnode_t *
nr_txn_save_if_slow_enough (nrtxn_t *txn, nrtime_t duration)
{
  int n;
  int parent;
  int priority_child;
 ...
 */
static nr_status_t
nr_txn_apply_url_rules (nrtxn_t *txn, const nrrules_t *rules)
{
  ...
  nr_free (output);
  nrl_verbosedebug (NRL_RULES, "url rules: ignore=%d before=" NRP_FMT " after=" NRP_FMT,
    txn->status.ignore,
    150, path_before,
    NRP_TXNNAME (txn->path));
 ...
 */
static nr_status_t
nr_txn_apply_txn_rules (nrtxn_t *txn, const nrrules_t *rules)
{
  nr_rules_result_t rv;
  nr_status_t ret;
  ...
  nr_free (output);
  nrl_verbosedebug (NRL_RULES, "txn rules: ignore=%d before=" NRP_FMT " after=" NRP_FMT,
    NR_RULES_RESULT_IGNORE == rv,
    150, txnname_before,
...
static void
nr_txn_apply_segment_terms (nrtxn_t *txn, const nr_segment_terms_t *terms)
{
  char *name;
...
static int
nr_txn_should_do_url_rules (int path_type, int is_background)
{
  if (is_background) {
    return 0;
  }
...
static void
nr_txn_update_apdex_if_key_txn (nrtxn_t *txn)
{
  double db = 0.0;
  ...
  key_txns = nro_get_hash_hash (txn->app_connect_reply, "web_transactions_apdex", 0);
  if (0 == key_txns) {
    return;
  }
...
nr_txn_freeze_name_update_apdex (nrtxn_t *txn)
{
   ...
   */
  if (txn->path) {
    if (nr_txn_should_do_url_rules (txn->status.path_type, txn->status.background)) {
...
static char *
nr_txn_replace_first_segment (const char *txnname, const char *new_prefix)
{
...
void
nr_txn_create_apdex_metrics (nrtxn_t *txn, nrtime_t duration)
{
  int satisfying = 0;
  int tolerating = 0;
  int failing = 0;
  ...
  nrm_force_add_apdex (txn->unscoped_metrics, "Apdex", satisfying, tolerating, failing, apdex);
  ...
  nrm_add_apdex (txn->unscoped_metrics, apdex_metric, satisfying, tolerating, failing, apdex);
  nr_free (apdex_metric);
}

void
nr_txn_create_error_metrics (nrtxn_t *txn, const char *txnname)
{
  int nl;
...
void
nr_txn_create_duration_metrics (nrtxn_t *txn, const char *txnname, nrtime_t duration)
{
  ...
  if (NULL == txnname_slash) {
    ...
    total_metric = nr_formatf ("%.*s%s%s", (int)(txnname_slash - txnname), txnname, TOTAL_TIME_SUFFIX, txnname_slash);
  }
  ...
  nrm_force_add_ex (txn->unscoped_metrics, total_metric, total_duration, total_duration);
  nrm_force_add_ex (txn->unscoped_metrics, rollup_metric, duration, exclusive);
  nrm_force_add_ex (txn->unscoped_metrics, rollup_total_metric, total_duration, total_duration);

  nro_set_hash_double (txn->intrinsics, "totalTime", ((double)total_duration) / NR_TIME_DIVISOR_D);

  nr_free (total_metric);
}

void
nr_txn_create_queue_metric (nrtxn_t *txn)
{
  nrtime_t qwait  = 0;
  ...
  if (txn->status.http_x_start > txn->root.start_time.when) {
    nrl_verbosedebug (NRL_TXN, "X-Request-Start is in the future: "
      NR_TIME_FMT " vs " NR_TIME_FMT, txn->status.http_x_start,
 ...
 */
static void
nr_txn_create_cpu_metrics (nrtxn_t *txn)
{
  nrtime_t user;
  nrtime_t sys;
  nrtime_t combined;
...
static void
nr_txn_add_datastore_rollup_metric (const char *name, int len, nrtxn_t *txn)
{
  char *dest;
  char *src;
...
void
nr_txn_create_rollup_metrics (nrtxn_t *txn)
{
  if (NULL == txn) {
    return;
  }
   ...
   */
  if (txn->status.background) {
    nrm_duplicate_metric (txn->unscoped_metrics, "Datastore/all", "Datastore/allOther");
    nrm_duplicate_metric (txn->unscoped_metrics, "External/all", "External/allOther");
  } else {
    nrm_duplicate_metric (txn->unscoped_metrics, "Datastore/all", "Datastore/allWeb");
    nrm_duplicate_metric (txn->unscoped_metrics, "External/all", "External/allWeb");
  }

  nr_string_pool_apply (txn->datastore_products,
...
nrtime_t
nr_txn_duration (const nrtxn_t *txn)
{
  if (NULL == txn) {
    return 0;
  }
...
void
nr_txn_add_error_attributes (nrtxn_t *txn)
{
  if (NULL == txn) {
    return;
  }
  ...
  nr_txn_set_string_attribute (txn, nr_txn_error_message, nr_error_get_message (txn->error));
  nr_txn_set_string_attribute (txn, nr_txn_error_type,    nr_error_get_klass (txn->error));
}

int
nr_txn_should_create_apdex_metrics (const nrtxn_t *txn)
{
  if (NULL == txn) {
    return 0;
  }
...
nr_txn_end (nrtxn_t *txn)
{
  ...
  nr_get_cpu_usage (&txn->user_cpu[NR_CPU_USAGE_END], &txn->sys_cpu[NR_CPU_USAGE_END]);
  nr_txn_set_time (txn, &txn->root.stop_time);

  duration = nr_txn_duration (txn);

  /*
   * Create the transaction, rollup, and queue metrics. Done in a separate functions
   ...
   */
  if (txn->synthetics) {
    nro_set_hash_string (txn->intrinsics, "synthetics_resource_id", nr_synthetics_resource_id (txn->synthetics));
    nro_set_hash_string (txn->intrinsics, "synthetics_job_id", nr_synthetics_job_id (txn->synthetics));
    nro_set_hash_string (txn->intrinsics, "synthetics_monitor_id", nr_synthetics_monitor_id (txn->synthetics));
  }
  ...
  nr_txn_assignment_t ok_to_override)
{
  ...
  if ((NR_NOT_OK_TO_OVERWRITE == ok_to_override) && (ptype == txn->status.path_type)) {
    return NR_FAILURE;
  }

  if (0 != whence) {
    nrl_debug (NRL_FRAMEWORK, NRP_FMT " naming is " NRP_FMT, NRP_CONFIG (whence), NRP_FWNAME (path));
  }
...
void
nr_txn_set_request_uri (nrtxn_t *txn, const char *uri)
{
  int i;
  char *without_params;
...
nr_status_t
nr_txn_record_error_worthy (const nrtxn_t *txn, int priority)
{
  if (nrunlikely ((0 == txn) || (0 == txn->options.err_enabled) || (0 == txn->status.recording))) {
    return NR_FAILURE;
  }
...
void
nr_txn_record_error (nrtxn_t *txn, int priority, const char *errmsg,
                     const char *errclass, const char *stacktrace_json)
{
  if (nrunlikely ((0 == txn) || (0 == txn->options.err_enabled) || (0 == errmsg) || (0 == errclass) ||
                  (0 == txn->status.recording) || (0 == errmsg[0]) || (0 == errclass[0]) || (0 == stacktrace_json))) {
    return;
  }
  ...
  txn->error = nr_error_create (priority, errmsg, errclass, stacktrace_json, nr_get_time ());

  nrl_verbosedebug (NRL_TXN, "recording error priority=%d msg='%.48s' cls='%.48s'",
    priority, NRSAFESTR (errmsg), NRSAFESTR (errclass));
}

char *
nr_txn_create_fn_supportability_metric (const char *function_name, const char *class_name)
{
...
void
nr_txn_force_single_count (nrtxn_t *txn, const char *metric_name)
{
  if (NULL == txn) {
    return;
  }
...
int
nr_txn_should_force_persist (const nrtxn_t *txn)
{
  if (0 == txn) {
    return 0;
  }

  if (txn->status.has_inbound_record_tt ||
...
static void
nr_txn_change_background_status (nrtxn_t *txn, const char *reason, int is_background)
{
  if (0 == txn) {
    return;
  }
  if (txn->status.path_is_frozen) {
     ...
     */
    nrm_force_add (txn->unscoped_metrics, "Supportability/background_status_change_prevented", 0);
    return;
  }

  txn->status.background = is_background;

  nrl_debug (NRL_INIT, "%.128s: marking txn as %.32s",
    reason ? reason : "",
    is_background ? "background job" : "web transaction");
}

void
nr_txn_set_as_background_job (nrtxn_t *txn, const char *reason)
{
  nr_txn_change_background_status (txn, reason, 1);
}

void
nr_txn_set_as_web_transaction (nrtxn_t *txn, const char *reason)
{
  nr_txn_change_background_status (txn, reason, 0);
}

void
nr_txn_set_http_status (nrtxn_t *txn, int http_code)
{
  char buf[32];
...
nr_status_t
nr_txn_add_user_custom_parameter (nrtxn_t *txn, const char *key, const nrobj_t *value)
{
  if (0 == txn) {
    return NR_FAILURE;
  }
  ...
  return nr_attributes_user_add (txn->attributes, NR_DEFAULT_USER_ATTRIBUTE_DESTINATIONS, key, value);
}

void
nr_txn_add_request_parameter (nrtxn_t *txn, const char *key, const char *value, int legacy_enable)
{
  char *buf = NULL;
   ...
   */
  if (legacy_enable) {
    default_destinations = NR_ATTRIBUTE_DESTINATION_TXN_TRACE |
                           NR_ATTRIBUTE_DESTINATION_ERROR;
  } else {
  ...
  nr_attributes_agent_add_string (txn->attributes, default_destinations, buf, value);
  nr_free (buf);
}

nr_status_t
nr_txn_set_stop_time (nrtxn_t *txn, const nrtxntime_t *start, nrtxntime_t *stop)
{
...
void
nr_txn_set_request_referer (nrtxn_t *txn, const char *request_referer)
{
  char *clean_referer;
...
void
nr_txn_set_request_content_length (nrtxn_t *txn, const char *content_length)
{
  ...
  nr_txn_set_long_attribute (txn, nr_txn_request_content_length, content_length_int);
}
...
void
nr_txn_set_queue_start (nrtxn_t *txn, const char *x_request_start)
{
  ...
  /*
   * (per https://docs.newrelic.com/docs/apm/other-features/request-queueing/configuring-request-queue-reporting)
   *
  ...
  if (0 == queue_start) {
    ...
    nrl_debug (NRL_TXN, "unable to parse HTTP_X_REQUEST_START header " NRP_FMT " expected something like 't=%.0f'",
      NRP_PHP (x_request_start),
      now_microseconds);
  } else {
...
void
nr_txn_record_custom_event_internal (nrtxn_t *txn, const char *type, const nrobj_t *params, nrtime_t now)
{
  nr_random_t *rnd;
...
void
nr_txn_record_custom_event (nrtxn_t *txn, const char *type, const nrobj_t *params)
{
  nr_txn_record_custom_event_internal (txn, type, params, nr_get_time ());
}

int
nr_txn_is_synthetics (const nrtxn_t *txn)
{
  if (NULL == txn) {
    return 0;
  }
...
static int
nr_txn_is_cat (const nrtxn_t *txn)
{
  if (NULL == txn) {
    return 0;
  }
...
nr_tt_recordsql_t
nr_txn_sql_recording_level (const nrtxn_t *txn)
{
  if (NULL == txn) {
    return NR_SQL_NONE;
  }
...
void
nr_txn_add_cat_analytics_intrinsics (const nrtxn_t *txn, nrobj_t *intrinsics)
{
  char *alternate_path_hashes = NULL;
  char *path_hash = NULL;
  const char *trip_id = NULL;

  if ((0 == nr_txn_is_cat (txn)) ||
      (NULL == intrinsics) || (NR_OBJECT_HASH != nro_type (intrinsics))) {
    return;
  }
  ...
  if (txn->cat.referring_path_hash) {
    nro_set_hash_string (intrinsics, "nr.referringPathHash",        txn->cat.referring_path_hash);
  }

  if (txn->cat.inbound_guid) {
    nro_set_hash_string (intrinsics, "nr.referringTransactionGuid", txn->cat.inbound_guid);
  }

  if (alternate_path_hashes) {
    nro_set_hash_string (intrinsics, "nr.alternatePathHashes",    alternate_path_hashes);
  }
...
nr_txn_add_alternate_path_hash (nrtxn_t *txn, const char *path_hash)
{
   ...
   */
  if (nro_getsize (txn->cat.alternate_path_hashes) >= NR_TXN_MAX_ALTERNATE_PATH_HASHES) {
    return;
  }
...
nr_apdex_zone_t
nr_txn_apdex_zone (const nrtxn_t *txn, nrtime_t duration)
{
...
struct hash_state {
  ...
  char *path_hash;      /* The final path hash, which will be filtered out of the alternate list */
};

static nr_status_t
nr_txn_iter_path_hash (const char *key, const nrobj_t *val NRUNUSED, struct hash_state *hash_state)
{
...
static int
nr_txn_compare_path_hashes (const void *pa, const void *pb)
{
...
nr_txn_get_alternate_path_hashes (const nrtxn_t *txn)
{
  ...
  hash_state.hashes = (const char **) nr_calloc (hash_state.capacity,
                                                 sizeof (const char *));
  hash_state.length = 0;
  hash_state.used = 0;
...
char *
nr_txn_get_path_hash (nrtxn_t *txn)
{
...
int
nr_txn_is_account_trusted (const nrtxn_t *txn, int account_id)
{
  int idx = 0;
  ...
  trusted_account_ids = nro_get_hash_array (txn->app_connect_reply, "trusted_account_ids", 0);
...
int
nr_txn_should_save_trace (const nrtxn_t *txn, nrtime_t duration)
{
  if (NULL == txn) {
    return 0;
  }
...
void
nr_txn_adjust_exclusive_time (nrtxn_t *txn, nrtime_t duration)
{
  if (NULL == txn) {
    return;
  }
...
void
nr_txn_add_async_duration (nrtxn_t *txn, nrtime_t duration)
{
  if (NULL == txn) {
    return;
  }
...
int
nr_txn_event_should_add_guid (const nrtxn_t *txn)
{
  if (NULL == txn) {
    return 0;
  }
...
double
nr_txn_start_time_secs (const nrtxn_t *txn)
{
...
nrtime_t
nr_txn_start_time (const nrtxn_t *txn)
{
  if (NULL == txn) {
    return 0;
  }
...
void
nr_txn_add_file_naming_pattern (nrtxn_t *txn, const char *user_pattern)
{
  if (NULL == txn) {
    return;
  }
  ...
  txn->match_filenames = nr_file_namer_append (txn->match_filenames, user_pattern);
}

void
nr_txn_add_match_files (nrtxn_t *txn, const char *comma_separated_list)
{
    nrobj_t *rs;
    int ns;
    int i;
...
void
nr_txn_match_file (nrtxn_t *txn, const char *filename)
{
  char *match;
  ...
  nr_txn_set_path ("File naming", txn, match, NR_PATH_TYPE_ACTION, NR_NOT_OK_TO_OVERWRITE);
  ...
  const char *attribute_name)
{
  ...
  if (metric) {
    ...
    nro_set_hash_double (attributes, attribute_name, ((double)total) / NR_TIME_DIVISOR_D);
  }
}

static void
nr_txn_add_metric_count_as_attribute (
  nrobj_t    *attributes,
  nrmtable_t *metrics,
  const char *metric_name,
  const char *attribute_name)
...
nr_error_to_event (const nrtxn_t *txn)
{
  ...
  nro_set_hash_string (params, "error.message",   nr_error_get_message (txn->error));
  nro_set_hash_string (params, "transactionName", txn->name);
  nro_set_hash_double (params, "duration",        ((double)duration) / NR_TIME_DIVISOR_D);

  nr_txn_add_metric_total_as_attribute (params, txn->unscoped_metrics, "WebFrontend/QueueTime", "queueDuration");
  nr_txn_add_metric_total_as_attribute (params, txn->unscoped_metrics, "External/all",          "externalDuration");
  nr_txn_add_metric_total_as_attribute (params, txn->unscoped_metrics, "Datastore/all",         "databaseDuration");

  nr_txn_add_metric_count_as_attribute (params, txn->unscoped_metrics, "Datastore/all", "databaseCallCount");
  nr_txn_add_metric_count_as_attribute (params, txn->unscoped_metrics, "External/all",  "externalCallCount");

  nro_set_hash_string (params, "nr.transactionGuid", txn->guid);

  if (txn->cat.inbound_guid) {
    nro_set_hash_string (params, "nr.referringTransactionGuid", txn->cat.inbound_guid);
  }

  if (txn->synthetics) {
    nro_set_hash_string (params, "nr.syntheticsResourceId", nr_synthetics_resource_id (txn->synthetics));
    nro_set_hash_string (params, "nr.syntheticsJobId", nr_synthetics_job_id (txn->synthetics));
    nro_set_hash_string (params, "nr.syntheticsMonitorId", nr_synthetics_monitor_id (txn->synthetics));
  }
...
nr_txn_event_intrinsics (const nrtxn_t *txn)
{
  ...
  nro_set_hash_double (params, "totalTime", ((double)duration + txn->async_duration) / NR_TIME_DIVISOR_D);
 ...
 */
nr_analytics_event_t *
nr_txn_to_event (const nrtxn_t *txn)
{
  nr_analytics_event_t *event;
  nrobj_t *params;
  nrobj_t *agent_attributes;
  ...
  agent_attributes = nr_attributes_agent_to_obj (txn->attributes, NR_ATTRIBUTE_DESTINATION_TXN_EVENT);
  user_attributes = nr_attributes_user_to_obj (txn->attributes, NR_ATTRIBUTE_DESTINATION_TXN_EVENT);
  event = nr_analytics_event_create (params, agent_attributes, user_attributes);
...
void
nr_txn_name_from_function (nrtxn_t *txn, const char *funcname, const char *classname)
{
  ...
  nr_txn_set_path ("name from function", txn, name, NR_PATH_TYPE_FUNCTION, NR_NOT_OK_TO_OVERWRITE);
  nr_free (buf);
}

void
nr_txn_ignore (nrtxn_t *txn)
{
  if (NULL == txn) {
    return;
  }
...
nr_status_t
nr_txn_add_custom_metric (nrtxn_t *txn, const char *name, double value_ms)
{
  if (NULL == txn) {
    return NR_FAILURE;
  }
  ...
  if (isnan (value_ms) || isinf (value_ms)) {
    ...
    nrl_warning (NRL_API, "unable to add custom metric '%s': "
      "invalid custom metric value %s", NRSAFESTR (name), kind);
    return NR_FAILURE;
  }

  nrm_add (txn->unscoped_metrics, name, (nrtime_t)(NR_TIME_DIVISOR_MS_D * value_ms));
...
bool
nr_txn_is_current_path_named (const nrtxn_t *txn, const char *path)
{
  if (NULL == txn) {
    return false;
  }  
