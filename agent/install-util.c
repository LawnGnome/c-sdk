...
static int
do_stat (int argc NRUNUSED, char *const argv[])
{
  struct stat sb;
...
static int
do_realpath (int argc NRUNUSED, char *const argv[])
{
...
usage (void)
{
  printf ("Usage: newrelic-iutil {stat filename} | {realpath filename} | {version}\n");
  exit (2);
}
