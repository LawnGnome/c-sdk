 ...
 */
static void
nr_php_rum_log_content_type (void *data TSRMLS_DC)
{
  ...
  if (0 == nr_strnicmp (hdr->header, NR_PSTR ("Content-Type:"))) {
    nrl_verbosedebug (NRL_AUTORUM, "autorum: " NRP_FMT, NRSAFELEN (hdr->header_len), hdr->header);
  }
}
 ...
 */
void
nr_php_rum_output_handler (char *output, 
                           ...
                           int mode TSRMLS_DC)
{
  ...
  if (debug_autorum) {
    nrl_verbosedebug (NRL_AUTORUM, "autorum: output handler starting: mode=%d", mode);
  }
  ...
  if (debug_autorum) {
    zend_llist_apply (nr_php_response_headers (TSRMLS_C), nr_php_rum_log_content_type TSRMLS_CC);
  }
  ...
  nr_rum_output_handler_worker (
    ...
    handled_output, &handled_output_len_temp,
    has_response_content_length,
    mimetype,
    debug_autorum);

  if (handled_output_len) {
    *handled_output_len = (nr_output_buffer_string_len_t) handled_output_len_temp;
  }
