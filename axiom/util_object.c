...
/*
 * This file implements the generic object. Unlike its use in the agent where
 * the internals of this type are visible to all, in this implementation the
 * object is completely opaque, and all member access is done through the
...
static nr_status_t nro_internal_setvalue_array   (nrintobj_t *op, int idx, nrobj_t *nobj);
static nr_status_t nro_internal_setvalue_hash    (nrintobj_t *op, const char *key, nrobj_t *nobj);

nrotype_t
nro_type (const nrobj_t *obj)
{
  if (0 == obj) {
...
/*
 * Assert that the given object is of the specified type.
 * Returns the object if successful, or NULL if not.
 *
 * Use nro_cassert for constant objects.
 */
nrobj_t *
nro_assert (nrobj_t *obj, nrotype_t type)
{
 ...
 */
static const nrobj_t *
nro_cassert (const nrobj_t *obj, nrotype_t type)
{
  if (nrunlikely (NULL == obj)) {
    return NULL;
  }
...
static void
nro_internal_new (nrintobj_t *op)
{
  switch (op->type) {
    ...
    case NR_OBJECT_HASH:
      ...
      op->u.hval.data = (nrintobj_t **)nr_calloc (NRO_CHUNK_SIZE, sizeof (nrintobj_t *));
      break;

    case NR_OBJECT_ARRAY:
      op->u.aval.allocated = NRO_CHUNK_SIZE;
      op->u.aval.data = (nrintobj_t **)nr_calloc (NRO_CHUNK_SIZE, sizeof (nrintobj_t *));
      break;
  }
}

nrobj_t *
nro_new (nrotype_t type)
{
  nrintobj_t *op;
...
static nrintobj_t *
nro_internal_new_and_construct (nrotype_t type)
{
  nrintobj_t *op;
...
static void
nro_internal_delete (nrintobj_t *op, int freebase)
{
  int i;
 ...
 */
static int
nro_find_key_in_hash (const nrintobj_t *op, const char *key)
{
  int i;
...
static nr_status_t
nro_internal_setvalue_array (nrintobj_t *op, int idx, nrobj_t *nobj)
{
  int i;
  ...
  if (0 == idx) {
    if (op->u.aval.size == op->u.aval.allocated) {
      op->u.aval.allocated += NRO_CHUNK_SIZE;
      op->u.aval.data = (nrintobj_t **)nr_realloc ((void *)op->u.aval.data,
        op->u.aval.allocated * sizeof (nrintobj_t *));
...
static nr_status_t
nro_internal_setvalue_hash (nrintobj_t *op, const char *key, nrobj_t *nobj)
{
  int i;
  int idx;
  ...
  if (-2 != idx) {
    ...
    if (idx == op->u.hval.allocated) {
      op->u.hval.allocated += NRO_CHUNK_SIZE;
      op->u.hval.keys = (char **)nr_realloc (op->u.hval.keys, op->u.hval.allocated * sizeof (char *));
      op->u.hval.data = (nrintobj_t **)nr_realloc (op->u.hval.data, op->u.hval.allocated * sizeof (nrintobj_t *));
...
nr_status_t
nro_set_hash (nrobj_t *obj, const char *key, const nrobj_t *value)
{
  nr_status_t rv;
  nrobj_t *dup;
...
nr_status_t
nro_set_array (nrobj_t *obj, int idx, const nrobj_t *value)
{
  nr_status_t rv;
  nrobj_t *dup;
...
nr_status_t
nro_set_hash_none (nrobj_t *obj, const char *key)
{
  nr_status_t rv;
...
nr_status_t
nro_set_hash_boolean (nrobj_t *obj, const char *key, int value)
{
  nr_status_t rv;
...
nr_status_t
nro_set_hash_int (nrobj_t *obj, const char *key, int value)
{
  nr_status_t rv;
...
nr_status_t
nro_set_hash_long (nrobj_t *obj, const char *key, int64_t value)
{
  nr_status_t rv;
...
nr_status_t
nro_set_hash_double (nrobj_t *obj, const char *key, double value)
{
  nr_status_t rv;
...
nr_status_t
nro_set_hash_string (nrobj_t *obj, const char *key, const char *value)
{
  nr_status_t rv;
...
nr_status_t
nro_set_hash_jstring (nrobj_t *obj, const char *key, const char *value)
{
  nr_status_t rv;
...
nr_status_t
nro_set_array_none (nrobj_t *obj, int idx)
{
  nr_status_t rv;
  nrobj_t *ob = nro_new_none ();
...
nr_status_t
nro_set_array_boolean (nrobj_t *obj, int idx, int value)
{
  nr_status_t rv;
  nrobj_t *ob = nro_new_boolean (value);
...
nr_status_t
nro_set_array_int (nrobj_t *obj, int idx, int value)
{
  nr_status_t rv;
  nrobj_t *ob = nro_new_int (value);
...
nr_status_t
nro_set_array_long (nrobj_t *obj, int idx, int64_t value)
{
  nr_status_t rv;
  nrobj_t *ob = nro_new_long (value);
...
nr_status_t
nro_set_array_double (nrobj_t *obj, int idx, double value)
{
  nr_status_t rv;
  nrobj_t *ob = nro_new_double (value);
...
nr_status_t
nro_set_array_string (nrobj_t *obj, int idx, const char *value)
{
  nr_status_t rv;
  nrobj_t *ob = nro_new_string (value);
...
nr_status_t
nro_set_array_jstring (nrobj_t *obj, int idx, const char *value)
{
  nr_status_t rv;
  nrobj_t *ob = nro_new_jstring (value);
 ...
 */
int
nro_get_hash_boolean (const nrobj_t *obj, const char *key, nr_status_t *errp)
{
  return nro_get_boolean (nro_get_hash_value (obj, key, 0), errp);
}

int
nro_get_hash_int (const nrobj_t *obj, const char *key, nr_status_t *errp)
{
  return nro_get_int (nro_get_hash_value (obj, key, 0), errp);
}

int64_t
nro_get_hash_long (const nrobj_t *obj, const char *key, nr_status_t *errp)
{
  return nro_get_long (nro_get_hash_value (obj, key, 0), errp);
}

const char *
nro_get_hash_string (const nrobj_t *obj, const char *key, nr_status_t *errp)
{
  return nro_get_string (nro_get_hash_value (obj, key, 0), errp);
}

const char *
nro_get_hash_jstring (const nrobj_t *obj, const char *key, nr_status_t *errp)
{
  return nro_get_jstring (nro_get_hash_value (obj, key, 0), errp);
}

double
nro_get_hash_double (const nrobj_t *obj, const char *key, nr_status_t *errp)
{
  return nro_get_double (nro_get_hash_value (obj, key, 0), errp);
}

const nrobj_t *
nro_get_hash_hash (const nrobj_t *obj, const char *key, nr_status_t *errp)
{
  return nro_cassert (nro_get_hash_value (obj, key, errp), NR_OBJECT_HASH);
}

const nrobj_t *
nro_get_hash_array (const nrobj_t *obj, const char *key, nr_status_t *errp)
{
  return nro_cassert (nro_get_hash_value (obj, key, errp), NR_OBJECT_ARRAY);
}

int
nro_get_array_boolean (const nrobj_t *obj, int key, nr_status_t *errp)
{
  return nro_get_boolean (nro_get_array_value (obj, key, 0), errp);
}

int
nro_get_array_int (const nrobj_t *obj, int key, nr_status_t *errp)
{
  return nro_get_int (nro_get_array_value (obj, key, 0), errp);
}

int64_t
nro_get_array_long (const nrobj_t *obj, int key, nr_status_t *errp)
{
  return nro_get_long (nro_get_array_value (obj, key, 0), errp);
}

int
nro_get_array_ival (const nrobj_t *obj, int key, nr_status_t *errp)
{
  return nro_get_ival (nro_get_array_value (obj, key, 0), errp);
}

const char *
nro_get_array_string (const nrobj_t *obj, int key, nr_status_t *errp)
{
  return nro_get_string (nro_get_array_value (obj, key, 0), errp);
}

const char *
nro_get_array_jstring (const nrobj_t *obj, int key, nr_status_t *errp)
{
  return nro_get_jstring (nro_get_array_value (obj, key, 0), errp);
}

double
nro_get_array_double (const nrobj_t *obj, int key, nr_status_t *errp)
{
  return nro_get_double (nro_get_array_value (obj, key, 0), errp);
}

const nrobj_t *
nro_get_array_hash (const nrobj_t *obj, int key, nr_status_t *errp)
{
  return nro_cassert (nro_get_array_value (obj, key, errp), NR_OBJECT_HASH);
}

const nrobj_t *
nro_get_array_array (const nrobj_t *obj, int key, nr_status_t *errp)
{
  return nro_cassert (nro_get_array_value (obj, key, errp), NR_OBJECT_ARRAY);
}

void
nro_iteratehash (const nrobj_t *obj, nrhashiter_t func, void *ptr)
{
...
nro_copy (const nrobj_t *obj)
{
  ...
  switch (op->type) {
    ...
    case NR_OBJECT_HASH:
      ...
      np->u.hval.data = (nrintobj_t **)nr_calloc (np->u.hval.size, sizeof (nrintobj_t *));
    ...
    case NR_OBJECT_ARRAY:
      ...
      np->u.aval.data = (nrintobj_t **)nr_calloc (np->u.aval.size, sizeof (nrintobj_t *));
...
int
nro_get_boolean (const nrobj_t *obj, nr_status_t *errp)
{
  const nrintobj_t *op = (const nrintobj_t *)obj;
...
error:
  ...
  return -1; /* TODO(rrh): dodgy */
}

int
nro_get_int (const nrobj_t *obj, nr_status_t *errp)
{
  const nrintobj_t *op = (const nrintobj_t *)obj;
...
int
nro_get_ival (const nrobj_t *obj, nr_status_t *errp)
{
  const nrintobj_t *op = (const nrintobj_t *)obj;
  int iret = 0;
...
int64_t
nro_get_long (const nrobj_t *obj, nr_status_t *errp)
{
  const nrintobj_t *op = (const nrintobj_t *)obj;
...
error:
  ...
  return ((int64_t)-1);  /* TODO(rrh): dodgy */
}

double
nro_get_double (const nrobj_t *obj, nr_status_t *errp)
{
  const nrintobj_t *op = (const nrintobj_t *)obj;
...
error:
  ...
  return (-1.0); /* TODO (rrh): dodgy */
}

const char *
nro_get_string (const nrobj_t *obj, nr_status_t *errp)
{
  const nrintobj_t *op = (const nrintobj_t *)obj;
...
const char *
nro_get_jstring (const nrobj_t *obj, nr_status_t *errp)
{
  const nrintobj_t *op = (const nrintobj_t *)obj;
...
const nrobj_t *
nro_get_hash_value (const nrobj_t *obj, const char *key, nr_status_t *errp)
{
  const nrintobj_t *op = (const nrintobj_t *)obj;
  int idx;
...
const nrobj_t *
nro_get_hash_value_by_index (const nrobj_t *obj, int idx, nr_status_t *errp, const char **keyp)
{
  const nrintobj_t *op = (const nrintobj_t *)obj;
...
const nrobj_t *
nro_get_array_value (const nrobj_t *array, int idx, nr_status_t *errp)
{
...
static void
add_obj_jfmt (nrbuf_t *buf, const char *fmt, ...)
{
  char tbuf[1024];
  char *tmp = tbuf;
  va_list ap;
...
static void
add_obj_double (nrbuf_t *buf, const double d)
{
  char tbuf[1024];
  int reqlen;
...
static void
recursive_obj_to_json (const nrintobj_t *op, nrbuf_t *buf)
{
  int l;
  int i;
...
/*
 * This parser is shamelessly stolen from cJSON. When time permits (i.e as soon
 * as humanly possible) get rid of it in favour of the state machine I started
 * above.
 *
 * If we recode this to use nothing from cJSON, be sure to drop that license
 * from the license manifest found in php_agent/LicenseData.
 */
 ...
 */
static const char *
parse_number (nrintobj_t *item, const char *num)
{
  double d;
  char *ret = 0;
  long i;
...
static const char *
parse_string (nrintobj_t *item, const char *str)
{
  ...
  out = (char *)nr_malloc (len + 1); /* This is how long we need for the string, roughly. */
...
nrobj_t *
nro_create_from_json (const char *json)
{
  nrintobj_t *rv;
...
nrobj_t *
nro_create_from_json_unterminated (const char *json, int len)
{
  nrobj_t *obj;
  char *terminated;
...
static const char *
parse_value (nrintobj_t *item, const char *value)
{
  if (!value) {
...
static const char *
parse_array (nrintobj_t *item, const char *value)
{
  nrintobj_t *child;
  ...
  item->u.aval.data = (nrintobj_t **)nr_calloc (item->u.aval.allocated, sizeof (nrintobj_t *));
  ...
  value = json_skip (parse_value (child, json_skip (value))); /* json_skip any spacing, get the value. */
  if (!value) {
...
static const char *
parse_object (nrintobj_t *item, const char *value)
{
  nrintobj_t *child;
  char *key;
  ...
  item->u.hval.data = (nrintobj_t **)nr_calloc (item->u.aval.allocated, sizeof (nrintobj_t *));
  ...
  value = json_skip (parse_value (child, json_skip (value + 1))); /* Skip any spacing, get the value. */
  if (!value) {
    nr_free (key);
    nro_internal_delete (item, 0);
    nro_internal_delete (child, 1);
  ...
  while (*value == ',') {
    ...
    value = json_skip (parse_value (child, json_skip (value + 1))); /* json_skip any spacing, get the value.    */
    if (!value) {
      nr_free (key);
      nro_internal_delete (item, 0);
      nro_internal_delete (child, 1);
...
int
nro_find_array_int (const nrobj_t *array, int x)
{
  int i;
...
static void
nro_dump_internal (const nrobj_t *obj, int level, char *retstr)
{
  const nrintobj_t *op = (const nrintobj_t *)obj;
  int i;
