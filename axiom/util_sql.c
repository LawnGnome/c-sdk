...
/*
 * (I suppose we could use PCRE if we wanted to link in that library.)
 */
char *
nr_sql_obfuscate (const char *raw)
{
  char *obf;
  const char *p;
  char *q;
...
char *
nr_sql_normalize (const char *obfuscated_sql)
{
  char *normalized;
  int state;
  const char *p;
      ...
      case 3:   /* Seen '[Ii][Nn][::iswhite:](' */
        if (('?' == *p) || (',' == *p) || nr_isspace (*p)) {
          ...
          *q = ')';
          ...
          state = 4;    /* Covers case where there is something other than ?,?,? in the IN clause */
          *q = *p;
          q++;
        }
        p++;
...
uint32_t
nr_sql_normalized_id (const char *obfuscated_sql)
{
  uint32_t ret;
  char *normalized;
 ...
 * what the general structure of the SQL statement is after the SQL operator keyword.
 */
...
static const char *
nr_sql_parse_type_string (nr_sql_parse_type_t from_into_none)
{
 ...
 */
const char *
nr_sql_whitespace_comment_prefix (const char *sql, int show_sql_parsing)
{
  int x;
  const char *s = sql;
...
static char *
nr_sql_parse_over (const char *str, int character, int show_sql_parsing)
{
...
void
nr_sql_get_operation_and_table (const char *sql, const char **operation_ptr, char **table_ptr, int show_sql_parsing)
{
  int i;
  const char *start = 0;
  const char *end = 0;
  ...
  static sql_parse_expectation_t operations[] = {
     ...
     * alter, drop, ...  as per the Java and dot-NET agents. We could also
     * expand on the show to parse the table name from "show columns in",
     * etc.
     */ 
  ...
  if ('(' == *x) {
    ...
    if (('`' == *x) || ('\'' == *x) || ('"' == *x)) {
      ...
      if ((x[sl] == ')') || (x[sl] == ',')) {
        ...
        /* this is a subquery */
        if (show_sql_parsing) {
          nrl_verbosedebug (NRL_SQL, "SQL parser: returning success: " NRP_FMT, NRP_SQL (subquery));
        }
  ...
  *table_ptr = nr_strndup (start, (int)(end - start));
  ...
  if (show_sql_parsing) {
    nrl_verbosedebug (NRL_SQL, "SQL parser: returning success: " NRP_FMT, 100, *table_ptr);
  }
}
