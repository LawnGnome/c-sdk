...
static zval *
nr_guzzle6_requesthandler_get_request (zval *obj TSRMLS_DC)
{
  zval *prop;
...
/*
 * Proto   : void RequestHandler::__construct(Psr\Http\Message\RequestInterface $request)
 */
static
PHP_NAMED_FUNCTION (nr_guzzle6_requesthandler_construct)
{
  zval *request = NULL;
  ...
  if (FAILURE == zend_parse_parameters_ex (ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS () TSRMLS_CC, "o", &request)) {
...
/*
 * Proto   : void RequestHandler::onFulfilled(Psr\Http\Message\ResponseInterface $response)
 *
 ...
 */
static
PHP_NAMED_FUNCTION (nr_guzzle6_requesthandler_onfulfilled)
{
  zval *response = NULL;
  ...
  if (FAILURE == zend_parse_parameters_ex (ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS () TSRMLS_CC, "o", &response)) {
    nrl_verbosedebug (NRL_FRAMEWORK, "%s: did not get response", __func__);
    return;
  }

  nr_guzzle6_requesthandler_handle_response (NR_PHP_USER_FN_THIS, response TSRMLS_CC);
}

/*
 * Proto   : void RequestHandler::onRejected(GuzzleHttp\Exception\TransferException $e)
 *
 ...
 */
static
PHP_NAMED_FUNCTION (nr_guzzle6_requesthandler_onrejected)
{
  zval *exc = NULL;
  zval *response = NULL;
  ...
  if (FAILURE == zend_parse_parameters_ex (ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS () TSRMLS_CC, "o", &exc)) {
   ...
   */
  if (!nr_php_object_instanceof_class (exc, "GuzzleHttp\\Exception\\BadResponseException" TSRMLS_CC)) {
    return;
  }
...
const zend_function_entry nr_guzzle6_requesthandler_functions[] = {
  ZEND_FENTRY (__construct, nr_guzzle6_requesthandler_construct,   nr_guzzle6_requesthandler_construct_arginfo,   ZEND_ACC_PUBLIC)
  ZEND_FENTRY (onFulfilled, nr_guzzle6_requesthandler_onfulfilled, nr_guzzle6_requesthandler_onfulfilled_arginfo, ZEND_ACC_PUBLIC)
  ZEND_FENTRY (onRejected,  nr_guzzle6_requesthandler_onrejected,  nr_guzzle6_requesthandler_onrejected_arginfo,  ZEND_ACC_PUBLIC)
  PHP_FE_END
};
...
NR_PHP_WRAPPER_START (nr_guzzle6_client_construct)
{
  ...
  if (!nr_php_is_zval_valid_callable (middleware TSRMLS_CC)) {
    ...
    nrm_force_add (NRTXN (unscoped_metrics),
                   "Supportability/library/Guzzle 6/MiddlewareNotCallable",
                   0);
  ...
  config = nr_php_get_zval_object_property_with_class (this_var,
                                                       guzzle_client_ce,
                                                       "config" TSRMLS_CC);
  ...
  if (!nr_php_object_instanceof_class (handler_stack, "GuzzleHttp\\HandlerStack" TSRMLS_CC)) {
    goto end;
  }
...
nr_guzzle6_enable (TSRMLS_D)
{
  ...
  retval = zend_eval_string (
    ...
    /*
     * Set up the RequestHandler object and attach it to the promise so that we
     * create an external node and deal with the CAT headers coming back from
     * the far end.
     */
...
nr_guzzle6_minit (TSRMLS_D)
{
  ...
  INIT_CLASS_ENTRY (ce, "newrelic\\Guzzle6\\RequestHandler", nr_guzzle6_requesthandler_functions);
  nr_guzzle6_requesthandler_ce = nr_php_zend_register_internal_class_ex (&ce, NULL TSRMLS_CC);
