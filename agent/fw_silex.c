...
NR_PHP_WRAPPER (nr_silex_name_the_wt)
{
  ...
  if (!nr_php_object_instanceof_class (request, "Symfony\\Component\\HttpFoundation\\Request" TSRMLS_CC)) {
    nrl_verbosedebug (NRL_INSTRUMENT, "%s: first parameter isn't a Request object", __func__);
    NR_PHP_WRAPPER_CALL;
    goto end;
  }
  ...
  if (!nr_php_object_instanceof_class (attributes, "Symfony\\Component\\HttpFoundation\\ParameterBag" TSRMLS_CC)) {
    nrl_verbosedebug (NRL_INSTRUMENT, "%s: Request::$attributes isn't a ParameterBag object", __func__);
    goto end;
  }
  ...
  if (!nr_php_is_zval_non_empty_string (route)) {
    nrl_verbosedebug (NRL_INSTRUMENT, "%s: _route is not a valid string", __func__);
    goto end;
  }
...
void
nr_silex_enable (TSRMLS_D)
{
  NR_UNUSED_TSRMLS;

  nr_php_wrap_user_function (NR_PSTR ("Symfony\\Component\\HttpKernel\\HttpKernel::handleRaw"),
