...
static zval *
nr_php_get_zval_object_property_with_class_internal (zval *object,
                                                     zend_class_entry *ce,
                                                     const char *cname
                                                     TSRMLS_DC)
{
...
#else
  /*
   * This mechanism uses an interface to zend that is implemented in both zend
   * and hhvm. Empirically, in zend at least, attempts to read uninitialized (or
   * non existing) properties always return uninitialized_zval_ptr, even in the
   * case where we read a property during pre-hook time on a constructor.
   */
...
zval *
nr_php_get_zval_object_property (zval *object, const char *cname TSRMLS_DC)
{
  char *name;
  ...
  if (nr_php_is_zval_valid_object (object)) {
    return nr_php_get_zval_object_property_with_class_internal (object,
                                                                Z_OBJCE_P (object),
                                                                name TSRMLS_CC);
...
zval *
nr_php_get_zval_object_property_with_class (zval *object,
                                            zend_class_entry *ce,
                                            const char *cname TSRMLS_DC)
{
  if ((!nr_php_is_zval_valid_object (object)) || (NULL == ce) ||
      (NULL == cname) || ('\0' == cname[0])) {
    return NULL;
  }
...
int
nr_php_object_has_method (zval *object, const char *lcname TSRMLS_DC)
{
  zend_class_entry *ce;
  int namelen;
  char *vname;
  ...
  if (nr_php_zend_hash_exists (&ce->function_table, vname)) {
    ...
    if (NULL == Z_OBJ_HT_P (object)->get_method) {  /* nr_php_object_has_method */
      ...
      func = (void *) Z_OBJ_HT_P (object)->get_method (&Z_OBJ_P (object), name_str, NULL TSRMLS_CC);

      zend_string_release (name_str);
#elif ZEND_MODULE_API_NO >= ZEND_5_4_X_API_NO
      /*
       * Fun side note: this can leak if the object has a __call() method, as in
       * that situation only, zend_std_get_method() will indirectly allocate a
       * new zend_function in zend_get_user_call_function().
       *
       ...
       * in Valgrind, though.
       */
      func = (void *)Z_OBJ_HT_P (object)->get_method (&object, vname, namelen, NULL TSRMLS_CC);  /* nr_php_object_has_method */
#else /* PHP < 5.4 */
      func = (void *)Z_OBJ_HT_P (object)->get_method (&object, vname, namelen TSRMLS_CC);  /* nr_php_object_has_method */
#endif
...
int
nr_php_object_has_concrete_method (zval *object, const char *lcname TSRMLS_DC)
{
...
zend_function *
nr_php_find_function (const char *name TSRMLS_DC)
{
  if (NULL == name) {
    return NULL;
  }
 ...
 */
zend_class_entry *
nr_php_find_class (const char *name TSRMLS_DC)
{
#ifdef PHP7
  if (NULL == name) {
    return NULL;
  ...
  ce_ptr = (zend_class_entry **) nr_php_zend_hash_find_ptr (EG (class_table),
                                                            name);
...
zend_function *
nr_php_find_class_method (const zend_class_entry *klass, const char *name)
{
  if (0 == klass) {
    return 0;
  }
...
int
nr_php_class_entry_instanceof_class (const zend_class_entry *ce, const char *class_name TSRMLS_DC)
{
  int found = 0;

  if (NULL != ce) {
    char *class_name_lower = nr_string_to_lowercase (class_name);
    zend_class_entry *class_name_ce = nr_php_find_class (class_name_lower TSRMLS_CC);
...
int
nr_php_object_instanceof_class (const zval *object, const char *class_name TSRMLS_DC)
{
  if (!nr_php_is_zval_valid_object (object)) {
    return 0;
  }
...
zend_function *
nr_php_zval_to_function (zval *zv TSRMLS_DC)
{
 ...
 */
static zend_execute_data *
nr_get_zend_execute_data (NR_EXECUTE_PROTO TSRMLS_DC)
{
  zend_execute_data *ptrg = EG (current_execute_data);  /* via zend engine global data structure */

  NR_UNUSED_SPECIALFN;
#if ZEND_MODULE_API_NO >= ZEND_5_5_X_API_NO
  {
    /*
     * ptra is argument passed in to us:
     * watch out! may be 0 if caller doesn't have that info
     *
     * TODO(rrh): consider adding a supportability metric to keep
     * track of when our assumptions about the zend engine data structures
     * aren't being met.
     */
 ...
 */
#if ZEND_MODULE_API_NO >= ZEND_5_5_X_API_NO && !defined(PHP7) /* PHP 5.5 and 5.6 */
static void **
nr_php_get_php55_stack_arguments (int legitimate_frame_delta, NR_EXECUTE_PROTO TSRMLS_DC)
{
  ...
  arguments = ex->function_state.arguments;
  if (0 == arguments) {
    return 0;  /* PHP stack appears to be be malformed */
  }

  for (i = 0; i < legitimate_frame_delta; i++) {
    ex = ex->prev_execute_data;
    if (0 == ex) {
      return 0; /* No caller; we're at the bottom of the stack */
    }
    arguments = ex->function_state.arguments;
    if (0 == arguments) {
      return 0;  /* PHP stack appears to be be malformed */
 ...
 */
static zval *
nr_php_get_user_func_arg_via_h (int requested_arg_index, int *arg_count_p, NR_EXECUTE_PROTO TSRMLS_DC)
{
  void **p = 0;
  zval **argp = 0;
  zval *arg = 0;
  ...
  *arg_count_p = -1;
  if (0 == nr_get_zend_execute_data (NR_EXECUTE_ORIG_ARGS TSRMLS_CC)) {
    return 0;
  }
...
#else
  p = nr_get_zend_execute_data (NR_EXECUTE_ORIG_ARGS TSRMLS_CC)->function_state.arguments;
  if (0 == p) {
    return 0;
  }
  *arg_count_p = (int)(zend_uintptr_t)*p;
...
/*
 * WATCH OUT! requested_arg_index is a 1-based value, not a 0-based value!
 */
zval *
nr_php_get_user_func_arg (size_t requested_arg_index, NR_EXECUTE_PROTO TSRMLS_DC)
{
  zval *arg_via_h = 0;
...
#else
  arg_via_h = nr_php_get_user_func_arg_via_h (requested_arg_index, &arg_count_via_h, NR_EXECUTE_ORIG_ARGS TSRMLS_CC);
#endif /* PHP7 */
...
size_t
nr_php_get_user_func_arg_count (NR_EXECUTE_PROTO TSRMLS_DC)
{
#ifdef PHP7
  ...
  if (NULL == nr_php_get_user_func_arg_via_h (1, &arg_count_via_h,
                                              NR_EXECUTE_ORIG_ARGS TSRMLS_CC)) {
    return 0;
  } else if (arg_count_via_h < 0) {
    nrl_verbosedebug (NRL_AGENT, "%s: unexpected argument count %d",
                      __func__, arg_count_via_h);
    return 0;
  }
...
zend_execute_data *
nr_php_get_caller_execute_data (NR_EXECUTE_PROTO, ssize_t offset TSRMLS_DC)
{
...
const zend_function *
nr_php_get_caller (NR_EXECUTE_PROTO, ssize_t offset TSRMLS_DC)
{
  const zend_execute_data *ped = nr_php_get_caller_execute_data (NR_EXECUTE_ORIG_ARGS, offset TSRMLS_CC);
...
zval *
nr_php_get_active_php_variable (const char *name TSRMLS_DC)
{
  HashTable *table;
...
int
nr_php_silence_errors (TSRMLS_D)
{
...
void
nr_php_restore_errors (int error_reporting TSRMLS_DC)
{
  EG (error_reporting) = error_reporting;
}

zval *
nr_php_get_constant (const char *name TSRMLS_DC)
{
#ifdef PHP7
  zval *constant;
  zval *copy = NULL;
...
/*
 * TODO(msl): Determine whether zend_update_class_constants should be called
 * prior to performing any lookups. The reflection and spl extensions appear
 * to do so, but it's unclear why.
 */
zval *
nr_php_get_class_constant (const zend_class_entry *ce, const char *name)
{
...
int
nr_php_is_zval_named_constant  (const zval *zv, const char *name TSRMLS_DC)
{
  int is_equal = 0;
  zval *constant;
  ...
  if ((IS_LONG == Z_TYPE_P (constant)) &&
      (Z_LVAL_P (zv) == Z_LVAL_P (constant))) {
    is_equal = 1;
  }
...
int
nr_php_zend_is_auto_global (const char *name, size_t name_len TSRMLS_DC)
{
#ifdef PHP7
  zend_bool rv;
...
const char *
nr_php_use_license (const char *api_license TSRMLS_DC)
{
  const char *lic_to_use = api_license;

  if ((0 == lic_to_use) || (0 == lic_to_use[0])) {
...
char *
nr_php_get_server_global (const char *name TSRMLS_DC)
{
  zval *data = 0;
  zval *global = 0;
  ...
  if (0 == nr_php_is_zval_valid_array (global)) {
    return 0;
  }

  if (0 == Z_ARRVAL_P (global)) {
    return 0;
  }

  data = nr_php_zend_hash_find (Z_ARRVAL_P (global), name);

  if (0 == nr_php_is_zval_non_empty_string (data)) {
    return 0;
  }
...
int
nr_php_extension_loaded (const char *name)
{
  int found = 0;
  char *lcname;
 ...
 */
static char *
nr_php_callable_array_to_string (zval *callable TSRMLS_DC)
{
  zval *function;
  char *name = NULL;
  ...
  if (2 != num_elements) {
    nrl_verbosedebug (NRL_TXN,
                      "nr_php_callable_array_to_string: unexpected number of elements in callable array: got %zu, expected 2",
                      num_elements);
    return NULL;
  }
  ...
  if (NULL == scope) {
     ...
     * hash table.
     */
    nrl_warning (NRL_TXN, "nr_php_callable_array_to_string: finding element 0 of a callable array with 2 elements failed");
    return NULL;
  }
  ...
  if (NULL == function) {
     ...
     * hash table.
     */
    nrl_warning (NRL_TXN, "nr_php_callable_array_to_string: finding element 1 of a callable array with 2 elements failed");
    return NULL;
  }

  if (0 == nr_php_is_zval_valid_string (function)) {
    nrl_verbosedebug (NRL_TXN,
                      "%s: unexpected type for function: got %d",
  ...
  if (nr_php_is_zval_valid_string (scope)) {
    ...
    if (NULL != ce) {
      ...
      nrl_warning (NRL_TXN, "nr_php_callable_array_to_string: object does not have a class entry");
    }
...
char *
nr_php_callable_to_string (zval *callable TSRMLS_DC)
{
  char *name = NULL;

  if (NULL == callable) {
    nrl_verbosedebug (NRL_TXN, "nr_php_callable_to_string: cannot create name from NULL callable");
    return NULL;
  }
  ...
  if (nr_php_is_zval_valid_string (callable)) {
    ...
    if (NULL != ce) {
      ...
      nrl_warning (NRL_TXN, "nr_php_callable_to_string: object does not have a class entry");
...
static int
nr_php_filter_class_methods (zend_function *func,
...
void
nr_php_remove_interface_from_class (zend_class_entry *class_ce,
                                    const zend_class_entry *iface_ce
                                    TSRMLS_DC)
{
  zend_uint i;
  ...
  for (i = 0; i < class_ce->num_interfaces; i++) {
    ...
    if (class_iface == iface_ce) {
      if (1 == class_ce->num_interfaces) {
         ...
         */
        class_ce->interfaces[i] = class_ce->interfaces[--class_ce->num_interfaces];
      }
    }
  }
...
nr_status_t
nr_php_swap_user_functions (zend_function *a, zend_function *b)
{
  zend_op_array temp;
...
char *
nr_php_class_name_from_full_name (const char *full_name)
{
  int i;
...
char *
nr_php_function_name_from_full_name (const char *full_name)
{
  int i;
...
char *
nr_php_function_debug_name (const zend_function *func)
{
  char *name = NULL;
  ...
  name = nr_formatf ("%s%s%s",
                     (scope ? NRSAFESTR (nr_php_class_entry_name (scope))
                            : ""),
                     (scope ? "::" : ""),
                     NRSAFESTR (nr_php_function_name (func)));
  ...
  if ((ZEND_USER_FUNCTION == func->type) &&
    ...
    name = nr_formatf ("%s declared at %s:%d",
                       orig_name,
                       NRSAFESTR (filename),
...
const char *
nr_php_function_filename (zend_function *func)
{
  /*
...
zval *
nr_php_json_decode (zval *json TSRMLS_DC)
{
  zval *decoded = NULL;
...
zval *
nr_php_json_encode (zval *zv TSRMLS_DC)
{
  zval *json = NULL;
...
zval *
nr_php_parse_str (const char *str, size_t len TSRMLS_DC)
{
  zval *arr;
  char *mutable;
...
bool
nr_php_function_is_static_method (const zend_function *func)
{
  if (NULL == func) {
    return false;
  }
