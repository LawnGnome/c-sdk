...
static char *
nr_url_clean_internal (const char *url, int urllen, int mask_creds)
{
   ...
   * iteration can stop.
   */
  len = nr_strncspn (url, urllen, NR_URL_QUERY_CHARS "\0", sizeof (NR_URL_QUERY_CHARS));
  if (len <= 0) {
    return 0;
  }
...
char *
nr_url_clean (const char *url, int urllen)
{
  return nr_url_clean_internal (url, urllen, 0);
}

char *
nr_url_proxy_clean (const char *url)
{
  return nr_url_clean_internal (url, nr_strlen (url), 1);
}

const char *
nr_url_extract_domain (const char *url, int urllen, int *dnlen)
{
  int i;
  ...
  *dnlen = -1;
  ...
  for (i = 0; i < urllen; i++) {
    ...
    } else if ((0 == url[i]) || (0 != nr_strchr ("/" NR_URL_QUERY_CHARS, url[i]))) {
      break;
    }
  }
