...
static void
cufa_pre (zend_function *func, const zend_function *caller TSRMLS_DC)
{
...
static void
define_cufa_function_f (TSRMLS_D)
{
  tlib_php_request_eval ("function f() { return 42; }" TSRMLS_CC);
}

static void
cufa_assert_f_called_by_g (zend_function *func, const zend_function *caller TSRMLS_DC)
{
  char *message;
...
test_cufa_direct (TSRMLS_D)
{
  ...
  tlib_php_request_eval ("function g() { return call_user_func_array('f', array()); }" TSRMLS_CC);
...
test_cufa_indirect (TSRMLS_D)
{
  ...
  define_cufa_function_f (TSRMLS_C);
  tlib_php_request_eval ("function g() { $cufa = 'call_user_func_array'; return $cufa('f', array()); }" TSRMLS_CC);
