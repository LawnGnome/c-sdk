...
# We also need to manipulate $(NEWRELIC_SHARED_LIBADD) when this is an nrlamp
...
#
# TODO(aharvey): When we eventually get rid of nrlamp, get rid of this horrific
# hack.
...
ifeq (/opt/nr/lamp/lib,$(findstring /opt/nr/lamp/lib,$(PHP_EMBED_LIBRARY)))
	...
	# upstream (https://github.com/libevent/libevent/issues/376), but until this
	# gets pulled in by whatever in nrcamp/nrlamp is pulling in libevent in the
	# first place (since it's not an explicit thing we're building), we'll set an
	# environment variable to hush it on all versions of OS X/macOS (at least
...
# Yet another nrlamp hack: ZTS builds need -pthread specified again at the end
# of the build line, so let's abuse TEST_NEWRELIC_SHARED_LIBADD to do that.
ifeq (/opt/nr/lamp/lib,$(findstring /opt/nr/lamp/lib,$(PHP_EMBED_LIBRARY)))
