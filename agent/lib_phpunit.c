 ...
 * data for a field, we send an empty string or 0 and log a message.
 *
...
static const size_t num_statuses = sizeof (nr_phpunit_test_statuses) / sizeof (nr_phpunit_test_status_t);
...
static char *
nr_phpunit_get_suite_name (zval *result TSRMLS_DC)
{
  zval *suite = NULL;
  char *name = NULL;
  zval *name_zv = NULL;
  ...
  if (!nr_php_object_instanceof_class (suite, "PHPUnit_Framework_TestSuite" TSRMLS_CC)) {
    nrl_verbosedebug (NRL_INSTRUMENT, "%s: unable to obtain test suite", __func__);
    goto leave;
  }
 ...
 */
static const char *
nr_phpunit_get_unique_identifier (TSRMLS_D)
{
  if (NULL == NRPRG (txn)) {
    return NULL;
  }
...
static int
nr_phpunit_was_test_successful (zval *result TSRMLS_DC)
{
  ...
  if (!nr_php_is_zval_valid_bool (successful_zv)) {
    nrl_verbosedebug (NRL_INSTRUMENT,
      "%s: unable to determine whether suite was successful", __func__);
    goto leave;
  }
...
static zend_long
nr_phpunit_get_count (zval *result, const char *method_name TSRMLS_DC)
{
  zval *value_zv = NULL;
  zend_long count = 0;
  ...
  if (0 == nr_php_is_zval_valid_integer (value_zv)) {
    nrl_verbosedebug (NRL_INSTRUMENT, "%s: method call to \"%s\" did not return a long",
...
static size_t
nr_phpunit_get_passed_count (zval *result TSRMLS_DC)
{
  zval *passed_zv = NULL;
  size_t passed = 0;
  ...
  if (!nr_php_is_zval_valid_array (passed_zv)) {
    nrl_verbosedebug (NRL_INSTRUMENT, "%s: unable to obtain number of passed tests", __func__);
    goto leave;
  }
...
static zend_long
nr_phpunit_get_num_assertions (zval *printer TSRMLS_DC)
{
  zval *assertions_zv = NULL;

  assertions_zv = nr_php_get_zval_object_property (printer, "numAssertions" TSRMLS_CC);
  if (!nr_php_is_zval_valid_integer (assertions_zv)) {
    nrl_verbosedebug (NRL_INSTRUMENT, "%s: unable to obtain number of test assertions", __func__);
    return 0;
  }
...
static double
nr_phpunit_get_duration (zval *result TSRMLS_DC)
{
  ...
  if (!nr_php_is_zval_valid_double (duration_zv)) {
    nrl_verbosedebug (NRL_INSTRUMENT, "%s: unable to obtain test duration", __func__);
    return 0;
  }
...
NR_PHP_WRAPPER (nr_phpunit_instrument_resultprinter_printresult)
{
  ...
  if (!nr_php_object_instanceof_class (result, "PHPUnit_Framework_TestResult" TSRMLS_CC)) {
    nrl_verbosedebug (NRL_INSTRUMENT, "%s: unable to obtain test result", __func__);
    NR_PHP_WRAPPER_CALL;
    goto end;
  }
  ...
  nro_set_hash_string (event, "runId", nr_phpunit_get_unique_identifier (TSRMLS_C));
  nro_set_hash_boolean (event, "successful", nr_phpunit_was_test_successful (result TSRMLS_CC));
  nro_set_hash_long (event, "testCount", nr_phpunit_get_count (result, "count" TSRMLS_CC));
  nro_set_hash_int (event, "passedCount", nr_phpunit_get_passed_count (result TSRMLS_CC));
  nro_set_hash_long (event, "failedCount", nr_phpunit_get_count (result, "failureCount" TSRMLS_CC));
  nro_set_hash_long (event, "skippedCount", nr_phpunit_get_count (result, "skippedCount" TSRMLS_CC));
  nro_set_hash_long (event, "errorCount", nr_phpunit_get_count (result, "errorCount" TSRMLS_CC));
  nro_set_hash_long (event, "riskyCount", nr_phpunit_get_count (result, "riskyCount" TSRMLS_CC));
  nro_set_hash_long (event, "incompleteCount", nr_phpunit_get_count (result, "notImplementedCount" TSRMLS_CC));
  nro_set_hash_long (event, "warningCount", nr_phpunit_get_count (result, "warningCount" TSRMLS_CC));
  nro_set_hash_long (event, "assertionCount", nr_phpunit_get_num_assertions (this_var TSRMLS_CC));
  nro_set_hash_double (event, "duration", nr_phpunit_get_duration (result TSRMLS_CC));
...
static int
nr_phpunit_did_last_test_fail (zval *result TSRMLS_DC)
{
  zval *last_test_failed = NULL;

  last_test_failed = nr_php_get_zval_object_property (result, "lastTestFailed" TSRMLS_CC);
  if (!nr_php_is_zval_valid_bool (last_test_failed)) {
    nrl_verbosedebug (NRL_INSTRUMENT, "%s: unable to determine whether last test failed", __func__);
    return 0;
  }
...
static const char *
nr_phpunit_determine_test_outcome (zval *this_var, zval *test_case TSRMLS_DC)
{
  ...
  if (!nr_php_is_zval_valid_integer (outcome_zv)) {
    nrl_verbosedebug (NRL_INSTRUMENT, "%s: unable to obtain test outcome", __func__);
    goto leave;
  }
  ...
  if ((index >= 0) && ((size_t)index <= num_statuses - 1)) {
      ...
      nrl_verbosedebug (NRL_INSTRUMENT, "%s: unknown test status: %li", __func__, (long)index);
      goto leave;
  }
   ...
   * TODO(earnold): Make this less brittle.
   */
...
static void
nr_phpunit_send_test_event (const nr_phpunit_test_event_fields_t *fields TSRMLS_DC)
{
  nrobj_t *event;
 ...
 */
static char *
nr_phpunit_get_message_for_test (zval *result, const char *test_type TSRMLS_DC)
{
  zval *tests = NULL;
  size_t num_tests = 0;
  ...
  if (!nr_php_object_instanceof_class (failure_zv, "PHPUnit_Framework_TestFailure" TSRMLS_CC)) {
    goto leave;
  }
...
NR_PHP_WRAPPER (nr_phpunit_instrument_testresult_endtest)
{
  ...
  if (!nr_php_object_instanceof_class (test_case, "PHPUnit_Framework_TestCase" TSRMLS_CC)) {
    nrl_verbosedebug (NRL_INSTRUMENT, "%s: unable to obtain test case", __func__);
      NR_PHP_WRAPPER_CALL;
      goto end;
  }
  ...
  fields.num_assertions = nr_phpunit_get_count (test_case, "getNumAssertions" TSRMLS_CC);
...
NR_PHP_WRAPPER (nr_phpunit_instrument_testresult_adderror)
{
  ...
  if (!nr_php_object_instanceof_class (exception, "PHPUnit_Framework_SkippedTest" TSRMLS_CC)) {
    NR_PHP_WRAPPER_CALL;
    goto end;
  }
 ...
 */
static int
nr_phpunit_are_statuses_valid (TSRMLS_D)
{
  zend_class_entry *class_entry = nr_php_find_class ("phpunit_runner_basetestrunner" TSRMLS_CC);
  if (0 == class_entry) {
    nrl_verbosedebug (NRL_INSTRUMENT, "could not find PHPUnit_Runner_BaseTestRunner");
    return 0;
  }
  ...
  for (size_t i = 0; i < 5; i++) {
    ...
    if ((!nr_php_is_zval_valid_integer (constant)) || (Z_LVAL_P (constant) != (int)i)) {
      nrl_verbosedebug (NRL_INSTRUMENT,
        "%s: %s constant has an unexpected value", __func__, status);
...
nr_phpunit_enable (TSRMLS_D)
{
  ...
  nr_php_wrap_user_function (NR_PSTR ("PHPUnit_TextUI_ResultPrinter::printResult"),
  ...
  nr_php_wrap_user_function (NR_PSTR ("PHPUnit_Framework_TestResult::addError"),
