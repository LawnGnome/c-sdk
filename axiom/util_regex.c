 ...
 */
static inline void
nr_regex_handle_option (int *pcre_option_set, int nr_option_set,
                        int pcre_option, int nr_option)
{
...
nr_regex_t *
nr_regex_create (const char *pattern, int options, int do_study)
{
  const char *err = NULL;
  int erroffset = 0;
  int pcre_options = 0;
   ...
   */
  nr_regex_handle_option (&pcre_options, options,
                          PCRE_ANCHORED,       NR_REGEX_ANCHORED);
  nr_regex_handle_option (&pcre_options, options,
                          PCRE_CASELESS,       NR_REGEX_CASELESS);
  nr_regex_handle_option (&pcre_options, options,
                          PCRE_DOLLAR_ENDONLY, NR_REGEX_DOLLAR_ENDONLY);
  nr_regex_handle_option (&pcre_options, options,
                          PCRE_DOTALL,         NR_REGEX_DOTALL);
  nr_regex_handle_option (&pcre_options, options,
                          PCRE_MULTILINE,      NR_REGEX_MULTILINE);
  ...
  if (NULL == regex->code) {
    nrl_verbosedebug (NRL_MISC,
                      "%s: regex compilation error %s at offset %d",
...
void
nr_regex_destroy (nr_regex_t **regex_ptr)
{
  nr_regex_t *regex;
  ...
  /*
   * The below block is in place for our FreeBSD 7 x64 build slave, which has a
   * relatively ancient version of PCRE installed.
   *
   * TODO(aharvey): fix the nrcamp deployed on freebsd7-64-nrcamp to include
   * the same PCRE version as the other build slaves.
   */
...
nr_status_t
nr_regex_match (const nr_regex_t *regex, const char *str, int str_len)
{
  int rc;
...
nr_regex_substrings_t *
nr_regex_match_capture (const nr_regex_t *regex, const char *str, int str_len)
{
  int rc;
  ...
  ss = nr_regex_substrings_create (regex->capture_count);
  if (NULL == ss) {
    return NULL;
  }

  rc = pcre_exec (regex->code, regex->extra, str, str_len, 0, 0,
                  ss->ovector, ss->ovector_size);
  if (rc < 0) {
    ...
    nrl_verbosedebug (NRL_MISC,
                      "%s: pcre_exec returned 0 (too many matches); expected >0 or PCRE_ERROR_NOMATCH",
                      __func__);
...
int
nr_regex_capture_count (const nr_regex_t *regex)
{
  int count;
  int retval;
  ...
  if (0 != retval) {
    nrl_verbosedebug (NRL_MISC,
                      "%s: pcre_fullinfo returned %d; expected 0",
                      __func__, retval);
    return -1;
  }
...
nr_regex_substrings_t *
nr_regex_substrings_create (int count)
{
  nr_regex_substrings_t *ss;
...
void
nr_regex_substrings_destroy (nr_regex_substrings_t **ss_ptr)
{
  nr_regex_substrings_t *ss;
...
int
nr_regex_substrings_count (const nr_regex_substrings_t *ss)
{
  if (NULL == ss) {
    return -1;
  }
...
char *
nr_regex_substrings_get (const nr_regex_substrings_t *ss, int index)
{
  int offsets[2];
...
nr_status_t
nr_regex_substrings_get_offsets (const nr_regex_substrings_t *ss, int index,
                                 int offsets[2])
{
...
const char *
nr_regex_pcre_version (void)
{
  return pcre_version ();
}

char *
nr_regex_quote (const char *str, size_t str_len, size_t *quoted_len)
{
...
void
nr_regex_add_quoted_to_buffer (nrbuf_t *buf, const char *str, size_t str_len)
{
  size_t i;
