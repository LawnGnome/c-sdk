...
func listenAndServe(address string, errorChan chan<- error, p *newrelic.Processor) <-chan struct{} {
	...
	go func() {
		...
		if err != nil {
			...
			// creating the listener socket. See PHP-1103.
...
//
// TODO(msl): This would be better if we had a custom type so parsing
// and validation could occur when during argument and configuration
// parsing.
func parseBindAddr(s string) (address net.Addr, err error) {
	// TODO(msl): Use the correct maximum for each platform. Be certain
	// to account for the NUL terminator.
