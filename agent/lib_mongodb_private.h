...
/*
 * Purpose : Retrieve the host name or port from a MongoDB\Driver\Server object.
 *
 * Params  : 1. The server object.
 *
 * Returns : A newly allocated string, which is owned by the caller, or NULL if an
 *           error occurred.
 */
 ...
 */
extern void
nr_mongodb_get_host_and_port_path_or_id (zval *server, char **host, char **port TSRMLS_DC);
