...
void
nr_php_httprequest_send_request_headers (zval *this_var TSRMLS_DC)
{
  ...
  if (NRPRG (txn) && NRTXN (special_flags.debug_cat)) {
    nrl_verbosedebug (NRL_CAT,
      "CAT: outbound request: transport='pecl_http 1' %s=" NRP_FMT " %s=" NRP_FMT,
...
char *
nr_php_httprequest_send_response_header (zval *this_var TSRMLS_DC)
{
  zval *retval = 0;
  zval *header_name = 0;
...
char *
nr_php_httprequest_send_get_url (zval *this_var TSRMLS_DC)
{
  char *url = 0;
  zval *urlz = 0;
