...
static void *
test_threads_thread1 (void *vp)
{
  test_threads_state_t *p = (test_threads_state_t *)vp;

  (void) p;
  nr_msleep (SLEEP_SCALE * 10); /* sleep for less than thread2 sleeps to avoid output ordering issues. */
...
/*
 * Test 4: lock and force deadlock and unlock mutex.
 */
static void
test_threads_test4 (test_threads_state_t *p)
{
  nr_status_t rv;
  ...
  rv = nrt_mutex_lock (&p->mutex);
  tlib_pass_if_true ("dynamic relock fails with deadlock", NR_FAILURE == rv, "rv=%d", (int)rv);

  rv = nrt_mutex_unlock (&p->mutex);
  tlib_pass_if_true ("dynamic unlock ok", NR_SUCCESS == rv, "rv=%d", (int)rv);

  rv = nrt_mutex_lock (&p->mutex);
  tlib_pass_if_true ("relock after failed unlock ok", NR_SUCCESS == rv, "rv=%d", (int)rv);
  rv = nrt_mutex_unlock (&p->mutex);
  tlib_pass_if_true ("unlock after relock ok", NR_SUCCESS == rv, "rv=%d", (int)rv);
}

static void
test_static_mutex (test_threads_state_t *p)
{
  nr_status_t rv;
  ...
  rv = nrt_mutex_lock (&p->static_mutex);
  tlib_pass_if_true ("mutex acquired after release", NR_SUCCESS == rv, "rv=%d", (int)rv);
  rv = nrt_mutex_unlock (&p->static_mutex);
  tlib_pass_if_true ("mutex release after relock ok", NR_SUCCESS == rv, "rv=%d", (int)rv);
}

/*
 * The test itself is crafted to test parallelism.
 *
 * Running the test multiple times in parallel does not
 * yet (08Jan2015) work.
 */
tlib_parallel_info_t parallel_info = {.suggested_nthreads = -1, .state_size = sizeof (test_threads_state_t)};
...
test_main (void *vp)
{
  ...
  /* snprintf (tmpfilename, sizeof (tmpfilename), "./threadslog.%05d.tmp", nr_gettid ()); */
  ...
  tlib_pass_if_true ("simple thread create OK", NR_SUCCESS == rv, "rv=%d", (int)rv);
  nrt_join (t1, 0);
}
