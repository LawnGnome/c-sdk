...
static nrl_subsys_names_t subsys_names[] = {
  { "autorum",                  NRL_AUTORUM },
  { "metrics",                  NRL_METRICS },
...
nr_status_t
nrl_set_log_file (const char *filename)
{
  if (-1 != logfile_fd) {
    return NR_FAILURE;
  }
...
void
nrl_format_timestamp (char *buf, size_t buflen, const struct timeval *tv)
{
  struct tm tm;
...
static nr_status_t
nrl_send_log_message_internal (int fd, nrloglev_t level, const char *fmt, va_list ap)
{
  char preamble[128];
  struct iovec miov[3];
  struct timeval tv;
  ...
  preamble_len = snprintf (preamble, sizeof (preamble), "%s (%d %d) %s: ",
    log_timestamp,
    nr_getpid (),
    nr_gettid (),
    level_names[level]);
...
nr_status_t
nrl_send_log_message (nrloglev_t level, const char *fmt, ...)
{
  nr_status_t rv;
  va_list ap;
...
static void
set_all_up_to (nrloglev_t level, uint32_t flags)
{
  int i;
...
nr_status_t
nrl_set_log_level (const char *level)
{
  nrobj_t *args = NULL;
  char *ep;
  int nargs = 0;
...
void
nrl_vlog (nrloglev_t level, uint32_t subsystem, const char *fmt, va_list ap)
{
