...
/*
 * This file deals with INI variables in all their glory.
 *
 ...
 *
 * + It acts as a cross bar switch, moving data from PHPs realm into
 * the agent's realm in various ways, although
 * not necessarily the other direction.
 *
 * + It implements visibility and modifiability semantics
 * for various ini settings, depending on the minit/rinit cycle.
 *
 * + It implements display semantics, as needed for php -i or phpinfo(),
 * generating straight text or html, as appropriate.
 *
 * + It papers over the complexity of what the underlying engine,
 * be it zend or hhvm, does under the covers.
 *
 * Note that as of Oct2014, there are bugs in the implementation
 * of HHVM regarding the value of ini settings upon
 * request initialization in server mode: the thread never sees
 * the ini settings already consumed.  See some of these bugs which may/may not
 * have been fixed:
 *  https://github.com/facebook/hhvm/issues/2964
 *  https://github.com/facebook/hhvm/issues/3654
 */

/*
 * The cross bar and visibility semantics are implemented by calls to PHP standard macros
 *      PHP_INI_ENTRY_EX has no mechanisms to route data structure pointers into modify handlers
 *  STD_PHP_INI_ENTRY_EX has    mechanisms to route data structure pointers into modify handlers
 * Use of these macros are wrapped between PHP_INI_BEGIN and PHP_INI_END.
 *
 * At some point in the cycle (TODO: minit only? rinit only?), the
 * static const array data structure of type zend_ini_entry[] which is implicit
 * between PHP_INI_BEGIN and PHP_INI_END is copied into a hash table
 * indexed by the ini entry name, such as "newrelic.license". This hash
 * table contains one entry for all ini entries, across all module
 * names.
 *
 * (TODO: It isn't clear if this hash table is copied and recreated at
 * rinit time; HHVM appears to be buggy in this regard. See hhvm issue
 * 3654.)
 *
 ...
 * of truth.
 *
 * COPIES ARE EVIL, but that's what we have: PHP's string copy of the
 ...
 *   2: the value comes from the table between PHP_INI_BEGIN and PHP_INI_END (stage == ZEND_INI_STAGE_STARTUP)
 * (TODO: The reassignment seems broken/backwards, but that's what is discernable from gdb;
 * this 2nd assignment doesn't seem to stem from deactivation.)
 *
 * Case 2: php not invoked with -dvar=value
 *   1: the value comes from the newrelic.ini file (stage == ZEND_INI_STAGE_STARTUP) (minit time)
 *
 * Case 3: newrelic.ini file present, but the value is commented out
 *   1: the value comes from the table between PHP_INI_BEGIN and PHP_INI_END (stage == ZEND_INI_STAGE_STARTUP)
 *
 * Then, the value given in the .htaccess file, if any, would be acted on through the sapi.
 * An example .htaccess file might contain:
 *   php_value newrelic.appname "special app name"
 *
 ...
 * with ZTS. This mode is not officially supported by New Relic, but
 * is a stepping stone to understanding what HHVM must do in order to
 * achieve parity.
 *
...
/*
 *
 * For HHVM as of 23Oct2014, this is the order of actions leading up to
 * calls to calling a modify handler.
 *
 * 1: zm_startup_newrelic is called (module init time)
 *
 * 2: The modify handler is called* from zm_startup_newrelic
 *    with the value scraped from newrelic.ini
 *
 * 3: zm_activiate_newrelic is called* from Extension::RequestInitModules
 *
 * 4: BUG: There is no call the modify handler.
 *
 * Look at ResetSystemDefault
 */
 ...
 *   to receive the value immediately.  An example is the modifiy handler nr_logfile_mh
 *   which is called when the ini setting "newrelic.logfile" is changed.
 *   tHe handler disseminates its value immediately by calling the function
 *   nrl_set_log_file.
 *
 * Pattern B:
 ...
 */
  ...
  #define NR_PHP_INI_DEFAULT_DAEMON_LOCATION "/opt/newrelic/bin/newrelic-daemon"
#else
...
static void
foreach_list (const char *str, foreach_fn_t f_eachname TSRMLS_DC)
{
  nrobj_t *rs;
  int ns = 0;
  int i;
...
static nrtime_t
nr_parse_time_from_config (const char *str)
{
  return nr_parse_time (str);
}

static void
check_ini_int_min_max (int *var, int minval, int maxval)
{
  if (*var < minval) {
    *var = minval;
 ...
 */
static int
nr_bool_val (php_ini_entry *ini_entry, int type)
{
 ...
 *  void *mh_arg1          the offset into the blob of data holding all ini values
 *  void *mh_arg2                          the blob of data holding all ini values
 *  void *mh_arg3          unused for our purposes
 *  int stage              taken from the set shown below
 *  TSRMLS_DC
 *
...
static PHP_INI_MH (nr_loglevel_mh)
{
  ...
  if (0 != NEW_VALUE_LEN) {
    ...
    if (NR_FAILURE == rv) {
       ...
       */
      nrl_warning (NRL_INIT, "unknown loglevel \"%.8s\"; using \"info\" instead", NEW_VALUE);
    }
  } else {
...
static PHP_INI_MH (nr_daemon_port_mh)
{
  ...
  if (NEW_VALUE_LEN > 0) {
    ...
    /*
     * TODO(msl): Failed port validation should result in actionable messages in the log.
     *            Should we also trigger a user level error?
     */
    if ('/' == NEW_VALUE[0]) {
      NR_PHP_PROCESS_GLOBALS (udspath) = nr_strdup (NEW_VALUE);  /*NOTTESTED*/
    } else {
...
static PHP_INI_MH (nr_daemon_proxy_mh)
{
  ...
  if (NEW_VALUE_LEN > 0) {
    NR_PHP_PROCESS_GLOBALS (proxy) = nr_strdup (NEW_VALUE);  /*NOTTESTED*/
  }
  return SUCCESS;
}
...
static PHP_INI_MH (nr_daemon_pidfile_mh)
{
  ...
  if (NEW_VALUE_LEN > 0) {
    NR_PHP_PROCESS_GLOBALS (pidfile) = nr_strdup (NEW_VALUE);  /*NOTTESTED*/
  }
  return SUCCESS;
}
...
static void
foreach_special_control_flag (const char *str, int str_len TSRMLS_DC)
{
  NR_UNUSED_TSRMLS;
...
static void
foreach_feature_flag (const char *str NRUNUSED, int str_len TSRMLS_DC)
{
  NR_UNUSED_TSRMLS;
...
static PHP_INI_MH (nr_feature_flag_mh)
{
  ...
  /*
   * Set default feature flag values. For example, for a feature flag called "foo":
   *
...
static void
foreach_disable_instrumentation (const char *str, int str_len TSRMLS_DC)
{
  nrinternalfn_t *w;
...
static PHP_INI_MH (nr_special_expensive_node_min_mh)
{
  ...
  if (0 != NEW_VALUE_LEN) {
    NR_PHP_PROCESS_GLOBALS (expensive_min) = nr_parse_time_from_config (NEW_VALUE);
  } else {
...
static PHP_INI_MH (nr_max_nesting_level_mh)
{
  ...
  if (0 != NEW_VALUE_LEN) {
    val = (int) strtol (NEW_VALUE, 0, 0);
    check_ini_int_min_max (&val, -1, 100000);  /* some ludicrous high value, documented in agent/scripts/newrelic.ini.private.template */
    p->value = (zend_uint)val;
    p->where = stage;
  }
 ...
 * global structure.
 *
 * Please always remember when thinking about this stuff that
 * when I use the term "global structure" I am referring to the
 * ZEND_BEGIN_MODULE_GLOBALS() structure, which is set up per-request
 * (and can be different for each thread in HHVM or in a ZTS-enabled PHP). Only
 ...
 *
 * Almost all options are actually queried through
 * Axiom and not directly through PHP (because most of the "juicy bits"
 * are implemented in Axiom and are not specific to PHP). Therefore,
 * allowing the user to use ini_set() gives the false impression that
 * doing so will have any effect. It won't.
 *
...
PHP_INI_BEGIN() /* { */
   ...
   */
  PHP_INI_ENTRY_EX ("newrelic.logfile",               NR_PHP_INI_DEFAULT_LOG_FILE,        NR_PHP_SYSTEM, nr_logfile_mh,               0)
  PHP_INI_ENTRY_EX ("newrelic.loglevel",              NR_PHP_INI_DEFAULT_LOG_LEVEL,       NR_PHP_SYSTEM, nr_loglevel_mh,              0)

  /*
   * High security mode is a system setting since it affects daemon spawn.
   */
  PHP_INI_ENTRY_EX ("newrelic.high_security", "0", NR_PHP_SYSTEM, nr_high_security_mh, 0)

  /*
   * Feature flag handling. Called feature_flag singular because that's what
   * Node uses.
   */
  PHP_INI_ENTRY_EX ("newrelic.feature_flag", "", NR_PHP_SYSTEM, nr_feature_flag_mh, 0)

  /*
   * Daemon
   */
  PHP_INI_ENTRY_EX ("newrelic.daemon.auditlog",       "",                                 NR_PHP_SYSTEM, nr_daemon_auditlog_mh,       0)
  PHP_INI_ENTRY_EX ("newrelic.daemon.logfile",        "",                                 NR_PHP_SYSTEM, nr_daemon_logfile_mh,        0)
  PHP_INI_ENTRY_EX ("newrelic.daemon.loglevel",       "",                                 NR_PHP_SYSTEM, nr_daemon_loglevel_mh,       0)
  PHP_INI_ENTRY_EX ("newrelic.daemon.port",           NR_PHP_INI_DEFAULT_PORT,            NR_PHP_SYSTEM, nr_daemon_port_mh,           0)
  PHP_INI_ENTRY_EX ("newrelic.daemon.ssl",            "1",                                NR_PHP_SYSTEM, nr_daemon_ssl_mh,            nr_enabled_disabled_dh)
  PHP_INI_ENTRY_EX ("newrelic.daemon.ssl_ca_bundle",  "",                                 NR_PHP_SYSTEM, nr_daemon_ssl_cafile_mh,     0)
  PHP_INI_ENTRY_EX ("newrelic.daemon.ssl_ca_path",    "",                                 NR_PHP_SYSTEM, nr_daemon_ssl_capath_mh,     0)
  PHP_INI_ENTRY_EX ("newrelic.daemon.collector_host", "collector.newrelic.com",           NR_PHP_SYSTEM, nr_daemon_collector_host_mh, 0)
  PHP_INI_ENTRY_EX ("newrelic.daemon.proxy",          "",                                 NR_PHP_SYSTEM, nr_daemon_proxy_mh,          nr_daemon_proxy_dh)
  PHP_INI_ENTRY_EX ("newrelic.daemon.location",       NR_PHP_INI_DEFAULT_DAEMON_LOCATION, NR_PHP_SYSTEM, nr_daemon_location_mh,       0)
  PHP_INI_ENTRY_EX ("newrelic.daemon.pidfile",        "",                                 NR_PHP_SYSTEM, nr_daemon_pidfile_mh,        0)
  PHP_INI_ENTRY_EX ("newrelic.daemon.dont_launch",    "",                                 NR_PHP_SYSTEM, nr_daemon_dont_launch_mh,    0)
  PHP_INI_ENTRY_EX ("newrelic.daemon.app_timeout",    "",                                 NR_PHP_SYSTEM, nr_daemon_app_timeout_mh,    0)

  /*
   * Utilization
   */
  PHP_INI_ENTRY_EX ("newrelic.daemon.utilization.detect_aws",     "1",                    NR_PHP_SYSTEM, NR_PHP_UTILIZATION_MH_NAME (aws),    nr_enabled_disabled_dh)
  PHP_INI_ENTRY_EX ("newrelic.daemon.utilization.detect_azure",   "1",                    NR_PHP_SYSTEM, NR_PHP_UTILIZATION_MH_NAME (azure),  nr_enabled_disabled_dh)
  PHP_INI_ENTRY_EX ("newrelic.daemon.utilization.detect_gcp",     "1",                    NR_PHP_SYSTEM, NR_PHP_UTILIZATION_MH_NAME (gcp),    nr_enabled_disabled_dh)
  PHP_INI_ENTRY_EX ("newrelic.daemon.utilization.detect_pcf",     "1",                    NR_PHP_SYSTEM, NR_PHP_UTILIZATION_MH_NAME (pcf),    nr_enabled_disabled_dh)
  PHP_INI_ENTRY_EX ("newrelic.daemon.utilization.detect_docker",  "1",                    NR_PHP_SYSTEM, NR_PHP_UTILIZATION_MH_NAME (docker), nr_enabled_disabled_dh)

  /*
   * This daemon flag is for internal development use only.  It should not be documented to customers.
   */
  PHP_INI_ENTRY_EX ("newrelic.daemon.special.integration",     "",  NR_PHP_SYSTEM, nr_daemon_special_integration_mh,       0)

  /*
   * These entries are NOT documented anywhere in https://docs.newrelic.com/docs/agents/php-agent
   * These entries are intended to be private for in-house development or on-site debugging.
   * See scripts/newrelic.ini.private.template for some additional documentation.
   *
  ...
  PHP_INI_ENTRY_EX ("newrelic.special.appinfo_timeout",                  "", NR_PHP_SYSTEM,   nr_special_appinfo_timeout_mh,                  0)
  PHP_INI_ENTRY_EX ("newrelic.special.disable_instrumentation",          "", NR_PHP_SYSTEM,   nr_special_disable_instrumentation_mh,          0)
  PHP_INI_ENTRY_EX ("newrelic.special.expensive_node_min",               "", NR_PHP_SYSTEM,   nr_special_expensive_node_min_mh,               0)
  PHP_INI_ENTRY_EX ("newrelic.special.enable_extension_instrumentation", "", NR_PHP_SYSTEM,   nr_special_enable_extension_instrumentation_mh, 0)
  PHP_INI_ENTRY_EX ("newrelic.daemon.special.curl_verbose",              "", NR_PHP_SYSTEM,   nr_daemon_special_curl_verbose_mh,              0)
   ...
   */
  STD_PHP_INI_ENTRY_EX ("newrelic.enabled", "1", NR_PHP_REQUEST, nr_enabled_mh, enabled,
    zend_newrelic_globals, newrelic_globals, nr_yes_no_dh)
  STD_PHP_INI_ENTRY_EX ("newrelic.license", "", NR_PHP_REQUEST, nr_license_mh, license,
    zend_newrelic_globals, newrelic_globals, nr_license_dh)
  STD_PHP_INI_ENTRY_EX ("newrelic.appname", NR_PHP_APP_NAME_DEFAULT, NR_PHP_REQUEST, nr_string_mh, appnames,
    zend_newrelic_globals, newrelic_globals, 0)
  STD_PHP_INI_ENTRY_EX ("newrelic.webtransaction.name.remove_trailing_path", "0", NR_PHP_REQUEST, nr_boolean_mh, remove_trailing_path,
    zend_newrelic_globals, newrelic_globals, nr_yes_no_dh)
  STD_PHP_INI_ENTRY_EX ("newrelic.framework.drupal.modules", "1", NR_PHP_REQUEST, nr_boolean_mh, drupal_modules,
    zend_newrelic_globals, newrelic_globals, nr_on_off_dh)
  STD_PHP_INI_ENTRY_EX ("newrelic.framework.wordpress.hooks", "1", NR_PHP_REQUEST, nr_boolean_mh, wordpress_hooks,
    zend_newrelic_globals, newrelic_globals, nr_on_off_dh)
  STD_PHP_INI_ENTRY_EX ("newrelic.framework", "", NR_PHP_REQUEST, nr_framework_mh, force_framework,
    zend_newrelic_globals, newrelic_globals, nr_framework_dh)
  STD_PHP_INI_ENTRY_EX ("newrelic.cross_application_tracer.enabled", "1", NR_PHP_REQUEST, nr_boolean_mh, cross_process_enabled,
    zend_newrelic_globals, newrelic_globals, nr_enabled_disabled_dh)
  STD_PHP_INI_ENTRY_EX ("newrelic.special.max_nesting_level", "-1", NR_PHP_REQUEST, nr_max_nesting_level_mh, max_nesting_level,
    zend_newrelic_globals, newrelic_globals, 0)
  STD_PHP_INI_ENTRY_EX ("newrelic.labels", "", NR_PHP_REQUEST, nr_string_mh, labels,
    zend_newrelic_globals, newrelic_globals, 0)
  STD_PHP_INI_ENTRY_EX ("newrelic.process_host.display_name", "", NR_PHP_REQUEST, nr_string_mh, process_host_display_name,
    zend_newrelic_globals, newrelic_globals, 0)
  STD_PHP_INI_ENTRY_EX ("newrelic.webtransaction.name.files", "", NR_PHP_REQUEST, nr_string_mh, file_name_list,
    zend_newrelic_globals, newrelic_globals, 0)
  STD_PHP_INI_ENTRY_EX ("newrelic.guzzle.enabled", "1", NR_PHP_REQUEST, nr_boolean_mh, guzzle_enabled,
    zend_newrelic_globals, newrelic_globals, 0)

  /*
   * Attributes
   */
  STD_PHP_INI_ENTRY_EX ("newrelic.attributes.enabled", "1", NR_PHP_REQUEST, nr_boolean_mh, attributes.enabled,
    zend_newrelic_globals, newrelic_globals, nr_enabled_disabled_dh)
  STD_PHP_INI_ENTRY_EX ("newrelic.attributes.include", "", NR_PHP_REQUEST, nr_string_mh, attributes.include,
    zend_newrelic_globals, newrelic_globals, 0)
  STD_PHP_INI_ENTRY_EX ("newrelic.attributes.exclude", "", NR_PHP_REQUEST, nr_string_mh, attributes.exclude,
    zend_newrelic_globals, newrelic_globals, 0)
  /* DEPRECATED */ STD_PHP_INI_ENTRY_EX ("newrelic.capture_params", "0", NR_PHP_REQUEST, nr_boolean_mh, capture_params,
    zend_newrelic_globals, newrelic_globals, nr_on_off_dh)
  /* DEPRECATED */ STD_PHP_INI_ENTRY_EX ("newrelic.ignored_params", "", NR_PHP_REQUEST, nr_string_mh, ignored_params,
    zend_newrelic_globals, newrelic_globals, 0)

  /*
   * Transaction Tracer
   */
  /* DEPRECATED */ STD_PHP_INI_ENTRY_EX ("newrelic.transaction_tracer.capture_attributes", "1", NR_PHP_REQUEST, nr_boolean_mh, transaction_tracer_capture_attributes,
    zend_newrelic_globals, newrelic_globals, nr_enabled_disabled_dh)
  STD_PHP_INI_ENTRY_EX ("newrelic.transaction_tracer.attributes.enabled", "1", NR_PHP_REQUEST, nr_boolean_mh, transaction_tracer_attributes.enabled,
    zend_newrelic_globals, newrelic_globals, nr_enabled_disabled_dh)
  STD_PHP_INI_ENTRY_EX ("newrelic.transaction_tracer.attributes.include", "", NR_PHP_REQUEST, nr_string_mh, transaction_tracer_attributes.include,
    zend_newrelic_globals, newrelic_globals, 0)
  STD_PHP_INI_ENTRY_EX ("newrelic.transaction_tracer.attributes.exclude", "", NR_PHP_REQUEST, nr_string_mh, transaction_tracer_attributes.exclude,
    zend_newrelic_globals, newrelic_globals, 0)
  STD_PHP_INI_ENTRY_EX ("newrelic.transaction_tracer.enabled", "1", NR_PHP_REQUEST, nr_boolean_mh, tt_enabled,
    zend_newrelic_globals, newrelic_globals, nr_enabled_disabled_dh)
  STD_PHP_INI_ENTRY_EX ("newrelic.transaction_tracer.explain_enabled", "1", NR_PHP_REQUEST, nr_boolean_mh, ep_enabled,
    zend_newrelic_globals, newrelic_globals, nr_enabled_disabled_dh)
  STD_PHP_INI_ENTRY_EX ("newrelic.transaction_tracer.detail", "1", NR_PHP_REQUEST, nr_tt_detail_mh, tt_detail,
    zend_newrelic_globals, newrelic_globals, 0)
  STD_PHP_INI_ENTRY_EX ("newrelic.transaction_tracer.slow_sql", "1", NR_PHP_REQUEST, nr_boolean_mh, tt_slowsql,
    zend_newrelic_globals, newrelic_globals, nr_enabled_disabled_dh)
  STD_PHP_INI_ENTRY_EX ("newrelic.transaction_tracer.threshold", "apdex_f", NR_PHP_REQUEST, nr_tt_threshold_mh, tt_threshold,
    zend_newrelic_globals, newrelic_globals, 0)
  STD_PHP_INI_ENTRY_EX ("newrelic.transaction_tracer.explain_threshold", "500", NR_PHP_REQUEST, nr_time_mh, ep_threshold,
    zend_newrelic_globals, newrelic_globals, 0)
  STD_PHP_INI_ENTRY_EX ("newrelic.transaction_tracer.stack_trace_threshold", "500", NR_PHP_REQUEST, nr_time_mh, ss_threshold,
    zend_newrelic_globals, newrelic_globals, 0)
  STD_PHP_INI_ENTRY_EX ("newrelic.transaction_tracer.record_sql", "obfuscated", NR_PHP_REQUEST, nr_recordsql_mh, tt_recordsql,
    zend_newrelic_globals, newrelic_globals, 0)
  STD_PHP_INI_ENTRY_EX ("newrelic.transaction_tracer.gather_input_queries", "1", NR_PHP_REQUEST, nr_boolean_mh, tt_inputquery,
    zend_newrelic_globals, newrelic_globals, 0)
  PHP_INI_ENTRY_EX ("newrelic.transaction_tracer.internal_functions_enabled", "0", NR_PHP_SYSTEM, nr_tt_internal_mh, nr_enabled_disabled_dh)

  /*
   * Error Collector
   */
  STD_PHP_INI_ENTRY_EX ("newrelic.error_collector.enabled", "1", NR_PHP_REQUEST, nr_boolean_mh, errors_enabled,
    zend_newrelic_globals, newrelic_globals, nr_enabled_disabled_dh)
  STD_PHP_INI_ENTRY_EX ("newrelic.error_collector.ignore_user_exception_handler", "0", NR_PHP_REQUEST, nr_boolean_mh, ignore_user_exception_handler,
    zend_newrelic_globals, newrelic_globals, nr_yes_no_dh)
  STD_PHP_INI_ENTRY_EX ("newrelic.error_collector.ignore_errors", "", NR_PHP_REQUEST, nr_int_mh, ignore_errors,
    zend_newrelic_globals, newrelic_globals, 0)
  STD_PHP_INI_ENTRY_EX ("newrelic.error_collector.ignore_exceptions", "", NR_PHP_REQUEST, nr_string_mh, ignore_exceptions,
    zend_newrelic_globals, newrelic_globals, 0)
  STD_PHP_INI_ENTRY_EX ("newrelic.error_collector.record_database_errors", "1", NR_PHP_REQUEST, nr_boolean_mh, record_database_errors,
    zend_newrelic_globals, newrelic_globals, nr_yes_no_dh)
  STD_PHP_INI_ENTRY_EX ("newrelic.error_collector.prioritize_api_errors", "0", NR_PHP_REQUEST, nr_boolean_mh, prioritize_api_errors,
    zend_newrelic_globals, newrelic_globals, nr_yes_no_dh)
  /* DEPRECATED */ STD_PHP_INI_ENTRY_EX ("newrelic.error_collector.capture_attributes", "1", NR_PHP_REQUEST, nr_boolean_mh, error_collector_capture_attributes,
    zend_newrelic_globals, newrelic_globals, nr_enabled_disabled_dh)
  STD_PHP_INI_ENTRY_EX ("newrelic.error_collector.attributes.enabled", "1", NR_PHP_REQUEST, nr_boolean_mh, error_collector_attributes.enabled,
    zend_newrelic_globals, newrelic_globals, nr_enabled_disabled_dh)
  STD_PHP_INI_ENTRY_EX ("newrelic.error_collector.attributes.include", "", NR_PHP_REQUEST, nr_string_mh, error_collector_attributes.include,
    zend_newrelic_globals, newrelic_globals, 0)
  STD_PHP_INI_ENTRY_EX ("newrelic.error_collector.attributes.exclude", "", NR_PHP_REQUEST, nr_string_mh, error_collector_attributes.exclude,
    zend_newrelic_globals, newrelic_globals, 0)

  /*
   * Transaction Events
   */
  /* DEPRECATED */ STD_PHP_INI_ENTRY_EX ("newrelic.analytics_events.enabled", "1", NR_PHP_REQUEST, nr_boolean_mh, analytics_events_enabled,
    zend_newrelic_globals, newrelic_globals, nr_enabled_disabled_dh)
  /* DEPRECATED */ STD_PHP_INI_ENTRY_EX ("newrelic.analytics_events.capture_attributes", "1", NR_PHP_REQUEST, nr_boolean_mh, analytics_events_capture_attributes,
    zend_newrelic_globals, newrelic_globals, nr_enabled_disabled_dh)
  STD_PHP_INI_ENTRY_EX ("newrelic.transaction_events.enabled", "1", NR_PHP_REQUEST, nr_boolean_mh, transaction_events_enabled,
    zend_newrelic_globals, newrelic_globals, nr_enabled_disabled_dh)
  STD_PHP_INI_ENTRY_EX ("newrelic.error_collector.capture_events", "1", NR_PHP_REQUEST, nr_boolean_mh, error_events_enabled,
    zend_newrelic_globals, newrelic_globals, nr_enabled_disabled_dh)
  STD_PHP_INI_ENTRY_EX ("newrelic.transaction_events.attributes.enabled", "1", NR_PHP_REQUEST, nr_boolean_mh, transaction_events_attributes.enabled,
    zend_newrelic_globals, newrelic_globals, nr_enabled_disabled_dh)
  STD_PHP_INI_ENTRY_EX ("newrelic.transaction_events.attributes.include", "", NR_PHP_REQUEST, nr_string_mh, transaction_events_attributes.include,
    zend_newrelic_globals, newrelic_globals, 0)
  STD_PHP_INI_ENTRY_EX ("newrelic.transaction_events.attributes.exclude", "", NR_PHP_REQUEST, nr_string_mh, transaction_events_attributes.exclude,
    zend_newrelic_globals, newrelic_globals, 0)

  /*
   * Custom Events
   */
  STD_PHP_INI_ENTRY_EX ("newrelic.custom_insights_events.enabled", "1", NR_PHP_REQUEST, nr_boolean_mh, custom_events_enabled,
    zend_newrelic_globals, newrelic_globals, nr_enabled_disabled_dh)

  /*
   * Synthetics
   */
  STD_PHP_INI_ENTRY_EX ("newrelic.synthetics.enabled", "1", NR_PHP_REQUEST, nr_boolean_mh, synthetics_enabled,
    zend_newrelic_globals, newrelic_globals, nr_enabled_disabled_dh)

  /*
   * Datastore Tracer
   */
  STD_PHP_INI_ENTRY_EX ("newrelic.datastore_tracer.instance_reporting.enabled", "1", NR_PHP_REQUEST, nr_boolean_mh, instance_reporting_enabled,
    zend_newrelic_globals, newrelic_globals, nr_enabled_disabled_dh)
  STD_PHP_INI_ENTRY_EX ("newrelic.datastore_tracer.database_name_reporting.enabled", "1", NR_PHP_REQUEST, nr_boolean_mh, database_name_reporting_enabled,
    zend_newrelic_globals, newrelic_globals, nr_enabled_disabled_dh)

  /*
   * Library Support
   */
  STD_PHP_INI_ENTRY_EX ("newrelic.phpunit_events.enabled", "0", NR_PHP_REQUEST, nr_boolean_mh, phpunit_events_enabled,
    zend_newrelic_globals, newrelic_globals, nr_enabled_disabled_dh)

  /*
   * Browser Monitoring
   */
  STD_PHP_INI_ENTRY_EX ("newrelic.browser_monitoring.auto_instrument", "1", NR_PHP_REQUEST, nr_boolean_mh, browser_monitoring_auto_instrument,
    zend_newrelic_globals, newrelic_globals, nr_enabled_disabled_dh)
  STD_PHP_INI_ENTRY_EX ("newrelic.browser_monitoring.debug", "0", NR_PHP_REQUEST, nr_boolean_mh, browser_monitoring_debug,
    zend_newrelic_globals, newrelic_globals, nr_enabled_disabled_dh)
  STD_PHP_INI_ENTRY_EX ("newrelic.browser_monitoring.loader", "rum", NR_PHP_REQUEST, nr_rum_loader_mh, browser_monitoring_loader,
    zend_newrelic_globals, newrelic_globals, 0)
  /* DEPRECATED */ STD_PHP_INI_ENTRY_EX ("newrelic.browser_monitoring.capture_attributes", "0", NR_PHP_REQUEST, nr_boolean_mh, browser_monitoring_capture_attributes,
    zend_newrelic_globals, newrelic_globals, nr_enabled_disabled_dh)
  STD_PHP_INI_ENTRY_EX ("newrelic.browser_monitoring.attributes.enabled", "0", NR_PHP_REQUEST, nr_boolean_mh, browser_monitoring_attributes.enabled,
    zend_newrelic_globals, newrelic_globals, nr_enabled_disabled_dh)
  STD_PHP_INI_ENTRY_EX ("newrelic.browser_monitoring.attributes.include", "", NR_PHP_REQUEST, nr_string_mh, browser_monitoring_attributes.include,
    zend_newrelic_globals, newrelic_globals, 0)
  STD_PHP_INI_ENTRY_EX ("newrelic.browser_monitoring.attributes.exclude", "", NR_PHP_REQUEST, nr_string_mh, browser_monitoring_attributes.exclude,
    zend_newrelic_globals, newrelic_globals, 0)
  /*
   ...
   */
  PHP_INI_ENTRY_EX ("newrelic.webtransaction.name.functions", "", NR_PHP_REQUEST, nr_wtfuncs_mh, 0)
  PHP_INI_ENTRY_EX ("newrelic.transaction_tracer.custom", "", NR_PHP_REQUEST, nr_ttcustom_mh, 0)

PHP_INI_END() /* } */

void
nr_php_register_ini_entries (int module_number TSRMLS_DC)
{
  REGISTER_INI_ENTRIES();
}

void
nr_php_unregister_ini_entries (int module_number TSRMLS_DC)
{
  UNREGISTER_INI_ENTRIES();
}

static void
nr_ini_displayer_cb (zend_ini_entry *ini_entry, int type TSRMLS_DC)
{
 ...
 */
static int
nr_ini_displayer_global (zend_ini_entry *ini_entry, int *module_number,
                         zend_hash_key *key NRUNUSED TSRMLS_DC)
{
  ...
  /*
   * If there is no value, then don't print anything for the "special" ini settings.
   */
  if ((NULL == PHP_INI_ENTRY_VALUE (ini_entry)) ||
      (0 == PHP_INI_ENTRY_VALUE_LEN (ini_entry))) {
    if (0 == nr_strncmp (PHP_INI_ENTRY_NAME (ini_entry), NR_PSTR ("newrelic.special"))) {
      return ZEND_HASH_APPLY_KEEP;
    }
    if (0 == nr_strncmp (PHP_INI_ENTRY_NAME (ini_entry), NR_PSTR ("newrelic.daemon.special"))) {
      return ZEND_HASH_APPLY_KEEP;
    }
  }
  ...
  if (!sapi_module.phpinfo_as_text) {
    PUTS ("<tr>");
    PUTS ("<td class=\"e\">");
    PHPWRITE (PHP_INI_ENTRY_NAME (ini_entry), PHP_INI_ENTRY_NAME_LEN (ini_entry) - 1);
    PUTS ("</td><td class=\"v\">");
 ...
 */
static int
nr_ini_displayer_perdir (zend_ini_entry *ini_entry, int *module_number,
                         zend_hash_key *key NRUNUSED TSRMLS_DC)
{
  if (ini_entry->module_number != *module_number) {
    return ZEND_HASH_APPLY_KEEP;
...
static void
nr_display_ini_entries_global (zend_module_entry *module TSRMLS_DC)
{
  int module_number;
...
static void
nr_display_ini_entries_perdir (zend_module_entry *module TSRMLS_DC)
{
  int module_number;
  ...
  php_info_print_table_header (3, "Directive Name", "Local/Active Value", "Master/Default Value");
  if (0 != EG (ini_directives)) {
    nr_php_zend_hash_ptr_apply (EG (ini_directives),
...
PHP_MINFO_FUNCTION (newrelic)
{
  ...
  php_info_print_table_header (2, "New Relic RPM Monitoring", NR_PHP_PROCESS_GLOBALS (enabled) ? "enabled" :
    NR_PHP_PROCESS_GLOBALS (mpm_bad) ? "disabled due to threaded MPM" : "disabled");
...
/*
 * This function must be compatible with typedef int (*apply_func_arg_t)(void *pDest, void *argument TSRMLS_DC);
 */
static int
nr_ini_settings (zend_ini_entry *ini_entry, settings_st *setarg,
                 zend_hash_key *key NRUNUSED TSRMLS_DC)
{
  NR_UNUSED_TSRMLS;
  ...
  if (!(ini_entry->modifiable & PHP_INI_PERDIR)) {
    if ((NULL == PHP_INI_ENTRY_VALUE (ini_entry)) ||
        (0 == PHP_INI_ENTRY_VALUE_LEN (ini_entry))) {
      if (0 == nr_strncmp (PHP_INI_ENTRY_NAME (ini_entry),
  ...
  if ((NULL == PHP_INI_ENTRY_VALUE (ini_entry)) ||
    ...
    if (0 == nr_strncmp (PHP_INI_ENTRY_NAME (ini_entry),
      ...
      char *plicense = nr_app_create_printable_license (PHP_INI_ENTRY_VALUE (ini_entry));

      nro_set_hash_string (setarg->obj, PHP_INI_ENTRY_NAME (ini_entry),
                           plicense ? plicense : "INVALID_FORMAT");
      nr_free (plicense);
    } else if (0 == nr_strncmp (PHP_INI_ENTRY_NAME (ini_entry),
...
int
nr_php_ini_setting_is_set_by_user (const char *name)
{
#ifdef PHP7
  int found;
  zend_string *zs;
