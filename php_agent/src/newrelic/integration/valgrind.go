...
func (tx *ValgrindCLI) Execute() (http.Header, []byte, error) {
	...
	// the valgrind processes' pid instead.
	valgrindMu.Lock()
	...
	//
	// TODO(msl): Eventually we want to report valgrind output separately,
	// so we can treat valgrind errors similarly to failed test expectations.
	// i.e. We'd like to add them to Test.Failures. After all, each test
	// has an implicit expectation that it will not exhibit memory bugs!
	//
	// TODO(msl): Once the former is in place, we should be able to parse
	// the memory leak reports produced by the Zend Memory Manager from the
	// test output and report them the same way as valgrind errors.
	...
	// error to report.
	//
	// TODO(msl): Marry the two errors in a small, private type so the
	// developers can live happily ever after.
	...
	//
	// TODO(msl): Remove this when valgrind errors can be treated as failed
	// test expectations.
