...
static size_t
zval_len_without_trailing_slash (const zval *zstr)
{
...
static void
add_wildcard_path_component (nrbuf_t *buf)
{
  nr_buffer_add (buf, NR_PSTR ("/(.*?)/"));
}

static nr_regex_t *
compile_regex_for_path (const zval *path)
{
...
static nr_regex_t *
compile_regex_for_path_array (const zval *paths)
{
  ...
  if (first) {
    nrl_verbosedebug (NRL_FRAMEWORK,
                      "%s: no valid elements in the path array",
                      __func__);
  } else {
    regex = nr_regex_create (nr_buffer_cptr (buf), NR_REGEX_CASELESS, 1);
  }
...
static nr_regex_t *
compile_regex_for_constant (const char *constant, const char *suffix TSRMLS_DC)
{
  nr_regex_t *regex = NULL;
  ...
  if (nr_php_is_zval_valid_string (value)) {
    ...
    if (value) {
      nrl_verbosedebug (NRL_FRAMEWORK,
                        "%s: unexpected non-string value for %s",
                        __func__, constant);
    }
  }
...
static char *
try_match_regex (const nr_regex_t *regex, const char *filename)
{
  char *plugin = NULL;
...
static const nr_regex_t *
nr_wordpress_plugin_regex (TSRMLS_D)
{
  nr_regex_t *regex = NULL;
   ...
   *
   * If neither exists, we'll just look for "wp-content/plugins" and hope for
   * the best.
   */
  ...
  if (NULL == regex) {
    ...
    regex = nr_regex_create ("/wp-content/plugins/(.*?)/", NR_REGEX_CASELESS, 1);
  }
...
static const nr_regex_t *
nr_wordpress_theme_regex (TSRMLS_D)
{
  nr_regex_t *regex = NULL;
  zval *roots = NULL;
...
static void
nr_wordpress_create_metric (nrtxn_t *txn, const char *prefix, const char *name,
                            nrtime_t duration, nrtime_t exclusive)
{
...
static void
free_wordpress_metadata (void *metadata)
{
  nr_free (metadata);
}

static char *
nr_wordpress_plugin_from_function (zend_function *func TSRMLS_DC)
{
  ...
  if (NULL == filename) {
    nrl_verbosedebug (NRL_FRAMEWORK, "Wordpress: cannot determine plugin name:"
      " missing filename, tag=" NRP_FMT, NRP_PHP (NRPRG (wordpress_tag)));
    return NULL;
  }
  filename_len = nr_strlen (filename);

  if (NRPRG (wordpress_file_metadata)) {
    if (nr_hashmap_get_into (NRPRG (wordpress_file_metadata),
                             filename, filename_len, (void **) &plugin)) {
      return plugin;
    }
  } else {
...
NR_PHP_WRAPPER (nr_wordpress_wrap_hook)
{
  ...
  nr_wordpress_create_metric (NRPRG (txn), NR_WORDPRESS_HOOK_PREFIX, NRPRG (wordpress_tag), duration, exclusive);
  nr_wordpress_create_metric (NRPRG (txn), NR_WORDPRESS_PLUGIN_PREFIX, plugin, duration, exclusive);
} NR_PHP_WRAPPER_END
 ...
 */
static void
nr_wordpress_call_user_func_array (zend_function *func,
 ...
 */
static char *
nr_wordpress_clean_tag (const zval *tag TSRMLS_DC)
{
  char *orig_tag = NULL;
  char *clean_tag = NULL;
  nr_regex_t *regex = NULL;
...
NR_PHP_WRAPPER (nr_wordpress_exec_handle_tag)
{
  ...
  if (1 == nr_php_is_zval_non_empty_string (tag) || (0 != NRINI (wordpress_hooks))) {
    /* 
 ...
 */
static void
nr_wordpress_name_the_wt (const zval *tag, zval **retval_ptr TSRMLS_DC)
{
  int alen;
  char *action = 0;
  char *shortened_action = 0;
  char *s2 = 0;
...
void
nr_wordpress_enable (TSRMLS_D)
{
  nr_php_wrap_user_function (NR_PSTR ("apply_filters"), nr_wordpress_apply_filters TSRMLS_CC);

  nr_php_wrap_user_function (NR_PSTR ("apply_filters_ref_array"), nr_wordpress_exec_handle_tag TSRMLS_CC);
  ...
  nr_php_add_call_user_func_array_pre_callback (nr_wordpress_call_user_func_array TSRMLS_CC);
}
