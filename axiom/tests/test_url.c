...
static void
clean_testcase_fn (const char *testname, const char *expected, const char *in, const char *file, int line)
{
  int inlen = nr_strlen (in);
  char *rv = nr_url_clean (in, inlen);

  test_pass_if_true (testname, 0 == nr_strcmp (rv, expected),
    "rv=%s expected=%s", NRSAFESTR (rv), NRSAFESTR (expected));
  nr_free (rv);
}

static void
test_clean_normal (void)
{
  char *json = 0;
  nrobj_t *array = 0;
  nrotype_t otype;
...
test_clean_bad_params (void)
{
  ...
  tlib_pass_if_true ("early terminator", 0 == nr_strcmp ("domain.com", rv), "rv=%s", NRSAFESTR (rv));
  nr_free (rv);
  ...
  tlib_pass_if_true ("urllen obeyed", 0 == nr_strcmp (rv, "domain.com"), "rv=%s", rv);
  nr_free (rv);
}

#define extract_testcase(...) test_url_extract_testcase_fn (__VA_ARGS__,__FILE__,__LINE__)

static void
test_url_extract_testcase_fn (const char *expected, const char *in, const char *file, int line)
{
  int len = 0;
  int inlen = nr_strlen (in);
  ...
  if (expected) {
    ...
    test_pass_if_true (in, (0 != rv) && (expectedlen == len) && (0 == nr_strncmp (rv, expected, expectedlen)),
      "expected=%s expectedlen=%d len=%d rv=%.*s", expected, expectedlen, len, len, NRSAFESTR (rv));
  } else {
    /*
     * Failure is expected.
     */
    test_pass_if_true (in, (0 == rv) && (-1 == len), "len=%d rv=%.*s", len, len, NRSAFESTR (rv));
  }
}

static void
test_extract_domain_normal (void)
{
  char *json = 0;
  nrobj_t *array = 0;
  nrotype_t otype;
...
#define DOMAIN_EXTRACTION_TESTS_FILE CROSS_AGENT_TESTS_DIR "/url_domain_extraction.json"
...
static void
test_extract_domain_bad_params (void)
{
  const char *rv;
  int dnlen;
  ...
  tlib_pass_if_true ("zero url", (0 == rv) && (-1 == dnlen), "rv=%p dnlen=%d", rv, dnlen);

  rv = nr_url_extract_domain ("", 1, &dnlen);
  tlib_pass_if_true ("empty url", (0 == rv) && (-1 == dnlen), "rv=%p dnlen=%d", rv, dnlen);

  rv = nr_url_extract_domain ("a", 0, &dnlen);
  tlib_pass_if_true ("zero len", (0 == rv) && (-1 == dnlen), "rv=%p dnlen=%d", rv, dnlen);

  rv = nr_url_extract_domain ("a", -1, &dnlen);
  tlib_pass_if_true ("negative len", (0 == rv) && (-1 == dnlen), "rv=%p dnlen=%d", rv, dnlen);
  ...
  tlib_pass_if_true ("early terminator", (0 != rv) && (10 == dnlen) && (0 == nr_strncmp (rv, "domain.com", 10)),
  ...
  tlib_pass_if_true ("urllen obeyed", (0 != rv) && (6 == dnlen) && (0 == nr_strncmp (rv, "domain", 6)),
    "dnlen=%d rv=%.*s", dnlen, dnlen, NRSAFESTR (rv));
}

#define proxy_clean_testcase(...) test_url_proxy_clean_testcase_fn (__VA_ARGS__,__FILE__,__LINE__)

static void
test_url_proxy_clean_testcase_fn (const char *expected, const char *in, const char *file, int line)
{
  ...
  if (expected) {
     ...
     * Success is expected.
     */
    test_pass_if_true (in, 0 == nr_strcmp (rv, expected),
      "expected=%s rv=%s", expected, NRSAFESTR (rv));
  } else {
    /*
     * Failure is expected.
     */
...
static void
test_proxy_clean (void)
{
  /* TODO (aheusser): turn this into a cross-agent test! */
  ...
  proxy_clean_testcase ("****:****@hostname:port", "user:password@hostname:port");
  ...
  proxy_clean_testcase ("scheme://****@hostname:port", "scheme://user@hostname:port");
  proxy_clean_testcase ("scheme://****:****@hostname", "scheme://user:password@hostname");
  proxy_clean_testcase ("scheme://****:****@hostname:port", "scheme://user:password@hostname:port");  proxy_clean_testcase (NULL, NULL);
