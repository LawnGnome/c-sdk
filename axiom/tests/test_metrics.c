...
#define test_metric_json(...) test_metric_json_fn(__VA_ARGS__, __FILE__, __LINE__)

static void
test_metric_json_fn (const char *testname, const nrmtable_t *table,
                     const char *expected_json, const char *file, int line)
{
  ...
  test_pass_if_true (testname, 0 == nr_strcmp (expected_json, json),
    "json=%s\nexpected_json=%s", NRSAFESTR (json), NRSAFESTR (expected_json));
...
static void
test_find_internal_bad_parameters (void)
{
...
static void
test_accessor_bad_parameters (void)
{
  nrtime_t data;
...
test_find_create (void)
{
  ...
  tlib_pass_if_true ("metric created", 0 == nr_strcmp ("name", name), "name=%s", NRSAFESTR (name));
  ...
  tlib_pass_if_true ("metric found", 0 == nr_strcmp ("name", name), "name=%s", NRSAFESTR (name));
  ...
  tlib_pass_if_true ("metric", 0 == nr_strcmp ("name", name), "name=%s", NRSAFESTR (name));

  metric = nrm_create (table, "name2", hash);
  name = nrm_get_name (table, metric);
  tlib_pass_if_true ("metric with different name", 0 != metric, "metric=%p", metric);
  tlib_pass_if_true ("metric with different name", 0 == nr_strcmp ("name2", name), "name=%s", NRSAFESTR (name));
...
static void
test_metric_attribute_fn (const char *testname, const char *expression1, nrtime_t value1,
                          const char *expression2, nrtime_t value2, const char *file, int line)
{
  test_pass_if_true (testname, value1 == value2,
    "%s=" NR_TIME_FMT " %s=" NR_TIME_FMT,
...
test_add (void)
{
  ...
  test_metric_json (testname, table, "[{\"name\":\"metric_name\",\"data\":[1,10.00000,10.00000,10.00000,10.00000,100.00000]}]");
  name = nrm_get_name (table, metric);
  tlib_pass_if_true (testname, 0 == nr_strcmp ("metric_name", name), "name=%s", NRSAFESTR (name));
  ...
  test_metric_attribute (testname, nrm_sumsquares (metric), 100 * (NR_TIME_DIVISOR * NR_TIME_DIVISOR));
  ...
  metric = nrm_find (table, "metric_name");
  test_metric_json (testname, table, "[{\"name\":\"metric_name\",\"data\":[3,30.00000,30.00000,9.00000,11.00000,302.00000]}]");
  name = nrm_get_name (table, metric);
  tlib_pass_if_true (testname, 0 == nr_strcmp ("metric_name", name), "name=%s", NRSAFESTR (name));
  test_metric_attribute (testname, nrm_count      (metric), 3);
  test_metric_attribute (testname, nrm_total      (metric), (10 + 9 + 11) * NR_TIME_DIVISOR);
  test_metric_attribute (testname, nrm_exclusive  (metric), (10 + 9 + 11) * NR_TIME_DIVISOR);
  ...
  test_metric_attribute (testname, nrm_sumsquares (metric), (100 + 81 + 121) * (NR_TIME_DIVISOR * NR_TIME_DIVISOR));
...
test_add_ex (void)
{
  ...
  metric = nrm_find (table, "metric_name");
  test_metric_json (testname, table, "[{\"name\":\"metric_name\",\"data\":[1,10.00000,5.00000,10.00000,10.00000,100.00000]}]");
  name = nrm_get_name (table, metric);
  tlib_pass_if_true (testname, 0 == nr_strcmp ("metric_name", name), "name=%s", NRSAFESTR (name));
  test_metric_attribute (testname, nrm_count      (metric), 1);
  test_metric_attribute (testname, nrm_total      (metric), 10 * NR_TIME_DIVISOR);
  ...
  nrm_add_ex (table, "metric_name",  9 * NR_TIME_DIVISOR, 4 * NR_TIME_DIVISOR); /* Min */
  nrm_add_ex (table, "metric_name", 11 * NR_TIME_DIVISOR, 3 * NR_TIME_DIVISOR); /* Max */

  testname = "multiple nrm_add_ex";
  metric = nrm_find (table, "metric_name");
  test_metric_json (testname, table, "[{\"name\":\"metric_name\",\"data\":[3,30.00000,12.00000,9.00000,11.00000,302.00000]}]");
  name = nrm_get_name (table, metric);
  tlib_pass_if_true (testname, 0 == nr_strcmp ("metric_name", name), "name=%s", NRSAFESTR (name));
  test_metric_attribute (testname, nrm_count      (metric), 3);
  test_metric_attribute (testname, nrm_total      (metric), (10 + 9 + 11) * NR_TIME_DIVISOR);
  test_metric_attribute (testname, nrm_exclusive  (metric), (5 + 4 + 3) * NR_TIME_DIVISOR);
  test_metric_attribute (testname, nrm_min        (metric),  9 * NR_TIME_DIVISOR);
  test_metric_attribute (testname, nrm_max        (metric), 11 * NR_TIME_DIVISOR);
  test_metric_attribute (testname, nrm_sumsquares (metric), (100 + 81 + 121) * (NR_TIME_DIVISOR * NR_TIME_DIVISOR));
...
#define test_metrics_equal(...) test_metrics_equal_fn (__VA_ARGS__, __FILE__, __LINE__)

static void
test_metrics_equal_fn (const char *testname, nrmtable_t *table1, nrmtable_t *table2,
                       const char *name, const char *file, int line)
{
  ...
  if (metric1 && metric2) {
    test_metric_attribute_fn (testname, "nrm_count      (metric1)", nrm_count      (metric1),
                                        "nrm_count      (metric2)", nrm_count      (metric2), file, line);
    test_metric_attribute_fn (testname, "nrm_total      (metric1)", nrm_total      (metric1),
                                        "nrm_total      (metric2)", nrm_total      (metric2), file, line);
    test_metric_attribute_fn (testname, "nrm_exclusive  (metric1)", nrm_exclusive  (metric1),
                                        "nrm_exclusive  (metric2)", nrm_exclusive  (metric2), file, line);
    test_metric_attribute_fn (testname, "nrm_min        (metric1)", nrm_min        (metric1),
                                        "nrm_min        (metric2)", nrm_min        (metric2), file, line);
    test_metric_attribute_fn (testname, "nrm_max        (metric1)", nrm_max        (metric1),
                                        "nrm_max        (metric2)", nrm_max        (metric2), file, line);
    test_metric_attribute_fn (testname, "nrm_sumsquares (metric1)", nrm_sumsquares (metric1),
...
test_force_add (void)
{
  ...
  tlib_pass_if_true ("table full unforced metric", 0 == metric, "metric=%p", metric);
  nrm_force_add (table, "FORCED", 0);
  metric = nrm_find (table, "FORCED");
  tlib_pass_if_true ("table full forced metric", 0 != metric, "metric=%p", metric);
  nrm_table_destroy (&table);
...
test_force_add_ex (void)
{
  ...
  metric = nrm_find (table, "NOT_FORCED");
  tlib_pass_if_true ("table full unforced metric ex", 0 == metric, "metric=%p", metric);
  nrm_force_add_ex (table, "FORCED", 0, 0);
  metric = nrm_find (table, "FORCED");
  tlib_pass_if_true ("table full forced metric ex", 0 != metric, "metric=%p", metric);
  nrm_table_destroy (&table);
  ...
  nrm_force_add_ex (t2, "metric_name", 10 * NR_TIME_DIVISOR, 5 * NR_TIME_DIVISOR);
  ...
  nrm_force_add_ex (t2, "metric_name", 11 * NR_TIME_DIVISOR, 3 * NR_TIME_DIVISOR);
  test_metrics_equal ("nrm_add_ex and nrm_force_add_ex", t1, t2, "metric_name");
  nrm_table_destroy (&t1);
  nrm_table_destroy (&t2);
}
...
test_add_apdex (void)
{
  ...
  test_metric_json (testname, table, "[{\"name\":\"my_apdex\",\"data\":[11,22,33,5.00000,5.00000,0]}]");
  name = nrm_get_name (table, metric);
  tlib_pass_if_true (testname, 0 == nr_strcmp ("my_apdex", name), "name=%s", NRSAFESTR (name));
  ...
  testname = "multiple nrm_add";
  metric = nrm_find (table, "my_apdex");
  test_metric_json (testname, table, "[{\"name\":\"my_apdex\",\"data\":[46,67,88,4.00000,6.00000,0]}]");
  name = nrm_get_name (table, metric);
  tlib_pass_if_true (testname, 0 == nr_strcmp ("my_apdex", name), "name=%s", NRSAFESTR (name));
...
test_force_add_apdex (void)
{
  ...
  metric = nrm_find (table, "NOT_FORCED");
  tlib_pass_if_true ("table full unforced apdex", 0 == metric, "metric=%p", metric);
  nrm_force_add_apdex (table, "FORCED", 0, 0, 0, 0);
  metric = nrm_find (table, "FORCED");
  tlib_pass_if_true ("table full forced apdex", 0 != metric, "metric=%p", metric);
  nrm_table_destroy (&table);
  ...
  test_metrics_equal  ("nrm_add_apdex and nrm_force_add_apdex", t1, t2, "my_apdex");
...
test_add_internal (void)
{
  ...
  name = nrm_get_name (table, metric);
  tlib_pass_if_true ("nrm_add_internal", 0 == nr_strcmp ("name1", name), "name=%s", NRSAFESTR (name));
  test_metric_attribute ("nrm_add_internal", nrm_count      (metric), 1);
  test_metric_attribute ("nrm_add_internal", nrm_total      (metric), 2*NR_TIME_DIVISOR);
  test_metric_attribute ("nrm_add_internal", nrm_exclusive  (metric), 3*NR_TIME_DIVISOR);
  test_metric_attribute ("nrm_add_internal", nrm_min        (metric), 4*NR_TIME_DIVISOR);
  test_metric_attribute ("nrm_add_internal", nrm_max        (metric), 5*NR_TIME_DIVISOR);
  test_metric_attribute ("nrm_add_internal", nrm_sumsquares (metric), 6*NR_TIME_DIVISOR*NR_TIME_DIVISOR);
  ...
  test_metric_json ("test_add_internal", table,
    "[{\"name\":\"name1\","                        "\"data\":[1,2.00000,3.00000,4.00000,5.00000,6.00000]},"
     "{\"name\":\"Supportability\\/MetricsDropped\",\"data\":[1,0.00000,0.00000,0.00000,0.00000,0.00000],\"forced\":true},"
     "{\"name\":\"name3\","                        "\"data\":[0,0.00000,0.00000,0.00000,0.00000,0.00000],\"forced\":true}]");

  nrm_table_destroy (&table);
}

static void
test_add_apdex_internal (void)
{
  nrmtable_t *table = nrm_table_create (1);
  nrmetric_t *metric;
  const char *name;
  ...
  tlib_pass_if_true ("nrm_add_apdex_internal", 0 == nr_strcmp ("name1", name), "name=%s", NRSAFESTR (name));
  ...
  test_metric_attribute ("nrm_add_apdex_internal", nrm_min        (metric), 4 * NR_TIME_DIVISOR);
  test_metric_attribute ("nrm_add_apdex_internal", nrm_max        (metric), 5 * NR_TIME_DIVISOR);
  ...
  test_metric_json ("test_add_internal", table,
    "[{\"name\":\"name1\","                        "\"data\":[1,2,3,4.00000,5.00000,0]},"
     "{\"name\":\"Supportability\\/MetricsDropped\",\"data\":[1,0.00000,0.00000,0.00000,0.00000,0.00000],\"forced\":true},"
     "{\"name\":\"name3\","                        "\"data\":[0,0,0,0.00000,0.00000,0],\"forced\":true}]");

  nrm_table_destroy (&table);
}

static void
test_add_bad_parameters (void)
{
  nrmtable_t *table;
...
test_duplicate_metric (void)
{
  ...
  test_metric_json ("duplicate success", table,
    "[{\"name\":\"old\",\"data\":[1,123.00000,123.00000,123.00000,123.00000,15129.00000],\"forced\":true},"
     "{\"name\":\"new\",\"data\":[1,123.00000,123.00000,123.00000,123.00000,15129.00000],\"forced\":true}]");

  nrm_table_destroy (&table);
}

static void
test_metric_table_to_daemon_json (void)
{
  nrmtable_t *table;
  char *json;
  ...
  nrm_force_add_ex (table, "nrm_force_add_ex", 5 * NR_TIME_DIVISOR, 6 * NR_TIME_DIVISOR);
  nrm_add_apdex (table, "nrm_add_apdex", 1, 2, 3, 44 * NR_TIME_DIVISOR);
  nrm_force_add_apdex (table, "nrm_force_add_apdex", 5, 6, 7, 88 * NR_TIME_DIVISOR);

  json = nr_metric_table_to_daemon_json (table);
  tlib_pass_if_str_equal ("empty table", json,
    "["
      "{\"name\":\"nrm_add\",\"data\":[1,1.00000,1.00000,1.00000,1.00000,1.00000]},"
      "{\"name\":\"nrm_force_add\",\"data\":[1,2.00000,2.00000,2.00000,2.00000,4.00000],\"forced\":true},"
      "{\"name\":\"nrm_add_ex\",\"data\":[1,3.00000,4.00000,3.00000,3.00000,9.00000]},"
      "{\"name\":\"nrm_force_add_ex\",\"data\":[1,5.00000,6.00000,5.00000,5.00000,25.00000],\"forced\":true},"
      "{\"name\":\"nrm_add_apdex\",\"data\":[1,2,3,44.00000,44.00000,0]},"
      "{\"name\":\"nrm_force_add_apdex\",\"data\":[5,6,7,88.00000,88.00000,0],\"forced\":true}"
    "]");
  nr_free (json);
