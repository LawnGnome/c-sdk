...
static nr_slowsqls_params_t
sample_slowsql_params (void)
{
...
test_simple_add (void)
{
  ...
  tlib_pass_if_str_equal ("simply add", nr_slowsql_params (slow), "{\"backtrace\":[\"already\\/escaped\"]}");
...
test_min_max (void)
{
  ...
  params.duration = 5 * NR_TIME_DIVISOR; nr_slowsqls_add (slowsqls, &params);
  params.duration = 3 * NR_TIME_DIVISOR; nr_slowsqls_add (slowsqls, &params);
  params.duration = 4 * NR_TIME_DIVISOR; nr_slowsqls_add (slowsqls, &params);
  params.duration = 6 * NR_TIME_DIVISOR; nr_slowsqls_add (slowsqls, &params);
  params.duration = 7 * NR_TIME_DIVISOR; nr_slowsqls_add (slowsqls, &params);
  params.duration = 5 * NR_TIME_DIVISOR; nr_slowsqls_add (slowsqls, &params);
  ...
  tlib_pass_if_str_equal ("test min/max", nr_slowsql_params (slow), "{\"backtrace\":[\"already\\/escaped\"]}");

  nr_slowsqls_destroy (&slowsqls);
}

static void
test_raw_sql_aggregation (void)
{
  nr_slowsqls_t *slowsqls;
  const nr_slowsql_t *slow;
  nr_slowsqls_params_t params = sample_slowsql_params ();
  ...
  slowsqls = nr_slowsqls_create (1);
  params.sql = "SELECT * FROM test WHERE foo IN (1)";       nr_slowsqls_add (slowsqls, &params);
  params.sql = "SELECT * FROM test WHERE foo IN (2)";       nr_slowsqls_add (slowsqls, &params);
  params.sql = "SELECT * FROM test WHERE foo IN (2, 3, 4)"; nr_slowsqls_add (slowsqls, &params);

  slow = nr_slowsqls_at (slowsqls, 0);
  tlib_pass_if_uint32_t_equal ("simple raw aggregation", nr_slowsql_id (slow), 2676686092);
  tlib_pass_if_int_equal ("simple raw aggregation", nr_slowsql_count (slow), 3);
  tlib_pass_if_time_equal ("simple raw aggregation", nr_slowsql_min (slow), 5000000);
  tlib_pass_if_time_equal ("simple raw aggregation", nr_slowsql_max (slow), 5000000);
  tlib_pass_if_time_equal ("simple raw aggregation", nr_slowsql_total (slow), 15000000);
  tlib_pass_if_str_equal ("simple raw aggregation", nr_slowsql_metric (slow), "my/metric");
  tlib_pass_if_str_equal ("simple raw aggregation", nr_slowsql_query (slow), "SELECT * FROM test WHERE foo IN (1)");
  tlib_pass_if_str_equal ("simple raw aggregation", nr_slowsql_params (slow), "{\"backtrace\":[\"already\\/escaped\"]}");

  nr_slowsqls_destroy (&slowsqls);
}

static void
test_obfuscated_sql_aggregation (void)
{
  nr_slowsqls_t *slowsqls;
  const nr_slowsql_t *slow;
  nr_slowsqls_params_t params = sample_slowsql_params ();
  ...
  tlib_pass_if_str_equal ("obfuscated", nr_slowsql_query (slow), "SELECT * FROM test WHERE foo IN (?)");
  tlib_pass_if_str_equal ("obfuscated", nr_slowsql_params (slow), "{\"backtrace\":[\"already\\/escaped\"]}");

  nr_slowsqls_destroy (&slowsqls);
}

static void
test_mixed_aggregation (void)
{
  nr_slowsqls_t *slowsqls;
  const nr_slowsql_t *slow;
  nr_slowsqls_params_t params = sample_slowsql_params ();
  ...
  slow = nr_slowsqls_at (slowsqls, 0);
  tlib_pass_if_uint32_t_equal ("mixed aggregation", nr_slowsql_id (slow), 2676686092);
  ...
  tlib_pass_if_time_equal ("mixed aggregation", nr_slowsql_total (slow), 20000000);
  tlib_pass_if_str_equal ("mixed aggregation", nr_slowsql_metric (slow), "my/metric");
  tlib_pass_if_str_equal ("mixed aggregation", nr_slowsql_query (slow), "SELECT * FROM test WHERE foo IN (?)");
  tlib_pass_if_str_equal ("mixed aggregation", nr_slowsql_params (slow), "{\"backtrace\":[\"already\\/escaped\"]}");

  nr_slowsqls_destroy (&slowsqls);
}

static void
test_slowest_taken (void)
{
  int i;
  nr_slowsqls_t *slowsqls;
  const nr_slowsql_t *slow;
  ...
  tlib_pass_if_str_equal ("slowest taken", nr_slowsql_params (slow), "{\"backtrace\":[\"already\\/escaped\"]}");
...
test_max_time_ranked (void)
{
  ...
  tlib_pass_if_str_equal (testname, nr_slowsql_params (slow), "{\"backtrace\":[\"already\\/escaped\"]}");

  nr_slowsqls_destroy (&slows);
}

static void
test_destroy_bad_params (void)
{
...
static void
test_instance_info_disabled (void)
{
  nr_slowsqls_t *slowsqls;
  const nr_slowsql_t *slow;
...
static void
test_slowsqls_at_bad_params (void)
{
  nr_slowsqls_t *slowsqls;
  nr_slowsqls_params_t params = sample_slowsql_params ();
...
static void
test_slowsql_accessor_bad_params (void)
{
