...
typedef struct _nrtxnopt_t {
  ...
  int instance_reporting_enabled;      /* Whether to capture datastore instance host and port */
  int database_name_reporting_enabled; /* Whether to include database name in datastore instance */
  ...
  nr_tt_recordsql_t tt_recordsql;      /* How to record SQL statements in TT's (if at all) */
  int tt_slowsql;                      /* Whether to support the slow SQL feature */
  nrtime_t apdex_t;                    /* From app default unless key txn */
  nrtime_t tt_threshold;               /* TT threshold in usec - faster than this isn't a TT */
...
typedef enum _nrtxnstatus_cross_process_t {
  ...
  NR_STATUS_CROSS_PROCESS_START            = 1, /* The response header has not been created */
  NR_STATUS_CROSS_PROCESS_RESPONSE_CREATED = 2  /* The response header has been created */
 ...
 *
 * See https://newrelic.atlassian.net/wiki/display/eng/PHP+Agent+Troubleshooting#PHPAgentTroubleshooting-WebTransactionNaming
 *
...
typedef struct _nrtxnstatus_t {
  int has_inbound_record_tt;    /* 1 if the inbound request header has a true record_tt, 0 otherwise */
  int has_outbound_record_tt;   /* 1 if an outbound response header has a true record_tt, 0 otherwise */
  ...
  int rum_header;               /* 0 = header not sent, 1 = sent manually, 2 = auto */  /* TODO(rrh): use an enumeration */
  int rum_footer;               /* 0 = footer not sent, 1 = sent manually, 2 = auto */  /* TODO(rrh): use an enumeration */
...
typedef struct _nrtxn_t {
  ...
  int high_security;            /* From application: Whether the txn is in special high security mode */
  ...
  nrmtable_t *scoped_metrics;   /* Contains metrics that are both scoped and unscoped. */
  nrmtable_t *unscoped_metrics; /* Unscoped metric table for the txn */
  nrobj_t *intrinsics;          /* Attribute-like builtin fields sent along with traces and errors */
  nr_attributes_t *attributes;  /* Key+value pair tags put in txn event, txn trace, error, and browser */
  nrtime_t *cur_kids_duration;  /* Points to variable to increment for a child's duration */
  ...
  nr_analytics_events_t *custom_events; /* Custom events created through the API. */
  ...
  char *path;                   /* Request URI or action (txn name before rules applied & prefix) */
  char *name;                   /* Full transaction metric name */

  nrtxntype_t type;             /* The transaction type(s), as a bitfield */

  nrobj_t *app_connect_reply;   /* Contents of application collector connect command reply */
  char *primary_app_name;       /* The primary app name in use (ie the first rollup entry) */
  nr_synthetics_t *synthetics;  /* Synthetics metadata for the transaction */

  /*
   * Special control variables derived from named bits in nrphpglobals_t.special_flags
   * These are used to debug the agent, possibly in the field.
   */
  struct {
...
static inline int
nr_txn_recording (const nrtxn_t *txn)
{
 ...
 *
 * Params  :
 *           1. A descriptive name for the originator of the txn name, used for logging.
 *           2. The transaction to set the path in.
 *           3. The path.
 *           4. The path type (nr_path_type_t above).
 *           5. describes if it is OK to overwrite the existing transaction name at the same priority level.
 *
...
extern nr_status_t nr_txn_set_path (
  const char         *whence,
  nrtxn_t            *txn,
  const char         *path,
 ...
 */
static inline void
nr_txn_set_time (nrtxn_t *txn, nrtxntime_t *t)
{
  if (nrunlikely (0 == t)) {
    return;
  }
 ...
 */
extern nr_status_t nr_txn_set_stop_time (nrtxn_t *txn, const nrtxntime_t *start, nrtxntime_t *stop);

static inline void
nr_txn_copy_time (const nrtxntime_t *src, nrtxntime_t *dest)
{
 ...
 */
extern void nr_txn_save_trace_node (
  nrtxn_t              *txn,
 ...
 */
#define NR_TXN_HIGH_SECURITY_ERROR_MESSAGE "Message removed by New Relic high_security setting"
extern void nr_txn_record_error (nrtxn_t *txn, int priority, const char *errmsg,
 ...
 */
extern nr_status_t nr_txn_add_user_custom_parameter (nrtxn_t *txn, const char *key, const nrobj_t *value);
 ...
 */
extern void nr_txn_add_request_parameter (nrtxn_t *txn, const char *key, const char *value, int legacy_enable);
 ...
 */
extern void nr_txn_set_request_referer (nrtxn_t *txn, const char *request_referer);
extern void nr_txn_set_request_content_length (nrtxn_t *txn, const char *content_length);
...
extern void nr_txn_set_string_attribute (nrtxn_t *txn, const nr_txn_attribute_t *attribute, const char *value);
extern void nr_txn_set_long_attribute (nrtxn_t *txn, const nr_txn_attribute_t *attribute, long value);
 ...
 */
extern void nr_txn_record_custom_event (nrtxn_t *txn, const char *type, const nrobj_t *params);
 ...
 */
extern nr_status_t nr_txn_add_custom_metric (nrtxn_t *txn, const char *name, double value_ms);
 ...
 */
extern bool
nr_txn_is_current_path_named (const nrtxn_t *txn, const char *name);
