...
nr_segment_terms_t *
nr_segment_terms_create (int size)
{
  ...
  terms->rules = (nr_segment_terms_rule_t **) nr_calloc (size, sizeof (nr_segment_terms_rule_t));
  terms->capacity = size;
  terms->size = 0;
...
nr_segment_terms_t *
nr_segment_terms_create_from_obj (const nrobj_t *obj)
{
  int i;
  int num_terms;
  ...
  for (i = 1; i <= num_terms; i++) {
    if (NR_FAILURE == nr_segment_terms_add_from_obj (terms, nro_get_array_hash (obj, i, NULL))) {
...
void
nr_segment_terms_destroy (nr_segment_terms_t **terms_ptr)
{
  nr_segment_terms_t *terms;
  int i;
...
nr_status_t
nr_segment_terms_add (nr_segment_terms_t *segment_terms, const char *prefix,
                      const nrobj_t *terms)
{
...
nr_status_t
nr_segment_terms_add_from_obj (nr_segment_terms_t *segment_terms,
                               const nrobj_t *rule)
{
  const char *prefix;
  const nrobj_t *terms;
...
char *
nr_segment_terms_apply (const nr_segment_terms_t *segment_terms,
                        const char *name)
{
  char *result = NULL;
  int i;
...
nr_segment_terms_rule_create (const char *prefix, const nrobj_t *terms)
{
  char *regex = NULL;
   ...
   */
  rule->re = nr_regex_create (regex,
                              NR_REGEX_ANCHORED | NR_REGEX_CASELESS,
                              1);
  if (NULL == rule->re) {
    nr_segment_terms_rule_destroy (&rule);
    goto end;
...
void
nr_segment_terms_rule_destroy (nr_segment_terms_rule_t **rule_ptr)
{
  nr_segment_terms_rule_t *rule;
...
char *
nr_segment_terms_rule_apply (const nr_segment_terms_rule_t *rule,
                             const char *name)
{
  nrbuf_t *buf;
  int i;
  ...
  /*
   * The spec describes the algorithm that we need to apply here:
   * https://newrelic.atlassian.net/wiki/display/eng/Language+agent+transaction+segment+terms+rules#Languageagenttransactionsegmenttermsrules-Applicationoftherules
   *
...
char *
nr_segment_terms_rule_build_regex (const nrobj_t *terms)
{
  nrbuf_t *buf;
  char *regex;
  int regex_len;
