...
static void
test_default_port_host_and_socket (void)
{
  char *host = NULL;
  char *port = NULL;
  ...
  putenv ("PGUSER=uhura");
  database_name = nr_postgres_default_database_name ();
  tlib_pass_if_str_equal ("user is default database name", "uhura", database_name);
  nr_free (database_name);

  putenv ("PGDATABASE=scotty");
  database_name = nr_postgres_default_database_name ();
  tlib_pass_if_str_equal ("dbname has precedence over user", "scotty", database_name);
  nr_free (database_name);
...
static void
test_conn_info_early_return (void) {
...
static void
test_conn_info (const char *conn_info, const char *expected_host,
  const char *expected_port_path_or_id, const char *expected_database_name)
{
  char *host = NULL;
  char *port_path_or_id = NULL;
  char *database_name = NULL;

  nr_postgres_parse_conn_info (conn_info, &host, &port_path_or_id, &database_name);
  ...
  tlib_pass_if_str_equal ("correct database_name", expected_database_name, database_name);
...
test_parse_conn_info (void)
{
   ...
   */
  test_conn_info ("host = localhost port = 5432 user = scotty", "localhost", DEFAULT_PORT, "scotty");
  ...
  test_conn_info ("user=uhura dbname=mccoy", "localhost", DEFAULT_SOCKET, "mccoy");
  test_conn_info ("host=localhost user=chekov", "localhost", DEFAULT_PORT, "chekov");

  /*
   * Test: Non-localhost
   */
  test_conn_info ("hostaddr=12.34.56.78", "12.34.56.78", DEFAULT_PORT, "");
  test_conn_info ("host=spock port=5432 user=kirk password=enterprise", "spock", DEFAULT_PORT, "kirk");
}
