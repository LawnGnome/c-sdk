...
static int
nr_php_stack_iterator (zval *frame, nrobj_t *arr,
                       zend_hash_key *key NRUNUSED TSRMLS_DC)
{
  ...
  if (nr_php_is_zval_valid_integer (line)) {
    ...
    line_str[0] = '\0';
    line_str_len = snprintf (line_str, sizeof (line_str),
                             " (%ld)", Z_LVAL_P (line));
...
static char *
nr_php_backtrace_to_json_internal (zval *trace TSRMLS_DC)
{
  nrobj_t *arr;
  char *json;
...
char *
nr_php_backtrace_to_json (zval *itrace TSRMLS_DC)
{
  zval *trace;
  char *json;
...
#ifdef PHP7
  /*
   * There's a bug in pre-release versions of PHP 7.0.0 (up to and including
   * RC1) whereby zend_fetch_debug_backtrace() can segfault if the current
   * execute data is NULL. I've PRed a fix for that, but even if that lands,
   * we'll have to check here as well to avoid crashes.
   *
   * TODO(aharvey): at some point around January 2017, we may as well remove
   * this, because nobody in their right mind will still be running a
   * pre-release version of PHP 7.0.
   */
  if (NULL == EG (current_execute_data)) {
    /*
     * This effectively emulates the behaviour of zend_fetch_debug_backtrace()
     * when the execute data is NULL: the trace is set to an empty array.
     */
    array_init (trace);
    return trace;
  }
...
char *
nr_php_backtrace_callback (void)
{
  TSRMLS_FETCH ();
...
typedef struct _nr_php_frame_info_t {
  ...
  const char *func_name;  /* function or operator name (e.g. "foo", "eval", "include") */
...
static int
nr_php_is_include_or_eval (zend_execute_data *ex)
{
...
nr_php_backtrace_get_call_site (zend_execute_data *ex)
{
  zend_execute_data *prev;
...
static void
nr_php_frame_info (nr_php_frame_info_t *info, zend_execute_data *ex)
{
  ...
  if (callsite && callsite->func && ZEND_USER_CODE (callsite->func->common.type)) {
   ...
   */
  if ((ZEND_USER_FUNCTION == func->type) && (func->common.fn_flags & ZEND_ACC_CLOSURE)) {
  ...
  if (nr_php_is_include_or_eval (ex)) {
    switch (ex->prev_execute_data->opline->extended_value) {
      case ZEND_EVAL:         info->func_name = "eval";         break;
      case ZEND_INCLUDE:      info->func_name = "include";      break;
      case ZEND_REQUIRE:      info->func_name = "require";      break;
      case ZEND_INCLUDE_ONCE: info->func_name = "include_once"; break;
      case ZEND_REQUIRE_ONCE: info->func_name = "require_once"; break;
      default:                info->func_name = "ZEND_INCLUDE_OR_EVAL"; break;
    }
  }
}
...
static void
nr_php_frame_info (nr_php_frame_info_t *info, zend_execute_data *ex TSRMLS_DC)
{
  zend_function *func;
...
void
nr_php_backtrace_fd (int fd, int limit TSRMLS_DC)
{
