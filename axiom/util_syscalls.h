 ...
 * 1) Declare the wrapper function below with the same prototype and "nr_" prefix.
 ...
 */
extern int nr_accept (int sock, struct sockaddr *address, socklen_t *address_len_p);
extern int nr_access (const char *path, int amode);
extern int nr_bind (int sock, const struct sockaddr *address, socklen_t address_len);
extern int nr_close (int fd);
extern int nr_connect (int sock, const struct sockaddr *address, socklen_t address_len);
...
extern int nr_setsockopt (int sock, int level, int option_name, const void *option_value, socklen_t option_len);
