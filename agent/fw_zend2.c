...
NR_PHP_WRAPPER (nr_zend2_name_the_wt)
{
  ...
  if (0 == nr_php_object_has_method (this_var, "getMatchedRouteName" TSRMLS_CC)) {
    nrl_verbosedebug (NRL_INSTRUMENT,
      "%s: this_var doesn't have getMatchedRouteName.", __func__);
    NR_PHP_WRAPPER_CALL;
    goto leave;
  }
  ...
  if (nr_php_is_zval_valid_string (path)) {
    ...
    nr_txn_set_path ("Zend2", NRPRG (txn), path_term, NR_PATH_TYPE_ACTION, NR_OK_TO_OVERWRITE);
    nr_free (path_term);
  } else {
    nrl_verbosedebug (NRL_INSTRUMENT,
      "%s: couldn't getMatchedRouteName on setter hook.", __func__);
  }
...
void
nr_fw_zend2_enable (TSRMLS_D)
{
  nr_txn_set_path ("Zend2", NRPRG (txn), "unknown", NR_PATH_TYPE_ACTION, NR_OK_TO_OVERWRITE);
