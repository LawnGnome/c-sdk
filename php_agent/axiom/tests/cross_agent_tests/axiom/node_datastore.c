...
static char* create_metrics(nrtxn_t* txn,
                            nrtime_t duration,
                            const char* product,
...
static void handle_instance(nrtxn_t* txn,
                            nrtime_t duration,
  ...
  if (instance) {
    if (txn->options.instance_reporting_enabled) {
      ...
      nro_set_hash_string(data_hash, "port_path_or_id",
...
void nr_txn_end_node_datastore(nrtxn_t* txn,
  ...
  if (nr_datastore_is_sql(params->datastore.type)) {
    ...
    if ((NULL == params->collection) || (NULL == params->operation)) {
      collection_allocated = nr_txn_node_sql_get_operation_and_table(
   ...
   *
   * See:
   * https://source.datanerd.us/agents/agent-specs/blob/master/Datastore-Metrics-PORTED.md
   */
  scoped_metric
      = create_metrics(txn, duration, datastore_string, collection, operation);
  if (return_scoped_metric) {
  ...
  if (params->callbacks.backtrace
      && nr_txn_node_datastore_stack_worthy(txn, duration)) {
    ...
    switch (nr_txn_sql_recording_level(txn)) {
      ...
      case NR_SQL_OBFUSCATED:
        ...
        if (params->sql.input_query) {
          ...
          input_query_allocated.name = input_query_name
              = nr_strdup(params->sql.input_query->name);
...
int nr_txn_node_potential_explain_plan(const nrtxn_t* txn, nrtime_t duration) {
  if (NULL == txn) {
    return 0;
  }
...
int nr_txn_node_potential_slowsql(const nrtxn_t* txn, nrtime_t duration) {
  if (0 == txn) {
    return 0;
  }
...
char* nr_txn_node_sql_get_operation_and_table(
    const nrtxn_t* txn,
...
int nr_txn_node_datastore_stack_worthy(const nrtxn_t* txn, nrtime_t duration) {
  if (0 == txn) {
    return 0;
  }
