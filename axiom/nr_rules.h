...
#define NR_RULE_REPLACE_ALL     0x00000004      /* Replace entire metric with substitution */
#define NR_RULE_TERMINATE       0x00000008      /* Terminate the rule chain if this rule applied */
#define NR_RULE_HAS_ALTS        0x00000010      /* Does this rule have | alternatives */
#define NR_RULE_HAS_CAPTURES    0x00000020      /* Does this rule have \0-9 captures */

/*
 * If RPM does not specify an eval_order for a rule, this one should be used.
 */
 ...
 */
extern nr_status_t nr_rules_add (nrrules_t *rules, uint32_t flags, int order, const char *match, const char *repl);
...
extern nr_rules_result_t nr_rules_apply (const nrrules_t *rules, const char *name, char **new_name);
