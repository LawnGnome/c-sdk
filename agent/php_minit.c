...
static void
php_newrelic_init_globals (zend_newrelic_globals *nrg)
{
  if (nrunlikely (0 == nrg)) {
    return;
  }
 ...
 */
#ifdef TAGS
void zm_globals_ctor_newrelic (zend_newrelic_globals *newrelic_globals);  /* ctags landing pad only */
#endif
 ...
 */
#ifdef TAGS
void zm_globals_dtor_newrelic (zend_newrelic_globals *newrelic_globals);  /* ctags landing pad only */
#endif
 ...
 */
static nr_daemon_startup_mode_t
nr_php_get_daemon_startup_mode (void)
{
  /*
  ...
  if (3 == NR_PHP_PROCESS_GLOBALS (no_daemon_launch)) {
     ...
     * Use this if you are configuring the daemon via newrelic.cfg and starting it outside of the agent.
     */
    return NR_DAEMON_STARTUP_INIT;
  }
  ...
  if (NR_PHP_PROCESS_GLOBALS (cli)) {
    /*
     * If command line version of PHP was used, the agent will not start the daemon.
     */
    ...
    /*
     * If non-command line version of PHP was used (for example Apache or php-fpm)
     * then the agent will not start the daemon (only the command line usage will start the daemon).
     */
 ...
 */
static nr_status_t
nr_php_check_for_threaded_mpm (TSRMLS_D)
{
  if ((0 != NR_PHP_PROCESS_GLOBALS (is_apache)) && (0 != NR_PHP_PROCESS_GLOBALS (apache_threaded))) {
    NR_PHP_PROCESS_GLOBALS (mpm_bad) = 1;
    php_error_docref (NULL TSRMLS_CC, E_WARNING,
      "You attempted to load the New Relic module, but you appear to be using a "
...
static char *
nr_php_check_for_upgrade_license_key (void)
{
...
static char *
nr_php_get_agent_specific_info (void)
{
  ...
  if (NR_PHP_PROCESS_GLOBALS (is_apache)) {
    snprintf (web_server_info, sizeof (web_server_info), "apache='%d.%d.%d%s' mpm=%s",
      ...
      (1 == NR_PHP_PROCESS_GLOBALS (apache_threaded)) ? "threaded" : "prefork");
  }

  if (NR_PHP_PROCESS_GLOBALS (php_version) && (0 != NR_PHP_PROCESS_GLOBALS (php_version)[0])) {
  ...
  buf[0] = '\0';
  snprintf (buf, sizeof (buf), " php='%s' zts=%s sapi='%s' %s",
    php_version,
    zend_type,
    sapi_module.name,
    web_server_info);
...
static void
nr_php_populate_apache_process_globals (void)
{
  void *handle;
  void (*mpmptr) (int, int *);
  ...
  if (0 == verptr) {
    verptr = (void (*) (nr_ap_version_t *))dlsym (handle, "_ap_get_server_revision");
  }
...
static char *
nr_php_get_php_version_number (TSRMLS_D)
{
  char *version = 0;
...
PHP_MINIT_FUNCTION (newrelic)
{
  ...
  NR_PHP_PROCESS_GLOBALS (upgrade_license_key) = nr_php_check_for_upgrade_license_key ();
   ...
   * If we detect that we're running in that environment, just disable the agent.
...
disbad:
  ...
  if (0 == nr_php_use_license (0 TSRMLS_CC)) {
    nrl_warning (NRL_INIT,
      ...
      "in the newrelic.ini file, or ensure that a valid license is provided on a "
      "per-virtual host or per-directory basis.");
  }

  /*
   * Attempt to connect to the daemon here.  Note that we do this no matter
   * the startup mode.  This delay allows CLI processes enough time to connect.
   * Since they handle a single request, they cannot wait through a request for
   * the connection to finish.
   */
#define NR_PHP_MINIT_DAEMON_CONNECTION_TIMEOUT_MS 10
  daemon_connect_succeeded = nr_agent_try_daemon_connect (NR_PHP_MINIT_DAEMON_CONNECTION_TIMEOUT_MS);
  ...
  if (0 == daemon_connect_succeeded) {
    if (NR_DAEMON_STARTUP_AGENT == daemon_startup_mode) {
      ...
      if (NR_PHP_PROCESS_GLOBALS (high_security)) {
        /* High security mode requires TLS */
        daemon_args.tls_enabled = 1;
      } else {
        daemon_args.tls_enabled = NR_PHP_PROCESS_GLOBALS (use_https);
      }
...
#define NR_PHP_DAEMON_PIDFILE "newrelic-daemon.pid"
      ...
      if (0 == nr_php_ini_setting_is_set_by_user ("newrelic.daemon.pidfile")) {
        if (0 == nr_access ("/var/run", W_OK)) {
          ...
          nrl_warning (NRL_INIT, "unable to find suitable pidfile location, using none");
      ...
      daemon_args.integration_mode = NR_PHP_PROCESS_GLOBALS (daemon_special_integration);
      daemon_args.debug_http = NR_PHP_PROCESS_GLOBALS (daemon_special_curl_verbose);
      daemon_args.utilization = NR_PHP_PROCESS_GLOBALS (utilization);

      daemon_pid = nr_spawn_daemon (NR_PHP_PROCESS_GLOBALS (daemon), &daemon_args);
      ...
      nrl_warning (NRL_DAEMON,
        "failed to connect to the newrelic-daemon.  The agent expects a daemon "
        "to be started externally. "
        "Please refer to: https://newrelic.com/docs/php/newrelic-daemon-startup-modes#daemon-external");
    }
  }
   ...
   * ALL extensions have been loaded. Here during the MINIT phase there may still
   * be many other extensions to come and some, like XDEBUG, are not very well
   * behaved citizens and we need to ensure certain initialization tasks are
   * run only once the PHP VM engine is ticking over fully.
   */
  ...
  if (NR_PHP_PROCESS_GLOBALS (instrument_internal)) {
    nrl_info (NRL_AGENT, "enabling internal function instrumentation (this might be slow!)");
     ...
     * executor used.
     */
    NR_PHP_PROCESS_GLOBALS (orig_execute_internal) = zend_execute_internal ? zend_execute_internal : execute_internal;
...
static void
nr_php_fatal_signal_handler (int sig)
{
  int fd;
...
nr_status_t
nr_php_late_initialization (TSRMLS_D)
{
  ...
  if (0 == zend_get_extension ("Xdebug")) {
    ...
    nrl_warning (NRL_INIT, "the Xdebug extension prevents the New Relic agent from gathering errors. No errors will be recorded.");
  }
