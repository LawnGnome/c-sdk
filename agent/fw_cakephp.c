...
nr_cakephp_special_1 (const zend_op_array *op_array TSRMLS_DC)
{
  (void)op_array;
  NR_UNUSED_TSRMLS;
...
nr_framework_classification_t
nr_cakephp_special_2 (const zend_op_array *op_array TSRMLS_DC)
{
  (void)op_array;
  NR_UNUSED_TSRMLS;
...
NR_PHP_WRAPPER (nr_cakephp_name_the_wt_pre20)
{
  ...
  if (IS_ARRAY != Z_TYPE_P (params)) {
    nrl_verbosedebug (NRL_FRAMEWORK, "CakePHP: component params is not an array");
    goto end;
  }

  czval = nr_php_get_zval_object_property (params, "controller" TSRMLS_CC);
  if (0 == czval) {
    nrl_verbosedebug (NRL_FRAMEWORK, "CakePHP: no params['controller'] in component");
  } else {
  ...
  if (0 == azval) {
    nrl_verbosedebug (NRL_FRAMEWORK, "CakePHP: no params['action'] in component");
  } else {
  ...
  if ((0 == clen) && (0 == alen)) {
    nrl_verbosedebug (NRL_FRAMEWORK, "CakePHP: nothing to call the transaction (yet?)");
    goto end;
  }
  ...
  nr_txn_set_path ("CakePHP", NRPRG (txn), name, NR_PATH_TYPE_ACTION, NR_NOT_OK_TO_OVERWRITE);
...
NR_PHP_WRAPPER (nr_cakephp_name_the_wt_2)
{
  ...
  if (!nr_php_is_zval_valid_string (czval)) {
    ...
    clen = Z_STRLEN_P (czval);
    if (clen <= 0) {
      nrl_verbosedebug (NRL_FRAMEWORK, "CakePHP: controller name string is not long enough");
    } else {
  ...
  if (0 == azval) {
    ...
    if (!nr_php_is_zval_valid_string (azval)) {
      nrl_verbosedebug (NRL_FRAMEWORK, "CakePHP: no string-valued params['action'] in request");
    } else {
      alen = Z_STRLEN_P (azval);
      action = (char *)nr_alloca (alen + 1);
      nr_strxcpy (action, Z_STRVAL_P (azval), alen);
 ...
 * CakePHP 1.2, 1.3
 *
 * Dispatch::cakeError will be called if there is a problem during dispatch (action or controller not found).
 */
 ...
 * CakePHP 2.0+
 *
 * If the action or controller is not found during the dispatch process, the appropriate Exception will be
 * created and thrown. We wrap the CakeException constructor instead of the Exception handler, since CakePHP
 * allows for the handler to be completely replaced.
 */
