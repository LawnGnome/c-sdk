...
nrtime_t                                                 \
FN_NAME (const nrmetric_t *metric)                       \
{                                                        \
  if (0 == metric) {                                     \
    return 0;                                            \
  }                                                      \
...
void
nrm_table_destroy (nrmtable_t **table_p)
{
  nrmtable_t *table;
...
int
nrm_is_apdex (const nrmetric_t *metric)
{
  if (metric) {
...
int
nrm_is_forced (const nrmetric_t *metric)
{
  if (metric) {
...
static uint32_t
nrm_hash (const char *name)
{
  return nr_mkhash (name, 0);
}

nrmetric_t *
nrm_find_internal (nrmtable_t *table, const char *name, uint32_t hash)
{
  int i;
  ...
  while (-1 != i) {
    ...
    if (hash == metric->hash) {
      const char *metric_name = nr_string_get (table->strpool, metric->name_index);
...
nrmetric_t *
nrm_find (nrmtable_t *table, const char *name)
{
 ...
 *        exist within the function already.  The caller should therefore
 *        first use nrm_find.
 */
nrmetric_t *
nrm_create (nrmtable_t *table, const char *name, uint32_t hash)
{
  int i;
  ...
  if (table->number >= table->allocated) {
    ...
    table->metrics = (nrmetric_t *)nr_realloc (table->metrics, table->allocated * sizeof (nrmetric_t));
  }
...
const nrmetric_t *
nrm_get_metric (const nrmtable_t *table, int i)
{
...
static int
nrm_is_full (const nrmtable_t *table)
{
...
static nrmetric_t *
nrm_find_or_create (int force, nrmtable_t *table, const char *name)
{
  nrmetric_t *metric;
  uint32_t hash = nrm_hash (name);
...
void
nrm_add_internal (int force, nrmtable_t *table, const char *name,
                  nrtime_t count, nrtime_t total, nrtime_t exclusive, nrtime_t min,
                  nrtime_t max, nrtime_t sum_of_squares)
{
...
void
nrm_add_ex (nrmtable_t *table, const char *name, nrtime_t duration, nrtime_t exclusive)
{
...
void
nrm_force_add_ex (nrmtable_t *table, const char *name, nrtime_t duration, nrtime_t exclusive)
{
...
void
nrm_add (nrmtable_t *table, const char *name, nrtime_t duration)
{
...
void
nrm_force_add (nrmtable_t *table, const char *name, nrtime_t duration)
{
...
void
nrm_add_apdex_internal (int force, nrmtable_t *table, const char *name,
                        nrtime_t satisfying, nrtime_t tolerating, nrtime_t failing,
                        nrtime_t min_apdex, nrtime_t max_apdex)
{
  nrmetric_t *metric = nrm_find_or_create (force, table, name);
...
void
nrm_add_apdex (nrmtable_t *table, const char *name,
               nrtime_t satisfying, nrtime_t tolerating, nrtime_t failing, nrtime_t apdex)
{
  nrm_add_apdex_internal (0, table, name, satisfying, tolerating, failing, apdex, apdex);
}

void
nrm_force_add_apdex (nrmtable_t *table, const char *name,
                     nrtime_t satisfying, nrtime_t tolerating, nrtime_t failing, nrtime_t apdex)
{
  nrm_add_apdex_internal (1, table, name, satisfying, tolerating, failing, apdex, apdex);
}

int
nrm_table_size (const nrmtable_t *tp)
{
  if (0 == tp) {
    return 0;
  }
...
const char *
nrm_get_name (const nrmtable_t *table, const nrmetric_t *met)
{
...
static void
nr_metric_data_as_json_to_buffer (nrbuf_t *buf, const nrmetric_t *met)
{
  char tmp[512];
  int sl;
  ...
  if (met->flags & MET_IS_APDEX) {
     ...
     */
    nr_double_to_str (buf_min_apdex_seconds, sizeof (buf_min_apdex_seconds), min_apdex_seconds);
    nr_double_to_str (buf_max_apdex_seconds, sizeof (buf_max_apdex_seconds), max_apdex_seconds);
    sl = snprintf (tmp, sizeof (tmp), "[" NR_TIME_FMT "," NR_TIME_FMT "," NR_TIME_FMT ",%s,%s,0]",
      satisfying, tolerating, failing, buf_min_apdex_seconds, buf_max_apdex_seconds);
  } else {
    nrtime_t count                = met->mdata[NRM_COUNT];
    double total_seconds          = (double)met->mdata[NRM_TOTAL]      / NR_TIME_DIVISOR_D;
    double exclusive_seconds      = (double)met->mdata[NRM_EXCLUSIVE]  / NR_TIME_DIVISOR_D;
    ...
    double sum_of_squares_seconds = (double)met->mdata[NRM_SUMSQUARES] / NR_TIME_DIVISOR_D_SQUARE;
    ...
    nr_double_to_str (buf_total_seconds, sizeof (buf_total_seconds), total_seconds);
    nr_double_to_str (buf_exclusive_seconds, sizeof (buf_exclusive_seconds), exclusive_seconds);
    ...
    nr_double_to_str (buf_sum_of_squares_seconds, sizeof (buf_sum_of_squares_seconds), sum_of_squares_seconds);
    sl = snprintf (tmp, sizeof (tmp), "[" NR_TIME_FMT ",%s,%s,%s,%s,%s]", count,
      buf_total_seconds,
      buf_exclusive_seconds,
      buf_min_seconds,
      buf_max_seconds,
      buf_sum_of_squares_seconds);
  }
...
nr_status_t
nrm_table_validate (const nrmtable_t *table)
{
  int i;
  int used;
  ...
  if (used) {
    ...
    for (i = 0; i < used; i++) {
      ...
      const char *name_string = nr_string_get (table->strpool, metric->name_index);
...
void
nrm_duplicate_metric (nrmtable_t *table, const char *current_name, const char *new_name)
{
  int force;
...
static void
nr_metric_to_daemon_json_buffer (nrbuf_t *buf, const nrmetric_t *metric, const nrmtable_t *table)
{
...
char *
nr_metric_table_to_daemon_json (const nrmtable_t *table)
{
  int i;
  nrbuf_t *buf;
  char *json;
