...
test_range_bad_params (void)
{
  ...
  tlib_pass_if_ulong_equal ("negative max (max too large)", 0, nr_random_range (rnd, -1));
  tlib_pass_if_ulong_equal ("zero max", 0, nr_random_range (rnd, 0));
  tlib_pass_if_ulong_equal ("one max", 0, nr_random_range (rnd, 1));
  tlib_pass_if_ulong_equal ("max too large", 0, nr_random_range (rnd, NR_RANDOM_MAX_EXCLUSIVE_LIMIT + 1));
...
test_range (void)
{
   ...
   * Note that this tests that we can get 0 and 9 (the min and max return value).
   */
  ...
  /* Test the max. */
  tlib_pass_if_ulong_equal ("max max_exclusive", 1391330424, nr_random_range (rnd, NR_RANDOM_MAX_EXCLUSIVE_LIMIT));
  tlib_pass_if_ulong_equal ("max max_exclusive", 58941426,   nr_random_range (rnd, NR_RANDOM_MAX_EXCLUSIVE_LIMIT));
  tlib_pass_if_ulong_equal ("max max_exclusive", 2045540820, nr_random_range (rnd, NR_RANDOM_MAX_EXCLUSIVE_LIMIT));
