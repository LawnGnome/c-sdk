...
#
# Bail out if we don't like the operating system we are on.
#
# If New Relic doesn't release anything for the O/S we are on,
# the only reason why we would have gotten here is if the customer
# unpacked an tarball for a supported O/S on an unsupported O/S,
# and then attempt to run this shell script.
#
...
#
# First things first, we check to see if the user has run this script before
# and if so, we source in their previous settings so we get them as defaults.
#
#[ -f "${HOME}/.newrelic.cfg" ] && . "${HOME}/.newrelic.cfg"
...
# the absolute path without doing an exec. Sigh.
#
...
for pmv in "20090626" "20100525" "20121212" "20131226" "20151012" "20160303"; do
...
#   which is suitable compatible. Both bash and zsh are known to work. This
...
#  If set to any non-empty value, copy the agent don't create a symlink.
#

#
# This script will put the log file and the sysinfo file into a tar file so
# it is ready to send to NR support if anything goes pear shaped.
cleanup() {
  (cd /tmp
...
if [ -z "${ispkg}" ]; then
  # If they have yum installed, list the installed packages
...
# package the PATH is frequently restricted. We therefor want to add a few
...
#
# OpenSolaris on Joyent is problematic. Many customers have a read-only /usr
# file system. So on that platform we create (if required) and always install
# to /opt/newrelic/bin.
#
set_daemon_location() {
  daemonloc=
...
#   what we detect.
#
# Original comments before code was moved (they are still relevant):
#
# Gather the PHP specific info for this installation. We care about 4 things:
...
# life was that easy. Things are further complicated by the fact that on
...
# too. We can easily retrieve the scan directory from PHP, but the problem is
...
# will obviously return the CLI location. So by using both methods we can
# make a very educated guess about these two directories, and check to see
# if they contain a "conf.d" directory that is a link to a shared location.
...
"
  ...
  if [ -n "${havebin}" ]; then
    # Prevent daemon spawn below. Either by disabling ini parsing or by
...
# in case this is an upgrade. This is the main reason we do not list the config
# files in things like the RPM spec file, so that they don't get removed. In
# cases where a user really wants to remove New Relic from their system they
# can be instructed by support to run this script with the "purge" command and
