...
const char *
nr_error_get_message (const nr_error_t *error)
{
  if (NULL == error) {
    return NULL;
  }
...
const char *
nr_error_get_klass (const nr_error_t *error)
{
  if (NULL == error) {
    return NULL;
  }
...
nrtime_t
nr_error_get_time (const nr_error_t *error)
{
  if (NULL == error) {
    return 0;
  }
...
int
nr_error_priority (const nr_error_t *error)
{
  if (0 == error) {
    return 0;
  }
...
void
nr_error_destroy (nr_error_t **error_ptr)
{
  nr_error_t *error;
...
static nrobj_t *
nr_error_params_to_object (const char *stacktrace_json,
...
char *
nr_error_to_daemon_json (
  const nr_error_t *error,
  const char *txn_name,
  const nrobj_t *agent_attributes,
  const nrobj_t *user_attributes,
  const nrobj_t *intrinsics,
  const char *request_uri)
{
  ...
  params = nr_error_params_to_object (error->stacktrace_json,
    agent_attributes, user_attributes, intrinsics, request_uri);
