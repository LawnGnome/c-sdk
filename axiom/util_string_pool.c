...
typedef struct _nrstrpool_t {
  ...
  char **strings;          /* Pointers to stored strings. Separated from entries to minimize buffer use */
  nrstable_t *tables;      /* Linked list of tables containing the strings */
} nrstrpool_t;

int
nr_string_len (const nrstrpool_t *pool, int idx)
{
...
uint32_t
nr_string_hash (const nrstrpool_t *pool, int idx)
{
  if (nrunlikely ((0 == pool) || (idx <= 0) || (idx > pool->num_entries))) {
    return 0;
  }
...
const char *
nr_string_get (const nrstrpool_t *pool, int idx)
{
  if (nrunlikely ((0 == pool) || (idx <= 0) || (idx > pool->num_entries))) {
    return 0;
  }
...
void
nr_string_pool_destroy (nrpool_t **poolptr)
{
  nrstrpool_t *pool;
  nrstable_t *table;
...
static int
nr_string_find_internal (const nrstrpool_t *pool, const char *string, uint32_t hash, int length)
{
  int idx = 1;
...
int
nr_string_find (const nrstrpool_t *pool, const char *string)
{
  int length = 0;
...
int
nr_string_find_with_hash (const nrstrpool_t *pool, const char *string, uint32_t hash)
{
...
int
nr_string_find_with_hash_length (const nrstrpool_t *pool, const char *string, uint32_t hash, int length)
{
  return nr_string_find_internal (pool, string, hash, length);
}
 ...
 */
static int
nr_string_add_internal (nrstrpool_t *pool, const char *string, uint32_t hash, int length)
{
  int idx;
  int new_string;
  nrstable_t *table;
  ...
  if (pool->size == pool->num_entries) {
    ...
    pool->entries = (nrstring_t *)nr_realloc (pool->entries, pool->size * sizeof (nrstring_t));
    pool->strings = (char **)nr_realloc (pool->strings, pool->size * sizeof (char *));
  }
  ...
  table = pool->tables;
  if ((0 == table) || ((table->num_bytes_allocated - table->num_bytes_used) < (length + 1))) {
    nrstable_t *t;
    int required = length + 1;
    int size = (required > NR_STRPOOL_TABLE_SIZE) ? required : NR_STRPOOL_TABLE_SIZE;
...
int
nr_string_add (nrstrpool_t *pool, const char *string)
{
  int length = 0;
  uint32_t hash = nr_mkhash (string, &length);
...
int
nr_string_add_with_hash (nrpool_t *pool, const char *string, uint32_t hash)
{
  int length = nr_strlen (string);
...
int
nr_string_add_with_hash_length (nrpool_t *pool, const char *string, uint32_t hash, int length)
{
  return nr_string_add_internal (pool, string, hash, length);
}

char *
nr_string_pool_to_json (const nrpool_t *pool)
{
  int i;
  char *js;
  nrbuf_t *buf;
...
void
nr_string_pool_apply (const nrpool_t *pool,
