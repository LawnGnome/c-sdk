 ...
 */
void
nr_php_initialize_samplers (void)
{
...
get_physical_memory_used (void)
{
  ...
  nrl_verbosedebug (NRL_MISC, "/proc/self read: %lld of %d-byte pages", (long long int)rv, page_size);
  return (rv * page_size);
}
...
get_physical_memory_used (void)
{
  ...
  if (-1 == ret) {
    int err = errno;
    nrl_verbosedebug (NRL_MISC, "getrusage() failed with %d (%.16s)", err, nr_errno (err));
    return 0;
  }
...
void
nr_php_resource_usage_sampler_start (TSRMLS_D)
{
  int ret;
...
void
nr_php_resource_usage_sampler_end (TSRMLS_D)
{
  int ret;
  int num_cpus;
  ...
  if (nrunlikely (elapsed_time <= 0)) {
    nrl_verbosedebug (NRL_MISC, "elapsed time is not positive - no CPU sampler data available this cycle");
    return;
  }
  ...
  if (diff < 0) {
    nrl_verbosedebug (NRL_MISC, "user time difference is negative - no CPU sampler data available this cycle");
    return;
  }
  ...
  nrm_force_add_ex (NRTXN (unscoped_metrics), "CPU/User/Utilization", fraction_usage, 0);
}
