 ...
 *
 * This JSON format matches the format expected by the collector.
 */
...
nr_analytics_event_create_from_string (const char *str)
{
  return (nr_analytics_event_t *)nr_strdup (str);
}

static nr_analytics_event_t *
nr_analytics_event_duplicate (const nr_analytics_event_t *event)
{
  if (0 == event) {
    return 0;
  }
...
void
nr_analytics_event_destroy (nr_analytics_event_t **event_ptr)
{
  nr_realfree ((void **)event_ptr);
}

struct _nr_analytics_events_t {
  int events_allocated;              /* Maximum number of events to store in this data structure. */
...
int
nr_analytics_events_number_seen (const nr_analytics_events_t *events)
{
  if (0 == events) {
    return 0;
  }
...
int
nr_analytics_events_number_saved (const nr_analytics_events_t *events)
{
  if (0 == events) {
    return 0;
  }
...
nr_analytics_events_create (int max_events)
{
  ...
  events->events = (nr_analytics_event_t **)nr_zalloc (max_events * sizeof (nr_analytics_event_t *));
...
void
nr_analytics_events_destroy (nr_analytics_events_t **events_ptr)
{
  int i;
  nr_analytics_events_t *events;
  ...
  nr_random_t *rnd)
{
  ...
  if (events->events_used < events->events_allocated) {
    ...
    /*
     * If the reservoir is full, we sample using the following sampling algorithm:
