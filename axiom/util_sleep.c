 ...
 */
#define NR_EARLIEST_ACCEPTABLE_UNIX_TIME ((nrtime_t)(946684800ULL)  * NR_TIME_DIVISOR) /* 2000/1/1 */
#define NR_LATEST_ACCEPTABLE_UNIX_TIME   ((nrtime_t)(2524629600ULL) * NR_TIME_DIVISOR) /* 2050/1/1 */

nrtime_t
nr_parse_unix_time (const char *str)
{
  double db;
  nrtime_t val;
  ...
  if ((val > NR_EARLIEST_ACCEPTABLE_UNIX_TIME) && (val < NR_LATEST_ACCEPTABLE_UNIX_TIME)) {
    return val;
  }
...
nr_msleep (int millis)
{
  ...
  if (EINTR == rv) {
    return (int)(remaining.tv_sec * 1000) + (int)(remaining.tv_nsec / (1000 * 1000));
  }
  return 0;
}
