...
nrobj_t* nr_distributed_trace_convert_payload_to_object(const char* payload,
  ...
  const nrobj_t* obj_payload_data;
  /*
   * TODO for Span Event MMF.
   *
   * No 'id' (span id) is set currently, as span events are not
   * supported yet.
   *
   * As soon as span events are supported, either span id or transaction
   * id are required. This should be checked here.
   */
...
char* nr_distributed_trace_payload_as_text(
  ...
  /* At least one of guid or transaction id must be present in a payload
   * for the payload to be valid.
   *
   * TODO for Span Event MMF.
   *
   * Add tests for this once span events are supported and guid is set
   * to the current span identifier.
   */
   ...
   * TODO for Span Event MMF.
   *
   * Set the "id" field in the payload to the span identifier as per the spec,
   * the id is the "current span identifier. Only sent if span events are
   * being created (span events enabled and the transaction is sampled)."
   * add_field_if_set(data, "id", ???);
   */
