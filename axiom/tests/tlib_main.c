...
sig_handler (int sig)
{
  ...
  printf ("%24s: %6d of %6d tests passed, and %d failed\n", progname, tlib_passcount,
    tlib_passcount + tlib_unexpected_failcount, tlib_unexpected_failcount);
  exit (1);
}
...
static void
consume_args (int argc, char * const argv[])
{
  int opt;
  ...
  while ((opt = getopt (argc, argv, "UEr:c:w:j:")) != -1) {
    switch (opt) {
    case 'U': /* Ignore unexpected failures when computing process return code. */
...
main (int argc, char *const argv[])
{
  ...
  test_main_parallel_driver (parallel_info.suggested_nthreads, parallel_info.state_size);

  if (0 == tlib_unexpected_failcount) {
    printf ("%30s: all %6d tests passed", progname, tlib_passcount);
    return_value = 0;
  } else {
    printf ("%30s: %6d of %6d tests passed, %6d failed",
      progname, tlib_passcount,
      tlib_passcount + tlib_unexpected_failcount,
      tlib_unexpected_failcount);
...
void
test_obj_as_json_fn (const char *testname, const nrobj_t *obj, const char *expected_json, const char *file, int line)
{
...
void *
tlib_getspecific (void)
{
  return pthread_getspecific (thread_id_key);
}

static void *
test_main_parallel_driver_helper (void *p)
{
  ...
  if (force_sequential) {
    ...
    for (o = 0; o < par_outer; o++) {
      for (i = 0; i < par_inner; i++) {
        rv = nrt_create (&tid[i], &attr, test_main_parallel_driver_helper, state[i]);
        tlib_pass_if_true ("thread create OK", NR_SUCCESS == rv, "rv=%d", (int)rv);
      }
