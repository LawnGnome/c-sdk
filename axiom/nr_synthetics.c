...
nr_synthetics_t *
nr_synthetics_create (const char *header)
{
  size_t i = 0;
  ...
  for (i = 0; i < (sizeof (nr_synthetics_parsers) / sizeof (nr_synthetics_parser_table_t)); i++) {
    if (version == nr_synthetics_parsers[i].version) {
      ...
      nrl_verbosedebug (NRL_TXN,
                        "%s: invalid synthetics header of version %d",
                        __func__, version);
      return NULL;
    }
  }
   ...
   */
  nrl_verbosedebug (NRL_TXN,
                    "%s: unknown synthetics version %d",
                    __func__, version);
...
nr_status_t
nr_synthetics_parse_v1 (const nrobj_t *synth_obj, nr_synthetics_t *out)
{
  ...
  if (5 != nro_getsize (synth_obj)) {
    nrl_verbosedebug (NRL_TXN,
                      "%s: invalid number of synthetics fields; expected 5, got %d",
...
void
nr_synthetics_destroy (nr_synthetics_t **synthetics_ptr)
{
...
int
nr_synthetics_version (const nr_synthetics_t *synthetics)
{
  if (NULL == synthetics) {
    return 0;
  }
...
int
nr_synthetics_account_id (const nr_synthetics_t *synthetics)
{
  if (NULL == synthetics) {
    return 0;
  }
...
const char *
nr_synthetics_resource_id (const nr_synthetics_t *synthetics)
{
  if (NULL == synthetics) {
    return NULL;
  }
...
const char *
nr_synthetics_job_id (const nr_synthetics_t *synthetics)
{
  if (NULL == synthetics) {
    return NULL;
  }
...
const char *
nr_synthetics_monitor_id (const nr_synthetics_t *synthetics)
{
  if (NULL == synthetics) {
    return NULL;
  }
...
const char *
nr_synthetics_outbound_header (nr_synthetics_t *synthetics)
{
  nrobj_t *obj = NULL;
