...
nr_status_t nr_txn_node_rollup(nrtxn_t* txn,
 ...
 * Transactions External/allOther
 * Unscoped   Background Tasks External/{host}/all
 * Unscoped   Always External/{host}/all
 * Scoped     Cross-Process Absent (Used for Node Name)
 * ExternalApp/{host}/{external_id}/all                         Unscoped
 * Cross-Process Present
 * ExternalTransaction/{host}/{external_id}/{external_txnname}  Unscoped
 * Cross-Process Present
 * ExternalTransaction/{host}/{external_id}/{external_txnname}  Scoped
 * Cross-Process Present (Used for Node Name)
 *
 ...
 */
static char* node_external_create_metrics(nrtxn_t* txn,
                                          nrtime_t duration,
                                          const char* url,
                                          int urllen,
  ...
  if (external_id && external_txnname) {
     ...
     * Cross process present.
     */
    snprintf(buf, buflen, "ExternalApp/%.*s/%s/all", domainlen, domain,
             external_id);
    nrm_add(txn->unscoped_metrics, buf, duration);

    snprintf(buf, buflen, "ExternalTransaction/%.*s/%s/%s", domainlen, domain,
...
void nr_txn_end_node_external(nrtxn_t* txn,
  ...
  node_name = node_external_create_metrics(txn, duration, params->url, urllen,
                                           external_id, external_txnname);

  if (params->do_rollup) {
    if (NR_SUCCESS
        == nr_txn_node_rollup(txn, &params->start, &params->stop, node_name)) {
      goto leave;
    }
  }
