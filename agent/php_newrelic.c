 ...
 * required to initialize the agent and register it with PHP. In the past this
 * file has grown to house all sorts of random parts of the agent. This should
...
#define PRINT_STATUS(field, format) fprintf(fp, " txn->status." #field "=" format "\n", txn->status.field);
...
nr_print_globals (FILE *fp)
{
  ...
  fprintf (fp, "start_user_time=" NR_TIME_FMT ".%06d\n", (nrtime_t)NRPRG (start_user_time.tv_sec), (int)NRPRG (start_user_time.tv_usec));
  fprintf (fp, "start_sys_time=" NR_TIME_FMT ".%06d\n", (nrtime_t)NRPRG (start_sys_time.tv_sec), (int)NRPRG (start_user_time.tv_usec));
...
/*
 * TODO(aharvey): when we drop support for PHP 5.3, the callback parameter
 * should be hinted as a callable.
 */
