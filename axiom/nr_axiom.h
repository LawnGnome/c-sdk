...
NRINLINE void *
nr_remove_const (const void *ptr)
{
  return (void *)ptr;
}
 ...
 * Because of the way that we preprocess our source files to present to scan-build,
 * this redefinition happens late in the preprocessing pipeline, so that
 * an invocation of the assert macro is not expanded prematurely.
 */
 ...
 */
#define NR_PSTR(S) (S), sizeof (S) - 1 /* a C string, with a length that EXCLUDES the null terminator */
#define NR_HSTR(S) (S), sizeof (S)     /* a C string, with a length that INCLUDES the null terminator (used for php hash tables) */
