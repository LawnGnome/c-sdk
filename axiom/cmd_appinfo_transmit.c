 ...
 */
static uint32_t
nr_appinfo_prepend_labels (const nr_app_info_t *info, nr_flatbuffer_t *fb)
{
  char *json;
  nrobj_t *labels;
  uint32_t offset;
...
static uint32_t
nr_appinfo_prepend_settings (const nr_app_info_t *info, nr_flatbuffer_t *fb)
{
  char *json;
  uint32_t offset;
...
static nr_status_t
convert_appenv (const char *key, const nrobj_t *val, void *ptr)
{
 ...
 */
static uint32_t
nr_appinfo_prepend_env (const nr_app_info_t *info, nr_flatbuffer_t *fb)
{
  char *json;
  nrobj_t *env;
  uint32_t offset;
...
nr_flatbuffer_t *
nr_appinfo_create_query (const char *agent_run_id, const nr_app_info_t *info)
{
  nr_flatbuffer_t *fb;
  ...
  nr_flatbuffers_object_prepend_uoffset (fb, APP_FIELD_REDIRECT_COLLECTOR, collector,           0);
  nr_flatbuffers_object_prepend_bool    (fb, APP_FIELD_HIGH_SECURITY,      info->high_security, 0);
  nr_flatbuffers_object_prepend_uoffset (fb, APP_FIELD_AGENT_VERSION,      agent_version,       0);
  nr_flatbuffers_object_prepend_uoffset (fb, APP_FIELD_AGENT_LANGUAGE,     agent_lang,          0);
  ...
  nr_flatbuffers_object_prepend_u8      (fb, MESSAGE_FIELD_DATA_TYPE,    MESSAGE_BODY_APP,    0);
  nr_flatbuffers_object_prepend_uoffset (fb, MESSAGE_FIELD_AGENT_RUN_ID, agent_run_id_offset, 0);
...
int
nr_command_is_flatbuffer_invalid (nr_flatbuffer_t *msg, size_t msglen)
{
...
nr_status_t
nr_cmd_appinfo_process_reply (const uint8_t *data, int len, nrapp_t *app)
{
  ...
  data_type = nr_flatbuffers_table_read_u8 (&msg, MESSAGE_FIELD_DATA_TYPE, MESSAGE_BODY_NONE);
  ...
  status = nr_flatbuffers_table_read_i8 (&reply, APP_REPLY_FIELD_STATUS, APP_STATUS_UNKNOWN);

  switch (status) {
    case APP_STATUS_UNKNOWN:
      app->state = NR_APP_UNKNOWN;
      nrl_debug (NRL_ACCT, "APPINFO reply unknown app=" NRP_FMT, NRP_APPNAME (app->info.appname));
      return NR_SUCCESS;
    case APP_STATUS_DISCONNECTED:
      app->state = NR_APP_INVALID;
      nrl_info (NRL_ACCT, "APPINFO reply disconnected app=" NRP_FMT, NRP_APPNAME (app->info.appname));
      return NR_SUCCESS;
    case APP_STATUS_INVALID_LICENSE:
      app->state = NR_APP_INVALID;
      nrl_error (NRL_ACCT, "APPINFO reply invalid license app=" NRP_FMT " please check your license "
        "key and restart your web server.", NRP_APPNAME (app->info.appname));
      return NR_SUCCESS;
   ...
   */
  reply_len = (int)nr_flatbuffers_table_read_vector_len (&reply, APP_REPLY_FIELD_CONNECT_REPLY);
  reply_json = (const char *)nr_flatbuffers_table_read_bytes (&reply, APP_REPLY_FIELD_CONNECT_REPLY);
  ...
  if (NULL == app->connect_reply) {
    nrl_error (NRL_ACCT, "APPINFO reply bad connect reply: len=%d json=%p", reply_len, reply_json);
    return NR_FAILURE;
  }
...
nr_status_t
nr_cmd_appinfo_tx (int daemon_fd, nrapp_t *app)
{
  ...
  app->state = NR_APP_UNKNOWN;
  nrl_verbosedebug (NRL_DAEMON, "querying app=" NRP_FMT " from parent=%d", NRP_APPNAME (app->info.appname), daemon_fd);
  ...
  nr_agent_lock_daemon_mutex (); {
    st = nr_write_message (daemon_fd, nr_flatbuffers_data (query), querylen, deadline);
    if (NR_SUCCESS == st) {
  ...
  if (NR_SUCCESS != st) {
    app->state = NR_APP_UNKNOWN;
    nrl_error (NRL_DAEMON, "APPINFO failure: len=%zu errno=%s", querylen, nr_errno (errno));
