...
static void nr_php_gather_dynamic_modules(nrobj_t* env TSRMLS_DC) {
  ...
  nro_set_hash(env, "Plugin List",
               plugins); /* see environment_value_set.rb ~ line 21 */
...
static void call_phpinfo(TSRMLS_D) {
  ...
  sapi_module.phpinfo_as_text
      = 1; /* this is a hack to force the output to the text format */
...
static void nr_php_gather_php_information(nrobj_t* env TSRMLS_DC) {
  ...
  if (FAILURE
    ...
    return; /* don't call phpinfo() if we can't buffer because otherwise we're
  ...
  nr_parse_rocket_assignment_list(Z_STRVAL_P(tmp_obj), Z_STRLEN_P(tmp_obj),
                                  env);
