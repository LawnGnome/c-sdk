 ...
 *
 * The internal instrumention scheme within this file is designed for code
 ...
 * the following:
 *
 *   > Create an auto-generated outer wrapper function for the internal function.
 *     This is done using the NR_OUTER_WRAPPER macro.
 *
 ...
 *     nr_php_generate_internal_wrap_records using the NR_INTERNAL_WRAPREC macro.
 *     This wrap record will reference the function/method name, the outer
 ...
 */
typedef void (*nr_inner_wrapper_fn_t) (INTERNAL_FUNCTION_PARAMETERS, nrinternalfn_t *fn);

/*
 * Delegation function which bridges between the outer and inner wrappers.
 */
static void
nr_php_instrument_delegate (nrinternalfn_t *wraprec, INTERNAL_FUNCTION_PARAMETERS)
{
  if ((0 == wraprec) ||
      (0 == wraprec->oldhandler) ||
      (0 == wraprec->inner_wrapper)) {
    /*
 ...
 */
#define NR_INNER_WRAPPER(RAW) static void NR_INNER_WRAPPER_NAME(RAW) (INTERNAL_FUNCTION_PARAMETERS, nrinternalfn_t *nr_wrapper)
...
static void \
NR_OUTER_WRAPPER_NAME (RAW) (INTERNAL_FUNCTION_PARAMETERS) \
{ \
  nr_php_instrument_delegate (NR_OUTER_GLOBAL_NAME (RAW), INTERNAL_FUNCTION_PARAM_PASSTHRU); \
}
 ...
 */
int
nr_zend_call_old_handler (nrphpfn_t oldhandler, INTERNAL_FUNCTION_PARAMETERS)
{
 ...
 */
void
nr_php_wrap_internal_function (nrinternalfn_t *wraprec TSRMLS_DC)
{
  char *function_name;
  char *class_name;
  ...
  if (0 == class_name) {
    ...
    if (ZEND_INTERNAL_FUNCTION != orig_func->type) {
      nrl_warning (NRL_INSTRUMENT, NRP_FMT " in function_table is not an internal function", NRP_PHP (wraprec->full_name));
      return;
      /*
    ...
    if (0 == orig_func) {
      nrl_verbosedebug (NRL_INSTRUMENT, "did not find function in " NRP_FMT "'s class function_table",
...
static void
nr_php_internal_wraprec_destroy (nrinternalfn_t **wraprec_ptr)
{
  nrinternalfn_t *wraprec;
...
void
nr_php_destroy_internal_wrap_records (void)
{
  nrinternalfn_t *next_internal_wraprec;
...
record_mysql_error (TSRMLS_D)
{
  ...
  if (NR_SUCCESS == nr_txn_record_error_worthy (NRPRG (txn), errprio)) {
    ...
    if (NULL == retval_ptr) {
      ...
      nrl_debug (NRL_INSTRUMENT, "mysql_error returned %d", Z_TYPE_P (retval_ptr));
    }
 ...
 *
 *     foo => (pointer) - - -> anonymous function (params) {
 *                                 static var ptr = (pointer) - - - - - -> function foo_impl(params) {
 *                                 start timer;                       /-->     ...implementation of foo...
 *                                 call (*ptr)(params); --[call to]--/     }
 *                                 stop timer;
 *                             }
 *
 ...
 * a generic function, so my final scheme is:
 *
 *     foo => (pointer) - - -> function foo_wrapper(params) {
 *                                 static var ptr = (pointer) - - - -> function foo_impl(params) {
 *                                 call generic_wrapper(params,ptr);        ...implementation of foo...
 *                             }                |                      }
 *                                          [call to]                                ^
 *                                              |                                    |
 *                                              v                                    |
 *                              function generic_wrapper(params,ptr) {               |
 *                                  start timer;                                     |
 *                                  call (*ptr)(params);---------------[call to]-----+
 *                                  stop timer;
 *                              }
...
NR_INNER_WRAPPER (mysql_connect)
{
  ...
  if (0 == zend_ini_long (NR_HSTR ("sql.safe_mode"), 0)) {
    ...
    if (FAILURE == zend_parse_parameters_ex (ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS () TSRMLS_CC,
      "|s!s!s!l", &host_and_port, &host_len, &user, &user_len, &passwd, &passwd_len, &ignore1)) {

      if (FAILURE == zend_parse_parameters_ex (ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS () TSRMLS_CC,
        "|s!s!s!bl", &host_and_port, &host_len, &user, &user_len, &passwd, &passwd_len, &ignore2, &ignore1)) {
...
NR_INNER_WRAPPER (mysql_query)
{
  ...
  if (FAILURE == zend_parse_parameters_ex (ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS () TSRMLS_CC, "s|r", &sqlstr, &sqlstrlen, &mysql)) {
    /* If we can't find a string at all, call the handler without instrumentation */
    nr_wrapper->oldhandler (INTERNAL_FUNCTION_PARAM_PASSTHRU);
    return;
  }
   ...
   */
  if (NRINI (record_database_errors) &&
      nr_php_is_zval_valid_bool (return_value) &&
...
/*
 * Handle
 *   resource mysql_db_query ( string $db, string $query [, resource $link_identifier ] )
 */
NR_INNER_WRAPPER (mysql_db_query)
{
  ...
  if (FAILURE == zend_parse_parameters_ex (ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS () TSRMLS_CC,
      "ss|r", &dbstr, &dblen, &sqlstr, &sqlstrlen, &mysql)) {
    /* if we can't find a string at all, call the handler without instrumentation */
    nr_wrapper->oldhandler (INTERNAL_FUNCTION_PARAM_PASSTHRU);
    return;
  }
  ...
  nr_php_txn_end_node_sql (NRPRG (txn), &start, NULL, sqlstr, sqlstrlen, NULL, NR_DATASTORE_MYSQL, instance TSRMLS_CC);
...
/*
 * Handle
 *   object mysqli_connect ([ string $host = ini_get("mysqli.default_host") [, string $username = ini_get("mysqli.default_user") [, string $passwd = ini_get("mysqli.default_pw") [, string $dbname = "" [, int $port = ini_get("mysqli.default_port") [, string $socket = ini_get("mysqli.default_socket") ]]]]]]
 *   object mysqli::__construct ([ string $host = ini_get("mysqli.default_host") [, string $username = ini_get("mysqli.default_user") [, string $passwd = ini_get("mysqli.default_pw") [, string $dbname = "" [, int $port = ini_get("mysqli.default_port") [, string $socket = ini_get("mysqli.default_socket") ]]]]]]
 *
...
NR_INNER_WRAPPER (mysqli_construct)
{
  ...
  nr_php_mysqli_save_datastore_instance (mysqli_obj, host, port, socket, database TSRMLS_CC);
  ...
  if (mysqli_obj && (0 == NRPRG (generating_explain_plan))) {
    ...
    nr_mysqli_metadata_set_connect (NRPRG (mysqli_links),
                                    (nr_mysqli_metadata_link_handle_t) Z_OBJ_HANDLE_P (mysqli_obj),
                                    host_term,
                                    username_term,
                                    password_term,
                                    database_term,
...
NR_INNER_WRAPPER (mysqli_options)
{
  ...
  if (value &&
      (0 == NRPRG (generating_explain_plan)) &&
      nr_php_mysqli_zval_is_link (mysqli_obj TSRMLS_CC) &&
      nr_php_is_zval_true (return_value)) {
    char *value_term = nr_strndup (value, value_len);

    nr_mysqli_metadata_set_option (NRPRG (mysqli_links),
                                   Z_OBJ_HANDLE_P (mysqli_obj),
                                   option,
                                   value_term);
...
/*
 * Handle
 *   object mysqli_real_connect ( object $link [, string $host = ini_get("mysqli.default_host") [, string $username = ini_get("mysqli.default_user") [, string $passwd = ini_get("mysqli.default_pw") [, string $dbname = "" [, int $port = ini_get("mysqli.default_port") [, string $socket = ini_get("mysqli.default_socket") [, int $flags = 0 ]]]]]]]
 *   object mysqli::real_connect ([ string $host = ini_get("mysqli.default_host") [, string $username = ini_get("mysqli.default_user") [, string $passwd = ini_get("mysqli.default_pw") [, string $dbname = "" [, int $port = ini_get("mysqli.default_port") [, string $socket = ini_get("mysqli.default_socket") [, int $flags = 0 ]]]]]]]
 *
...
NR_INNER_WRAPPER (mysqli_general_query)
{
  ...
  if ((0 == NRPRG (generating_explain_plan)) &&
      nr_php_mysqli_zval_is_link (mysqli_obj TSRMLS_CC)) {
    plan = nr_php_explain_mysqli_query (NRPRG (txn), mysqli_obj,
                                        sqlstr, sqlstrlen,
                                        &start, &stop TSRMLS_CC);
  }

  nr_php_txn_end_node_sql (NRPRG (txn), &start, NULL, sqlstr, sqlstrlen, plan, NR_DATASTORE_MYSQL, instance TSRMLS_CC);
 ...
 */
static const char *
nr_php_prepared_statement_find_internal (const char *key TSRMLS_DC)
{
  const char *stmt;
 ...
 */
static const char *
nr_php_prepared_statement_find (const zval *obj, const char *extension TSRMLS_DC)
{
  char *key;
  const char *stmt;
 ...
 */
static void
nr_php_prepared_statement_save_internal (const char *key, const char *stmt, nr_string_len_t stmt_len TSRMLS_DC)
{
  if ((NULL == key) || (NULL == stmt) || (0 == stmt_len)) {
    return;
  }

  nr_hashmap_update (NRPRG (prepared_statements), key, nr_strlen (key), nr_strndup (stmt, stmt_len));
}
 ...
 */
static void
nr_php_prepared_statement_save (const zval *obj, const char *extension,
  const char *stmt, nr_string_len_t stmt_len TSRMLS_DC)
{
  char *key;
...
/*
 * Handle
 *   bool mysqli_stmt_bind_param ( object $mysqli_stmt, string $types, mixed &$var1 [, mixed &$... ] )
...
NR_INNER_WRAPPER (mysqli_stmt_bind_param)
{
  ...
  for (i = 0; i < argc; i++) {
    ...
    argv[i] = nr_php_get_user_func_arg (i + 1, EG (current_execute_data) TSRMLS_CC);
#else /* PHP < 5.5 */
  ...
  nr_php_mysqli_query_set_bind_params (Z_OBJ_HANDLE_P (mysqli_stmt_obj),
                                       Z_STRVAL_P (types), Z_STRLEN_P (types),
                                       argc - args_to_skip, &argv[args_to_skip]
                                       TSRMLS_CC);
...
NR_INNER_WRAPPER (mysqli_stmt_execute)
{
  ...
  if (FAILURE == zend_parse_parameters_ex (ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS () TSRMLS_CC, "o", &stmt_obj)) {
    stmt_obj = NR_PHP_INTERNAL_FN_THIS;
  }
  ...
  nr_php_txn_end_node_sql (NRPRG (txn), &start, NULL, sqlstr, sqlstrlen, plan, NR_DATASTORE_MYSQL, NULL TSRMLS_CC);
...
static void
nr_php_prepared_prepare_general (INTERNAL_FUNCTION_PARAMETERS, nrinternalfn_t *nr_wrapper, const char *extension)
{
  zval *conn_obj = 0;
  char *sqlstr;
  nr_string_len_t sqlstrlen;
  ...
  if (FAILURE == zend_parse_parameters_ex (ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS () TSRMLS_CC, "os", &conn_obj, &sqlstr, &sqlstrlen)) {
    if (zend_parse_parameters_ex (ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS () TSRMLS_CC, "s|z", &sqlstr, &sqlstrlen, &ignore1) == FAILURE) {
      nr_wrapper->oldhandler (INTERNAL_FUNCTION_PARAM_PASSTHRU);
      return;
    } else {
  ...
  nr_wrapper->oldhandler (INTERNAL_FUNCTION_PARAM_PASSTHRU);
  nr_php_prepared_statement_save (return_value, extension, sqlstr, sqlstrlen TSRMLS_CC);
  ...
  if ((0 == NRPRG (generating_explain_plan)) &&
    ...
    nr_php_mysqli_query_set_query (Z_OBJ_HANDLE_P (return_value),
                                   sqlstr, sqlstrlen TSRMLS_CC);
  }
}
...
{
  nr_php_prepared_prepare_general (INTERNAL_FUNCTION_PARAM_PASSTHRU, nr_wrapper, "mysqli");
}
...
{
  nr_php_prepared_prepare_general (INTERNAL_FUNCTION_PARAM_PASSTHRU, nr_wrapper, "pdo");
}
...
NR_INNER_WRAPPER (mysqli_stmt_construct)
{
  ...
  if (FAILURE == zend_parse_parameters_ex (ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS () TSRMLS_CC, "o|s", &mysqli_obj, &sqlstr, &sqlstrlen)) {
    nr_wrapper->oldhandler (INTERNAL_FUNCTION_PARAM_PASSTHRU);
    return;
  }
  ...
  if (sqlstr) {
    nr_php_prepared_statement_save (NR_PHP_INTERNAL_FN_THIS, "mysqli", sqlstr, sqlstrlen TSRMLS_CC);
  }
...
NR_INNER_WRAPPER (mysqli_stmt_init)
{
  ...
  if (FAILURE == zend_parse_parameters_ex (ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS () TSRMLS_CC, "o", &mysqli_obj)) {
    mysqli_obj = NR_PHP_INTERNAL_FN_THIS;
  }
...
{
  zval *mysqli_stmt_obj = 0;
  char *sqlstr;
  nr_string_len_t sqlstrlen;

  if (FAILURE == zend_parse_parameters_ex (ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS () TSRMLS_CC, "os", &mysqli_stmt_obj, &sqlstr, &sqlstrlen)) {
    if (FAILURE == zend_parse_parameters_ex (ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS () TSRMLS_CC, "s", &sqlstr, &sqlstrlen)) {
      nr_wrapper->oldhandler (INTERNAL_FUNCTION_PARAM_PASSTHRU);
      return;
    } else {
      mysqli_stmt_obj = NR_PHP_INTERNAL_FN_THIS;
  ...
  nr_wrapper->oldhandler (INTERNAL_FUNCTION_PARAM_PASSTHRU);
  nr_php_prepared_statement_save (mysqli_stmt_obj, "mysqli", sqlstr, sqlstrlen TSRMLS_CC);

  if ((0 == NRPRG (generating_explain_plan)) &&
      nr_php_mysqli_zval_is_stmt (mysqli_stmt_obj TSRMLS_CC) &&
      nr_php_explain_mysql_query_is_explainable (sqlstr, sqlstrlen)) {
    nr_php_mysqli_query_set_query (Z_OBJ_HANDLE_P (mysqli_stmt_obj),
                                   sqlstr, sqlstrlen TSRMLS_CC);
  }
}

static void
nr_php_instrument_datastore_operation_call (
...
{
  nr_php_instrument_datastore_operation_call (nr_wrapper, NR_DATASTORE_MEMCACHE, nr_wrapper->extra, NULL, INTERNAL_FUNCTION_PARAM_PASSTHRU);
}
...
NR_INNER_WRAPPER (redis_connect)
{
  ...
  nr_php_instrument_datastore_operation_call (nr_wrapper,
                                              NR_DATASTORE_REDIS,
                                              nr_wrapper->extra,
                                              instance,
...
{
  nr_php_redis_remove_datastore_instance (NR_PHP_INTERNAL_FN_THIS TSRMLS_CC);
...
NR_INNER_WRAPPER (redis_select)
{
  ...
  if (SUCCESS == zend_parse_parameters_ex (ZEND_PARSE_PARAMS_QUIET,
    ...
    instance = nr_php_redis_retrieve_datastore_instance (NR_PHP_INTERNAL_FN_THIS TSRMLS_CC);
    nr_datastore_instance_set_database_name (instance, database);
    nr_free (database);
  }
...
static char *
nr_php_prepared_statement_make_pgsql_key (const zval *conn, const char *stmtname, nr_string_len_t stmtname_len)
{
  char *key = NULL;
  ...
  if (nr_php_is_zval_valid_resource (conn)) {
    key = nr_formatf ("type=pgsql id=%ld name=%.*s",
      nr_php_zval_resource_id (conn), NRSAFELEN (stmtname_len), stmtname);
  } else {
    key = nr_formatf ("type=pgsql id=default name=%.*s", NRSAFELEN (stmtname_len), stmtname);
  }
...
NR_INNER_WRAPPER (pg_connect)
{
  ...
  /*
   * TODO(earnold): There is also a very old deprecated syntax with multiple
   * parameters. I couldn't empirically determine whether it still worked and
   * therefore left it out of the initial datastore instance instrumentation.
   */
  if (FAILURE == zend_parse_parameters_ex (ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS () TSRMLS_CC,
      "s|l", &conn_info, &conn_infolen, &ignore1)) {
    nr_wrapper->oldhandler (INTERNAL_FUNCTION_PARAM_PASSTHRU);
    return;
  }
...
NR_INNER_WRAPPER (pg_execute)
{
  ...
  switch (argc) {
    case 3:
      rv = zend_parse_parameters_ex (ZEND_PARSE_PARAMS_QUIET, argc TSRMLS_CC,
        "rsa/", &conn, &stmtname, &stmtname_len, &params);
      break;
    case 2:
      rv = zend_parse_parameters_ex (ZEND_PARSE_PARAMS_QUIET, argc TSRMLS_CC,
  ...
  if (SUCCESS == rv) {
     ...
     * generate the same key for lookup as for insert. PHP-1050
     */
    key = nr_php_prepared_statement_make_pgsql_key (conn, stmtname, stmtname_len);
...
NR_INNER_WRAPPER (pg_prepare)
{
  ...
  switch (argc) {
    case 3:
      rv = zend_parse_parameters_ex (ZEND_PARSE_PARAMS_QUIET, argc TSRMLS_CC,
        "rss", &conn, &stmtname, &stmtname_len, &query, &query_len);
      break;
    case 2:
      rv = zend_parse_parameters_ex (ZEND_PARSE_PARAMS_QUIET, argc TSRMLS_CC,
        "ss", &stmtname, &stmtname_len, &query, &query_len);
      break;
  }
...
NR_INNER_WRAPPER (pg_query)
{
  ...
  if (1 == argc) {
    ...
    if (FAILURE == zend_parse_parameters_ex (ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS () TSRMLS_CC, "rs", &pgsql_link, &sqlstr, &sqlstrlen)) {
      sqlstr = "(unknown sql)";
      sqlstrlen = nr_strlen (sqlstr);
    }
  }
  ...
  nr_php_txn_end_node_sql (NRPRG (txn), &start, NULL, sqlstr, sqlstrlen, NULL, NR_DATASTORE_POSTGRES, instance TSRMLS_CC);
...
/*
 * Handle
 *   pg_query_params (string, array) and pg_query_params (resource, string, array)
 *
 * NOTE: This function allows you to use variable placeholders in the query string like $1, $2 etc
 * whose values come from the array passed as the last argument. We do not currently attempt any
 * such expansion, but rather just use the raw SQL statement as passed to the PHP function. While
 * this will not be the exact SQL that PostgreSQL executes, it will at least be good enough to
 * measure time spent and give the user an idea if a given query is too slow. In the future we
 * may revisit this and attempt actual expansion to produce the full SQL that the server will see.
 */
NR_INNER_WRAPPER (pg_query_params)
{
  ...
  argc = ZEND_NUM_ARGS ();
  if (argc == 2) {
    if (FAILURE == zend_parse_parameters_ex (ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS () TSRMLS_CC, "sa", &sqlstr, &sqlstrlen, &param_array)) {
      sqlstr = "(unknown sql)";
      sqlstrlen = nr_strlen (sqlstr);
    }
  } else {
    if (FAILURE == zend_parse_parameters_ex (ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS () TSRMLS_CC, "rsa", &pgsql_link, &sqlstr, &sqlstrlen, &param_array)) {
      sqlstr = "(unknown sql)";
      sqlstrlen = nr_strlen (sqlstr);
    }
  }
  ...
  nr_php_txn_end_node_sql (NRPRG (txn), &start, NULL, sqlstr, sqlstrlen, NULL, NR_DATASTORE_POSTGRES, NULL TSRMLS_CC);
...
NR_INNER_WRAPPER (sqlite_query_function)
{
  ...
  if (obj) {
    if (FAILURE == zend_parse_parameters_ex (ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS () TSRMLS_CC, "s|lz/", &sqlstr, &sqlstrlen, &ignores, &errstr)) {
      sqlstr = "(unknown sql)";
      sqlstrlen = nr_strlen (sqlstr);
    }
  } else {
    if (FAILURE == zend_parse_parameters_ex (ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS () TSRMLS_CC, "sr|lz/", &sqlstr, &sqlstrlen, &zdb, &ignores, &errstr)) {
      if (FAILURE == zend_parse_parameters_ex (ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS () TSRMLS_CC, "rs|lz/", &zdb, &sqlstr, &sqlstrlen, &ignores, &errstr)) {
        sqlstr = "(unknown sql)";
        sqlstrlen = nr_strlen (sqlstr);
      }
    }
  ...
  nr_php_txn_end_node_sql (NRPRG (txn), &start, NULL, sqlstr, sqlstrlen, NULL, NR_DATASTORE_SQLITE, NULL TSRMLS_CC);
...
NR_INNER_WRAPPER (sqlite_exec_or_query)
{
  ...
  obj = NR_PHP_INTERNAL_FN_THIS;
  if (obj) {
    if (FAILURE == zend_parse_parameters_ex (ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS () TSRMLS_CC, "s|z/", &sqlstr, &sqlstrlen, &errstr)) {
      sqlstr = "(unknown sql)";
      sqlstrlen = nr_strlen (sqlstr);
    }
  } else {
    if (FAILURE == zend_parse_parameters_ex (ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS () TSRMLS_CC, "sr", &sqlstr, &sqlstrlen, &zdb)) {
      if (FAILURE == zend_parse_parameters_ex (ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS () TSRMLS_CC, "rs|z/", &zdb, &sqlstr, &sqlstrlen, &errstr)) {
        sqlstr = "(unknown sql)";
        sqlstrlen = nr_strlen (sqlstr);
      }
    }
...
NR_INNER_WRAPPER (sqlite3_querysingle)
{
  ...
  rv = zend_parse_parameters_ex (ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS () TSRMLS_CC,
    "s|b", &sqlstr, &sqlstrlen, &entire_row);
...
NR_INNER_WRAPPER (pdo_construct)
{
  ...
  if (FAILURE == zend_parse_parameters_ex (ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS () TSRMLS_CC, "s|s!s!a!", &dsn, &dsn_len, &username, &username_len, &password, &password_len, &options)) {
    options = NULL;
  }
   ...
   * for validity.
   */
  nr_php_pdo_options_save (NR_PHP_INTERNAL_FN_THIS, options TSRMLS_CC);
...
NR_INNER_WRAPPER (pdo_query)
{
   ...
   */
  if (FAILURE == zend_parse_parameters_ex (ZEND_PARSE_PARAMS_QUIET, 1 TSRMLS_CC, "s", &sqlstr, &sqlstrlen)) {
    sqlstr = "(unknown sql)";
    sqlstrlen = nr_strlen (sqlstr);
  }
...
NR_INNER_WRAPPER (curl_setopt)
{
  ...
  if (!NRPRG (curl_ignore_setopt)) {
    rv = zend_parse_parameters_ex (ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS () TSRMLS_CC, "zzz",
                                   &curlres, &curlopt, &curlval);
...
NR_INNER_WRAPPER (curl_setopt_array)
{
  ...
  if (!NRPRG (curl_ignore_setopt)) {
    rv = zend_parse_parameters_ex (ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS () TSRMLS_CC, "za", &curlres, &options);

    if (SUCCESS == rv) {
      nr_php_curl_setopt_array (curlres, options, nr_php_curl_setopt_pre TSRMLS_CC);
    }
  }
  ...
  if (!NRPRG (curl_ignore_setopt)) {
    if (SUCCESS == rv) {
      nr_php_curl_setopt_array (curlres, options, nr_php_curl_setopt_post TSRMLS_CC);
    }
  }
}
...
NR_INNER_WRAPPER (curl_exec)
{
  ...
  if (SUCCESS != zend_parse_parameters_ex (ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS () TSRMLS_CC, "r", &curlres)) {
    nr_wrapper->oldhandler (INTERNAL_FUNCTION_PARAM_PASSTHRU);
    return;
  }
...
NR_INNER_WRAPPER (call_user_func_array)
{
  if (NULL != NRPRG (cufa_callback)) {
    ...
    if (SUCCESS != zend_parse_parameters_ex (ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS () TSRMLS_CC, "fa/", &fci, &fcc, &args)) {
...
/*
 * Handle
 *   resource mssql_query ( string $query [, resource $link_identifier [, int $batch_size = 0]] )
 */
NR_INNER_WRAPPER (mssql_query)
{
  ...
  if (FAILURE == zend_parse_parameters_ex (ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS () TSRMLS_CC, "s|zz", &sqlstr, &sqlstrlen, &ignore1, &ignore2)) {
    nr_wrapper->oldhandler (INTERNAL_FUNCTION_PARAM_PASSTHRU);
    return;
  }
  ...
  nr_php_txn_end_node_sql (NRPRG (txn), &start, NULL, sqlstr, sqlstrlen, NULL, NR_DATASTORE_MSSQL, NULL TSRMLS_CC);
...
NR_INNER_WRAPPER (mongocollection_15)
{
  ...
  if (nr_php_is_zval_valid_string (collectionnamez)) {
    ...
    nr_strxcpy (collection_name, Z_STRVAL_P (collectionnamez), collection_name_len);
  } else if (NULL != collectionnamez) {
    nrl_warning (NRL_INSTRUMENT, "__toString does not return a string in MongoCollection (%d)", Z_TYPE_P (collectionnamez));
  } else {
...
NR_INNER_WRAPPER (mongodb_execute)
{
  nr_php_instrument_datastore_operation_call (nr_wrapper, NR_DATASTORE_MONGODB,
    "execute", NULL, INTERNAL_FUNCTION_PARAM_PASSTHRU);
}
...
NR_INNER_WRAPPER (oci_parse)
{
  ...
  if (FAILURE == zend_parse_parameters_ex (ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS () TSRMLS_CC, "rs", &ignore1, &sqlstr, &sqlstrlen)) {
    nr_wrapper->oldhandler (INTERNAL_FUNCTION_PARAM_PASSTHRU);
    return;
  }
  nr_wrapper->oldhandler (INTERNAL_FUNCTION_PARAM_PASSTHRU);
  nr_php_prepared_statement_save (return_value, "oci", sqlstr, sqlstrlen TSRMLS_CC);
...
NR_INNER_WRAPPER (oci_execute)
{
  ...
  if (FAILURE == zend_parse_parameters_ex (ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS () TSRMLS_CC, "r|l", &stmt_obj, &ignore1)) {
    nr_wrapper->oldhandler (INTERNAL_FUNCTION_PARAM_PASSTHRU);
    return;
  }
  ...
  nr_php_txn_end_node_sql (NRPRG (txn), &start, NULL, sqlstr, sqlstrlen, NULL, NR_DATASTORE_ORACLE, NULL TSRMLS_CC);
...
NR_INNER_WRAPPER (file_get_contents)
{
   ...
   */
  if ((SUCCESS != zend_rv) ||
      (0 == nr_php_is_zval_valid_string (file_zval)) ||
  ...
  if (NRPRG (txn)->options.cross_process_enabled && (0 == context)) {
     ...
     */
    rv = nr_php_file_get_contents_recurse_with_context (return_value, file_zval, use_include_path,
                                                        offset, maxlen TSRMLS_CC);
    if (NR_SUCCESS != rv) {
      goto leave;
    }
    return;
...
static inline int
nr_php_should_instrument_exception_handler (zval *handler TSRMLS_DC)
{
    ...
    if (handler) {
      ...
      if (nr_php_is_zval_valid_callable (handler TSRMLS_CC)) {
        if (nr_php_is_zval_valid_string (handler) &&
            (0 == nr_strncmp ("newrelic_exception_handler",
                              Z_STRVAL_P (handler), Z_STRLEN_P (handler)))) {
          return 0;
        }
...
NR_INNER_WRAPPER (exception_common)
{
  ...
  if (nr_php_should_instrument_exception_handler (exception_handler TSRMLS_CC)) {
    ...
    nr_php_remove_exception_function (func);
  }
...
nrinternalfn_t *nr_wrapped_internal_functions = 0;

static nrinternalfn_t *
nr_get_wraprec (void)
{
...
void
nr_php_generate_internal_wrap_records (void)
{
  nrinternalfn_t *w;
...
#define NR_INTERNAL_WRAPREC(FULL_NAME, OUTER, INNER, DISABLED, EXTRA) { \
  ...
  w->extra                  = EXTRA; \
  w->supportability_metric  = nr_txn_create_fn_supportability_metric (FULL_NAME, NULL); \
}
  /*
   *                   Function or Class::Method Name     Outer Wrapper              Inner Wrapper                  Disabled
   *                   |                                  |                          |                              |  Extra
   *                   v                                  v                          v                              v  v
   */
  ...
  NR_INTERNAL_WRAPREC ("mysql_unbuffered_query",          mysql_unbuffered_query,    mysql_query,                   0, 0)
  ...
  NR_INTERNAL_WRAPREC ("mysqli_multi_query",              mysqli_multi_query,        mysqli_general_query,          0, 0)
  ...
  NR_INTERNAL_WRAPREC ("mysqli_real_query",               mysqli_real_query,         mysqli_general_query,          0, 0)
  NR_INTERNAL_WRAPREC ("mysqli_real_connect",             mysqli_real_connect,       mysqli_real_connect,           0, 0)
  NR_INTERNAL_WRAPREC ("mysqli_select_db",                mysqli_select_db,          mysqli_select_db,              0, 0)
  NR_INTERNAL_WRAPREC ("mysqli_stmt_init",                mysqli_stmt_init,          mysqli_stmt_init,              0, 0)

#ifdef PHP7
  NR_INTERNAL_WRAPREC ("mysqli::__construct",             mysqliC_construct,         mysqli_construct,              0, 0)
#else
  NR_INTERNAL_WRAPREC ("mysqli::mysqli",                  mysqliC_construct,         mysqli_construct,              0, 0)
#endif /* PHP7 */

  NR_INTERNAL_WRAPREC ("mysqli::multi_query",             mysqliC_multi_query,       mysqli_general_query,          0, 0)
  ...
  NR_INTERNAL_WRAPREC ("mysqli::query",                   mysqliC_query,             mysqli_general_query,          0, 0)
  NR_INTERNAL_WRAPREC ("mysqli::real_connect",            mysqliC_real_connect,      mysqli_real_connect,           0, 0)
  NR_INTERNAL_WRAPREC ("mysqli::real_query",              mysqliC_real_query,        mysqli_general_query,          0, 0)
  NR_INTERNAL_WRAPREC ("mysqli::select_db",               mysqliC_select_db,         mysqli_select_db,              0, 0)
  NR_INTERNAL_WRAPREC ("mysqli::stmt_init",               mysqliC_stmt_init,         mysqli_stmt_init,              0, 0)

  NR_INTERNAL_WRAPREC ("mysqli_stmt_bind_param",          mysqli_stmt_bind_param,    mysqli_stmt_bind_param,        0, 0)
  NR_INTERNAL_WRAPREC ("mysqli_stmt_execute",             mysqli_stmt_execute,       mysqli_stmt_execute,           0, 0)
  NR_INTERNAL_WRAPREC ("mysqli_stmt_prepare",             mysqli_stmt_prepare,       mysqli_stmt_prepare,           0, 0)

  NR_INTERNAL_WRAPREC ("mysqli_stmt::__construct",        mysqli_stmtC_construct,    mysqli_stmt_construct,         0, 0)
  NR_INTERNAL_WRAPREC ("mysqli_stmt::bind_param",         mysqli_stmtC_bind_param,   mysqli_stmt_bind_param,        0, 0)
  NR_INTERNAL_WRAPREC ("mysqli_stmt::execute",            mysqli_stmtC_execute,      mysqli_stmt_execute,           0, 0)
  NR_INTERNAL_WRAPREC ("mysqli_stmt::prepare",            mysqli_stmtC_prepare,      mysqli_stmt_prepare,           0, 0)

  NR_INTERNAL_WRAPREC ("memcache_connect",                memcache_connect,          memcache_function, 0, "connect")
  NR_INTERNAL_WRAPREC ("memcache_pconnect",               memcache_pconnect,         memcache_function, 0, "connect")
  ...
  NR_INTERNAL_WRAPREC ("memcache_replace",                memcache_replace,          memcache_function, 0, "replace")
  NR_INTERNAL_WRAPREC ("memcache_get",                    memcache_get,              memcache_function, 0, "get")
  NR_INTERNAL_WRAPREC ("memcache_delete",                 memcache_delete,           memcache_function, 0, "delete")
  NR_INTERNAL_WRAPREC ("memcache_increment",              memcache_increment,        memcache_function, 0, "incr")
  NR_INTERNAL_WRAPREC ("memcache_decrement",              memcache_decrement,        memcache_function, 0, "decr")

  NR_INTERNAL_WRAPREC ("memcache::connect",               memcacheC_connect,         memcache_function, 0, "connect")
  NR_INTERNAL_WRAPREC ("memcache::pconnect",              memcacheC_pconnect,        memcache_function, 0, "connect")
  NR_INTERNAL_WRAPREC ("memcache::add",                   memcacheC_add,             memcache_function, 0, "add")
  NR_INTERNAL_WRAPREC ("memcache::set",                   memcacheC_set,             memcache_function, 0, "set")
  NR_INTERNAL_WRAPREC ("memcache::replace",               memcacheC_replace,         memcache_function, 0, "replace")
  NR_INTERNAL_WRAPREC ("memcache::get",                   memcacheC_get,             memcache_function, 0, "get")
  NR_INTERNAL_WRAPREC ("memcache::delete",                memcacheC_delete,          memcache_function, 0, "delete")
  NR_INTERNAL_WRAPREC ("memcache::increment",             memcacheC_increment,       memcache_function, 0, "incr")
  NR_INTERNAL_WRAPREC ("memcache::decrement",             memcacheC_decrement,       memcache_function, 0, "decr")

  NR_INTERNAL_WRAPREC ("memcached::add",                  memcached_add,             memcache_function, 0, "add")
  NR_INTERNAL_WRAPREC ("memcached::addbykey",             memcached_addbykey,        memcache_function, 0, "add")
  NR_INTERNAL_WRAPREC ("memcached::append",               memcached_append,          memcache_function, 0, "replace")
  NR_INTERNAL_WRAPREC ("memcached::appendbykey",          memcached_appendbykey,     memcache_function, 0, "replace")
  NR_INTERNAL_WRAPREC ("memcached::cas",                  memcached_cas,             memcache_function, 0, "replace")
  NR_INTERNAL_WRAPREC ("memcached::casbykey",             memcached_casbykey,        memcache_function, 0, "replace")
  NR_INTERNAL_WRAPREC ("memcached::decrement",            memcached_decrement,       memcache_function, 0, "decr")
  NR_INTERNAL_WRAPREC ("memcached::delete",               memcached_delete,          memcache_function, 0, "delete")
  NR_INTERNAL_WRAPREC ("memcached::deletebykey",          memcached_deletebykey,     memcache_function, 0, "delete")
  NR_INTERNAL_WRAPREC ("memcached::get",                  memcached_get,             memcache_function, 0, "get")
  NR_INTERNAL_WRAPREC ("memcached::getbykey",             memcached_getbykey,        memcache_function, 0, "get")
  NR_INTERNAL_WRAPREC ("memcached::getdelayed",           memcached_getdelayed,      memcache_function, 0, "get")
  NR_INTERNAL_WRAPREC ("memcached::getdelayedbykey",      memcached_getdelayedbykey, memcache_function, 0, "get")
  NR_INTERNAL_WRAPREC ("memcached::getmulti",             memcached_getmulti,        memcache_function, 0, "get")
  NR_INTERNAL_WRAPREC ("memcached::getmultibykey",        memcached_getmultibykey,   memcache_function, 0, "get")
  NR_INTERNAL_WRAPREC ("memcached::increment",            memcached_increment,       memcache_function, 0, "incr")
  NR_INTERNAL_WRAPREC ("memcached::prepend",              memcached_prepend,         memcache_function, 0, "replace")
  NR_INTERNAL_WRAPREC ("memcached::prependbykey",         memcached_prependbykey,    memcache_function, 0, "replace")
  NR_INTERNAL_WRAPREC ("memcached::replace",              memcached_replace,         memcache_function, 0, "replace")
  NR_INTERNAL_WRAPREC ("memcached::replacebykey",         memcached_replacebykey,    memcache_function, 0, "replace")
  NR_INTERNAL_WRAPREC ("memcached::set",                  memcached_set,             memcache_function, 0, "set")
  NR_INTERNAL_WRAPREC ("memcached::setbykey",             memcached_setbykey,        memcache_function, 0, "set")
  NR_INTERNAL_WRAPREC ("memcached::setmulti",             memcached_setmulti,        memcache_function, 0, "set")
  NR_INTERNAL_WRAPREC ("memcached::setmultibykey",        memcached_setmultibykey,   memcache_function, 0, "set")

  NR_INTERNAL_WRAPREC ("redis::connect",                  redis_connect,             redis_connect,  0, "connect")
  NR_INTERNAL_WRAPREC ("redis::pconnect",                 redis_pconnect,            redis_connect,  0, "pconnect")
  ...
  NR_INTERNAL_WRAPREC ("redis::delete",                   redis_delete,              redis_function, 0, "delete")
  ...
  NR_INTERNAL_WRAPREC ("redis::incrby",                   redis_incrby,              redis_function, 0, "incrby")
  NR_INTERNAL_WRAPREC ("redis::decr",                     redis_decr,                redis_function, 0, "decr")
  NR_INTERNAL_WRAPREC ("redis::decrby",                   redis_decrby,              redis_function, 0, "decrby")
  ...
  NR_INTERNAL_WRAPREC ("redis::lremove",                  redis_lremove,             redis_function, 0, "lremove")
  NR_INTERNAL_WRAPREC ("redis::lindex",                   redis_lindex,              redis_function, 0, "lindex")
  NR_INTERNAL_WRAPREC ("redis::hset",                     redis_hset,                redis_function, 0, "hset")
  NR_INTERNAL_WRAPREC ("redis::hsetnx",                   redis_hsetnx,              redis_function, 0, "hsetnx")
  ...
  NR_INTERNAL_WRAPREC ("sqlite_unbuffered_query",         sqlite_unbuffered_query,   sqlite_query_function,         0, 0)
  NR_INTERNAL_WRAPREC ("sqlite_exec",                     sqlite_exec,               sqlite_exec_or_query,          0, 0)

  NR_INTERNAL_WRAPREC ("sqlitedatabase::query",           sqlitedatabaseC_query,            sqlite_query_function,  0, 0)
  NR_INTERNAL_WRAPREC ("sqlitedatabase::unbufferedquery", sqlitedatabaseC_unbuffered_query, sqlite_query_function,  0, 0)
  NR_INTERNAL_WRAPREC ("sqlitedatabase::queryexec",       sqlitedatabaseC_exec,             sqlite_exec_or_query,   0, 0)

  NR_INTERNAL_WRAPREC ("sqlite3::query",                  sqlite3_query,             sqlite3,             0, 0)
  NR_INTERNAL_WRAPREC ("sqlite3::querysingle",            sqlite3_querysingle,       sqlite3_querysingle, 0, 0)
  ...
  NR_INTERNAL_WRAPREC ("pdostatement::execute",           pdostmt_execute,           pdostatement_execute,          0, 0)

  NR_INTERNAL_WRAPREC ("curl_exec",                       curl_exec,                 curl_exec,                     0, 0)
  NR_INTERNAL_WRAPREC ("curl_setopt",                     curl_setopt,               curl_setopt,                   0, 0)
  NR_INTERNAL_WRAPREC ("curl_setopt_array",               curl_setopt_array,         curl_setopt_array,             0, 0)
   ...
   * is detected.
   */
  NR_INTERNAL_WRAPREC ("call_user_func_array",            call_user_func_array,      call_user_func_array,          1, 0)

  NR_INTERNAL_WRAPREC ("mssql_query",                     mssql_query,               mssql_query,                   0, 0)

  NR_INTERNAL_WRAPREC ("mongocollection::find",           mongoC_find,               mongocollection_15,            0, "find")
  NR_INTERNAL_WRAPREC ("mongocollection::findone",        mongoC_findone,            mongocollection_15,            0, "findone")
  NR_INTERNAL_WRAPREC ("mongocollection::insert",         mongoC_insert,             mongocollection_15,            0, "insert")
  NR_INTERNAL_WRAPREC ("mongocollection::remove",         mongoC_remove,             mongocollection_15,            0, "remove")
  NR_INTERNAL_WRAPREC ("mongocollection::save",           mongoC_save,               mongocollection_15,            0, "save")
  NR_INTERNAL_WRAPREC ("mongocollection::update",         mongoC_update,             mongocollection_15,            0, "update")
  NR_INTERNAL_WRAPREC ("mongocollection::group",          mongoC_group,              mongocollection_15,            0, "group")

  NR_INTERNAL_WRAPREC ("mongodb::execute",                mongodb_execute,           mongodb_execute,               0, 0)

  NR_INTERNAL_WRAPREC ("oci_parse",                       oci_parse,                 oci_parse,                     0, 0)
  NR_INTERNAL_WRAPREC ("oci_execute",                     oci_execute,               oci_execute,                   0, 0)

  NR_INTERNAL_WRAPREC ("file_get_contents",               file_get_contents,         file_get_contents,             0, 0)
   ...
   */
  NR_INTERNAL_WRAPREC ("httprequest::send",               httprequest_send,          httprequest_send,              0, 0)
  ...
  NR_INTERNAL_WRAPREC ("ob_implicit_flush",               ob_implicit_flush,         ob_flush_common,                0, 0)
  NR_INTERNAL_WRAPREC ("ob_gzhandler",                    ob_gzhandler,              ob_flush_common,                0, 0)

  NR_INTERNAL_WRAPREC ("soapclient::__dorequest",         soapclient_dorequest,      soapclient_dorequest,           0, 0)

  NR_INTERNAL_WRAPREC ("dl",                              dl,                        dl,                             0, 0)

  NR_INTERNAL_WRAPREC ("set_exception_handler",           set_exception_handler,     exception_common,               0, 0)
  NR_INTERNAL_WRAPREC ("restore_exception_handler",       restore_exception_handler, exception_common,               0, 0)
}
 ...
 */
void
nr_php_add_internal_instrumentation (TSRMLS_D)
{
  nrinternalfn_t *w;
