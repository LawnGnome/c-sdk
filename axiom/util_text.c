...
char *
nr_read_file_contents (const char *file_name, size_t max_bytes)
{
  int stat_status;
  struct stat statbuf;
  ...
  while (bytes_to_read > 0) {
    const int nread = fread (file_contents + offset, sizeof (char), bytes_to_read, fp);
...
void
nr_parse_rocket_assignment_list (char *s, size_t len, nrobj_t *kv_hash)
{
  const char *key = NULL;
