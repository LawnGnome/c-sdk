/*
 * This file contains functions dealing with Real User Monitoring.
 */
#ifndef NR_RUM_HDR
#define NR_RUM_HDR
...
/*
 * Purpose : Scan html looking for a heuristically good place in <head> to put RUM header code.
 *
 * Params : 1. The input string, which is putatively well-formed HTML ready to be output.
 *          2. The length of the input string.
 *
 * Returns: The place in input where the text of the <head> tag starts.
 *
 * Watch out: This code uses a simplistic pure lexical approach for scanning the HTML.
 * It does not work if the input/input_len is only a fragment of the entire
 * HTML being generated, as for example when output buffer(s) are flushed.
 * Further, it will easily get confused if the HTML contains strings
 * (as for example inside of <script> that has strings) and those strings
 * contain HTML.
 */
extern const char *nr_rum_scan_html_for_head (const char *input, const uint input_len);

/*
 * This is a control block for the nr_run_output_handler_worker.
 * It contains state for handling ZTS vs non-ZTS,
 * and a bunch of function pointers acting as virtual functions.
 *
 * In this way, nr_run_output_handler_worker can be dis-associated from
 * the PHP engine for unit testing purposes.
 */
typedef struct _nr_rum_control_block nr_rum_control_block_t;

struct _nr_rum_control_block {
  /*
   * Typically bound to php's emalloc function.
   */
...
/*
 * Purpose: inject rum header and footer into output, returning new handled_output.
 *
 * Params:
 *         1.  A control block.
 *         2.  The transaction being worked on.
 *         3.  The original output string, which is putatively well formed HTML.
 *         4.  The original output string length to consider.
 *         5.  A pointer to a string to hold the transformed (header and footer injected) output.
 *         6.  A pointer to an int to hold the length of the transformed output.
 *         7.  non-zero if the response has a content length set, in which case we can't add rum js.
