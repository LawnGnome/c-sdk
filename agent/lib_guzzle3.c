...
static int
nr_guzzle3_in_redirect_iterator (zval *frame, int *in_guzzle_ptr,
                                 zend_hash_key *key NRUNUSED TSRMLS_DC)
{
  ...
  /*
   * NOTE(willhf): RedirectPlugin was added in Guzzle version v3.0.3
   * If we want to support redirects from previous versions then this
...
/*
 * TODO(willhf): Combine this with nr_guzzle_in_call_stack to reduce
 * code duplication.
 */
static int
nr_guzzle3_in_redirect (TSRMLS_D)
{
  int in_redirect = 0;
  ...
  if (nr_php_is_zval_valid_array (stack)) {
    nr_php_zend_hash_zval_apply (Z_ARRVAL_P (stack),
                                 (nr_php_zval_apply_t) nr_guzzle3_in_redirect_iterator,
 ...
 */
static int
nr_guzzle3_is_state (const char *expected, zval *state, zval *request TSRMLS_DC)
{
  ...
  if (SUCCESS == is_identical_function (&result, expected_const, state TSRMLS_CC)) {
    is_complete = nr_php_is_zval_true (&result);
  } else {
    nrl_verbosedebug (NRL_INSTRUMENT,
                      "Guzzle 3: is_identical_function failed when checking the request state");
  }
 ...
 */
static zval *
nr_guzzle3_response_get_info (const char *key, zval *response TSRMLS_DC)
{
 ...
 */
static void
nr_guzzle3_request_state_transfer (zval *request TSRMLS_DC)
{
  /*
...
nr_guzzle3_request_state_complete (zval *request TSRMLS_DC)
{
   ...
   * response object. Ergo, let's grab the response object.
   */
  response = nr_php_call (request, "getResponse");
  if (0 == nr_php_object_instanceof_class (response,
                                           "Guzzle\\Http\\Message\\Response" TSRMLS_CC)) {
    nrl_verbosedebug (NRL_INSTRUMENT,
...
NR_PHP_WRAPPER (nr_guzzle3_request_setstate)
{
  ...
  /*
   * TODO(willhf) It would be nice to have metrics/nodes for these
   * redirect calls.  We need to make sure that each one has a unique
   * async_context (distinct from each other and the call that resulted in
   * the redirect).
   */
