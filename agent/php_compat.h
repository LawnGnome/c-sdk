...
static inline zval *
nr_php_zval_dereference (zval *zv)
{
...
#define ZEND_HASH_FOREACH(ht) do { \
  ...
  for (zend_hash_internal_pointer_reset_ex ((ht), &pos); \
       SUCCESS == zend_hash_get_current_data_ex ((ht), (void **) &value_ptr, &pos); \
...
#define ZEND_HASH_FOREACH_KEY_VAL(ht, _index, _key, _value) \
  ...
  _value = *value_ptr; \
  \
  key_type = zend_hash_get_current_key_ex ((ht), &key_ptr, &key_len, &index, \
                                           0, &pos); \
  switch (key_type) { \
