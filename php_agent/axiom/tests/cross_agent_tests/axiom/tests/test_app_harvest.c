...
static void test_should_sample_subsequent_harvest(nr_random_t* rnd) {
   ...
   * affirmed that the threshold is getting correctly recalculated and
   * updated.  Moreover, it can be affirmed that the number seen are being
   * updated. */
  nr_app_harvest_private_should_sample(&ah, rnd, 171);
  ...
  nr_app_harvest_private_should_sample(&ah, rnd, 171);
  tlib_pass_if_uint64_t_equal("threshold", 3, ah.threshold);
  tlib_pass_if_uint64_t_equal("seen", target + 2, ah.transactions_seen);

  nr_app_harvest_private_should_sample(&ah, rnd, 171);
  tlib_pass_if_uint64_t_equal("threshold", 2, ah.threshold);
...
static void test_should_sample_skip_harvest(nr_random_t* rnd) {
   ...
   * Test : Skip harvests.
   *
   * More than one harvest later, affirm that the previous number of transactions
   * seen is correctly updated to 0. */
