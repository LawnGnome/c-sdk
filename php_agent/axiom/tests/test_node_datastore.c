...
static nr_node_datastore_params_t sample_node_datastore_params(
    nrtime_t duration) {
...
static nr_node_datastore_params_t sample_node_sql_params(nrtime_t duration) {
  nr_node_datastore_params_t params = {
      .start = {.when = 1 * NR_TIME_DIVISOR, .stamp = 1},
      .stop = {.when = 1 * NR_TIME_DIVISOR + duration, .stamp = 2},
...
static void test_table_and_operation(void) {
  ...
  test_one_node_populated(tname, txn,
                          "Datastore/statement/MongoDB/my_table/my_operation",
                          duration, "{}");
  ...
  test_metric_created(tname, txn->unscoped_metrics, 0, duration,
                      "Datastore/operation/MongoDB/my_operation");
  test_metric_created(tname, txn->scoped_metrics, 0, duration,
                      "Datastore/statement/MongoDB/my_table/my_operation");
  nr_txn_destroy(&txn);
}
...
static void test_no_table(void) {
  ...
  nr_txn_end_node_datastore(txn, &params, NULL);
  tlib_pass_if_time_equal(tname, txn->root_kids_duration, duration);
  test_one_node_populated(
      tname, txn, "Datastore/operation/MongoDB/my_operation", duration, "{}");
  test_metric_table_size(tname, txn->scoped_metrics, 1);
...
static void test_no_table_no_operation(void) {
  ...
  test_metric_created(tname, txn->unscoped_metrics, MET_FORCED, duration,
                      "Datastore/MongoDB/all");
  test_metric_created(tname, txn->scoped_metrics, 0, duration,
                      "Datastore/operation/MongoDB/other");
  nr_txn_destroy(&txn);
}

static void test_return_scoped_metric(void) {
  const char* tname = "table and operation";
  nrtxn_t* txn = new_txn(0);
  nrtime_t duration = 4 * NR_TIME_DIVISOR;
  ...
  tlib_pass_if_str_equal(tname, scoped_metric,
                         "Datastore/statement/MongoDB/my_table/my_operation");
  tlib_pass_if_time_equal(tname, txn->root_kids_duration, duration);
  test_one_node_populated(tname, txn,
                          "Datastore/statement/MongoDB/my_table/my_operation",
...
static void test_instance_info_with_data_hash(void) {
  ...
  tlib_pass_if_time_equal(tname, txn->root_kids_duration, duration);
  test_one_node_populated(
      tname, txn, "Datastore/statement/MongoDB/my_table/my_operation", duration,
      "{\"host\":\"super_db_host\",\"port_path_or_id\":\"3306\",\"database_"
      "name\":\"my_database\"}");
  test_metric_table_size(tname, txn->scoped_metrics, 1);
...
static void test_instance_database_name_reporting_disabled(void) {
  ...
  test_one_node_populated(
      tname, txn, "Datastore/statement/MongoDB/my_table/my_operation", duration,
      "{\"host\":\"super_db_host\",\"port_path_or_id\":\"3306\"}");
  test_metric_table_size(tname, txn->scoped_metrics, 1);
  test_metric_table_size(tname, txn->unscoped_metrics, 4);
  test_metric_created(tname, txn->unscoped_metrics, MET_FORCED, duration,
                      "Datastore/all");
...
static void test_instance_info_without_data_hash(void) {
  const char* tname = "instance info without data hash";
  nrtxn_t* txn = new_txn(0);
  nrtime_t duration = 4 * NR_TIME_DIVISOR;
  nr_node_datastore_params_t params = sample_node_datastore_params(duration);
...
static void test_instance_info_empty(void) {
  ...
  test_one_node_populated(
      tname, txn, "Datastore/statement/MongoDB/my_table/my_operation", duration,
      "{\"host\":\"unknown\",\"port_path_or_id\":\"unknown\",\"database_name\":"
      "\"unknown\"}");
  test_metric_table_size(tname, txn->scoped_metrics, 1);
  test_metric_table_size(tname, txn->unscoped_metrics, 4);
  test_metric_created(tname, txn->unscoped_metrics, MET_FORCED, duration,
...
static void test_instance_info_with_localhost(void) {
  ...
  char* expected_data = nr_formatf(
      "{\"host\":\"%s\",\"port_path_or_id\":\"3306\",\"database_name\":\"my_"
      "database\"}",
      system_host);
  char* expected_metric
      = nr_formatf("Datastore/instance/MongoDB/%s/3306", system_host);
  nr_free(system_host);

  params.instance
      = nr_datastore_instance_create("localhost", "3306", "my_database");

  nr_txn_end_node_datastore(txn, &params, NULL);
  tlib_pass_if_time_equal(tname, txn->root_kids_duration, duration);
  test_one_node_populated(tname, txn,
                          "Datastore/statement/MongoDB/my_table/my_operation",
                          duration, expected_data);
  test_metric_table_size(tname, txn->scoped_metrics, 1);
  test_metric_table_size(tname, txn->unscoped_metrics, 4);
  test_metric_created(tname, txn->unscoped_metrics, MET_FORCED, duration,
                      "Datastore/all");
...
static void test_instance_metric_with_slashes(void) {
  ...
  test_one_node_populated(
      tname, txn, "Datastore/statement/MongoDB/my_table/my_operation", duration,
      "{\"host\":\"super_db_host\",\"port_path_or_id\":\"\\/path\\/to\\/"
      "socket\",\"database_name\":\"my_database\"}");
  test_metric_table_size(tname, txn->scoped_metrics, 1);
  test_metric_table_size(tname, txn->unscoped_metrics, 4);
  test_metric_created(tname, txn->unscoped_metrics, MET_FORCED, duration,
                      "Datastore/all");
  ...
  test_metric_created(tname, txn->unscoped_metrics, 0, duration,
                      "Datastore/operation/MongoDB/my_operation");
  test_metric_created(
      tname, txn->unscoped_metrics, 0, duration,
      "Datastore/instance/MongoDB/super_db_host//path/to/socket");
  test_metric_created(tname, txn->scoped_metrics, 0, duration,
                      "Datastore/statement/MongoDB/my_table/my_operation");
  nr_txn_destroy(&txn);
  nr_datastore_instance_destroy(&params.instance);
...
static void test_explain_plan(void) {
  ...
  nr_txn_end_node_datastore(txn, &params, NULL);
  test_one_node_populated(tname, txn, "Datastore/statement/MySQL/table/select",
                          duration,
                          "{\"backtrace\":[\"Zip\",\"Zap\"],"
  ...
  tlib_pass_if_str_equal("raw slowsql saved", nr_slowsql_metric(slow),
                         "Datastore/statement/MySQL/table/select");
  tlib_pass_if_str_equal("raw slowsql saved", nr_slowsql_query(slow),
                         "SELECT * FROM table WHERE constant = 31");
  tlib_pass_if_str_equal("raw slowsql saved", nr_slowsql_params(slow),
                         "{\"explain_plan\":[[\"a\",\"b\"],[[1,2],[3,4]]],"
                         "\"backtrace\":[\"Zip\",\"Zap\"]}");
...
static void test_web_transaction_insert(void) {
  ...
  tlib_pass_if_time_equal(tname, txn->root_kids_duration, duration);
  test_one_node_populated(
      tname, txn, "Datastore/statement/MySQL/table/select", duration,
      "{\"backtrace\":[\"Zip\",\"Zap\"],"
...
static void test_stack_recorded(void) {
  ...
  test_one_node_populated(tname, txn, "Datastore/statement/MySQL/table/select",
                          duration, "{\"backtrace\":[\"Zip\",\"Zap\"]}");

  nr_txn_destroy(&txn);
}

static void test_missing_stack_callback(void) {
...
static void test_slowsql_raw_saved(void) {
  ...
  test_one_node_populated(tname, txn, "Datastore/statement/MySQL/table/select",
                          duration,
                          "{\"backtrace\":[\"Zip\",\"Zap\"],\"sql\":\"SELECT * "
                          "FROM table WHERE constant = 31\"}");

  nr_txn_destroy(&txn);
}

static void test_slowsql_obfuscated_saved(void) {
  ...
  tlib_pass_if_str_equal(tname, nr_slowsql_metric(slow),
                         "Datastore/statement/MySQL/table/select");
  tlib_pass_if_str_equal(tname, nr_slowsql_query(slow),
                         "SELECT * FROM table WHERE constant = ?");
  tlib_pass_if_str_equal(tname, nr_slowsql_params(slow),
                         "{\"backtrace\":[\"Zip\",\"Zap\"]}");
  test_one_node_populated(tname, txn, "Datastore/statement/MySQL/table/select",
                          duration,
...
static void test_table_not_found(void) {
  ...
  tlib_pass_if_time_equal(tname, txn->root_kids_duration, duration);
  test_one_node_populated(
      tname, txn, "Datastore/operation/MySQL/select", duration,
...
static void test_table_and_operation_not_found(void) {
  ...
  tlib_pass_if_time_equal(tname, txn->root_kids_duration, duration);
  test_one_node_populated(
      tname, txn, "Datastore/operation/MySQL/other", duration,
      "{\"backtrace\":[\"Zip\",\"Zap\"],\"sql_obfuscated\":\"*\"}");
  test_metric_table_size(tname, txn->scoped_metrics, 1);
  test_metric_table_size(tname, txn->unscoped_metrics, 2);
  test_metric_created(tname, txn->unscoped_metrics, MET_FORCED, duration,
  ...
  test_metric_created(tname, txn->unscoped_metrics, MET_FORCED, duration,
                      "Datastore/MySQL/all");
  test_metric_created(tname, txn->scoped_metrics, 0, duration,
                      "Datastore/operation/MySQL/other");
  nr_txn_destroy(&txn);
}
...
static void test_get_operation_and_table(void) {
  ...
  operation = NULL;
  table = nr_txn_node_sql_get_operation_and_table(
      NULL, &operation, "select * from my_table", &modify_table_name);
  ...
  table = nr_txn_node_sql_get_operation_and_table(
      &txn, &operation, "select * from my_table", &modify_table_name);
  ...
  operation = NULL;
  table = nr_txn_node_sql_get_operation_and_table(&txn, &operation, "select *",
                                                  &modify_table_name);
  ...
  operation = NULL;
  table = nr_txn_node_sql_get_operation_and_table(&txn, &operation,
                                                  "select * from fix_me", NULL);
  tlib_pass_if_str_equal("callback is NULL", table, "fix_me");
  tlib_pass_if_str_equal("callback is NULL", operation, "select");
  nr_free(table);
}

static void test_node_stack_worthy(void) {
  int rv;
  nrtxn_t txnv;
  nrtxn_t* txn = &txnv;
...
static void test_node_potential_explain_plan(void) {
  int rv;
  nrtxn_t txnv;
  nrtxn_t* txn = &txnv;
...
static void test_node_potential_slowsql(void) {
  int rv;
  nrtxn_t txnv;
  nrtxn_t* txn = &txnv;
