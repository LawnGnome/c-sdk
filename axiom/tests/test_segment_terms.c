...
static void
test_segment_terms_create_destroy (void)
{
...
static void
test_segment_terms_create_from_obj (void)
{
  nrobj_t *obj;
  nr_segment_terms_t *terms;
...
test_segment_terms_add (void)
{
   ...
   * Test : Bad parameters.
   */
  tlib_pass_if_status_failure ("NULL segment terms", nr_segment_terms_add (NULL, prefix, whitelist));
  tlib_pass_if_status_failure ("NULL prefix", nr_segment_terms_add (terms, NULL, whitelist));
  tlib_pass_if_status_failure ("empty prefix", nr_segment_terms_add (terms, "", whitelist));
  tlib_pass_if_status_failure ("NULL whitelist", nr_segment_terms_add (terms, prefix, NULL));

  /*
   * Test : Normal operation.
   */
  tlib_pass_if_status_success ("add term", nr_segment_terms_add (terms, prefix, whitelist));
   ...
   * Test : Full terms.
   */
  tlib_pass_if_status_failure ("add term", nr_segment_terms_add (terms, prefix, whitelist));
  tlib_pass_if_int_equal ("terms size", 2, terms->size);
...
static void
test_segment_terms_add_from_obj (void)
{
  nrobj_t *invalid_rule;
  nrobj_t *rule;
  nr_segment_terms_t *terms;

  rule = nro_create_from_json ("{\"prefix\":\"Foo/Bar\",\"terms\":[\"a\",\"b\"]}");
  terms = nr_segment_terms_create (2);

  /*
   * Test : Bad parameters.
   */
  tlib_pass_if_status_failure ("NULL segment terms", nr_segment_terms_add_from_obj (NULL, rule));
  tlib_pass_if_status_failure ("NULL rule", nr_segment_terms_add_from_obj (terms, NULL));

  invalid_rule = nro_new_array ();
  tlib_pass_if_status_failure ("non-object rule", nr_segment_terms_add_from_obj (terms, invalid_rule));
  nro_delete (invalid_rule);

  /*
   * Test : Normal operation.
   */
  tlib_pass_if_status_success ("add term", nr_segment_terms_add_from_obj (terms, rule));
  tlib_pass_if_int_equal ("terms size", 1, terms->size);
  tlib_pass_if_str_equal ("rule prefix", "Foo/Bar/", terms->rules[0]->prefix);
   ...
   * Test : Full terms.
   */
  tlib_pass_if_status_failure ("add term", nr_segment_terms_add_from_obj (terms, rule));
  tlib_pass_if_int_equal ("terms size", 2, terms->size);
...
static void
test_segment_terms_apply (void)
{
  int i;
  char *json;
  nrobj_t *tests;

#define SEGMENT_TERMS_TESTS_FILE CROSS_AGENT_TESTS_DIR "/transaction_segment_terms.json"
...
static void
test_segment_terms_rule_create_destroy (void)
{
  ...
  tlib_pass_if_null ("NULL terms", nr_segment_terms_rule_create ("Foo/Bar", NULL));
  tlib_pass_if_null ("invalid terms", nr_segment_terms_rule_create ("Foo/Bar", invalid_terms));
...
static void
test_segment_terms_rule_build_regex (void)
{
  char *regex = NULL;
  nrobj_t *invalid_terms;
  nrobj_t *terms;
  ...
  tlib_pass_if_null ("invalid terms", nr_segment_terms_rule_build_regex (invalid_terms));
