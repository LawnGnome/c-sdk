...
#define tlib_php_request_end() \
  tlib_php_request_end_impl (__FILE__, __LINE__)
...
typedef void (*tlib_php_internal_function_handler_t) (INTERNAL_FUNCTION_PARAMETERS);
 ...
 *              are persisted.
 */
extern tlib_php_internal_function_handler_t
 ...
 *
 * TODO(aharvey): Consider moving these into tlib_main.h.
 */
...
#define tlib_specific_message_alloc(NAME, STEM, SUFFIXLEN) \
  ...
  size_t tlib_test_message_ ## NAME ## _len = nr_strlen (tlib_test_message_ ## NAME ## _stem); \
  size_t tlib_test_message_ ## NAME ## _suffixlen = (SUFFIXLEN); \
  char *tlib_test_message_ ## NAME = nr_zalloc (tlib_test_message_ ## NAME ## _len + tlib_test_message_ ## NAME ## _suffixlen + 1);
...
#define tlib_specific_message_set(NAME, SUFFIX) \
  do { \
    if ('\0' == *tlib_test_message_ ## NAME) { \
      nr_memcpy (tlib_test_message_ ## NAME, tlib_test_message_ ## NAME ## _stem, tlib_test_message_ ## NAME ## _len); \
    } \
    \
    nr_strlcpy (tlib_test_message_ ## NAME + tlib_test_message_ ## NAME ## _len, (SUFFIX), tlib_test_message_ ## NAME ## _suffixlen + 1); \
  } while (0)

#define tlib_specific_message(NAME) tlib_test_message_ ## NAME

/*
 * Useful assertions for PHP specific types.
 *
 * TODO(aharvey): Implement fail macros; for now I haven't bothered because I
 * don't need them, but they should be straightforward enough if required.
 */
...
#define tlib_pass_if_zend_function_is(M, CLASSNAME, NAME, FUNC) \
  do { \
    ...
    if (tlib_test_classname) { \
      tlib_fail_if_null (tlib_specific_message (zfi), tlib_test_func->common.scope); \
      \
      tlib_specific_message_set (zfi, " class name"); \
      tlib_pass_if_str_equal (tlib_specific_message (zfi), tlib_test_classname, nr_php_class_entry_name (tlib_test_func->common.scope)); \
    } else { \
      tlib_pass_if_null (tlib_specific_message (zfi), tlib_test_func->common.scope); \
    } \
    \
    tlib_specific_message_set (zfi, " function name"); \
    tlib_pass_if_str_equal(tlib_specific_message (zfi), (NAME), nr_php_function_name (func)); \
    \
