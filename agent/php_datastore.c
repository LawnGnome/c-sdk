 ...
 */
static void
nr_php_modify_sql_table_name_magento2 (char *tablename)
{
 ...
 */
static void
nr_php_modify_sql_table_name_wordpress (char *tablename)
{
  if (NULL == tablename) {
    return;
  }
...
static nr_modify_table_name_fn_t
nr_php_modify_table_name_fn (TSRMLS_D)
{
...
void
nr_php_txn_end_node_sql (nrtxn_t *txn,
                         const nrtxntime_t *start, const nrtxntime_t *stop,
                         const char *sql, int sqllen,
...
char *
nr_php_datastore_make_key (const zval *conn, const char *extension)
{
  char *key = NULL;
  ...
  if (nr_php_is_zval_valid_resource (conn)) {
    ...
    nrl_verbosedebug (NRL_INSTRUMENT, "%s conn is unexpected type %d; expected resource, object, or conn to be NULL", NRSAFESTR (extension), Z_TYPE_P (conn));
  }
...
int
nr_php_datastore_has_conn (const char *key TSRMLS_DC)
{
  if (NULL == key) {
    return 0;
  }
...
void
nr_php_datastore_instance_save (const char *key, nr_datastore_instance_t *instance TSRMLS_DC)
{
  if ((NULL == key) || (NULL == instance)) {
    return;
  }

  nr_hashmap_update (NRPRG (datastore_connections), key, nr_strlen (key), instance);
}

nr_datastore_instance_t *
nr_php_datastore_instance_retrieve (const char *key TSRMLS_DC)
{
  if (NULL == key) {
    return NULL;
  }

  return (nr_datastore_instance_t *)nr_hashmap_get (NRPRG (datastore_connections), key, nr_strlen (key));
}

void
nr_php_datastore_instance_remove (const char *key TSRMLS_DC)
{
  if (NULL == key) {
    return;
  }
