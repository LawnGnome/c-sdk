 ...
 */
static void
nr_drupal8_add_method_callback (const zend_class_entry *ce,
                                ...
                                nrspecialfn_t callback TSRMLS_DC)
{
  ...
  if (NULL == ce) {
    nrl_verbosedebug (NRL_FRAMEWORK,
                      "Drupal 8: got NULL class entry in %s", __func__);
    return;
  }
 ...
 */
static int
nr_drupal8_is_function_in_call_stack (const char *function, const char *scope TSRMLS_DC)
{
  int found = 0;
  zval *frame = NULL;
  zval *trace = NULL;
  ...
  ZEND_HASH_FOREACH_VAL (Z_ARRVAL_P (trace), frame) {
    ...
    if (nr_php_is_zval_valid_string (frame_func)) {
      if ((NULL == scope) && (0 == nr_php_is_zval_valid_string (frame_scope))) {
        ...
        if (0 == nr_strnicmp (function, Z_STRVAL_P (frame_func), Z_STRLEN_P (frame_func))) {
          found = 1;
          goto out;
        }
        ...
        if ((0 == nr_strnicmp (function, Z_STRVAL_P (frame_func), Z_STRLEN_P (frame_func))) &&
            (0 == nr_strnicmp (scope, Z_STRVAL_P (frame_scope), Z_STRLEN_P (frame_scope)))) {
          found = 1;
          goto out;
        }
      }
...
NR_PHP_WRAPPER (nr_drupal8_name_the_wt)
{
  ...
  if (retval_ptr && *retval_ptr) {
     ...
     */
    if ((NULL != name) &&
        (0 == nr_drupal8_is_function_in_call_stack ("getTitle", "Drupal\\Core\\Controller\\TitleResolver" TSRMLS_CC))) {
      /*
       ...
       */
      nr_txn_set_path ("Drupal8", NRPRG (txn), name, NR_PATH_TYPE_ACTION, NR_OK_TO_OVERWRITE);
    }
...
NR_PHP_WRAPPER (nr_drupal8_wrap_view_execute)
{
  ...
  if ((0 == storage) ||
      (!nr_php_object_instanceof_class (storage, "Drupal\\views\\Entity\\View" TSRMLS_CC))) {
    nrl_verbosedebug (NRL_FRAMEWORK, "Drupal 8: ViewExecutable storage property isn't a View object");
    goto leave;
  }

  label = nr_php_call (storage, "label");
  if (0 == nr_php_is_zval_non_empty_string (label)) {
    nrl_verbosedebug (NRL_FRAMEWORK, "Drupal 8: View::label() didn't return a string");
    goto leave;
  }
 ...
 */
static int
nr_drupal8_apply_hook (zval *element, const zval *hook,
...
NR_PHP_WRAPPER (nr_drupal8_module_handler)
{
  ...
  if (0 == nr_php_object_instanceof_class (*retval_ptr,
...
nr_drupal8_enable (TSRMLS_D)
{
  ...
  nr_php_wrap_user_function (NR_PSTR ("Drupal\\page_cache\\StackMiddleware\\PageCache::get"),
