...
static const int nr_agent_desired_uds_max = sizeof (nr_agent_daemon_unaddr.sun_path) - 1;
...
nr_agent_connection_state_t nr_agent_connection_state = NR_AGENT_CONNECTION_STATE_START;

nr_status_t
nr_agent_initialize_daemon_connection_parameters (const char *listen_path, int external_port)
{
  /*
   * Check parameters
   */
  if (0 == external_port) {
    ...
    if (0 == listen_path) {
      nrl_error (NRL_DAEMON, "invalid daemon connection parameters: zero port and listen path");
      return NR_FAILURE;
    }
    ...
    if ('@' == listen_path[0]) {
      if (0 == listen_path[1]) {
        nrl_error (NRL_DAEMON, "invalid daemon abstract domain socket: name is missing");
        return NR_FAILURE;
      }
    } else if ('/' != listen_path[0]) {
      nrl_error (NRL_DAEMON, "invalid daemon UNIX-domain socket: path must be absolute");
      return NR_FAILURE;
    }
#else
    if ('/' != listen_path[0]) {
  ...
  if (0 == external_port) {
    ...
    nr_agent_daemon_sl = offsetof (struct sockaddr_un, sun_path) + nr_strlen (listen_path) + 1;
    nr_memset (nr_agent_daemon_sa, 0, sizeof (nr_agent_daemon_sa));

    nr_agent_daemon_unaddr.sun_family = AF_UNIX;
    nr_strlcpy (nr_agent_daemon_unaddr.sun_path, listen_path, sizeof (nr_agent_daemon_unaddr.sun_path));
    ...
    snprintf (nr_agent_connect_method_msg, sizeof (nr_agent_connect_method_msg), "uds=%s", listen_path);
  } else {
    /*
    ...
    nr_agent_connect_method_msg[0] = '\0';
    snprintf (nr_agent_connect_method_msg, sizeof (nr_agent_connect_method_msg), "port=%d", external_port);
  }
...
static int
nr_agent_create_socket (nr_socket_type_t listen_type)
{
  int fd;
  int fl;
  int err;
...
static void
nr_agent_warn_connect_failure (int connect_fd, int connect_rv, int connect_err)
{
  time_t now = time (0);

  if ((now - nr_agent_last_cant_connect_warning) < NR_AGENT_CANT_CONNECT_WARNING_BACKOFF_SECONDS) {
    return;
  }

  nr_agent_last_cant_connect_warning = now;

  nrl_warning (NRL_DAEMON | NRL_IPC, "daemon connect(fd=%d %.256s) returned %d errno=%.16s. "
...
static int
nr_get_daemon_fd_internal (int log_warning_on_connect_failure)
{
  int err;
  int fl;
  ...
  if (0 == fl) {
    nrl_verbosedebug (NRL_DAEMON | NRL_IPC, "daemon connect(fd=%d %.256s) succeeded",
      nr_agent_daemon_fd,
      nr_agent_connect_method_msg);
  } else {
    nrl_verbosedebug (NRL_DAEMON | NRL_IPC, "daemon connect(fd=%d %.256s) returned %d errno=%.16s",
      nr_agent_daemon_fd,
      nr_agent_connect_method_msg,
      fl,
      nr_errno (err));
  ...
  if ((0 == fl) || (EISCONN == err)) {
     ...
     * has completed.  Unix documentation says that this can be done with
     * select/poll along with getsockopt.  However, I (willhf) was unable to
     * get this to work with TCP.  Instead, we determine if the in-progress
     * connection has been completed by repeating the connect call.  Once the
...
int
nr_agent_try_daemon_connect (int time_limit_ms)
{
  int fd;
  int did_connect = 0;
  ...
  if (-1 != fd) {
    did_connect = 1;
  } else if (NR_AGENT_CONNECTION_STATE_IN_PROGRESS == nr_agent_connection_state) {
