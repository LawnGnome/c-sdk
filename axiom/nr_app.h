...
#define NR_PRINTABLE_LICENSE_WINDOW         "%." NR_STR2 (NR_PRINTABLE_LICENSE_WINDOW_SIZE) "s"
#define NR_PRINTABLE_LICENSE_FMT            NR_PRINTABLE_LICENSE_WINDOW "..." NR_PRINTABLE_LICENSE_WINDOW
#define NR_PRINTABLE_LICENSE_PREFIX_START   0
#define NR_PRINTABLE_LICENSE_SUFFIX_START   (NR_LICENSE_SIZE - NR_PRINTABLE_LICENSE_WINDOW_SIZE)
...
typedef struct _nr_app_info_t {
  int high_security;        /* Indicates if high security been set locally for this application */
  char *license;            /* License key provided */
  nrobj_t *settings;        /* New Relic settings */
  nrobj_t *environment;     /* Application environment */
  nrobj_t *labels;          /* Labels (https://newrelic.atlassian.net/wiki/display/eng/Labels+for+Language+Agents) */
  char *host_display_name;  /* Optional user-provided host name for UI (https://source.datanerd.us/agents/agent-specs/blob/master/2015-04-0001-CustomHostNames.md) */
  char *lang;               /* Language */
...
typedef struct _nrapp_t {
  ...
  time_t last_daemon_query;             /* Used by agent: Last time we queried daemon about this app */
  int failed_daemon_query_count;        /* Used by agent: Number of times daemon query has not returned valid */
  nrrules_t *url_rules;                 /* From RPM - rules for txn path. Only used by agent. */
  nrrules_t *txn_rules;                 /* From RPM - rules for full txn metric name. Only used by agent. */
  nr_segment_terms_t *segment_terms;    /* From RPM - rules for transaction segment terms. Only used by agent. */
 ...
 */
extern nrapp_t *nr_app_verify_id (nrapplist_t *applist, const char *agent_run_id);
