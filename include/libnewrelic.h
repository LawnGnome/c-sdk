...
typedef struct _newrelic_transaction_tracer_config_t {
  ...
  /*! Controls the format of the sql put into transaction traces. If this value
   *  is set to "off", transaction traces have no sql in them.  If set to
   *  "raw" the sql is untouched.  If set to "obfuscated", alphanumeric
   *  characters are set to '?'.  For example 'SELECT * FROM table WHERE
   *  foo = 42' is reported as 'SELECT * FROM table WHERE foo = ?'.
   *
