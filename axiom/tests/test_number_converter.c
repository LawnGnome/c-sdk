...
#define test_format_double(l,v,e) test_format_double_worker(l,v,e,__FILE__,__LINE__)
#define test_scan_double(l,s,e,n) test_scan_double_worker(l,s,e,n,__FILE__,__LINE__)

static void
test_scan_double_worker (const char *locale, const char *subject, double expect, const char *expect_end, const char *file, int line)
{
  double actual;
  char *end = 0;

  actual = nr_strtod (subject, &end);
  test_pass_if_true ("nr_strtod", actual == expect, "locale=%s actual=%g expect=%g", locale, actual, expect);
  if (0 != expect_end && 0 != end) {
    test_pass_if_true ("nr_strtod", 0 == nr_strcmp (end, expect_end), "locale=%s end=%s expect_end=%s", locale, end, expect_end);
  }
}

static void
test_format_double_worker (const char *locale, double val, const char *expect, const char *file, int line)
{
  int written;
  int expected_len;
  char actual[BUFSIZ];
   ...
   */
  test_pass_if_true ("nr_double_to_str", 0 == nr_stricmp (actual, expect), "locale=%s actual=%s expect=%s", locale, actual, expect);
  expected_len = nr_strlen (expect);
  test_pass_if_true ("nr_double_to_str", written == expected_len, "written=%d expected_len=%d", written, expected_len);
}

static void
test_format_doubles_buffering (void)
{
  char actual[BUFSIZ];
  double val = 256.0;
...
static void
test_scan_doubles (const char *locale)
{
  ...
  test_scan_double (locale, "65536.0,999", 65536.0, ",999");
  test_scan_double (locale, "65536,0,999", 65536.0, ",0,999");  /* hits corner case */
  test_scan_double (locale, "65536,0.999", 65536.0, ",0.999");  /* hits corner case */
  ...
  test_scan_double (locale, ";2.0", 0.0, ";2.0");  /* probably undefined what strtod returns when first non space char isn't legal */
...
static void
test_format_doubles (const char *locale)
{
  double nan;
  uint64_t int_nan;

  /*
   * Some implementations of the C library will print a Nan with the sign bit set as
   * a negative Nan.  Others won't.
   * So here we just smash the sign bit away.
   */
  nan = 0.0/0.0;
...
static void
test_format_doubles_locale (const char *new_locale)
{
 ...
 */
static void
test_format_doubles_locales (void)
{
  ...
  test_format_doubles_locale ("de_DE");  /* German (uses ',' for decimal radix separator) */
