...
int
nr_json_escape (char *dest, const char *json)
{
  char *ep;
      ...
      default:
        {
          ...
          if (0xc0 == (u_json[0] & 0xc0)) {  /* & 0b1100000 == 0b110000 */
            ...
            uint8_t lead_mask = 0x0;      /* bits of payload in byte 0 of the utf8 character */
            uint32_t code_point = 0x0;    /* the binary encoding of the code point */
            int fault = 0;                /* set if there's a decode fault somewhere in the subsidiary bytes */
            int i;
            ...
            if (0xc0 == (u_json[0] & 0xe0)) {
              ...
              /* NOTE: not fully implemented; no encoding possible with surrogate pairs */
            ...
            if (fault) {
              ...
              if (bits_in_code_point <= 16) {
                sprintf (ep, "\\u%04x", code_point & 0xffff);  /* TODO: Dangerous unchecked write */
                ...
                sprintf (ep, "\\u%04x\\u%04x", surrogate_0 & 0xffff, surrogate_1 & 0xffff);  /* TODO: Dangerous unchecked write */
                ep += (1 + 1 + 4  + 1 + 1 + 4);
              } else {
                goto fault;
              }
          ...
          fault: ;
            /*
             * Behavior of the encoder when presented with illegal UTF-8 is undefined.
             * Here we handle unknown or mis-encoded characters as a 16 bit UTF-8 encoding,
             * with the leading byte set to 0.
             */
