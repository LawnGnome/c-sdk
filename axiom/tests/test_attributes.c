...
static char *
nr_attribute_destination_modifier_to_json (const nr_attribute_destination_modifier_t *modifier)
{
  nrobj_t *obj;
  char *json;
  ...
  obj = nro_new_hash ();
  nro_set_hash_boolean (obj, "has_wildcard_suffix", modifier->has_wildcard_suffix);
  ...
  nro_set_hash_long (obj, "include_destinations", modifier->include_destinations);
  nro_set_hash_long (obj, "exclude_destinations", modifier->exclude_destinations);
...
static char *
nr_attribute_config_to_json (const nr_attribute_config_t *config)
{
  nrobj_t *obj;
  ...
  nro_set_hash_long (obj, "disabled_destinations", config->disabled_destinations);
...
#define test_modifier_as_json(...) test_modifier_as_json_fn (__VA_ARGS__,__FILE__,__LINE__)

static void
test_modifier_as_json_fn (
  const char *testname,
  const nr_attribute_destination_modifier_t *modifier,
  ...
  int line)
{
  ...
  test_pass_if_true (testname, 0 == nr_strcmp (expected_json, actual_json),
    "expected_json=%s actual_json=%s", expected_json, actual_json);
  nr_free (actual_json);
}

#define test_config_as_json(...) test_config_as_json_fn (__VA_ARGS__,__FILE__,__LINE__)

static void
test_config_as_json_fn (
  const char *testname,
...
static void
test_destination_modifier_match (void)
{
  nr_attribute_destination_modifier_t *modifier;
  uint32_t all = NR_ATTRIBUTE_DESTINATION_ALL;
  int rv;

  rv = nr_attribute_destination_modifier_match (0, "alpha", nr_mkhash ("alpha", 0));
  tlib_pass_if_true ("null modifier", 0 == rv, "rv=%d", rv);

  modifier = nr_attribute_destination_modifier_create ("alpha", all, all);

  rv = nr_attribute_destination_modifier_match (modifier, "alpha", nr_mkhash ("alpha", 0));
  tlib_pass_if_true ("exact match success", 1 == rv, "rv=%d", rv);

  rv = nr_attribute_destination_modifier_match (modifier, "alpha", nr_mkhash ("alpha", 0) + 1);
  tlib_pass_if_true ("wrong hash", 0 == rv, "rv=%d", rv);

  rv = nr_attribute_destination_modifier_match (modifier, "alphaa", nr_mkhash ("alpha", 0));
  ...
  rv = nr_attribute_destination_modifier_match (modifier, "alpha.beta", nr_mkhash ("alpha.beta", 0));
  tlib_pass_if_true ("wildcard match success", 1 == rv, "rv=%d", rv);

  rv = nr_attribute_destination_modifier_match (modifier, "AlPhA.BeTa", nr_mkhash ("AlPhA.BeTa", 0));
  tlib_pass_if_true ("case sensitive wildcard match success", 0 == rv, "rv=%d", rv);

  rv = nr_attribute_destination_modifier_match (modifier, "alpha.", nr_mkhash ("alpha.", 0));
  tlib_pass_if_true ("wildcard match success (no wildcard chars)", 1 == rv, "rv=%d", rv);
...
static void
test_destination_modifier_apply (void)
{
  uint32_t destinations;
  nr_attribute_destination_modifier_t *modifier;
  uint32_t all = NR_ATTRIBUTE_DESTINATION_ALL;

  destinations = nr_attribute_destination_modifier_apply (0, "alpha", nr_mkhash ("alpha", 0), all);
  tlib_pass_if_true ("null modifier", all == destinations, "destinations=%u", destinations);

  modifier = nr_attribute_destination_modifier_create ("alpha", all, 0);
  destinations = nr_attribute_destination_modifier_apply (modifier, "alpha", nr_mkhash ("alpha", 0), 0);
  tlib_pass_if_true ("include", all == destinations, "destinations=%u", destinations);
  nr_attribute_destination_modifier_destroy (&modifier);

  modifier = nr_attribute_destination_modifier_create ("alpha", 0, all);
  destinations = nr_attribute_destination_modifier_apply (modifier, "alpha", nr_mkhash ("alpha", 0), all);
  tlib_pass_if_true ("exclude", 0 == destinations, "destinations=%u", destinations);
  nr_attribute_destination_modifier_destroy (&modifier);

  modifier = nr_attribute_destination_modifier_create ("alpha",
  ...
  destinations = nr_attribute_destination_modifier_apply (modifier, "alpha", nr_mkhash ("alpha", 0),
    NR_ATTRIBUTE_DESTINATION_TXN_EVENT | NR_ATTRIBUTE_DESTINATION_BROWSER);
  tlib_pass_if_true ("include and exclude, exclude has priority", 
  (NR_ATTRIBUTE_DESTINATION_TXN_TRACE | NR_ATTRIBUTE_DESTINATION_BROWSER) == destinations,
  "destinations=%u", destinations);
  nr_attribute_destination_modifier_destroy (&modifier);
}

static void
test_modifier_destroy_bad_params (void)
{
  nr_attribute_destination_modifier_t *modifier;
...
static void
test_disable_destinations (void)
{
  ...
  tlib_pass_if_true ("config has no starting disabled destinations",
    0 == config->disabled_destinations, "config->disabled_destinations=%u",
  ...
  tlib_pass_if_true ("destinations successfully disabled",
    (event | error) == config->disabled_destinations, "config->disabled_destinations=%u",
    config->disabled_destinations);
  nr_attribute_config_destroy (&config);
}

static void
test_destination_modifier_create (void)
{
  nr_attribute_destination_modifier_t *modifier;
  uint32_t event = NR_ATTRIBUTE_DESTINATION_TXN_EVENT;
  uint32_t error = NR_ATTRIBUTE_DESTINATION_ERROR;
  ...
  tlib_pass_if_true ("null match string", 0 == modifier, "modifier=%p", modifier);
...
static void
test_config_modify_destinations (void)
{
  nr_attribute_config_t *config;
  uint32_t event = NR_ATTRIBUTE_DESTINATION_TXN_EVENT;
...
test_config_copy (void)
{
  ...
  tlib_pass_if_true ("empty config copied exactly", 0 == nr_strcmp (config_json, config_copy_json),
    "config_json=%s config_copy_json=%s", NRSAFESTR (config_json), NRSAFESTR (config_copy_json));
  nr_free (config_json);
  ...
  config_copy_json = nr_attribute_config_to_json (config_copy);
  config_json = nr_attribute_config_to_json (config);
  tlib_pass_if_true ("full config copied exactly", 0 == nr_strcmp (config_json, config_copy_json),
    "config_json=%s config_copy_json=%s", NRSAFESTR (config_json), NRSAFESTR (config_copy_json));
  nr_free (config_json);
  nr_free (config_copy_json);
  nr_attribute_config_destroy (&config);
...
test_config_apply (void)
{
  ...
  tlib_pass_if_true ("zero input", 0 == destinations, "destinations=%u", destinations);

  destinations = nr_attribute_config_apply (0, "alpha", nr_mkhash ("alpha", 0), event);
  tlib_pass_if_true ("null config", event == destinations, "destinations=%u", destinations);

  destinations = nr_attribute_config_apply (config, 0, nr_mkhash ("alpha", 0), event);
  tlib_pass_if_true ("null key", 0 == destinations, "destinations=%u", destinations);

  /*
   * Test that the destination modifier are applied in the correct order.
   */
  nr_attribute_config_modify_destinations (config, "alpha.*", browser | trace, 0);
  nr_attribute_config_modify_destinations (config, "alpha.beta", error, browser);

  destinations = nr_attribute_config_apply (config, "alpha.beta", nr_mkhash ("alpha.beta", 0), event);
  tlib_pass_if_true ("destinations correctly modified", (trace | error | event) == destinations,
    "destinations=%u", destinations);
  ...
  destinations = nr_attribute_config_apply (config, "alpha.beta", nr_mkhash ("alpha.beta", 0), event);
  tlib_pass_if_true ("destinations disabled after modification", (error | event) == destinations,
    "destinations=%u", destinations);
...
static void
test_config_destroy_bad_params (void)
{
  nr_attribute_config_t *config;
...
static void
test_attribute_destroy_bad_params (void)
{
...
static void
test_attributes_destroy_bad_params (void)
{
...
#define test_user_attributes_as_json(...)  test_attributes_as_json_fn (__VA_ARGS__,1,__FILE__,__LINE__)
#define test_agent_attributes_as_json(...) test_attributes_as_json_fn (__VA_ARGS__,0,__FILE__,__LINE__)

static void
test_attributes_as_json_fn (
  const char *testname,
...
test_remove_duplicate (void)
{
  ...
  nr_attributes_agent_add_long (attributes, event, "zap", 1);
  nr_attributes_agent_add_long (attributes, event, "alpha", 1);
  nr_attributes_remove_duplicate (attributes, "alpha", nr_mkhash ("alpha", 0) + 1, 0);
  test_agent_attributes_as_json ("hash correctly used to find duplicated", attributes, all,
    "{\"alpha\":1,\"zap\":1,\"zip\":1}");
...
test_add (void)
{
  ...
  tlib_pass_if_true ("add success", NR_SUCCESS == st, "st=%d", (int)st);
  st = nr_attributes_agent_add_string (attributes, browser | error, "theta", "789");
  tlib_pass_if_true ("add success", NR_SUCCESS == st, "st=%d", (int)st);

  st = nr_attributes_agent_add_string (attributes, 0, "no_destinations_ignore_me", "789");
  tlib_pass_if_true ("attribute with no destinations", NR_FAILURE == st, "st=%d", (int)st);

  st = nr_attributes_agent_add_string (attributes, 1 << 10, "no_valid_destinations_ignore_me", "789");
  tlib_pass_if_true ("add success", NR_SUCCESS == st, "st=%d", (int)st);

  test_user_attributes_as_json ("user attributes: all", attributes, all,
    "{\"gamma\":\"789\",\"beta\":456,\"alpha\":123}");
  test_agent_attributes_as_json ("agent attributes: all", attributes, all,
  ...
  test_user_attributes_as_json ("user attributes: browser", attributes, browser,
    "{}");
  test_agent_attributes_as_json ("agent attributes: browser", attributes, browser,
    "{\"theta\":\"789\",\"omega\":456,\"psi\":123}");
...
static void
test_attributes_to_obj_bad_params (void)
{
  /* Don't blow up! */
...
static void
test_attribute_string_length_limits (void)
{
  nr_attributes_t *attributes;
  nr_attribute_config_t *config;
  uint32_t all = NR_ATTRIBUTE_DESTINATION_ALL;
  ...
  tlib_pass_if_true ("tests valid", NR_ATTRIBUTE_KEY_LENGTH_LIMIT   == 255,
    "NR_ATTRIBUTE_KEY_LENGTH_LIMIT=%d", NR_ATTRIBUTE_KEY_LENGTH_LIMIT);
  tlib_pass_if_true ("tests valid", NR_ATTRIBUTE_VALUE_LENGTH_LIMIT == 255,
    "NR_ATTRIBUTE_VALUE_LENGTH_LIMIT=%d", NR_ATTRIBUTE_VALUE_LENGTH_LIMIT);
  ...
  st = nr_attributes_user_add_string  (attributes, all,
    ...
    "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
    "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
    "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", /* 320 chars */
    "alpha");
  tlib_pass_if_true ("key exceeds limit", NR_FAILURE == st, "st=%d", (int)st);
  test_user_attributes_as_json ("key exceeds limit", attributes, all,
    "null");
  ...
  test_user_attributes_as_json ("value gets truncated", attributes, all,
    "{"
      "\""
      "alpha"
      "\":\""
...
static void
test_user_attribute_limit (void)
{
  int i;
  char buf[128];
  nr_attributes_t *attributes;
  ...
  st = nr_attributes_user_add_string (attributes, all, "cant_add_me", "cant_add_me");
  tlib_pass_if_true ("user attribute limit upheld", NR_FAILURE == st, "st=%d", (int)st);
  ...
  tlib_pass_if_true ("replacement still works when limit reached", NR_SUCCESS == st, "st=%d", (int)st);

  test_user_attributes_as_json ("replacement still works when limit reached", attributes, all,
    "{\"0\":\"BEEN_REPLACED\",\"63\":\"63\",\"62\":\"62\",\"61\":\"61\",\"60\":\"60\","
     "\"59\":\"59\",\"58\":\"58\","
     "\"57\":\"57\",\"56\":\"56\",\"55\":\"55\",\"54\":\"54\",\"53\":\"53\",\"52\":\"52\","
     "\"51\":\"51\",\"50\":\"50\",\"49\":\"49\",\"48\":\"48\",\"47\":\"47\",\"46\":\"46\","
     "\"45\":\"45\",\"44\":\"44\",\"43\":\"43\",\"42\":\"42\",\"41\":\"41\",\"40\":\"40\","
...
static nr_attribute_config_t *
cross_agent_attribute_config_from_obj (const nrobj_t *obj)
{
  int i;
  ...
  struct {
    ...
    {"transaction_events.attributes.enabled", 1, NR_ATTRIBUTE_DESTINATION_TXN_EVENT},
    {"transaction_tracer.attributes.enabled", 1, NR_ATTRIBUTE_DESTINATION_TXN_TRACE},
    {"error_collector.attributes.enabled",    1, NR_ATTRIBUTE_DESTINATION_ERROR},
    {"browser_monitoring.attributes.enabled", 0, NR_ATTRIBUTE_DESTINATION_BROWSER},
    {0, 0, 0}
  };
  struct {
    const char *name;
    ...
    {"transaction_events.attributes.exclude",   NR_ATTRIBUTE_DESTINATION_TXN_EVENT, 0},
    {"transaction_tracer.attributes.exclude",   NR_ATTRIBUTE_DESTINATION_TXN_TRACE, 0},
    {"error_collector.attributes.exclude",      NR_ATTRIBUTE_DESTINATION_ERROR,     0},
    {"browser_monitoring.attributes.exclude",   NR_ATTRIBUTE_DESTINATION_BROWSER,   0},

    {"attributes.include",                      0, NR_ATTRIBUTE_DESTINATION_ALL},
    {"transaction_events.attributes.include",   0, NR_ATTRIBUTE_DESTINATION_TXN_EVENT},
    {"transaction_tracer.attributes.include",   0, NR_ATTRIBUTE_DESTINATION_TXN_TRACE},
    {"error_collector.attributes.include",      0, NR_ATTRIBUTE_DESTINATION_ERROR},
    {"browser_monitoring.attributes.include",   0, NR_ATTRIBUTE_DESTINATION_BROWSER},
    {0, 0, 0}
  };
  ...
  for (i = 0; enabled_settings[i].name; i++) {
    int enabled = nr_reply_get_bool (obj, enabled_settings[i].name, enabled_settings[i].dflt);

    if (0 == enabled) {
      nr_attribute_config_disable_destinations (attribute_config, enabled_settings[i].destinations);
    }
  }
  ...
  for (i = 0; include_exclude_settings[i].name; i++) {
    int j;
    const nrobj_t *arr = nro_get_hash_array (obj, include_exclude_settings[i].name, 0);
    ...
    for (j = 0; j < arr_size; j++) {
      ...
      if (entry) {
        nr_attribute_config_modify_destinations (attribute_config, entry,
          include_exclude_settings[i].include_destinations, include_exclude_settings[i].exclude_destinations);
      }
    }
  }
...
static uint32_t
cross_agent_destinations_from_array (const nrobj_t *arr)
{
...
static uint32_t
cross_agent_tests_get_actual_destinations (const nr_attributes_t *attributes, const char *input_key)
{
...
static void
test_cross_agent_attribute_configuration (void)
{
  char *json = 0;
  nrobj_t *array = 0;
  nrotype_t otype;
...
#define TEST_ATTRIBUTES_CONFIGURATION_TEST_FILE CROSS_AGENT_TESTS_DIR "/attribute_configuration.json"
  json = nr_read_file_contents (TEST_ATTRIBUTES_CONFIGURATION_TEST_FILE, 10 * 1000 * 1000);
  ...
  if (array && (NR_OBJECT_ARRAY == nro_type (array))) {
    for (i = 1; i <= nro_getsize (array); i++) {
      ...
      tlib_pass_if_true ("tests valid", 0 != input_key, "input_key=%p", input_key);
      ...
      nr_attributes_agent_add_long (attributes, input_default_destinations, input_key, 12345);
      actual_destinations = cross_agent_tests_get_actual_destinations (attributes, input_key);

      tlib_pass_if_true (testname ? testname : "unknown", expected_destinations == actual_destinations,
        "expected_destinations=%u actual_destinations=%u", expected_destinations, actual_destinations);
...
static void
test_null_and_bools_and_double (void)
{
  ...
  obj = nr_attributes_user_to_obj (atts, event);
  test_obj_as_json ("true, false, null", obj,
