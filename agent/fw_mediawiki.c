 ...
 * contain a member named 'action'. A bit convoluted but there you have it.
 */
NR_PHP_WRAPPER (nr_mediawiki_name_the_wt_non_api)
{
  ...
  if (nr_php_is_zval_non_empty_string (arg1)) {
    ...
    if (nr_php_is_zval_non_empty_string (arg2)) {
      ...
      nr_txn_set_path ("MediaWiki non-API", NRPRG (txn), name, NR_PATH_TYPE_ACTION, NR_NOT_OK_TO_OVERWRITE);
    }
  }
...
NR_PHP_WRAPPER (nr_mediawiki_name_the_wt_api)
{
  ...
  if (nr_php_is_zval_valid_object (arg1)) {
    ...
    if (nr_php_is_zval_valid_string (action)) {
      ...
      nr_txn_set_path ("MediaWiki_API", NRPRG (txn), name, NR_PATH_TYPE_ACTION, NR_NOT_OK_TO_OVERWRITE);
    }
  }
...
NR_PHP_WRAPPER (nr_mediawiki_getaction)
{
  ...
  if ((NULL == return_value) || (0 == nr_php_is_zval_non_empty_string (*return_value))) {
   ...
   * anyway).
   */
  nr_txn_set_path ("MediaWiki non-API", NRPRG (txn), name, NR_PATH_TYPE_ACTION, NR_OK_TO_OVERWRITE);
 ...
 */
void
nr_mediawiki_enable (TSRMLS_D)
{
  /*
   * Instrumentation for MediaWiki before version 1.18.0.
   */
  nr_php_wrap_user_function (NR_PSTR ("MediaWiki::setVal"), nr_mediawiki_name_the_wt_non_api TSRMLS_CC);
  nr_php_wrap_user_function (NR_PSTR ("ApiMain::__construct"), nr_mediawiki_name_the_wt_api TSRMLS_CC);

  /*
   * Instrumentation for MediaWiki 1.18.0 and later.
   */
  nr_php_wrap_user_function (NR_PSTR ("MediaWiki::getAction"), nr_mediawiki_getaction TSRMLS_CC);
  nr_php_wrap_user_function (NR_PSTR ("ApiMain::setupExecuteAction"), nr_mediawiki_apimain_setupexecuteaction TSRMLS_CC);
}
