...
// To take control of BigChange, change the following variables
// to point to your origin and branch and regenerate using
// the PHP_Refresh_Jobs job.
//
// Defaults:
//   ownerAndRepo = 'php-agent/php_agent'
//   branchSpec = '*/master'
def ownerAndRepo = 'php-agent/php_agent'
def branchSpec = '*/master'

// Whether to produce an optimized or debug build. Useful when
// BigChange is used to produce a build for a customer to try.
...
use(extensions) {
  matrixJob('php-bigchange-agent') {
    description 'Build the agent on the current big change branch.'
    ...
    publishers {
      ...
      downstream('php-bigchange-packages')
      downstream('php-bigchange-tarballs')
  ...
  matrixJob('php-bigchange-packages') {
    description 'Create the packages for the current big change branch.'
    ...
    steps {
      ...
      for (lbl in ['centos5-32-nrcamp', 'centos5-64-nrcamp']) {
        copyArtifacts("php-bigchange-agent/label=${lbl}") {
  ...
  freeStyleJob('php-bigchange-tarballs') {
    description 'Create the tarballs for the current big change branch.'
