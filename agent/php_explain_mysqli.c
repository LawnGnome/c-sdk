 ...
 */
static int
nr_php_explain_mysqli_add_field_to_plan (zval *field,
 ...
 */
static nr_status_t
nr_php_explain_mysqli_add_fields_to_plan (zval *fields,
 ...
 */
static nr_status_t
nr_php_explain_mysqli_execute (zval *stmt TSRMLS_DC);
...
nr_php_explain_mysqli_fetch_plan (zval *stmt TSRMLS_DC);
 ...
 */
static nr_explain_plan_t *
nr_php_explain_mysqli_issue (zval *link, nr_php_object_handle_t handle,
 ...
 */
static zval *
nr_php_explain_mysqli_prepare (zval *link, const char *query TSRMLS_DC);

nr_explain_plan_t *
nr_php_explain_mysqli_query (const nrtxn_t *txn, zval *link,
...
nr_explain_plan_t *
nr_php_explain_mysqli_stmt (const nrtxn_t *txn, nr_php_object_handle_t handle,
                            const nrtxntime_t *start, const nrtxntime_t *stop
                            TSRMLS_DC)
{
  nrtime_t duration;
                                          ...
                                          nr_explain_plan_t *plan TSRMLS_DC)
{
  ...
  nr_php_zend_hash_zval_apply (Z_ARRVAL_P (fields),
...
nr_php_explain_mysqli_fetch_plan (zval *stmt TSRMLS_DC)
{
   ...
   * libmysqlclients build don't support this, so we have to use the horrible
  ...
  if (NR_FAILURE == nr_php_explain_mysqli_add_fields_to_plan (fields,
                                                              plan TSRMLS_CC)) {
   ...
   */
  bind_retval = nr_php_call_user_func (stmt, "bind_result",
                                       num_fields, results TSRMLS_CC);
