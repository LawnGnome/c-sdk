...
static void nr_execute_handle_framework(const nr_framework_table_t frameworks[],
  ...
  if (NR_FW_UNSET == NRINI(force_framework)) {
    ...
    forced_framework = nr_try_force_framework(
        frameworks, num_frameworks, NRINI(force_framework), op_array TSRMLS_CC);
    ...
    if (OP_ARRAY_IS_FILE(op_array, frameworks[i].file_to_check)) {
      ...
      if (frameworks[i].special) {
        nr_framework_classification_t special
            = frameworks[i].special(op_array TSRMLS_CC);
...
static void nr_php_max_nesting_level_reached(TSRMLS_D) {
   ...
   * longjmp to discard all of the C frames and PHP frames, that the stack depth
   * is correct. Execution will probably not continue after E_ERROR; that
   * decision may rest on the error handler(s) registered as callbacks.
   */
