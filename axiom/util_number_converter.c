...
void
nr_itoa (char *buf, size_t len, int x)
{
  char scratch[24];
  char *p;
  char sign;
...
int
nr_double_to_str (char *buf, int buf_len, double input)
{
  int i;
  int natural_width;
   ...
   * merely prints "123456".  %.17g on 0.333 prints something like "0.3330000000005
   */
  natural_width = snprintf (buf, buf_len, "%.5f", input);

  actually_written = (natural_width < (buf_len - 1)) ? natural_width : (buf_len - 1);
   ...
   *
   * The story isn't so clean, however, for nr_strtod, which is scanning numbers from
   * a variety of sources.
   *
   * The ice is very thin here, especially across platforms.
   */
...
/*
 * For our uses, nr_strtod will only be used to scan numbers that obey JSON conventions
 * from http://json.org/
 *
 * Because of the JSON spec, we only expect to see numbers written by a "C"-locale printf
 * which uses '.' as the decimal radix.
 *
 ...
 *
 * In both cases we make a local copy of the incoming number, fiddle with characters,
 * and rescan with the locale sensitive strtod.
 *
 * For the common case of "C" locale, or "en_EN", we'll only call srtod on the string once,
 * which is the most expensive part.
 */
double
nr_strtod (const char *buf, char **endptr)
{
  typedef enum {
    OK,
    UNDERSHOT,
  ...
  if (0 == buf) {
    ...
    return 0.0;  /* we could also return NaN, perhaps */
  }
  ...
  for (i = 0; &buf[i] != myendptr; i++) {
    if ('.' == buf[i]) {
      /*
       * strtod scanned through a '.', so that must have been the current locale's
    ...
    if (',' == buf[i]) {
      /*
       * strtod scanned through a ',', so that must have been the current locale's
       * decimal radix character, which is not what the writer intended.
       */
      work_code = OVERSHOT;
      position_radix = i;
      break;
  ...
  /*
   * Copy into a local buffer, so we can try to convert the fixed incoming decimal radix '.'
   * to the locale's presumed radix character of ','.
   */
  nr_strlcpy (copy, buf, sizeof (copy));

  if (UNDERSHOT == work_code) {
    copy[position_radix] = ','; /* strtod refused to scan a '.', and it presumably would scan a ','.  */
  } else {
...
#if defined(__clang_) || ((__GNUC__ > 4) || ((__GNUC__ == 4) && (__GNUC_MINOR__ > 5)))
