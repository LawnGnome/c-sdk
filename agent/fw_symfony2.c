 ...
 */
static int
nr_symfony2_name_the_wt_from_zval (const zval *name TSRMLS_DC)
{
  if (nrlikely (nr_php_is_zval_non_empty_string (name))) {
    char *path = nr_strndup (Z_STRVAL_P (name), Z_STRLEN_P (name));

    nr_txn_set_path ("Symfony2", NRPRG (txn), path, NR_PATH_TYPE_ACTION, NR_OK_TO_OVERWRITE);  /* Watch out: this name is OK to overwrite */
 ...
 */
static zval *
nr_symfony2_object_get_string (zval *obj, const char *param TSRMLS_DC)
{
  zval *rval = 0;
...
NR_PHP_WRAPPER (nr_symfony2_name_the_wt)
{
  ...
  if (0 == nr_php_is_zval_valid_object (event)) {
    nrl_verbosedebug (NRL_TXN, "Symfony 2: RouterListener::onKernelRequest() does not have an event parameter");
    NR_PHP_WRAPPER_CALL;
    goto end;
  }
  ...
  if (nr_php_object_instanceof_class (request, "Symfony\\Component\\HttpFoundation\\Request" TSRMLS_CC)) {
    /* Let's look for _route first. */
    zval *route_rval = nr_symfony2_object_get_string (request, "_route" TSRMLS_CC);

    if (route_rval) {
      if (NR_SUCCESS != nr_symfony2_name_the_wt_from_zval (route_rval TSRMLS_CC)) {
        nrl_verbosedebug (NRL_TXN, "Symfony 2: Request::get('_route') returned a non-string");
      }
      nr_php_zval_free (&route_rval);
    } else {
      /* No _route. Look for _controller. */
      zval *controller_rval = nr_symfony2_object_get_string (request, "_controller" TSRMLS_CC);

      if (controller_rval) {
        if (NR_SUCCESS != nr_symfony2_name_the_wt_from_zval (controller_rval TSRMLS_CC)) {
          nrl_verbosedebug (NRL_TXN, "Symfony 2: Request::get('_controller') returned a non-string");
        }
        nr_php_zval_free (&controller_rval);
      } else {
        nrl_verbosedebug (NRL_TXN, "Symfony 2: Neither _controller nor _route is set");
      }
    }
  } else {
    nrl_verbosedebug (NRL_TXN, "Symfony 2: GetResponseEvent::getRequest() returned a non-Request object");
  }
...
void
nr_symfony2_enable (TSRMLS_D)
{
  /*
   * We set the path to 'unknown' to prevent having to name routing errors.
   */
  nr_txn_set_path ("Symfony2", NRPRG (txn), "unknown", NR_PATH_TYPE_ACTION, NR_NOT_OK_TO_OVERWRITE);
