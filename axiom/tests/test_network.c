...
static void
test_write_parse_preamble (void)
{
...
static void
test_parse_preamble_bad_params (void)
{
  nrbuf_t *buf = nr_buffer_create (0, 0);
  uint32_t datalen;
  nr_status_t rv;
...
static void
test_parse_preamble_corrupted (void)
{
  nrbuf_t *buf;
  uint32_t datalen = 0;
  nr_status_t rv;
...
static void
test_send_receive_success (void)
{
  int socks[2];
  nr_status_t st;
  nrbuf_t *buf;
...
test_send_bad_params (void)
{
  ...
  deadline = nr_get_time () + (TEST_NETWORK_TIMEOUT_MS * NR_TIME_DIVISOR_MS);
  st = nr_write_message (socks[0], "hello world", NR_PROTOCOL_CMDLEN_MAX_BYTES + 1, deadline);
...
test_read_times_out (void)
{
  ...
  /*
   * TODO(msl): Figure out why this had to be so large on BSD and sometimes on linux
   */
...
static void
test_set_nonblocking_bad_param (void)
{
