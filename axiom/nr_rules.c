...
void
nr_rules_destroy (nrrules_t **rules_p)
{
  int i;
  nrrules_t *rules;
 ...
 */
const int nr_rules_regex_options = NR_REGEX_CASELESS | NR_REGEX_DOLLAR_ENDONLY | NR_REGEX_DOTALL;

nr_status_t
nr_rules_add (nrrules_t *rules, uint32_t flags, int order, const char *match, const char *repl)
{
  nr_regex_t *regex;
  nrrule_t *rule;
  ...
  if (NULL == regex) {
     ...
     */
    nrl_warning (NRL_RULES, "RPM rule " NRP_FMT " failed to compile", NRP_REGEXP (match));
    return NR_FAILURE;
  }

  if (rules->nalloc == rules->nrules) {
    rules->nalloc += 8;
    rules->rules = (nrrule_t *)nr_realloc (rules->rules, rules->nalloc * sizeof (nrrule_t));
  }
...
static int
qsort_comparator_for_rules (const void *e1, const void *e2)
{
...
void
nr_rules_sort (nrrules_t *rules)
{
  if ((0 == rules) || (0 == rules->rules) || (0 == rules->nrules)) {
    return;
  }

  qsort ((void *)rules->rules, rules->nrules, sizeof (nrrule_t), qsort_comparator_for_rules);
}

void
nr_rule_replace_string (const char *repl, char *dest, size_t dest_len, const nr_regex_substrings_t *ss)
{
  int state = 0;
  int ch;
  int num = 0;
  ...
  while (0 != (ch = *repl)) {
    if (0 == state) {
      ...
      if ((ch >= '0') && (ch <= '9')) {
        ...
        if (num > count) {
          *dest = 0;
          len = snprintf (dest, dest_len - (dest - original_dest), "\\%d", num);
          dest += len;
        } else {
 ...
 */
static nr_rules_result_t
nr_rule_apply (char *str, char *work, char *repl, size_t repl_len, const nrrule_t *rule)
{
  int changed = 0;
  char *wp = work;

  /*
   * A quick word on why ovector is 90 ints long. PCRE uses the bottom two
   * thirds of whatever array you pass for storing captures, and the top
   * third for its internal work. Captures each require 2 ints out of this
   * array. The array must be a multiple of 3. This allows us to have up
   * to 30 captures in a single replacement (\1 ... \30).
   */
   ...
   */
  if ((rule->rflags & NR_RULE_IGNORE) || (0 == (rule->rflags & (NR_RULE_EACH_SEGMENT | NR_RULE_REPLACE_ALL)))) {
     ...
     * is correct. It would be a very nice optimization if we only needed to
     * calculate the replacement string once, and I am sure for 99% of all
     * URL rules we are likely to encounter this will be the case. However,
     * we can not rely on this, because the RE could have alternatives, which
     * change the capture pattern, and so we must calculate the replacement
     * every time. Although, if we were to do a pre-scan of the string, say
     * when we actually get the rules, and set a flag saying the RE has | in
     * it which indicates alternatives, and that flag is not set, then we can
     * in fact only calculate the replacement once. Let's do that.
     */
    int startpos = 0;
    ...
    while (NULL != (ss = nr_regex_match_capture (rule->regex, str + startpos, slen - startpos))) {
      int offsets[2];
      int mstart;
      int mend;
      ...
      if (need_replacement) {
        ...
        need_replacement = (rule->rflags & (NR_RULE_HAS_ALTS | NR_RULE_HAS_CAPTURES));
      }
...
nr_rules_result_t
nr_rules_apply (const nrrules_t *rules, const char *name, char **new_name)
{
  int i;
  int changed = 0;
  char *str;
  ...
  for (i = 0; i < rules->nrules; i++) {
    ...
    int rv = nr_rule_apply (str, work, repl, sizeof (repl), rule);
...
void
nr_rules_process_rule (nrrules_t *rules, const nrobj_t *rule)
{
  uint32_t flags = 0;
  int order;
  const char *mstr;
...
nrrules_t *
nr_rules_create_from_obj (const nrobj_t *obj)
{
  int i;
  int nrules;
  nrrules_t *rs;
