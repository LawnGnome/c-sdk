...
static void test_nr_php_get_daemon_startup_mode(void) {
  ...
  /*
   * Test no daemon start if a manual configuration file exists. If the
  ...
  if (0 == nr_access("/etc/newrelic/newrelic.cfg", F_OK)) {
    ...
    tlib_pass_if_int_equal(
        "no daemon start if a manual configuration file exists",
  ...
  /*
   * Test no daemon start if command line version of PHP was used.
   */
  NR_PHP_PROCESS_GLOBALS(cli) = 1;
  NR_PHP_PROCESS_GLOBALS(no_daemon_launch) = 1;
  daemon_startup_mode = nr_php_get_daemon_startup_mode();
  tlib_pass_if_int_equal(
      "no daemon start if command line version of PHP was used",
      NR_DAEMON_STARTUP_INIT, daemon_startup_mode);

  /*
   * Test no daemon start if non-command line version of PHP was used (for
  ...
  daemon_startup_mode = nr_php_get_daemon_startup_mode();
  tlib_pass_if_int_equal(
      "no daemon start if non-command line version of PHP was used",
      NR_DAEMON_STARTUP_INIT, daemon_startup_mode);

  /*
   * Test no daemon start if daemon is configured via newrelic.cfg and starting
   * it outside of the agent.
   */
  NR_PHP_PROCESS_GLOBALS(no_daemon_launch) = 3;
  daemon_startup_mode = nr_php_get_daemon_startup_mode();
  tlib_pass_if_int_equal(
      "no daemon start if daemon is configured via newrelic.cfg and starting "
      "it outside of the agent.",
      NR_DAEMON_STARTUP_INIT, daemon_startup_mode);

  /*
   * Test no daemon start if the daemon connection settings specify
  ...
  daemon_startup_mode = nr_php_get_daemon_startup_mode();
  tlib_pass_if_int_equal(
      "no daemon start if the daemon connection settings specify a host "
