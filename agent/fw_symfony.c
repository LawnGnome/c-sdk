...
NR_PHP_WRAPPER (nr_symfony1_name_the_wt)
{
   ...
   *   ..calls..
   * 2. (php function) ...->forward (module_name, action_name)  (This function pre-call wrapped)
   *
  ...
  if (0 == NRPRG (symfony1_in_dispatch)) {
    nrl_debug (NRL_FRAMEWORK,
               "%s: forward() called, but not from dispatch()",
               __func__);
  ...
  {
    char *name = nr_formatf ("%.*s/%.*s",
      NRSAFELEN (Z_STRLEN_P (module_name)), Z_STRVAL_P (module_name),
      NRSAFELEN (Z_STRLEN_P (action_name)), Z_STRVAL_P (action_name));
    ...
    nr_txn_set_path ("Symfony1", NRPRG (txn), name, NR_PATH_TYPE_ACTION, ok_to_override);
...
NR_PHP_WRAPPER (nr_symfony1_context_loadfactories)
{
  ...
  if (nr_php_is_zval_valid_object (controller)) {
    ...
    nrl_verbosedebug (NRL_FRAMEWORK,
                      "%s: the controller factory is not an object",
                      __func__);
  }
...
nr_symfony1_enable (TSRMLS_D)
{
   ...
   */
  nr_php_wrap_user_function (NR_PSTR ("sfContext::loadFactories"), nr_symfony1_context_loadfactories TSRMLS_CC);

  nr_php_wrap_user_function (NR_PSTR ("sfError404Exception::printStackTrace"), nr_symfony1_error404exception_printstacktrace TSRMLS_CC);
}
