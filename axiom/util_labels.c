...
static int
nr_labels_get_whitespace_prefix_len (const char *input)
{
  int i;
...
static int
nr_labels_get_whitespace_suffix_len (const char *input, int len_to_test)
{
  const char *current;
  const char *end;
...
static nrobj_t *
nr_labels_parse_internal (const char *str)
{
  nrobj_t *labels = 0;
  int count = 0;
  int i;
  ...
  for (i = 0; i < count; i++) {
    ...
    snprintf (key_buf, sizeof (key_buf), "%.*s", trimmed_len, raw + ws_prefix_len);
    ...
    snprintf (val_buf, sizeof (val_buf), "%.*s", trimmed_len, raw + ws_prefix_len);
...
static char *
nr_labels_strip_surrounding_semicolons_whitespace (const char *str)
{
  int i;
  char *dup;
  int len;
...
nrobj_t *
nr_labels_parse (const char *str)
{
  char *dup;
  nrobj_t *obj;
...
static nr_status_t
nr_labels_connector_format_iterator (const char *key, const nrobj_t *val, void *ptr)
{
...
nrobj_t *
nr_labels_connector_format (const nrobj_t *object)
{
