...
test_cat_path (void)
{
  ...
  /*
   * The below inputs and outputs are taken from the CATMQ/CAT Map spec:
...
static void
test_cat_path_cross_agent (void)
{
  int i;
  char *json;
  nrobj_t *tests;
...
test_md5 (void)
{
  ...
  tlib_pass_if_md5_equal      ("empty string", "d41d8cd98f00b204e9800998ecf8427e", md5);

  tlib_pass_if_status_success ("non-empty string", nr_hash_md5 (md5, NR_PSTR ("foobar")));
  tlib_pass_if_md5_equal      ("non-empty string", "3858f62230ac3c915f300c664312c63f", md5);
}
...
test_md5_low32 (void)
{
  ...
  tlib_pass_if_uint32_t_equal ("empty string", 0xecf8427e, nr_hash_md5_low32 (md5));

  (void) nr_hash_md5 (md5, NR_PSTR ("foobar"));
  tlib_pass_if_uint32_t_equal ("non-empty string", 0x4312c63f, nr_hash_md5_low32 (md5));
}
...
test_mkhash (void)
{
  ...
  tlib_pass_if_true ("hashes match", ui1 == ui2, "hash1=0x%x hash2=0x%x", ui1, ui2);
  ...
  tlib_pass_if_true ("hash not 0", (0 != ui1) && (6 == l), "hash=0x%x, l=%d", ui1, (int)l);
}
