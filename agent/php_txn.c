...
static void
nr_php_collect_x_request_start (TSRMLS_D)
{
...
static void
nr_php_set_initial_path (nrtxn_t *txn TSRMLS_DC)
{
  ...
  if (nr_php_is_zval_valid_array (server)) {
    if ((NR_PHP_PROCESS_GLOBALS (special_flags).enable_path_translated) &&
        (NULL != (uri = nr_php_zend_hash_find (Z_ARRVAL_P (server),
                                               "PATH_TRANSLATED")))) {
      whence = "WT_IS_FILENAME & PATH_TRANSLATED";
      remove_document_root = 1;
      /* uri has a zval with the translated path */
    } else if (NULL != (uri = nr_php_zend_hash_find (Z_ARRVAL_P (server),
                                                     "SCRIPT_FILENAME"))) {
    ...
    if ((NULL == uri) && (NULL == suri)) {
      ...
      if (nr_php_is_zval_valid_array (argv)) {
        ...
        if (nr_php_is_zval_valid_string (filename)) {
          ...
          if (NULL == nr_php_zend_hash_find (Z_ARRVAL_P (server),
                                             "REQUEST_METHOD")) {
  ...
  if (uri || suri) {
    ...
    if (NRINI (remove_trailing_path)) {  /* remove the tailing /xyz/zy from foo/bar.php/xyz/zy */
      q = nr_strstr (s, ".php");
      if (0 != q) {
        q[4] = 0;
      }
    ...
    if (remove_document_root && docroot) {
      if (0 == nr_strncmp (s, Z_STRVAL_P (docroot), Z_STRLEN_P (docroot))) {
        char *s2 = nr_strndup (s + Z_STRLEN_P (docroot),
                               len - Z_STRLEN_P (docroot));
    ...
    /* couldn't find _SERVER[REQUEST_URI] or argv[0] thus we'll have no path information */
    nr_txn_set_path ("default initial path", txn, "<unknown>", NR_PATH_TYPE_UNKNOWN, NR_NOT_OK_TO_OVERWRITE);
  }
}

static int
nr_php_capture_request_parameter (zval *element, nrtxn_t *txn,
...
#define NR_MAX_STRLEN 512
  ...
  if (nr_php_zend_hash_key_is_string (key)) {
    ...
    snprintf (keystr, sizeof (keystr), "%ld",
    ...
    case IS_STRING:
      {
        ...
        len = Z_STRLEN_P (element) < NR_MAX_STRLEN ? Z_STRLEN_P (element) : NR_MAX_STRLEN - 1;
...
static void
nr_php_capture_request_parameters (nrtxn_t *txn TSRMLS_DC)
{
  zval *data = 0;
  ...
  nr_php_zend_hash_zval_apply (Z_ARRVAL_P (data),
                               (nr_php_zval_apply_t) nr_php_capture_request_parameter,
                               txn TSRMLS_CC);
}

static void
nr_php_attribute_helper (nrtxn_t *txn, const char *name, const nr_txn_attribute_t *attribute TSRMLS_DC)
{
...
static void
nr_php_gather_global_params (nrtxn_t *txn TSRMLS_DC)
{
  ...
  nr_php_attribute_helper (txn, "HTTP_ACCEPT",     nr_txn_request_accept_header TSRMLS_CC);
  nr_php_attribute_helper (txn, "CONTENT_TYPE",    nr_txn_request_content_type TSRMLS_CC);
  nr_php_attribute_helper (txn, "HTTP_HOST",       nr_txn_request_host TSRMLS_CC);
  nr_php_attribute_helper (txn, "REQUEST_METHOD",  nr_txn_request_method TSRMLS_CC);
  nr_php_attribute_helper (txn, "HTTP_USER_AGENT", nr_txn_request_user_agent TSRMLS_CC);
 ...
 */
static uint32_t
nr_php_attribute_disable_destination_helper (
...
nr_php_create_attribute_config (TSRMLS_D)
{
  nr_attribute_config_t *config;
  ...
  nr_php_modify_attribute_destinations (config, 0,
    NRINI (transaction_tracer_attributes.include), NR_ATTRIBUTE_DESTINATION_TXN_TRACE, 0);
  nr_php_modify_attribute_destinations (config, 0,
    NRINI (transaction_tracer_attributes.exclude), 0, NR_ATTRIBUTE_DESTINATION_TXN_TRACE);

  nr_php_modify_attribute_destinations (config, 0,
    NRINI (error_collector_attributes.include), NR_ATTRIBUTE_DESTINATION_ERROR, 0);
  nr_php_modify_attribute_destinations (config, 0,
    NRINI (error_collector_attributes.exclude), 0, NR_ATTRIBUTE_DESTINATION_ERROR);

  nr_php_modify_attribute_destinations (config, 0,
    NRINI (transaction_events_attributes.include), NR_ATTRIBUTE_DESTINATION_TXN_EVENT, 0);
  nr_php_modify_attribute_destinations (config, 0,
    NRINI (transaction_events_attributes.exclude), 0, NR_ATTRIBUTE_DESTINATION_TXN_EVENT);

  nr_php_modify_attribute_destinations (config, 0,
    NRINI (browser_monitoring_attributes.include), NR_ATTRIBUTE_DESTINATION_BROWSER, 0);
  nr_php_modify_attribute_destinations (config, 0,
    NRINI (browser_monitoring_attributes.exclude), 0, NR_ATTRIBUTE_DESTINATION_BROWSER);

  nr_php_modify_attribute_destinations (config, 0,
    NRINI (attributes.include), NR_ATTRIBUTE_DESTINATION_ALL, 0);
  nr_php_modify_attribute_destinations (config, 0,
    NRINI (attributes.exclude), 0, NR_ATTRIBUTE_DESTINATION_ALL);

  /*
   * Handle deprecated ignored request parameters.
   */
  nr_php_modify_attribute_destinations (config, NR_TXN_REQUEST_PARAMETER_ATTRIBUTE_PREFIX,
    NRINI (ignored_params), 0, NR_ATTRIBUTE_DESTINATION_ALL);
...
static nrobj_t *
nr_php_txn_get_labels (TSRMLS_D)
{
  const char *input;
...
static void
nr_php_txn_prepared_statement_destroy (void *sql)
{
  nr_free (sql);
}

nr_status_t
nr_php_txn_begin (const char *appnames, const char *license TSRMLS_DC)
{
  nrtxnopt_t opts;
  ...
  if (0 == lic_to_use) {
     ...
     * RINIT.
     */
    nrl_debug (NRL_INIT, "unable to begin transaction: license key is missing "
  ...
  opts.database_name_reporting_enabled = (int)NRINI (database_name_reporting_enabled);
  ...
  app = nr_agent_find_or_add_app (nr_agent_applist, &info,
    /*
     * Settings are provided through a callback: They cannot be calculated once
     * per process, since they depend on per request settings.  They are not
     * calculated each transaction to reduce overhead.
     */
    &nr_php_app_settings);
  nr_app_info_destroy_fields (&info);

  if (0 == app) {
    nrl_debug (NRL_INIT, "unable to begin transaction: app '%.128s' is unknown", appnames ? appnames : "");
    return NR_FAILURE;
  }
  ...
  NRPRG (txn)->special_flags.no_sql_parsing   = NR_PHP_PROCESS_GLOBALS (special_flags).no_sql_parsing;
  NRPRG (txn)->special_flags.show_sql_parsing = NR_PHP_PROCESS_GLOBALS (special_flags).show_sql_parsing;
  NRPRG (txn)->special_flags.debug_cat        = NR_PHP_PROCESS_GLOBALS (special_flags).debug_cat;

  NRPRG (prepared_statements) = nr_hashmap_create (nr_php_txn_prepared_statement_destroy);
  ...
  if (NRPRG (txn)->options.cross_process_enabled) {
    ...
    x_newrelic_transaction = nr_php_get_request_header ("HTTP_X_NEWRELIC_TRANSACTION" TSRMLS_CC);

    (void) nr_header_set_cat_txn (NRPRG (txn),
                                  x_newrelic_id,
...
static int
nr_php_txn_should_ignore (int ignoretxn TSRMLS_DC)
{
  if (NRTXN (status.ignore)) {
    ignoretxn = 1;
  }

  if ((NRTXN (status.background)) && (NR_PHP_PROCESS_GLOBALS (special_flags).no_background_jobs)) {
    ignoretxn = 1;
  }
...
/* Actually shutdown, without checking if we should ignore or if the txn is NULL. */
static void
nr_php_txn_do_shutdown (nrtxn_t *txn TSRMLS_DC)
{
  char *request_uri;
...
static void
nr_php_txn_set_response_header_attributes (nrtxn_t *txn TSRMLS_DC)
{
  char *content_type;
  int content_length;
  ...
  nr_txn_set_long_attribute (txn, nr_txn_response_content_length, content_length);
}

nr_status_t
nr_php_txn_end (int ignoretxn, int in_post_deactivate TSRMLS_DC)
{
  nr_status_t ret;
  ...
  if (0 == ignoretxn) {
    ...
    nrm_force_add (txn->unscoped_metrics, "Supportability/execute/user/call_count", NRPRG (execute_count));
