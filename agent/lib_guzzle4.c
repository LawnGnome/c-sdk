 ...
 */
static int
nr_guzzle4_is_zval_an_event (zval *obj TSRMLS_DC)
{
  return nr_php_object_instanceof_class (obj, "GuzzleHttp\\Event\\EventInterface" TSRMLS_CC);
}
 ...
 */
static int
nr_guzzle4_is_zval_an_emitter (zval *obj TSRMLS_DC)
{
  return nr_php_object_instanceof_class (obj, "GuzzleHttp\\Event\\EmitterInterface" TSRMLS_CC);
}
 ...
 */
static int
nr_guzzle4_is_zval_a_request (zval *obj TSRMLS_DC)
{
  return nr_php_object_instanceof_class (obj, "GuzzleHttp\\Message\\RequestInterface" TSRMLS_CC);
}
 ...
 */
static int
nr_guzzle4_is_zval_a_response (zval *obj TSRMLS_DC)
{
  return nr_php_object_instanceof_class (obj, "GuzzleHttp\\Message\\ResponseInterface" TSRMLS_CC);
}
 ...
 */
static void
nr_guzzle4_add_event_to_events_array (zval *events, const char *event, const char *method, long priority)
{
 ...
 */
static nr_status_t
nr_guzzle4_subscriber_event_get_args (nr_guzzle4_subscriber_event_args_t *args, INTERNAL_FUNCTION_PARAMETERS)
{
  /*
 ...
 */
static
PHP_NAMED_FUNCTION (nr_guzzle4_subscriber_get_events)
{
  /*
  ...
  nr_guzzle4_add_event_to_events_array (return_value, "complete", "onComplete", 0);
}
 ...
 */
static
PHP_NAMED_FUNCTION (nr_guzzle4_subscriber_on_before)
{
 ...
 */
static
PHP_NAMED_FUNCTION (nr_guzzle4_subscriber_on_complete)
{
  nr_guzzle4_subscriber_event_args_t args;
  zval *request = NULL;
  zval *response = NULL;
...
const zend_function_entry nr_guzzle4_subscriber_functions[] = {
  ZEND_FENTRY (getEvents,  nr_guzzle4_subscriber_get_events,  nr_guzzle4_subscriber_get_events_arginfo,  ZEND_ACC_PUBLIC)
  ZEND_FENTRY (onBefore,   nr_guzzle4_subscriber_on_before,   nr_guzzle4_subscriber_on_before_arginfo,   ZEND_ACC_PUBLIC)
  ZEND_FENTRY (onComplete, nr_guzzle4_subscriber_on_complete, nr_guzzle4_subscriber_on_complete_arginfo, ZEND_ACC_PUBLIC)
  PHP_FE_END
};
...
NR_PHP_WRAPPER_START (nr_guzzle4_client_construct)
{
   ...
   */
  if (0 == nr_php_class_entry_instanceof_class (nr_guzzle4_subscriber_ce, "GuzzleHttp\\Event\\SubscriberInterface" TSRMLS_CC)) {
    zend_class_entry *subscriber_interface = nr_php_find_class ("guzzlehttp\\event\\subscriberinterface" TSRMLS_CC);
...
nr_guzzle4_minit (TSRMLS_D)
{
   ...
   */
  INIT_CLASS_ENTRY (ce, "newrelic\\Guzzle4\\Subscriber", nr_guzzle4_subscriber_functions);
  nr_guzzle4_subscriber_ce = nr_php_zend_register_internal_class_ex (&ce, NULL TSRMLS_CC);
...
nr_guzzle4_rshutdown (TSRMLS_D)
{
   ...
   */
  iface_ce = nr_php_find_class ("guzzlehttp\\event\\subscriberinterface" TSRMLS_CC);
  if (NULL == iface_ce) {
    return;
  }

  nr_php_remove_interface_from_class (nr_guzzle4_subscriber_ce, iface_ce TSRMLS_CC);
}
