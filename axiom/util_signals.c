...
nr_signal_tracer_prep (void)
{
  ...
  if (signal_tracer_fd >= 0) {
    signal_tracer_bannerlen = snprintf (signal_tracer_banner, sizeof (signal_tracer_banner),
      "process id %d fatal signal (SIGSEGV, SIGFPE, SIGILL, SIGBUS, ...)  - stack dump follows (code=%p bss=%p):\n",
 ...
 * is in its own DSO.
 *
 * See https://newrelic.atlassian.net/browse/PHP-697
 */
void
nr_signal_tracer_common (int sig)
{
  char buf[256];
  ...
  snprintf (buf, sizeof (buf), "Process %d (version %s) received signal %2d: %s\n", nr_getpid (), nr_version (), sig, signal_name);
 ...
 */
static void
default_fatal_signal_handler (int sig)
{
...
void
nr_signal_handler_install (void (*handler)(int sig))
{
  struct sigaction sa;
