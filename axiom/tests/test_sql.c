...
#define test_get_operation_and_table(...) test_get_operation_and_table_f (__VA_ARGS__,__FILE__,__LINE__)

static void
test_get_operation_and_table_f (
  const char *test,
  const char *sql,
  ...
  int         line)
{
  ...
  if (expected_operation) {
    test_pass_if_true (test, 0 == nr_strcmp (expected_operation, operation),
      "expected_operation=%s operation=%s", expected_operation, NRSAFESTR (operation));
  } else {
  ...
  if (expected_table) {
    test_pass_if_true (test, 0 == nr_strcmp (expected_table, table),
      "expected_table=%s table=%s", expected_table, NRSAFESTR (table));
  } else {
...
#define whitespace_comment_testcase(...) whitespace_comment_testcase_fn (__VA_ARGS__,__FILE__,__LINE__)

static void
whitespace_comment_testcase_fn (const char *input, const char *expected_output, const char *file, int line)
{
  const char *rv;
  int show_sql_parsing = 1;
  ...
  test_pass_if_true (NRSAFESTR (input),
    0 == nr_strcmp (rv, expected_output),
    "rv=%s expected_output=%s", NRSAFESTR (rv), NRSAFESTR (expected_output));
}

static void
test_whitespace_comment_prefix (void)
{
  /*
   * Test : Bad Parameters
   */
  ...
  whitespace_comment_testcase ("\t/**/SELECT",     "SELECT");
  whitespace_comment_testcase ("/**/\fSELECT",     "SELECT");
  whitespace_comment_testcase ("/*/**/*/\fSELECT", "*/\fSELECT"); /* Nested comments not supported */
  whitespace_comment_testcase (" /* arma */ /* virumque */ /* cano */ SELECT", "SELECT");
  whitespace_comment_testcase ("/* * */ SELECT",   "SELECT");
  /*
...
#define sql_obfuscate_testcase(...) sql_obfuscate_testcase_fn(__VA_ARGS__, __FILE__, __LINE__)

static void
sql_obfuscate_testcase_fn (const char* testname, const char *sql, const char *expected, const char *file, int line)
{
  char *output = nr_sql_obfuscate (sql);
  char *idempotent = output ? nr_sql_obfuscate (output) : 0;

  test_pass_if_true (testname, 0 == nr_strcmp (expected, output), "expected=%s output=%s", NRSAFESTR (expected), NRSAFESTR (output));
  test_pass_if_true (testname, 0 == nr_strcmp (idempotent, output), "idempotent=%s output=%s", NRSAFESTR (idempotent), NRSAFESTR (output));
...
test_sql_obfuscate (void)
{
  ...
  sql_obfuscate_testcase ("missing closing double quote",
    "SELECT * FROM test WHERE foo IN (1,\"missing closing double quote)",
    "SELECT * FROM test WHERE foo IN (?,?");

  sql_obfuscate_testcase ("missing closing single quote",
    "SELECT * FROM test WHERE foo IN (1,\'missing closing single quote)",
    "SELECT * FROM test WHERE foo IN (?,?");

  sql_obfuscate_testcase ("digit strings",
    "SELECT 12345 FROM test WHERE foo IN (1,\"foo\",'baz')",
    "SELECT ? FROM test WHERE foo IN (?,?,?)");

  sql_obfuscate_testcase ("floating point number",
    "SELECT 12345.78 FROM test WHERE foo IN (1,\"foo\",'baz')",
    "SELECT ?.? FROM test WHERE foo IN (?,?,?)");

  sql_obfuscate_testcase ("floating point number with exponent",
    "SELECT 12345.78e01 FROM test WHERE foo IN (1,\"foo\",'baz')",
    "SELECT ?.?e? FROM test WHERE foo IN (?,?,?)");

  sql_obfuscate_testcase ("Comment, SQL style",
    "SELECT * FROM PASSWORDS -- hunter2 -- WHERE foo IN (1)",
    "SELECT * FROM PASSWORDS ?");

  sql_obfuscate_testcase ("Comment, C style",
    "SELECT * FROM PASSWORDS /* hunter2 */ WHERE foo IN (1)",
    "SELECT * FROM PASSWORDS ?");
  ...
  sql_obfuscate_testcase ("Mixed comments",
    "SELECT * -- FROM PASSWORDS /* hunter2 */ WHERE foo IN (1)",
    "SELECT * ?");
  ...
  sql_obfuscate_testcase ("C-style comment start inside single quotes, comment outside.",
    "SELECT * FROM PASSWORDS WHERE foo IN (\"/*\" /* HIDING */)",
    "SELECT * FROM PASSWORDS WHERE foo IN (? ?");

  sql_obfuscate_testcase ("SQL-style comment start inside single quotes, comment outside.",
    "SELECT * FROM PASSWORDS WHERE foo IN (\"--\" --)",
    "SELECT * FROM PASSWORDS WHERE foo IN (? ?");

  sql_obfuscate_testcase ("C-style comment start inside single quotes, comment end only outside.",
    "SELECT * FROM PASSWORDS WHERE foo IN (\"/*\" HIDING */)",
    "SELECT * FROM PASSWORDS WHERE foo IN (? HIDING */)");

  sql_obfuscate_testcase ("escaped quotes with comments",
    "SELECT * FROM test WHERE foo IN (1,\"--\\\"\",'/*\\'')",
    "SELECT * FROM test WHERE foo IN (?,?,?)");

  sql_obfuscate_testcase ("stuttered quotes with comments",
    "SELECT * FROM test WHERE foo IN (1,\"--,/*\"\"\",'''/*',14)",
    "SELECT * FROM test WHERE foo IN (?,?,?,?)");
  ...
  for (lg = 1; lg < 20; lg++) {
    ...
    s2 = "?";
    tlib_pass_if_true ("monstrous obfuscation", 0 == nr_strcmp (s1, s2), "s1=%s s2=%s", NRSAFESTR (s1), NRSAFESTR (s2));
    nr_free (buffer);
    nr_free (s1);
  }
}
...
test_sql_normalize (void)
{
  ...
  tlib_pass_if_true ("nr_sql_normalize", (0 == nr_strcmp (s1, s2)), "s1=%s s2=%s", s1, s2);
  nr_free (s1);
...
static void
test_find_table_with_from (void)
{
  /* Empty table name */
  test_get_operation_and_table (NULL, "SELECT * FROM `` WHERE x > y", "select", NULL);
}

static void
test_real_world_things (void)
{
  const char *sql;

  /* Real-world stuff that has caused errors */
  sql = "(SELECT SQL_CALC_FOUND_ROWS c., e.entry_id, e.entry_title, `ee`.`entry_extra_image` AS `entry_image` "
    "FROM `hp_comments` c, `mt_entry` e , mt_entry_extra ee WHERE e.`entry_id` = c.`entry_id` AND `ee`.`entry_extra_id` = `e`.`entry_id` AND `published` = ? AND `removed` = ? AND `user_id` = ? ) "
    "UNION ALL (SELECT c., e.entry_id, e.entry_title, `ee`.`entry_extra_image` AS `entry_image` FROM `HPCommentsArchive?` c, "
    "`mt_entry` e , mt_entry_extra ee WHERE e.`entry_id` = c.`entry_id` AND `ee`.`entry_extra_id` = `e`.`entry_id` AND `published` = ? AND `removed` = ? AND `user_id` = ? ) "
    "UNION ALL (SELECT c., e.entry_id, e.entry_title, `ee`.`entry_extra_image` AS `entry_image` FROM `HPCommentsArchive?` c, "
    "`mt_entry` e , mt_entry_extra ee WHERE e.`entry_id` = c.`entry_id` AND `ee`.`entry_extra_id` = `e`.`entry_id` AND `published` = ? AND `removed` = ? AND `user_id` = ? ) "
    "UNION ALL (SELECT c., e.entry_id, e.entry_title, `ee`.`entry_extra_image` AS `entry_image` FROM `HPCommentsArchive?` c, "
    "`mt_entry` e , mt_entry_extra ee WHERE e.`entry_id` = c.`entry_id` AND `ee`.`entry_extra_id` = `e`.`entry_id` AND `published` = ? AND `removed` = ? AND `user_id` = ? ) "
   ...
   * TODO(willhf) Is this correct?
   */
  test_get_operation_and_table ("Huffington Post Bad Parse 1", sql, NULL, NULL);

  sql = "SELECT `mt_entry`.`entry_id`, `mt_entry`.`entry_title`, `mt_entry`.`entry_blog_id`, `mt_entry`.`entry_basename`, "
    "`mt_entry_extra`.`entry_extra_image`, `mt_entry`.`entry_author_id`, `mt_entry`.`entry_created_on`, "
    "`mt_author`.`author_name`, `mt_author`.`author_nickname` FROM `hp_prod`.`mt_entry` as `mt_entry` "
    "INNER JOIN `hp_prod`.`mt_objecttag` as `mt_objecttag` ON `mt_objecttag`.`objecttag_object_id` = `mt_entry`.`entry_id` "
    "AND `mt_objecttag`.`objecttag_tag_id` = ? AND `mt_objecttag`.`objecttag_object_id` "
    "NOT IN ("
    "?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, "
    "?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, "
    "?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, "
    ...
    "?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, "
    "?, ?, ?, ?, ?, ?, ?) "
    "INNER JOIN `hp_prod`.`mt_author` as `mt_author` ON `mt_author`.`author_id` = `mt_entry`.`entry_author_id` "
    "INNER JOIN `hp_prod`.`mt_entry_extra` as `mt_entry_extra` ON `mt_entry_extra`.`entry_extra_id` = `mt_entry`.`entry_id` "
    "AND `mt_entry_extra`.`entry_extra_image` LIKE \"?\" INNER JOIN `hp_prod`.`mt_placement` as `mt_placement` "
    "ON `mt_placement`.`placement_entry_id` = `mt_entry`.`entry_id` AND `mt_placement`.`placement_category_id` "
    "IN (?, ?) AND `mt_placement`.`placement_is_primary` = ? WHERE `mt_entry`.`entry_created_on` > \"?\" "
    "AND `mt_entry`.`entry_status` = ? GROUP BY `mt_entry`.`entry_id` ORDER BY `mt_entry`.`entry_created_on` DESC LIMIT ? / app?.ewr.huffpo.net, slave-db /";
  test_get_operation_and_table ("Huffington Post Bad Parse 2", sql, "select", "mt_entry");

  sql = "SELECT SQL_NO_CACHE `stats`.`entity_id` as `entry_id`, COUNT(`stats`.`count`) as `c`, "
    "GROUP_CONCAT(DISTINCT `stats`.`user_id`) as `friends_ids`, COUNT(DISTINCT `stats`.`user_id`) "
    "as `friends_count`, `ee`.`entry_extra_image` as `entry_image` FROM `hp_prod`.`stats_user_actions` "
    "as `stats` INNER JOIN `hp_prod`.`mt_entry` as `e` ON `e`.`entry_id` = `stats`.`entity_id` AND "
    "`e`.`entry_created_on` > NOW() - INTERVAL ? DAY LEFT JOIN `hp_prod`.`mt_entry_extra` as "
    "`ee` ON `ee`.`entry_extra_id` = `stats`.`entity_id` AND `ee`.`entry_extra_image` LIKE \"?\" WHERE `stats`.`user_id` IN";
  test_get_operation_and_table ("Huffington Post Bad Parse 3", sql, "select", "stats_user_actions");

  sql = "select City.name, Country.name, City.population from (Country, City) where Country.population > ? and City.population > ?";
  test_get_operation_and_table ("Richard multiple-table test", sql, "select", "Country");

  sql = "SELECT CONV(SUBSTRING(MD5(LOWER('what breast pathology involves malignant cells with halos invading the epidermis of the skin?')), 1, 8), 16, 10)";
  test_get_operation_and_table ("Quizlet sub-string bad parse 1", sql, "select", NULL);

  sql = "SELECT CONV(SUBSTRING(MD5(LOWER('Who is the Thrilla from Manila?')), 1, 8), 16, 10)";
  test_get_operation_and_table ("Quizlet sub-string bad parse 2", sql, "select", NULL);

  sql = " SELECT n.message, n.subject, ne.name as email_name, ne.email as email_from FROM notifications "
    "n INNER JOIN notification_emails ne ON ne.id = n.notification_email_id WHERE n.notification_event_id = :event_id AND n.locale_id = :locale_id AND n.is_active = ? LIMIT ? ";
  test_get_operation_and_table ("_from in alias", sql, "select", "notifications");
}
...
#if 0  /* TODO: spaces in quotes before keywords are not handled */
...
#if 0  /* TODO: ANSI_QUOTES are not handled */
...
static void
test_get_operation_and_table_in_sql_with_info (void)
{
  const char *sql;
   ...
   * TODO(willhf) Is this correct behavior?
   */
  test_get_operation_and_table (NULL, "IINTO foobar INSERT InTo baz(a,b) VALUES(1,2)", NULL, NULL);

  /* Real-world stuff that has caused errors */
  sql = "INSERT INTO gm1_gross_margin_report_audit(id,parent_id,field_name,data_type,before_value_string,after_value_string,date_created,created_by) VALUES('?','?','?','?','?','?','?','?')";
  test_get_operation_and_table ("Bjorn's customer test", sql, "insert", "gm1_gross_margin_report_audit");
}

static void
test_weird_and_wonderful (void)
{
  const char *sql;

  /* Caused Megento SIGSEGV */
  sql = "SELECT `main_table`.*, `main_table`.`total_item_count` AS `items_count`, CONCAT(main_table.customer_firstname,\" \", "
    "main_table.customer_lastname) AS `customer`, "
    "(main_table.base_grand_total * main_table.base_to_global_rate) AS `revenue` "
    "FROM `sales_flat_order` AS `main_table` ORDER BY created_at DESC LIMIT 5";
  test_get_operation_and_table ("Magento SIGSEGV", sql, "select", "sales_flat_order");

  /* Caused SIGSEGV for user barry in a support ticket */
  sql = "select imageclass,concat(imageclass,' [',c,']') from category_stat where c > 15 order by rand() limit 5";
  test_get_operation_and_table ("Barry SIGSEGV", sql, "select", "category_stat");

  sql = "SELECT r.nid, MATCH(r.body, r.title) AGAINST ('%s') AS score FROM {node_revisions} r "
    "INNER JOIN {node} n ON r.nid = n.nid AND r.vid = n.vid INNER JOIN {term_node} t ON n.nid = t.nid AND t.tid IN (%s) "
    "WHERE n.status <> 0 AND r.nid <> %d AND n.type IN ($types) GROUP BY n.nid HAVING score > 0 ORDER BY score DESC, r.vid DESC";
  test_get_operation_and_table ("Table in braces", sql, "select", "node_revisions");

  /* Causes lots of warnings about illegal characters - need to parse comments */
  sql = "UPDATE /* 1.2.3.4 */ `iw_page` SET page_counter = page_counter + 1 WHERE page_id = 824' rv=''";
  test_get_operation_and_table ("C-style comments in SQL", sql, "update", "iw_page");

  sql = "SELECT * FROM /* zip */ /* zap */ /* zop */  alpha";
  test_get_operation_and_table ("multiple comments before table name in select", sql, "select", "alpha");

  sql = "UPDATE /* zip */ /* zap */ /* zop */ alpha SET page_counter = page_counter + 1 WHERE page_id = 824' rv=''";
  test_get_operation_and_table ("multiple comments before table name in update", sql, "update", "alpha");

  sql = "/* zip */ /* zap */ /* zop */ UPDATE alpha SET page_counter = page_counter + 1 WHERE page_id = 824' rv=''";
  test_get_operation_and_table ("multiple comments before update in update", sql, "update", "alpha");

  sql = "/* zip */ UPDATE /* zap */ alpha SET page_counter = page_counter + 1 WHERE page_id = 824' rv=''";
  test_get_operation_and_table ("comment before and after update", sql, "update", "alpha");
  ...
  sql = "update /* alpha";
  test_get_operation_and_table ("unterminated comment before update tablename", sql, "update", NULL);
...
static void
test_get_operation_and_table_bad_params (void)
{
  ...
  nr_sql_get_operation_and_table (sql, 0, 0, show_sql_parsing); /* Don't blow up */

  nr_sql_get_operation_and_table (sql, &operation, 0, show_sql_parsing);
  tlib_pass_if_true ("null table ptr", 0 == operation, "operation=%p", operation);
  tlib_pass_if_true ("null table ptr", 0 == table, "table=%p", table);

  nr_sql_get_operation_and_table (sql, 0, &table, show_sql_parsing);
  tlib_pass_if_true ("null operation ptr", 0 == operation, "operation=%p", operation);
  ...
  if (array && (NR_OBJECT_ARRAY == nro_type (array))) {
    for (i = 1; i <= nro_getsize (array); i++) {
      ...
      if (input && table && operation) {
        test_get_operation_and_table (testname ? testname : input, input, operation, table);
      }
    }
  }
