...
static void
test_parse_v1_invalid_json (const char *message, const char *json)
{
  nrobj_t *obj = NULL;
...
test_parse_v1 (void)
{
  ...
  tlib_pass_if_status_failure ("NULL synth_obj", nr_synthetics_parse_v1 (NULL, synthetics));
  ...
  tlib_pass_if_status_success ("valid JSON", nr_synthetics_parse_v1 (obj, synthetics));
...
test_version (void)
{
  nr_synthetics_t *synthetics = nr_synthetics_create ("[1,100,\"a\",\"b\",\"c\"]");
   ...
   */
  tlib_pass_if_int_equal ("valid synthetics", 1, nr_synthetics_version (synthetics));

  nr_synthetics_destroy (&synthetics);
}

static void
test_account_id (void)
{
  nr_synthetics_t *synthetics = nr_synthetics_create ("[1,100,\"a\",\"b\",\"c\"]");
   ...
   * Test : Good parameters.
   */
  tlib_pass_if_int_equal ("valid synthetics", 100, nr_synthetics_account_id (synthetics));

  nr_synthetics_destroy (&synthetics);
}

static void
test_resource_id (void)
{
  nr_synthetics_t *synthetics = nr_synthetics_create ("[1,100,\"a\",\"b\",\"c\"]");
   ...
   * Test : Good parameters.
   */
  tlib_pass_if_str_equal ("valid synthetics", "a", nr_synthetics_resource_id (synthetics));
...
test_job_id (void)
{
   ...
   * Test : Good parameters.
   */
  tlib_pass_if_str_equal ("valid synthetics", "b", nr_synthetics_job_id (synthetics));

  nr_synthetics_destroy (&synthetics);
}

static void
test_monitor_id (void)
{
  nr_synthetics_t *synthetics = nr_synthetics_create ("[1,100,\"a\",\"b\",\"c\"]");
   ...
   * Test : Good parameters.
   */
  tlib_pass_if_str_equal ("valid synthetics", "c", nr_synthetics_monitor_id (synthetics));

  nr_synthetics_destroy (&synthetics);
}

static void
test_outbound_header (void)
{
  ...
  tlib_pass_if_ptr_equal ("header is cached", header, nr_synthetics_outbound_header (synthetics));

  nr_synthetics_destroy (&synthetics);
}

#define TEST_SYNTHETICS_TEST_FILE CROSS_AGENT_TESTS_DIR "/synthetics/synthetics.json"

static void
test_synthetics_cross_agent_tests (void)
{
  char *file_json;
  nrobj_t *arr;
  int i;

  file_json = nr_read_file_contents (TEST_SYNTHETICS_TEST_FILE, 10 * 1000 * 1000);
  ...
  for (i = 1; i <= nro_getsize (arr); i++) {
    ...
    const nrobj_t *input_obfuscated_header = nro_get_hash_hash (hash, "inputObfuscatedHeader", NULL);
    const char *x_newrelic_synthetics = nro_get_hash_string (input_obfuscated_header, "X-NewRelic-Synthetics", NULL);
    const nrobj_t *input_header_payload = nro_get_hash_array (hash, "inputHeaderPayload", NULL);
    const nrobj_t *output_txn_trace = nro_get_hash_hash (hash, "outputTransactionTrace", NULL);
    const nrobj_t *output_txn_event = nro_get_hash_hash (hash, "outputTransactionEvent", NULL);
    const nrobj_t *output_request_header = nro_get_hash_hash (hash, "outputExternalRequestHeader", NULL);
    const char *synthetics_encoding_key = nro_get_hash_string (settings, "syntheticsEncodingKey", NULL);

    const char *expected_output_header = nro_get_hash_string (nro_get_hash_hash (nro_get_hash_hash (hash,
        "outputExternalRequestHeader", NULL),
        "expectedHeader", NULL),
    ...
    txn->guid = nr_strdup (nro_get_hash_string (settings, "transactionGuid", NULL));
    txn->app_connect_reply = nro_new_hash ();
    nro_set_hash_string (txn->app_connect_reply, "encoding_key", nro_get_hash_string (settings, "agentEncodingKey", NULL));
    nro_set_hash (txn->app_connect_reply, "trusted_account_ids", nro_get_hash_array (settings, "trustedAccountIds", NULL));

    if (x_newrelic_synthetics) {
      char *json = nro_to_json (input_header_payload);
      char *obfuscated_input_payload = nr_obfuscate (json, synthetics_encoding_key, 0);

      tlib_pass_if_str_equal (testname, obfuscated_input_payload, x_newrelic_synthetics);
      nr_free (json);
    ...
    /*
     * NOTE (willhf): This cross agent testing is incomplete, the txn event and
     * txn trace fields are not compared.
     */
