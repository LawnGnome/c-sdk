 ...
 *
 * For full information, see the spec here:
 ...
 *
 * All configuration is applied to identically to attributes created by the
 * user and agent.  They are only kept seperately so that collisions can
 * be handled by the collector/beacon rather than in the agent.
 * 
 ...
 *           instead or-ed.
 *
 *           If the include/exclude destinations conflic, the exclude setting
 *           will have priority.
 *
 ...
 */
extern nr_attributes_t *nr_attributes_create (const nr_attribute_config_t *config);

/*
 * Purpose : Create a new attribute.
 *
 * IMPORTANT : If you create a new attribute, you must document it on the list:
 *             https://source.datanerd.us/agents/agent-specs/blob/master/Agent-Attributes-PORTED.md
 *
 */
extern nr_status_t nr_attributes_user_add (nr_attributes_t *ats, uint32_t default_destinations, const char *key, const nrobj_t *value);
extern nr_status_t nr_attributes_agent_add_long   (nr_attributes_t *ats, uint32_t default_destinations, const char *key, int64_t lng);
extern nr_status_t nr_attributes_agent_add_string (nr_attributes_t *ats, uint32_t default_destinations, const char *key, const char *str);

/* Convenience testing functions. */
extern nr_status_t nr_attributes_user_add_string (nr_attributes_t *ats, uint32_t default_destinations, const char *key, const char *value);
extern nr_status_t nr_attributes_user_add_long (nr_attributes_t *ats, uint32_t default_destinations, const char *key, int64_t lng);
 ...
 *           included.
 */
extern nrobj_t *nr_attributes_user_to_obj (const nr_attributes_t *attributes, uint32_t destination);
extern nrobj_t *nr_attributes_agent_to_obj (const nr_attributes_t *attributes, uint32_t destination);
