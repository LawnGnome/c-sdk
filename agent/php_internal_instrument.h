 ...
 * to compiler allocated strings from "...", and not from strings dynamically allocated
 * using, say, nr_strdup.  Thus, when freeing instances of this structure,
 * you do not need to free the outgoing references.
 */
typedef struct _nrinternalfn_t {
  ...
  /*
   * Refer to the documentation in php_internal_instrument.c for information regarding
   * the inner and outer wrappers, and the outer_wrapper_global.
   */
  struct _nrinternalfn_t **outer_wrapper_global;
  nrphpfn_t                outer_wrapper;
  void (*inner_wrapper)(INTERNAL_FUNCTION_PARAMETERS, struct _nrinternalfn_t *fn); /* Can't use nr_inner_wrapper_fn_t due to circular reference */
...
extern int nr_zend_call_old_handler (nrphpfn_t oldhandler, INTERNAL_FUNCTION_PARAMETERS);
...
extern void
nr_php_add_call_user_func_array_pre_callback (nrphpcufafn_t callback TSRMLS_DC);
