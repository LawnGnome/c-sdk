/*
 * This file contains string utility functions, unencumbered by license.
 */
#include "nr_axiom.h"
...
char *
nr_string_to_lowercase (const char *str)
{
  int i;
  char *low;
...
nrobj_t *
nr_strsplit (const char *orig, const char *delim, int use_empty)
{
  nrobj_t *arr;
  const char *s;
  ...
  if (0 == delim[0]) {
    ...
    /* Here we do not strip whitespace, is this correct? */
    return arr;
  }
...
char *
nr_strxcpy (char *dest, const char *src, int len)
{
 ...
 */
int
nr_strnlen (const char *str, int maxlen)
{
...
char *
nr_strcpy (char *dest, const char *src)
{
...
char *
nr_strlcpy (char *dest, const char *src, int len)
{
  if (nrlikely ((0 != dest) && (0 != src) && (len > 0))) {
    int slen = (int)(strlen) (src);
...
char *
nr_strcat (char *dest, const char *src)
{
  int dl;
  int sl;
...
char *
nr_strncat (char *dest, const char *src, int len)
{
  int dl;
  int sl;
...
int
nr_stridx (const char *str, const char *needle)
{
...
int
nr_strcaseidx (const char *str, const char *needle)
{
  char c;
  int i;
  int needle_len;
...
int
nr_strncaseidx (const char *str, const char *needle, int len)
{
  char c;
  int i;
  int needle_len;
...
int
nr_strncaseidx_last_match (const char *str, const char *needle, int len)
{
  char c;
  int i;
  int needle_len;
...
int
nr_strnidx (const char *str, const char *needle, int str_len)
{
...
int
nr_strnspn (const char *s1, int s1len, const char *s2, int s2len)
{
  const char *es1;
  const char *es2;
  const char *wp = s1;
...
int
nr_strncspn (const char *s1, int s1len, const char *s2, int s2len)
{
  const char *es1;
  const char *es2;
  const char *wp;
...
nr_str_char_count (const char *s, char c)
{
  ...
  /*
   * TODO(msl): Why doesn't this function call strchr in a loop?
   */
