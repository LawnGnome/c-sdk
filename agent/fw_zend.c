 ...
 * =====================
 *
 * In a standard ZF1 application, requests enter the front controller (Zend_Controller_Front)
 * where a route is selected based on the request URL. Once a route has been found,
 * the front controller then enters the dispatch loop where it will determine
 * which controller(s) and action(s) to invoke. In most cases, this is the controller
 * and action associated with the route. However, invoking a different action, or
 * even multiple actions is a normal practice in Zend applications. For example, to
 * forward a request to another controller, or redirect to another URL. Zend also
 * provides a plugin interface to isolate cross-cutting routing concerns into separate
 * classes. For example, implementing authentication. The following pseudo-php code
 ...
 * transaction. There two complications that prevent us from doing so.
 *
 * 1. An action or plugin can end the request early by calling the `exit()`
 *    function. If this occurs during the dispatch loop, `dispatchLoopShutdown()`
 *    will never be invoked. There are at least three standard Zend components
 *    that do this: Redirect, Json, and AutoComplete.
 ...
 * loop and record it within the response. Thus far, this strategy has not
 * caused any issues with transaction naming.
 *
 * Note: Previously, we hooked `routeShutdown()` instead. This occurs too
 * early. The controller and action could be changed by a plugin, or by any
 * of the actions invoked during the dispatch loop. This scheme was suggested
 * by Nick Calugar of IGN Entertainment ncalugar@ign.com.
 */

/*
 * Purpose : Name the transaction based on the current controller and action.
 *
 * Params  : 1. An object that quacks like a Zend_Controller_Request_Abstract.
 *
 * Returns : Nothing.
 */
static void
nr_zend_name_the_wt (zval *request TSRMLS_DC)
{
  zval *module = 0;
  zval *controller = 0;
  zval *action = 0;
  ...
  if (module || controller || action) {
    ...
    snprintf (buf, sizeof (buf), "%.*s/%.*s/%.*s",
      ...
      nr_php_is_zval_non_empty_string (module) ? Z_STRVAL_P (module) : "NoModule",

      nr_php_is_zval_non_empty_string (controller) ? Z_STRLEN_P (controller) : 32,
      nr_php_is_zval_non_empty_string (controller) ? Z_STRVAL_P (controller) : "NoController",

      nr_php_is_zval_non_empty_string (action) ? Z_STRLEN_P (action) : 32,
      nr_php_is_zval_non_empty_string (action) ? Z_STRVAL_P (action) : "NoAction");

    nr_txn_set_path ("Zend", NRPRG (txn), buf, NR_PATH_TYPE_ACTION, NR_OK_TO_OVERWRITE);
  }
 ...
 */
static zval*
nr_zend_plugin_broker_get_request (zval *plugins TSRMLS_DC)
{
  zval *request = 0;
...
void
nr_zend_enable (TSRMLS_D)
{
  nr_php_wrap_user_function (NR_PSTR ("Zend_Controller_Plugin_Broker::dispatchLoopShutdown"),
    nr_zend_plugin_broker_dispatch_loop_shutdown TSRMLS_CC);
  nr_php_wrap_user_function (NR_PSTR ("Zend_Controller_Plugin_Broker::preDispatch"),
