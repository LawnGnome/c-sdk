 ...
 */
#ifdef PHP7
# define NR_SPECIALFNPTR_PROTO          struct _nruserfn_t *wraprec, zend_execute_data *execute_data
...
# define NR_ZEND_EXECUTE_HOOK           zend_execute_ex
#else /* PHP < 5.5 */
# define NR_SPECIALFNPTR_PROTO          struct _nruserfn_t *wraprec, zend_op_array *op_array_arg
...
typedef void (*nrphperrfn_t) (int type, const char *filenm, uint lineno, const char *fmt, va_list args) ZEND_ATTRIBUTE_PTR_FORMAT(printf, 4, 0);
typedef zend_op_array *(*nrphpcfile_t) (zend_file_handle *file_handle, int type TSRMLS_DC);
typedef zend_op_array *(*nrphpcstr_t) (zval *source_string, char *filename TSRMLS_DC);
typedef void (*nrphpexecfn_t) (NR_EXECUTE_PROTO TSRMLS_DC);
typedef void (*nrphpcufafn_t) (zend_function *func, const zend_function *caller TSRMLS_DC);
typedef int (*nrphphdrfn_t)(sapi_header_struct *sapi_header, sapi_header_op_enum op, sapi_headers_struct *sapi_headers TSRMLS_DC);

#if defined(PHP7)
typedef void (*nr_php_execute_internal_function_t) (zend_execute_data *execute_data, zval *return_value);
#elif ZEND_MODULE_API_NO >= ZEND_5_5_X_API_NO
typedef void (*nr_php_execute_internal_function_t) (zend_execute_data *execute_data, zend_fcall_info *fci, int return_value_used TSRMLS_DC);
#else
typedef void (*nr_php_execute_internal_function_t) (zend_execute_data *execute_data, int return_value_used TSRMLS_DC);
#endif
...
ZEND_BEGIN_MODULE_GLOBALS(newrelic)
  ...
  nrinibool_t errors_enabled;           /* newrelic.error_collector.enabled - paired with RPM */
  nrinibool_t ignore_user_exception_handler;      /* newrelic.error_collector.ignore_user_exception_handler */
  ...
  nrinibool_t record_database_errors;             /* newrelic.error_collector.record_database_errors */
  nrinibool_t prioritize_api_errors;              /* newrelic.error_collector.prioritize_api_errors */
  nrinibool_t remove_trailing_path;               /* newrelic.webtransaction.name.remove_trailing_path */

  nrinibool_t browser_monitoring_auto_instrument; /* newrelic.browser_monitoring.auto_instrument */
  ...
  nrinibool_t analytics_events_enabled;              /* DEPRECATED newrelic.analytics_events.enabled */
  nrinibool_t transaction_tracer_capture_attributes; /* DEPRECATED newrelic.transaction_tracer.capture_attributes */
  nrinibool_t error_collector_capture_attributes;    /* DEPRECATED newrelic.error_collector.capture_attributes */
  nrinibool_t analytics_events_capture_attributes;   /* DEPRECATED newrelic.analytics_events.capture_attributes */
  nrinibool_t browser_monitoring_capture_attributes; /* DEPRECATED newrelic.browser_monitoring.capture_attributes */

  nrinibool_t transaction_events_enabled;            /* newrelic.transaction_events.enabled */
  ...
  nr_php_ini_attribute_config_t transaction_tracer_attributes; /* newrelic.transaction_tracer.attributes.*  */
  nr_php_ini_attribute_config_t error_collector_attributes;    /* newrelic.error_collector.attributes.*  */
  nr_php_ini_attribute_config_t transaction_events_attributes; /* newrelic.transaction_events.attributes.*  */
  nr_php_ini_attribute_config_t browser_monitoring_attributes; /* newrelic.browser_monitoring.attributes.*  */
  ...
  nrinibool_t instance_reporting_enabled;      /* newrelic.datastore_tracer.instance_reporting.enabled */
  nrinibool_t database_name_reporting_enabled; /* newrelic.datastore_tracer.database_name_reporting.enabled */
  ...
  nrinibool_t tt_enabled;               /* newrelic.transaction_tracer.enabled - paired with RPM */
  ...
  nrinitime_t ss_threshold;             /* newrelic.transaction_tracer.stack_trace_threshold */
  nrinibool_t cross_process_enabled;    /* newrelic.cross_application_tracer.enabled */

  nriniuint_t max_nesting_level;        /* newrelic.special.max_nesting_level (named after like-used variable in xdebug) */
  ...
  nrinibool_t tt_inputquery;            /* newrelic.transaction_tracer.gather_input_queries */
...
#define NR_PHP_RECORDSQL_OBFUSCATED     NR_SQL_OBFUSCATED
  ...
  nrframework_t current_framework;      /* Current request framework (forced or detected) */
  int framework_version;                /* Current framework version */

  nrtime_t *cur_drupal_module_kids_duration; /* Points to variable to increment for a Drupal Module child's duration */
  nrtime_t *cur_drupal_view_kids_duration;   /* Points to variable to increment for a Drupal View child's duration */
  char *drupal_module_invoke_all_hook;       /* The current Drupal hook */
  size_t drupal_module_invoke_all_hook_len;  /* The length of the current Drupal hook */
  size_t drupal_http_request_depth;          /* The current depth of drupal_http_request() calls */

  int symfony1_in_dispatch;             /* Whether we are currently within a sfFrontWebController::dispatch() frame */
  int symfony1_in_error404;             /* Whether we are currently within a sfError404Exception::printStackTrace() frame */

  nrtime_t *cur_wordpress_hook_kids_duration; /* Points to variable to increment for a WordPress hook child's duration */
  ...
  nr_hashmap_t *wordpress_file_metadata;    /* Metadata for plugin and theme names given a filename */

  char *doctrine_dql;                    /* The current Doctrine DQL. Only non-NULL while a Doctrine object is on the stack. */

  int execute_count;                    /* How many times nr_php_execute_enabled was called */
  int php_cur_stack_depth;              /* Total current depth of PHP stack, measured in PHP call frames */

  nrphpcufafn_t cufa_callback;          /* The current call_user_func_array callback, if any */
  ...
  nr_hashmap_t *guzzle_objs;            /* Guzzle request object storage: requests that are currently in progress are stored here */
  ...
  char *curl_exec_x_newrelic_app_data;  /* Header information saved by curl_exec callback */
