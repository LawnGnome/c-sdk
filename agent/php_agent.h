...
/*
 * config.h is generated by standard the PHP build pipeline: phpize and configure.
 * This must be included first because some of the defines toggle platform-
 * specific code in axiom.
 */
#ifdef HAVE_CONFIG_H
#include "config.h"
#endif
...
typedef struct _nrphpglobals_t {
  ...
  int   daemon_special_curl_verbose;      /* Cause the daemon to enter curl verbose mode */
  int   daemon_special_integration;       /* Cause daemon to dump special log entries to help integration testing. */
  int   zend_offset;            /* Zend extension offset */
  int   done_instrumentation;   /* Set to true if we have installed instrumentation handlers */
  nrtime_t expensive_min;       /* newrelic.special.expensive_node_min */
  char  *upgrade_license_key;   /* License key from special file created during 2.9 upgrades */
  ...
  struct {
    uint8_t laravel_queue:1;  /* enables Laravel queue support */
...
/*
 * Purpose : Free all of the global structure's allocated fields and zero
 *           all fields.
 */
extern void nr_php_per_process_globals_dispose (void);
...
/*
 * Purpose : Same as nr_php_backtrace_to_json but designed as a callback parameter
 *           for nr_txn_end_node_sql.
 */
 ...
 */
extern zval *nr_php_get_zval_object_property (zval *object, const char *cname TSRMLS_DC);
 ...
 *           method table.
 */
extern int nr_php_object_has_concrete_method (zval *object, const char *lcname TSRMLS_DC);
 ...
 */
extern nr_status_t nr_php_txn_begin (const char *appnames, const char *license TSRMLS_DC);
 ...
 */
extern nr_status_t nr_php_txn_end (int ignoretxn, int in_post_deactivate TSRMLS_DC);
 ...
 */
extern zend_function *nr_php_find_class_method (const zend_class_entry *klass, const char *name);
 ...
 * Returns : 0 or 1.
 */
extern int nr_php_class_entry_instanceof_class (const zend_class_entry *ce, const char *class_name TSRMLS_DC);
 ...
 * Returns : 0 or 1.
 */
extern int nr_php_object_instanceof_class (const zval *object, const char *class_name TSRMLS_DC);
 ...
 */
static inline zval *
nr_php_get_return_value (NR_EXECUTE_PROTO TSRMLS_DC)
{
#ifdef PHP7
 ...
 */
extern zval *nr_php_get_user_func_arg (size_t requested_arg_index, NR_EXECUTE_PROTO TSRMLS_DC);
...
nr_php_execute_function (NR_EXECUTE_PROTO TSRMLS_DC)
{
  NR_UNUSED_TSRMLS;
...
static inline zval *
nr_php_execute_scope (zend_execute_data *execute_data)
{
  if (NULL == execute_data) {
    return NULL;
  }
...
static inline zend_op_array *
nr_php_current_op_array (TSRMLS_D)
{
#ifdef PHP7
 ...
 *           after use.
 */
extern zval *nr_php_get_class_constant (const zend_class_entry *ce, const char *name);
 ...
 */
extern int nr_php_is_zval_named_constant  (const zval *zv, const char *name TSRMLS_DC);
...
nr_php_zend_register_internal_class_ex (zend_class_entry *ce,
...
static inline char *
nr_php_zend_ini_string (char *name, nr_string_len_t name_len, int orig)
{
#ifdef PHP7
...
{
#ifdef PHP7
  return (func->common.function_name && func->common.function_name->len) ? func->common.function_name->val : NULL;
#else
...
{
#ifdef PHP7
  return (op_array->filename && op_array->filename->len) ? op_array->filename->val : NULL;
#else
...
{
#ifdef PHP7
  return (op_array->function_name && op_array->function_name->len) ? op_array->function_name->val : NULL;
#else
