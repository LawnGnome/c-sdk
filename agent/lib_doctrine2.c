 ...
 *
 * Currently (2/2016) this is only SQL Input Query Spec:
 * https://source.datanerd.us/agents/agent-specs/blob/master/SQL-Input-Query.md
 */
#include "php_agent.h"
 ...
 */
static int
nr_doctrine2_dql_enabled (TSRMLS_D)
{
...
NR_PHP_WRAPPER (nr_doctrine2_cache_dql)
{
  ...
  if (nr_doctrine2_dql_enabled (TSRMLS_C)) {
    ...
    if (0 != nr_php_object_instanceof_class (this_var, "Doctrine\\ORM\\Query" TSRMLS_CC) ) {
