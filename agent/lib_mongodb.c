...
static int
nr_mongodb_is_server (const zval *obj TSRMLS_DC)
{
  return nr_php_object_instanceof_class (obj, "MongoDB\\Driver\\Server" TSRMLS_CC);
}

char *
nr_mongodb_get_host (zval *server TSRMLS_DC)
{
  char *host = NULL;
  zval *host_zval;

  if (!nr_mongodb_is_server (server TSRMLS_CC)) {
    nrl_verbosedebug (NRL_FRAMEWORK, "%s: MongoDB server does not seem to be a server", __func__);
    return NULL;
  }
  ...
  if (nr_php_is_zval_valid_string (host_zval)) {
    /*
     * Cool, we got a valid string. We check whether it's empty or localhost;
...
char *
nr_mongodb_get_port (zval *server TSRMLS_DC)
{
  char *port = NULL;
  zval *port_zval;
...
void
nr_mongodb_get_host_and_port_path_or_id (zval *server, char **host,
...
NR_PHP_WRAPPER (nr_mongodb_operation)
{
  ...
  if (!nr_php_object_instanceof_class (this_var, this_klass TSRMLS_CC)) {
    nrl_verbosedebug (NRL_FRAMEWORK, "%s: operation is not %s", __func__, this_klass);
    NR_PHP_WRAPPER_CALL;
    goto leave;
  }

  collection = nr_php_get_zval_object_property (this_var, "collectionName" TSRMLS_CC);
  if (nr_php_is_zval_valid_string (collection)) {
    params.collection = Z_STRVAL_P (collection);
  }

  database = nr_php_get_zval_object_property (this_var, "databaseName" TSRMLS_CC);
...
void
nr_mongodb_enable (TSRMLS_D)
{
  /*
   * We instrument interesting methods on the MongoDB\Collection class via their
   * associated MongoDB\Operation classes.
   */
  nr_php_wrap_user_function_extra (NR_PSTR ("MongoDB\\Operation\\Aggregate::execute"),
    nr_mongodb_operation, "aggregate" TSRMLS_CC);
  nr_php_wrap_user_function_extra (NR_PSTR ("MongoDB\\Operation\\BulkWrite::execute"),
    nr_mongodb_operation, "bulkWrite" TSRMLS_CC);
   ...
   */
  nr_php_wrap_user_function_extra (NR_PSTR ("MongoDB\\Operation\\CreateIndexes::execute"),
   ...
   */
  nr_php_wrap_user_function_extra (NR_PSTR ("MongoDB\\Operation\\Delete::execute"),
    nr_mongodb_operation, "delete" TSRMLS_CC);

  nr_php_wrap_user_function_extra (NR_PSTR ("MongoDB\\Operation\\Distinct::execute"),
    nr_mongodb_operation, "distinct" TSRMLS_CC);

  /*
   * This also catches MongoDB\Collection::drop
   */
  nr_php_wrap_user_function_extra (NR_PSTR ("MongoDB\\Operation\\DropCollection::execute"),
    nr_mongodb_operation, "dropCollection" TSRMLS_CC);

  /*
   * This also catches MongoDB\Collection::dropIndex
   */
  nr_php_wrap_user_function_extra (NR_PSTR ("MongoDB\\Operation\\DropIndexes::execute"),
    nr_mongodb_operation, "dropIndexes" TSRMLS_CC);
   ...
   */
  nr_php_wrap_user_function_extra (NR_PSTR ("MongoDB\\Operation\\FindAndModify::execute"),
    nr_mongodb_operation, "findAndModify" TSRMLS_CC);

  nr_php_wrap_user_function_extra (NR_PSTR ("MongoDB\\Operation\\InsertMany::execute"),
    nr_mongodb_operation, "insertMany" TSRMLS_CC);
  nr_php_wrap_user_function_extra (NR_PSTR ("MongoDB\\Operation\\InsertOne::execute"),
    nr_mongodb_operation, "insertOne" TSRMLS_CC);
  nr_php_wrap_user_function_extra (NR_PSTR ("MongoDB\\Operation\\ListIndexes::execute"),
    nr_mongodb_operation, "listIndexes" TSRMLS_CC);
   ...
   */
  nr_php_wrap_user_function_extra (NR_PSTR ("MongoDB\\Operation\\Update::execute"),
    nr_mongodb_operation, "update" TSRMLS_CC);
   ...
   */
  nr_php_wrap_user_function_extra (NR_PSTR ("MongoDB\\Operation\\DatabaseCommand::execute"),
