...
static const char DEFAULT_INI[] =
  ...
  "newrelic.license=4e8c0ad1c4e63058f78cea33765a9ef36004393b\n"
 ...
 */
static char *
tlib_php_create_output_filename (const char *ext)
{
...
static size_t
tlib_php_engine_ub_write (const char *str, size_t len)
#else
static int
tlib_php_engine_ub_write (const char *str, uint len TSRMLS_DC)
#endif /* PHP >= 7.0 */
{
  NR_UNUSED_TSRMLS;
...
static void
tlib_php_interned_strings_snapshot (TSRMLS_D)
{
  NR_UNUSED_TSRMLS;
}
...
nr_status_t
tlib_php_engine_create (const char *extra_ini PTSRMLS_DC)
{
...
# endif /* PHP7 */
  ...
  /*
   * TODO(aharvey): It's unfortunate that we create real files for the agent
   * log and output, since they'll be deleted on successful runs. Let's see if
   * we can refactor this to keep them in memory during execution and only dump
  ...
  ini = nr_formatf ("%s\nnewrelic.logfile=%s\n%s",
                    DEFAULT_INI, logfile, NRBLANKSTR (extra_ini));
...
void
tlib_php_engine_destroy (TSRMLS_D)
{
  ...
  if (tlib_unexpected_failcount > 0) {
     ...
     */
    printf ("Errors occurred. Output files:\n"
            "\tAgent log:  %s\n"
            "\tPHP output: %s\n",
            logfile, outfile);
  } else {
...
nr_status_t
tlib_php_request_start_impl (const char *file, int line TSRMLS_DC)
{
  assert (NULL != out);
  assert (NULL == argv);
  ...
  /*
   * Much of the below is cargo culted from php_embed_init().
   *
...
void
tlib_php_request_end_impl (const char *file, int line)
{
  assert (NULL != out);
...
bool
tlib_php_request_is_active (void)
{
  return (NULL != argv);
}

void
tlib_php_request_eval (const char *code TSRMLS_DC)
{
...
zval *
tlib_php_request_eval_expr (const char *code TSRMLS_DC)
{
  char *copy = nr_strdup (code);
...
int
tlib_php_require_extension (const char *extension TSRMLS_DC)
{
  char *file = NULL;
  int loaded = 0;
...
tlib_php_internal_function_handler_t
tlib_php_replace_internal_function (const char *klass, const char *function,
                                    tlib_php_internal_function_handler_t handler
                                    TSRMLS_DC)
{
  ...
  nrl_verbosedebug (NRL_AGENT, "%s: replacing %s%s%s",
                    __func__,
                    klass ? klass : "",
                    klass ? "::" : "",
                    function);
...
#ifdef PHP7
    ce = (zend_class_entry *) nr_php_zend_hash_find_ptr (CG (class_table),
                                                         lcclass);
#else
    zend_class_entry **ce_ptr = nr_php_zend_hash_find_ptr (CG (class_table),
                                                           lcclass);
...
zval *
tlib_php_zval_create_default (zend_uchar type TSRMLS_DC)
{
  ...
  switch (type) {
    ...
    case IS_RESOURCE:
      /*
       * PHP 7 tends to be unhappy if you don't give it a non-NULL pointer for
       * the resource. The actual pointer doesn't matter much, since we're never
...
zval **
tlib_php_zvals_of_all_types (TSRMLS_D)
{
  zval **arr;
  size_t i;
  static const size_t num_types = sizeof (default_zval_types) /
                                  sizeof (default_zval_types[0]);
...
zval **
tlib_php_zvals_not_of_type (zend_uchar type TSRMLS_DC)
{
  zval **arr;
  size_t i;
  size_t idx = 0;
...
void
tlib_php_free_zval_array (zval ***arr_ptr)
{
  zval **arr;
  size_t i;
...
const char *app_connect_reply =
  ...
  "   ],"
  "   \"apdex_t\" : 0.5,"
  "   \"cross_process_id\" : \"204549#18303\","
  "   \"episodes_url\" : \"https://js-agent.newrelic.com/nr-106.js\","
  ...
  "            \"phpinfo.php\""
  "         ],"
  "         \"prefix\" : \"Browser/PageView/localhost/internal_white_terms/\""
  "      }"
  "   ],"
...
static nr_status_t
stub_cmd_appinfo_tx (int daemon_fd NRUNUSED, nrapp_t *app)
{
  /*
  ...
  app->agent_run_id = nr_strdup (nro_get_hash_string (app->connect_reply, "agent_run_id", NULL));
  app->state = NR_APP_OK;
  nr_rules_destroy (&app->url_rules);
  app->url_rules = nr_rules_create_from_obj (nro_get_hash_array (app->connect_reply, "url_rules", 0));
  nr_rules_destroy (&app->txn_rules);
  app->txn_rules = nr_rules_create_from_obj (nro_get_hash_array (app->connect_reply, "transaction_name_rules", 0));
  nr_segment_terms_destroy (&app->segment_terms);
  app->segment_terms = nr_segment_terms_create_from_obj (nro_get_hash_array (app->connect_reply, "transaction_segment_terms", 0));

  return NR_SUCCESS;
}

static nr_status_t
stub_cmd_txndata_tx (int daemon_fd NRUNUSED, const nrtxn_t *txn NRUNUSED)
{
  /*
