...
static nr_status_t
nr_custom_events_iter (const char *key, const nrobj_t *val, void *ptr)
{
...
static int
nr_custom_events_valid_event_type (const char *type)
{
  int idx;
  ...
  if (nr_strlen (type) > NR_ATTRIBUTE_KEY_LENGTH_LIMIT) {
    nrl_warning (NRL_TXN, "unable to add custom event: type string exceeds length limit of %d",
  ...
  if ((idx <= 0) || (0 != type[idx])) {
    nrl_warning (NRL_TXN, "unable to add custom event: event type does not match ^[a-zA-Z0-9:_ ]+$");
    return 0;
  }
  ...
  nr_random_t *rnd)
{
  ...
  nro_set_hash_double (intrinsics, "timestamp", ((double)now) / NR_TIME_DIVISOR_D);
  ...
  validated = nr_attributes_user_to_obj (atts, NR_ATTRIBUTE_DESTINATION_TXN_EVENT);
